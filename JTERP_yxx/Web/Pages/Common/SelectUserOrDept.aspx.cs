...
public partial class Pages_Common_SelectUserOrDept : System.Web.UI.Page
{
    ...
    private void BindTree()
    {
        ...
        if (ShowType == "2" && OprtType == "2")
        {
            btnOk.Visible = true;//如果为多选，确定按钮要显示 宋凯歌
        }
        ...
        if (!string.IsNullOrEmpty(ShowType) && Convert.ToInt32(ShowType) > 1)
        {
            ...
            if (this.RblIsShowLizhi.Checked)
            {
             ...
             Userdt = UserDeptSelectBus.GetUserInfo(ShowType, OprtType, isShowLizhi,EmpType);
        }
        //根节点
        TreeNode rootNode = new TreeNode();
        rootNode.Text = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyName + "组织机构";
    ...
    //by:zyy
    protected void getculture()
    {
        DataTable dt = UserDeptSelectBus.GetCulture(ShowType);
        //根节点
        TreeNode rootNode = new TreeNode();
        rootNode.Text = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyName + "--企业文化类型";
        rootNode.Value = "0";
        ...
        if (dt != null && dt.Rows.Count > 0)
        {
            ...
            {
                childNode = new TreeNode();
                Userdt = UserDeptSelectBus.GetCultureInfo(dt.Rows[i]["ID"].ToString());
                childNode.Value = dt.Rows[i]["ID"].ToString();
                childNode.Text = dt.Rows[i]["DeptName"].ToString();
                childNode.NavigateUrl = "javascript:void(0);";

                TreeNode Usernode = null;
                if (Userdt != null && Userdt.Rows.Count > 0)
                {
                    string UserExprssion = "SuperDeptID='" + childNode.Value + "'";
                    DataRow[] UserRow = Userdt.Select(UserExprssion);
                    if (UserRow.Length > 0)
                    {
                        for (int j = 0; j < UserRow.Length; j++)
