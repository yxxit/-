...
public partial class Pages_Office_SystemManager_RoleInfo_Add :BasePage
{
    ...
    private void InitPage(int RoleID )
    {
        ...
        if (DataTable.Rows.Count > 0)
        {
            ...
            if (row["RoleRange"].ToString() == "0")
            {
                RoleRange0.Checked = true;
            }
            if (row["RoleRange"].ToString() == "1")
            {
                RoleRange1.Checked = true;
            }
            if (row["RoleRange"].ToString() == "2")
            {
                RoleRange2.Checked = true;
            }
            if (row["RoleRange"].ToString() == "3")
            {
                RoleRange3.Checked = true;
            }
