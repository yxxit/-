...
public partial class Pages_Office_HumanManager_SalaryCommission : BasePage
{
    /// <summary>
    /// 类名：SalaryCommission
    /// 描述：提成工资设置
    ...
    {
        //是否含有生产模块
        bool isHaveInfo = CommissionItemBus.IsHaveSellInfo();
        //设置是否有生产模块标识
        if (isHaveInfo)
        {
            txtIsHaveSell.Value = "1";
        }
        else
        {
            txtIsHaveSell.Value = "0";
        }
        //设置工资表格内容
    ...
    /// <summary>
    /// 设置工资内容
    /// <param name="isHaveInfo">是否包含销售模块区分</param>
    /// </summary>
    private string InitSalaryDetailInfo(bool isHaveInfo)
    {
        ...
        //获取数据
        DataTable dtSalaryInfo = CommissionItemBus.GetCommissionItemInfo(false);
        //数据存在时
        if (dtSalaryInfo != null && dtSalaryInfo.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dtSalaryInfo.Rows.Count; i++)
            {
                ...
                sbSalaryInfo.AppendLine("<td class='tdColInputCenter'>"
                            ...
                            + GetSafeData.ValidateDataRow_String(dtSalaryInfo.Rows[i], "Rate")
                            + "' class='tdinput' id='txtRate_" + (i + 1).ToString() + "' style='text-align:center'  onchange='Number_round(this,\"2\");'  onkeydown='Numeric_OnKeyDown();'/></td>");
                //启用状态
                //变量定义
                ...
                sbSalaryInfo.AppendLine("<td class='tdColInputCenter'>"
                            + "<input type='hidden' id='txtUsedStatus_" + (i + 1).ToString() + "' value='" + usedStatus + "' / >"
    ...
    private string CreateSalaryListTable()
    {
        ...
        table.AppendLine("		</td>                                                             ");
        table.AppendLine("		<td  class='td_list_fields' style='width:32%'>项目编号</td>             ");
        table.AppendLine("		<td  class='td_list_fields' style='width:32%'>项目名称</td>             ");
        table.AppendLine("		<td  class='td_list_fields' style='width:22%'><span style=\"color:Red\">*</span>提成率(%)</td>               ");
        table.AppendLine("		<td  class='td_list_fields' style='width:10%'><span style=\"color:Red\">*</span>启用状态</td>             ");
        table.AppendLine("	</tr>                                                                 ");
        //返回表格语句
