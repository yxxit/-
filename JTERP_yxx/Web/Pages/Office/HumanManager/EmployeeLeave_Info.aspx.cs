...
public partial class Pages_Office_HumanManager_EmployeeLeave_Info : BasePage
{
    /// <summary>
    /// 类名：EmployeeLeave_Info
    /// 描述：离职人员列表
    /// 
    /// 作者：吴志强
    /// 创建时间：2009/03/25
        ...
        if (!IsPostBack)
        {
            ...
            if (firstIndex > 0)
            {
                ...
                if ("1".Equals(flag))
                {
                    ...
                    txtEnteryEndDate.Value = Request.QueryString["EnteryEndDate"];
                    //离职时间
                    txtLeaveDate.Value = Request.QueryString["LeaveDate"];
                    txtLeaveEndDate.Value = Request.QueryString["LeaveEndDate"];
                    ...
                    ClientScript.RegisterStartupScript(this.GetType(), "DoSearch"
                            , "<script language=javascript>this.pageCount = parseInt(" + pageCount + ");SearchEmployeeLeave('" + pageIndex + "');</script>");
                }
            }
        }
