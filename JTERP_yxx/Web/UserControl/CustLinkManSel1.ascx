<%@ Control Language="C#" AutoEventWireup="true" CodeFile="CustLinkManSel1.ascx.cs" Inherits="UserControl_CustLinkManSel1" %>
 ...
 function lmTurnToPage(pageIndex)
    {
           ...
           var CustNo = document.getElementById("hfCustID").value;
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                      if(item.id != null && item.id != "")                      
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+"<input onclick=\"GetLinkMan('"+item.id+"','"+item.LinkManName+"','"+item.WorkTel+"','"+item.MSN+"','"+item.MailAddress+"')\" id='Checkbox1' value="+item.id+"  type='radio'/>"+"</td>"+                                                                                                       
...
function GetLinkMan(ID,LinkManName,WorkTel,MSN,MailAddress)
{
    ...
    try
    {
        ...
        $("#txtCustLinkTel").attr("value",WorkTel);//联系人名  
        $("#txtCustLinkMSN").attr("value",MSN);//客户联系电话    
        $("#txtCustLinkEmail").attr("value",MailAddress);//客户联系电话 
...
function LinkManClear()
{
    ...
    try {
        ...
        $("#txtCustLinkTel").attr("value", ""); //联系人电话
        $("#txtCustLinkMSN").attr("value", ""); //客户联系MSN    
        $("#txtCustLinkEmail").attr("value", ""); //客户联系邮箱 
    }
    catch (Error) { }
...
<input id="txtUcLinkMan"  type="text" readonly="readonly" class="tdinput1" /><asp:Image ID="Image4" runat="server" ImageUrl="~/Images/default/search.gif" ImageAlign=AbsMiddle onclick="SearchLinkMan();" />              <!--class="tdinput" -->
...
<table width="99%" border="0" align="center" cellpadding="0" id="lmTable1"  cellspacing="0" bgcolor="#CCCCCC">
    ...
    <table width="99%" border="0" align="center" cellpadding="0" id="searchtable"  cellspacing="0" bgcolor="#CCCCCC">
      <tr>
        <td bgcolor="#FFFFFF"><table width="100%" border="0"  cellpadding="2" cellspacing="1" bgcolor="#CCCCCC" class="table">
          <tr class="table-item">
            <td width="10%" height="20" class="td_list_title" align="right"> 联系人姓名</td>
            <td width="23%" bgcolor="#FFFFFF"><input id="txtLinkMan"  class="tdinput"  type="text" style="width:95%" /></td>
            
            <td width="10%"  class="td_list_fields"  align="right">称谓</td>
    ...
    <tbody>
      <tr>
        ...
        <th align="center"  class="td_list_fields"  ><div class="orderClick" onclick="OrderBy('LinkManName','oLinkManName');return false;">联系人姓名<span id="oLinkManName" class="orderTip"></span></div></th>        
        <th align="center"  class="td_list_fields"  ><div class="orderClick" onclick="OrderBy('Appellation','oAppellation');return false;">称谓<span id="oAppellation" class="orderTip"></span></div></th>        
        <th align="center"  class="td_list_fields"  ><div class="orderClick" onclick="OrderBy('Department','oDepartment');return false;">部门<span id="oDepartment" class="orderTip"></span></div></th>        
