...
<!-- Start 物品库存快照 -->
<div id="divSnapshot" style="border: solid 10px #93BCDD; background: #fff; padding: 20px;
    width: 455px; height: 220px; overflow: scroll; z-index: 1001; position: absolute;
    display: none; top: 50px; left: 65%; margin: 30px 0 0 -400px; scrollbar-face-color: #ffffff;
    scrollbar-highlight-color: #ffffff; scrollbar-shadow-color: COLOR:#000000; scrollbar-3dlight-color: #ffffff;
    scrollbar-darkshadow-color: #ffffff;" onmousedown="javascript:moveStart(event,this.id);">
    <table width="445">
        <tr>
            <td>
    ...
    function ShowStorageSnapshot(intProductID, snapProductName, snapProductNo) {
        ...
        $.ajax({
            ...
            success: function(msg) {
                var divHtml = "<table width=\"445\" border=\"0\" cellspacing=\"1\" bgcolor=\"#CCCCCC\"><tr><td width=\"120\" bgcolor=\"#93BCDD\">仓库名称</td><td width=\"65\" bgcolor=\"#93BCDD\">现有存量</td><td width=\"65\" bgcolor=\"#93BCDD\">在途量</td><td width=\"65\" bgcolor=\"#93BCDD\">待发货量</td><td width=\"65\" bgcolor=\"#93BCDD\">借出量</td><td width=\"65\" bgcolor=\"#93BCDD\">借入量</td><td width=\"120\" bgcolor=\"#93BCDD\">批次</td></tr>";
                var tmpHtml = '';
                var bgColor = '';
                var rowsCount = 0;
                //库存快照
                if (typeof (msg.dataSto) != 'undefined') {
                    $.each(msg.dataSto, function(i, item) {
                        ...
                        tmpHtml = tmpHtml + '<tr><td width=\"120\" bgcolor=\"' + bgColor + '\">' + item.StorageName + '</td><td width=\"65\" bgcolor=\"' + bgColor + '\">' + item.ProductCount + '</td><td width=\"65\" bgcolor=\"' + bgColor + '\">' + item.RoadCount + '</td><td width=\"65\" bgcolor=\"' + bgColor + '\">' + item.OrderCount + '</td><td width=\"65\" bgcolor=\"' + bgColor + '\">' + item.OutCount + '</td><td width=\"65\" bgcolor=\"' + bgColor + '\">' + item.InCount + '</td><td width=\"120\" bgcolor=\"' + bgColor + '\">' + item.BatchNo + '</td></tr>';
