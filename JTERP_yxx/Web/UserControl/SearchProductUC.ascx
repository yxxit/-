<%@ Control Language="C#" AutoEventWireup="true" CodeFile="SearchProductUC.ascx.cs" Inherits="UserControl_SearchProductUC" %>
    ...
    <div id="divStorageProduct" style="border: solid 10px #93BCDD; background: #fff;
        ...
        <div id="divquery" style="width: 100%; left: 0px;">
                   <%-- <a onclick="closeProductdiv('divStorageProduct')" style="text-align: right; cursor: pointer">关闭</a>--%>
                     <a onclick="closeProductdiv()" style="text-align: right; cursor: pointer">关闭</a>
                       <a onclick="ClearProductInfo()" style="text-align: right; cursor: pointer ;" id="ClearInputProduct">清空</a>
                                ...
                                <tr id="trNewAttr" style="display:none">  
                                     <td height="20" align="right" class="td_list_fields"width="10%">
                                      <span id="spanOther" style="display:none">其他条件</span>
                                        ...
                                        <td height="20" align="right" class="td_list_fields"colspan="6">
                ...
                <tr>
                    ...
                    <th align="center" class="td_list_fields">
                        <div class="orderClick" onclick="OrderByProduct('StandardCost','StandardCost');return false;">
                            标准成本<span id="StandardCost" class="orderTip"></span></div>
                    </th>
...
popTechObj.InputObj = null;
 popTechObj.Condition=null;
 popTechObj .CleraInput=null;
 popTechObj .CleraHidden=null;
popTechObj.ShowList=function(CleraInput,CleraHidden,Condition)
{
    popTechObj.Condition=Condition;
  ...
  Fun_Search_Product();
    ...
    function TurnToPageProduct(pageIndex)
    {
          ...
          if(document.getElementById("trNewAttr").style.display!="none")
          {
           ...
           url:  '../../../Handler/Office/SupplyChain/SearchProduct.ashx',//目标地址
           cache:false,
                     data: "pageIndex="+pageIndex+"&pageCount="+pageCountproduct+"&action="+action+"&orderByP="+orderByP+"&ProductID="+escape(ProdNo)+"&ProdName="+escape(ProdName)+"&PYShort="+escape(PYShort)+"&typeid="+escape(typeid)+"&Condition="+escape(popTechObj.Condition)+"&EFIndex="+escape(EFIndex)+"&EFDesc="+escape(EFDesc)+"",//数据
         ...
         success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        {
                        var tempOnclick="";
                         tempOnclick= ""+  item.ID+",'"+item.ProdNo+"','"+item.ProductName+"','"+item.StandardSell+"','"+item.UnitID+"','"+item.CodeName+"','"+item.TaxRate+"','"+item.SellTax+"','"+item.Discount+"','"+item.Specification+"','"+item.CodeTypeName+"','"+item.TypeID+"'";
