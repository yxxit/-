<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ProductInfoBatchControl.ascx.cs" Inherits="UserControl_ProductInfoBatchControl" %>
...
<div id="divzhezhao1" style="filter: Alpha(opacity=0); width: 93%; padding: 1px;
    ...
    <iframe style="border: 0; width: 100%; height: 800px; position: absolute;" ></iframe>
</div>
                    ...
                    <table width="100%" border="0" align="left" cellpadding="0" id="searchtable12" cellspacing="0"
                        ...
                        <div class="orderClick">
                            查看<span id="oID" class="orderTip"></span></div>
                                                        ...
                                                        <th align="center" class="td_list_fields">
                                                            <div class="orderClick" onclick="OrderByProduct('BatchNo','Public_BatchNo');return false;">
                                                                批次<span id="Public_BatchNo" class="orderTip"></span></div>
                                                        </th>
                                                        <th align="center" class="td_list_fields"
                                                            style="display: none">
...
var glbHidAuthority =null;
try
{
    glbHidAuthority=document.getElementById('ProductInfoControl1_hidIsAuthority').value; 
} catch(err)
{
    glbHidAuthority=document.getElementById('ProductInfoBatchControl1_hidIsAuthority').value; 
}
    ...
    function TurnToPageProduct(pageIndex) {
        ...
        $.ajax({
            type: "POST", //用POST方式传输
            dataType: "json", //数据格式:JSON
            url: '../../../Handler/Office/SupplyChain/ProductInfoBatchList.ashx', //目标地址
            cache: false,
                ...
                $.each(msg.data, function(i, item) {
                    if (item.ID != null && item.ID != "") {
                        ...
                        if (popTechObj.Special != null) {
                         ...
                         '" + item.MakeUnitID + "','" + item.MakeUnitName + "', '" + item.IsBatchNo + "', '" + item.BatchNo + "','"+item.ColorName+ "','"+item.StorageID+"'";
                        }
                        else {
                       ...
                       "<td height='22' align='center'>" + item.ProductCount + "</td>" +TempPrice+
