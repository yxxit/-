jQuery(document).ready(function ($) {
    $('#Report').flexigrid({
        url: 'JtPurDetail.ashx?action=SearchAll',
        dataType: 'json',
        method:'POST',
        colModel: [
        ...
        sortname: "CustName", //根据需要修改，可以不写（默认为id），但是如果写就必须要正确
        sortorder: "asc",
        usepager: true,//是否分页
        title: '采购统计表',
        ...
        searchitems: [  //检索条件,显示可以，如何使用呢？
			{display: '供应商', name: 'CustName', isdefault: true },
            { display: '合同数量', name: 'iQuantity', isdefault: false },
			{ display: '合同金额', name: 'iMoney', isdefault: false }
			]
    });
