...
FROM   jt_xsfh t1   
       ...
       left join ContractHead_Sale t7 on t7.id=t1.FromBillID 
       JOIN dbo.ContractDetails_Sale t8 ON t7.id = t8.ContractID and      t2.ProductID=t8.cInvCCode 
where   t1.BusiType=1 and t1.BillStatus in (2,9) and t7.BillStatus in (2,9)
and  t1.CompanyCD='" + CompanyCD + @"'";
   ...
   (t2.ProductCount-t2.SttlCount) as wCount,
    ...
    t2.SttlTotalPrice,
   (t2.totalfee - t2.SttlTotalPrice) as wFee,
    replace( convert(varchar(12),t8.SignDate,23),'1900-01-01','') SignDate ,
    replace( convert(varchar(12),t8.EffectiveDate,23),'1900-01-01','') EffectiveDate ,
    replace( convert(varchar(12),t8.EndDate,23),'1900-01-01','') EndDate ,
	t7.DeptName,
    t6.EmployeeName  
from  jt_xsfh t1  
	  left join jt_xsfh_mx t2 on t1.id=t2.sendno
	  left join officedba.ProviderInfo t3 on t1.providerid=t3.id
	  left join officedba.ProductInfo t4 on t2.productid=t4.id  
	  left join officedba.CustInfo t5 on t1.custid=t5.id
	  left join officedba.EmployeeInfo t6 on t1.Seller=t6.id     
	  left join officedba.DeptInfo t7 on t7.id=t1.sellDeptId 
      left join ContractHead_Sale t8 on t8.id=t1.FromBillID  
      left JOIN dbo.ContractDetails_Sale t9 ON t8.id = t9.ContractID         
where t1.BillStatus in (2,9) and t1.BusiType=2 and t8.BillStatus in (2,9)   
and  t1.CompanyCD='" + CompanyCD + "'"; //这个是必须要有的

        if (CustName != null && CustName != "")//这样方便定制
        {
            sqltop1 += " and t4.CustName like  '%" + CustName + "%'";
            sqltop2 += " and t5.CustName like  '%" + CustName + "%'";
        }
        if (Contractid != null && Contractid != "")
        {
            sqltop1 += " and t7.Contractid like  '%" + Contractid + "%'";
            sqltop2 += " and t8.Contractid like  '%" + Contractid + "%'";
        }
        if (StartTime != null && StartTime != "")
        {
            sqltop1 += " and  t7.ConfirmDate>= '" + StartTime + "'";
            sqltop2 += " and t8.ConfirmDate>= '" + StartTime + "'";
        }
        if (EndTime != null && EndTime != "")
        {
            sqltop1 += " and  t7.ConfirmDate<= '" + EndTime + "'";
            sqltop2 += " and t8.ConfirmDate<= '" + EndTime + "'";
        }
        if (Dept != null && Dept != "")
        {
            sqltop1 += " and t6.DeptName =  '" + Dept + "'";
            sqltop2 += " and t7.DeptName =  '" + Dept + "'";
        }
...
DeptName,
EmployeeName  
from (" + sqltop1 + " union all " + sqltop2 + ") tempTable";
