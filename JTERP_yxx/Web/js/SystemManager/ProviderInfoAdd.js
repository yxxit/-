...
//    recordnoparam = intMasterProviderID.toString();
    recordnoparam = requestobj['intMasterProviderID'];
...
//        document.getElementById("TableDTN").style.display="block";
        $("#hidIsliebiao").val(Isliebiao);
        $("#TableDTB").attr("style", "display:block;");  
        $("#TableDTN").attr("style", "display:block;");
    }
    else
    {
//        document.getElementById("TableDTB").style.display="none";
//        document.getElementById("TableDTN").style.display="none";
        $("#TableDTB").css("display", "none"); 
        $("#TableDTN").css("display", "none"); 
    }
...
//            if(typeof(recordnoparam)!="undefined")
            if(recordnoparam !=0&&typeof(recordnoparam)!="undefined")
            {
//                $("#btn_back").show();
                //isnew="2";
...
function GetProviderInfo(ID)
{
     $.ajax({
       ...
       url:"../../../Handler/JTHY/sysmanage/ProviderInfoEdit.ashx",//目标地址
//       data:'ID='+escape(ID),
        data:"ID="+ID,    //原语句为 data:"ID="+ID+"",
...
//                       {
//                        $("#txtusedata").attr("value",item.Usedata.substring(0,item.Usedata.length-7));
//                        $("#txtcertidata").attr("value",item.Certidata.substring(0,item.Certidata.length-7));
//                        $("#txtwardata").attr("value",item.Wardata.substring(0,item.Wardata.length-7));
//                        $("#txtgmspdata").attr("value",item.Gmspdata.substring(0,item.Gmspdata.length-7));
//                        $("#txtpowerdata").attr("value",item.Powerdata.substring(0,item.Powerdata.length-7));
//                        }
                        //--------------------------------------------------------------------------------//
                        //辅助信息
                      ...
                      //  $("#txtCustNote").attr("value",item.CustNote);//供应商简介
                        document.getElementById("txtCustNote").innerHTML=item.CustNote;//供应商简介  20121126
...
{ 
        ...
        var action = null;
         ...
         if($("#txtAction").val()=="2"){
            custNo=document.getElementById("divProviderInfoNo").innerText;
         }
...
//        var custNote=document.getElementById("txtCustNote").value.Trim();//供应商简介 
 var custNote=document.getElementById("txtCustNote").innerHTML;//供应商简介  20121126
...
//          if(document.getElementById("Tb_yy")!=null)
//          {
//         var usedata=document.getElementById("txtusedata").value;//营业执照有效期
//         var certidata=document.getElementById("txtcertidata").value;//经营许可证有效期
//         var wardata=document.getElementById("txtwardata").value;//质保书有效期
//         var powerdata=document.getElementById("txtpowerdata").value;//法人委托书有效期
//         var gmspdata=document.getElementById("txtgmspdata").value;//GMS有效期
//         
//         var usedata=$("#txtusedata").val();//营业执照有效期
//         var certidata=$("#txtcertidata").val();//经营许可证有效期
//         var wardata=$("#txtwardata").val();//质保书有效期
//         var powerdata=$("#txtpowerdata").val();//法人委托书有效期
//         var gmspdata=$("#txtgmspdata").val();//GMS有效期
//      }
//       else
//       {
//         var usedata="";//营业执照有效期
...
//       }
        ...
        else
        {
         ...
         var no= document.getElementById("divProviderInfoNo").innerHTML;  //供应商编号
...
//                  url: "../../../Handler/Office/PurchaseManager/ProviderInfoAdd.ashx?"+UrlParam,
                  url: "../../../Handler/JTHY/sysmanage/ProviderInfoAdd.ashx",
                  dataType:'json',//返回json格式数据
                  data: "custNo="+escape(custNo)+"&custName="+escape(custName)+"&custNam="+escape(custNam)+"\
                        &pYShort="+escape(pYShort)+"&manager="+escape(manager)+"&managerName="+escape(managerName)+"&areaID="+escape(areaID)+"\
                        &setupAddress="+escape(setupAddress)+"&custNote="+escape(custNote)+"\
                        &contactName="+escape(contactName)+"&tel="+escape(tel)+"&mobile="+escape(mobile)+"\
                        &openBank="+escape(openBank)+"&accountMan="+escape(accountMan)+"\
                        &accountNum="+escape(accountNum)+"&usedStatus="+escape(usedStatus)+"\
                        &creator="+escape(creator)+"&creatorName="+escape(creatorName)+"\
                        &createDate="+escape(createDate)+"&modifiedUserID="+escape(modifiedUserID)+"&modifiedUserName="+escape(modifiedUserName)+"\
                        &sendAddress="+escape(sendAddress)+"&action="+escape(action)+"\
                        &CodeType="+escape(CodeType)+"&cno="+escape(no)+"\
                        &AnnFileName="+escape(AnnFileName)+"\
                        &AnnRemark="+escape(AnnRemark)+"&UpDateTime="+UpDateTime+"\
                        &AnnAddr="+escape(AnnAddr.replace(/\\/g, "\\\\"))+"&ID="+escape(txtIndentityID)+"",//数据
...
function AfterUploadFile(url, docName) {
    if (url != "") {
        ...
        //下载删除显示       
//        document.getElementById("divDealAttachment").style.display = "block";
        var upDateTime = document.getElementById("SystemTime").value;
