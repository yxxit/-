...
$(document).ready(function() {
    ...
    if (document.getElementById("headid").value != "" && document.getElementById("headid").value != "undefined") {
        GetInfoById(document.getElementById("headid").value); //根据id获取基本信息
        GetDetailById(document.getElementById("headid").value);//根据id获取详细信息
        $("#labTitle_Write1").html("销售结算单查看");
        $("#txtCustomerName").css("display", "none");
        $("#search").css("display", "none");
    }
    else {
        $("#labTitle_Write1").html("销售结算单新建");
    }
...
function GetInfoById(headid) {
    var action = "SearchPayOne";
   
    $.ajax({
        type: "POST", //用POST方式传输
        dataType: "json", //数据格式:JSON
        url: '../../../Handler/JTHY/Expenses/PaySettle.ashx', //目标地址
        cache: false,
...
function SearchPaySettle() {
    ...
    $.ajax({
        type: "POST", //用POST方式传输
        dataType: "json", //数据格式:JSON
        url: '../../../Handler/JTHY/Expenses/PaySettle.ashx', //标地址
        cache: false,
        data: "action=" + action + "&custName=" + escape(CustName) + "&beginDate=" + escape(BeginT) + "&endDate=" + escape(EndT) + "&iscount=" + escape(iscount),
        beforeSend: function() {
...
function GetDetailById(headid) {
    var action = "SearchPaySettleDetail";
    $.ajax({
        type: "POST", //用POST方式传输
        dataType: "json", //数据格式:JSON
        url: '../../../Handler/JTHY/Expenses/PaySettle.ashx', //目标地址
        ...
        success: function(msg) {
            ...
            var j = 1;
            $.each(msg.data, function(i, item) {
                if (item.id != null && item.id != "") {//id没有获取到
                    FillSignRow(i, item);
                }
            });
        },
...
function FillSignRow(i, item) { //读取最后一行的行号，存放在txtTRLastIndex文本框中
    ...
    newFitDesctd.innerHTML = "<input id='txtSendNo" + rowID + "' value=\"" + item.SendNo + "\"  disabled='disabled' type='text' class='tdinput' style=' width:90%;'  /> "; //添加列内容
    m++;
    ...
    newFitDesctd.innerHTML = "<input id='txtMoney" + rowID + "' value=\"" + Number(item.sttlTotalPrice).toFixed(2) + "\"       onblur=\"getTotalMoney();getMoney(" + rowID + ",7);\" onkeyup='return ValidateNumber(this,value)'  maxlength='10' type='text' class='tdinput' style=' width:90%;'  /> "; //添加列内容
    m++;
    ...
    var newFitDesctd = newTR.insertCell(m); //添加列:结算时备注
    newFitDesctd.className = "cell";
    ...
    // newFitDesctd.innerHTML = "<input id='txtSttlRemark" + rowID + "'   disabled='disabled'   value=\"" + item.sttlCounts + "\"     type='text' class='tdinput'    style='width:90%;'/>"; //添加列内容
    m++;
    var newFitDesctd = newTR.insertCell(m); //添加列:已结算金额
    newFitDesctd.className = "cell";
    newFitDesctd.innerHTML = "<input id='txtsttlTotalPrice" + rowID + "'  disabled='disabled'  value=\"" + Number(item.sttlTotalPrices).toFixed(2) + "\"      type='text' class='tdinput'    style='width:90%;'/>"; //添加列内容
    m++;
    var newFitDesctd = newTR.insertCell(m);
    newFitDesctd.className = "cell";
    newFitDesctd.innerHTML = "<input id='txtProductId" + rowID + "' value=\"" + item.ProductID + "\"    type='hidden'  style='display:none;' class='tdinput' />";
    m++;

    var newFitDesctd = newTR.insertCell(m);
    newFitDesctd.className = "cell";
    newFitDesctd.innerHTML = "<input id='txtSendId" + rowID + "' value=\"" + item.sendId + "\"    type='hidden'  style='display:none;' class='tdinput' />";
    m++;
}
...
function SaveSellOrder() {
    ...
    if ($("#txtCustomerName").val() == "" ) {
        isFlag = false;
        fieldText = fieldText + "客户信息|";
        msgText = msgText + "客户信息不能为空|";
    }
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
      if (signFrame.rows[i].style.display != "none") {
          var Quantity = document.getElementById("txtQuantity" + i).value; //结算数量
          var UnitCost = document.getElementById("txtUnitCost" + i).value; //结算单价
          var txtMoney = document.getElementById("txtMoney" + i).value; //总金额
           var TtlNum = document.getElementById("txtNum" + i).value;  // 单据总数量
           var JslCounts = document.getElementById("txtlCounts" + i).value; //已结算数量

           var txtNum = document.getElementById("txtNum" + i).value; //单据数量
           var txtTaxPrice = document.getElementById("txtTaxPrice" + i).value; //单据单价
       //单据数量
//           if (parseInt(txtNum) == 0 || document.getElementById("txtNum" + i).value == "0" || document.getElementById("txtNum" + i).value=="") {
//           isFlag = false;
//           fieldText = fieldText + "明细信息|";
//           msgText = msgText + "第" + i + "行单据数量不能为空|";
//       }
//       //单据单价
//       if (parseInt(txtTaxPrice) == 0 || document.getElementById("txtTaxPrice" + i).value == "0"||document.getElementById("txtTaxPrice" + i).value == "") {
//           isFlag = false;
//           fieldText = fieldText + "明细信息|";
//           msgText = msgText + "第" + i + "行单据单价不能为空|";
//       }
    ...
    $.ajax({
        type: "POST",
        url: "../../../Handler/JTHY/Expenses/PaySettle.ashx",
        data: 'strfitinfo=' + escape(strfitinfo) + '&action=insertPaySettle' + "&headid=" + headid + '&custId=' + custId,
...
function getDropValue() {
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            j = j + 1;
            var rowid = signFrame.rows[i].id;
            var ArriveNo = $("#txtSendNo" + rowid).val(); //单据编号
            var Id = $("#detailsid" + rowid).val(); //单据编号的Id
            var CustName = $("#txtCustName" + rowid).val(); //客户编号
            var Num = $("#txtNum" + rowid).val(); //单据数量
            var TaxPrice = $("#txtTaxPrice" + rowid).val(); //单据单价
            ...
            var SttlRemark = $("#txtSttlRemark" + rowid).val(); //备注
            var ProductId = $("#txtProductId" + rowid).val();  //商品的编号         
            SendOrderFit_Item[j] = [[ArriveNo], [Id], [CustName], [Num], [TaxPrice], [Quantity], [UnitCost], [Money], [CreateDate], [SttlRemark], [ProductId]];
...
function FillSignRows(i, item) { //读取最后一行的行号，存放在txtTRLastIndex文本框中
    ...
    newFitDesctd.className = "cell";
    var TotalFee = Number(item.SttlTotalPrice).toFixed(2);
    newFitDesctd.innerHTML = "<input id='txtMoney" + rowID + "' value='0.00'      onblur=\"getTotalMoney();getMoney(" + rowID + ",7);\" onkeyup='return ValidateNumber(this,value)'  maxlength='10' type='text' class='tdinput' style=' width:90%;'  /> "; //添加列内容
    m++;
    ...
    var newFitDesctd = newTR.insertCell(m); //添加列:已结算数量
    newFitDesctd.className = "cell";
    //newFitDesctd.innerHTML = "<input id='txtlCounts" + rowID + "'  disabled='disabled'     value=\"" + Math.abs(item.sttlCounts) + "\"     type='text' class='tdinput'    style='width:90%;'/>"; //添加列内容
    newFitDesctd.innerHTML = "<input id='txtlCounts" + rowID + "'  disabled='disabled'     value=\"" +item.sttlCounts + "\"     type='text' class='tdinput'    style='width:90%;'/>"; //添加列内容
    m++;
