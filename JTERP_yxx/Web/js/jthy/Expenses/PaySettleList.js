        ...
        var total2 = 0;
            ...
            $.each(msg.data, function(i, item) {
                if (item.id != null && item.id != "") {
                    ...
                    if (parseFloat(item.sttlCount) != 0 && item.sttlCount != "") {//这个地方出现的问题！
                        total1 += parseFloat(item.sttlCount);
                    }
                    if (parseFloat(item.sttlTotalPrice) != 0 &&item.sttlTotalPrice != "") {
                    ...
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + "<input id='Checkbox1'  Title='" + item.id + "' name='Checkbox1'  value=" + j + "  type='checkbox' onclick=IfSelectAll('Checkbox1','checkall') />" + "</td>" +
                         ...
                         "<td height='22' align='center'><a href='#' onclick=GetLinkParam0('" + item.id + "')><span title=\"" + item.xsfpNo + "\">" + item.xsfpNo + "</a></td>" +
                        ...
                        "<td height='22' align='center'>" + item.sttlCounts + "</a></td>" +
                        "<td height='22' align='center'>" + QfwFormat((item.ProductCount - item.sttlCounts),2) + "</a></td>" +
                        "<td height='22' align='center'>" + item.sttlTotalPrices + "</a></td>" +
            ...
            $("<tr class='newrow'></tr>").append("<td height='22' align='center'>本页合计</td>" +
                        ...
                        "<td height='22' colspan='8'></td>").appendTo($("#pageDataList1 tbody"));//去除冗余
...
function CreateNew() {
    //Pages/JTHY/Expenses/PaySettle.aspx?ModuleID=8880125
    //window.parent.addTab(null, '90031', '登记到货', 'Pages/JTHY/BusinessManage/OutBus_ADD.aspx?typeflag=0&ModuleID=90031');
    window.parent.addTab(null, '8880125', '销售结算单', 'Pages/JTHY/Expenses/PaySettle.aspx?ModuleID=8880125&typeflag=0');
...
function GetLinkParam0(id) {
    ...
    window.parent.addTab(null, '8880125', '销售结算单', 'Pages/JTHY/Expenses/PaySettle.aspx?ModuleID=8880125&intMasterID=' + escape(id));
...
function Fun_DeletePaySettle() {
    ...
    for (var i = 0; i < ck.length; i++) {
        if (ck[i].checked) {
            ...
            status = table.rows[ck[i].value].cells[15].innerText;
            if (status == "确认") {
    ...
    if (c == true) {
        $.ajax({
            ...
            dataType: "json", //数据格式:JSON
            url: "../../../Handler/JTHY/Expenses/PaySettle.ashx", //目标地址
            data: URLParams + "&allPaySettle=" + escape(IsId) + '&billtype=' + escape(typeflag) + ' ', //数据
            cache: false,
