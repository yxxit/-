...
//            var EndCreateDate = document.getElementById("txtEndCreateDate").value;//建挡日期止
           ...
           $.ajax({
               ...
               url: '../../../Handler/JTHY/sysmanage/ProviderInfoInfo.ashx', //目标地址
               cache: false,
...
/// <reference path="file:///C:/Program Files/Baidu/BaiduPinyin/2.9.2.42/dict/Default//6c036bb170fa186a654934e7c2873cd6.png" />
                       ...
                       "<td height='22' align='center'><a href='#' onclick=GetLinkParam1('" + item.ID + "')><span title=\"" + item.CustNo + "\">" + CustNo + "</a></td>" +
                        ...
                        "<td height='22' align='center' style='display: none'><span  style='display: none' title=\"" + item.PYShort + "\">" + PYShort + "</a></td>" +
                        "<td height='22' align='center'>" + item.ContactName + "</a></td>" +
                        "<td height='22' align='center'>" + item.Mobile + "</a></td>" +
                        //"<td height='22' align='center' >" + item.AreaName + "</a></td>" +
                        "<td height='22' align='center' >" + item.ManagerName + "</a></td>" +
                        "<td height='22' align='center'style='display: none'>" + item.CreditGradeName + "</a></td>" +
...
function GetLinkParam1(id) {
    ...
    var flag = "0"; //默认为未点击查询的时候
    if (searchCondition != "") flag = "1"; //设置了查询条件时
    parent.addTab(null,ModuleID,'供应商档案','Pages/JTHY/SysManage/ProviderInfo_Add.aspx?ModuleID='+ModuleID+'&intMasterProviderID='+id+'&'+searchCondition+'&Flag='+flag);
}
...
//    //-----------------------------------医药行业专属字段 2012-09-24--------------------------------------------//
//    if(document.getElementById("searchtable1")!=null)//判断当前系统版本是否为医药版本
//    {
...
//    currentPageIndex = pageIndex;
    ...
    var URLParams = "&Isliebiao="+escape(Isliebiao)+"&CustNo="+escape(CustNo)+"&CustName="+escape(CustName)+
                     
                     "&Manager="+escape(Manager)+"&ManagerName="+escape(ManagerName)+""; 
//---------------------------------------医药行业专用信息 2012-09-24-------------------------------------------------------//
...
//                    "&Certibegin="+reescape(Certibegin)+"&Certiend="+reescape(Certiend)+"&Usebegin="+reescape(Usebegin)+"&Useend="+reescape(Useend)+"&Warbegin="+reescape(Warbegin)+"&Warend="+reescape(Warend)+
//                    "&Powerbegin="+reescape(Powerbegin)+"&Powerend="+reescape(Powerend)+"&Gmspbegin="+reescape(Gmspbegin)+"&Gmspend="+reescape(Gmspend)+
...
//    currentPageIndex = pageIndex;
    var Isliebiao = 1;
    var URLParams = "&Isliebiao="+escape(Isliebiao)+"&ModuleID="+escape(ModuleID)+"&PageIndex=" + currentPageIndex + "&PageCount="+currentpageCount+"&CustNo="+escape(CustNo)+"&CustName="+escape(CustName)+
...
//    return linkParam;
    ...
    parent.addTab(null, '2041101', '新建供应商档案', 'Pages/JTHY/SysManage/ProviderInfo_Add.aspx?ModuleID=555241');
   }
...
//            URLParams+="&CustNo"+(index++)+"="+table.rows[ck[i].value].cells[2].innerText;
            var CustID = table.rows[ck[i].value].cells[3].innerText;
            URLParams+="&CustNo"+(index++)+"="+table.rows[ck[i].value].cells[2].innerText+"&CustID"+(index++)+"="+CustID;
        }       
    }
    if(index == 0)
    {
    ...
    $.ajax({
           type: "POST",//用POST方式传输
           dataType:"json",//数据格式:JSON
           url:  '../../../Handler/JTHY/sysmanage/ProviderInfoInfo.ashx?'+URLParams,//目标地址
           cache:false,
