        ...
        success: function (msg) {
            ...
            $.each(msg.data, function (i, item) {
                if (item.id != null && item.id != "") {
                    ...
                    document.getElementById("hidBillStatus").value = item.billstatusid; //这个是否有问题
                    if (document.getElementById("hidBillStatus").value == "1") {
                        document.getElementById("txtBillStatusName").value = "制单";
                        fnStatus('2'); //是否应该修改成1这个点击保存后的状态
                    }
            ...
            if (typeof (msg.dataAttach) != 'undefined') {
                ...
                $.each(msg.dataAttach, function (i, item) {
                    ...
                    $("#Tb_05").append("<tr class='table-item newrow'><td height='22' align='right'class='td_list_fields' style='width:10%;' >附件名称</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><a href='#' name='aFileName' onclick=\"DealAttachment('download'," + addi + ");\" style=\"cursor: pointer;width:98%;\"> " + item.AnnFileName + "</a></td><td height='22'  class='td_list_fields' align='right' style='width:10%;'>附件说明</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><input type='text' id='txtAnnRemark" + addi + "' class='tdinput' name='txtAnnRemark' value='" + item.AnnRemark + "' style='width:98%' /></td><td height='22'  class='td_list_fields' align='right'style='width:10%;'>上传时间</td><td height='22' bgcolor='#FFFFFF'style='width:20%;'><input type='text' id='txtUpDateTime" + addi + "' name='txtUpDateTime' class='tdinput' value = '" + item.UpDateTime + "' style='width:98%' /></td><td height='22' bgcolor='#FFFFFF'><a href='#' onclick='javascript:DelFileInput(this)' style=\"cursor: pointer;\">删除附件</a><input type='hidden' id='txtaddr" + addi + "' name='txtaddr' value='" + item.AnnAddr + "'  /></td></tr>");
...
function SaveSellOrder() {
    ...
    {     
      ...
      if (headid == "undefined" || headid == "null" || headid == "") { headid = "0";}
      $.ajax({
        type: "POST",
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.sta > 0) {
                    ...
                    // fnStatus($("#ddlBillStatus").val(),$("#Isyinyong").val());
                    //fnFlowStatus($("#FlowStatus").val());
                    GetInfoById(document.getElementById("headid").value); //为其赋值
                    var glb_BillTypeFlag = "30";
                    var glb_BillTypeCode = "2";
                    var glb_BillID = document.getElementById('headid').value;                                //单据ID
                    var glb_IsComplete = false;                                          //是否需要结单和取消结单(小写字母)
