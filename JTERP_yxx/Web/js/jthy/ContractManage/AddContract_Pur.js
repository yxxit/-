        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
                if (item.id != null && item.id != "") {
                    ...
                    document.getElementById("ddlContractID_txtCode").value = item.Contractid;
                    document.getElementById("txtProviderID").value = item.cVenCode;
                    document.getElementById("txtProviderName").value = item.cvenname;
...
//                    GetFlowButton_DisplayControl();
            ...
            //-----------------------加载附件信息----------------------//
            $("#Tb_05 tbody").find("tr.newrow").remove();
            if (typeof (msg.dataAttach) != 'undefined') {
                ...
                $.each(msg.dataAttach, function(i, item) {
                    ...
                    $("#Tb_05").append("<tr class='newrow'><td height='22' align='right'class='td_list_fields' style='width:10%;' >附件名称</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><a href='#' name='aFileName' onclick=\"DealAttachment('download'," + addi + ");\" style=\"cursor: pointer;width:98%;\"> " + item.AnnFileName + "</a></td><td height='22'  class='td_list_fields' align='right' style='width:10%;'>附件说明</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><input type='text' id='txtAnnRemark" + addi + "' class='tdinput' name='txtAnnRemark' value='" + item.AnnRemark + "' style='width:98%' /></td><td height='22'  class='td_list_fields' align='right'style='width:10%;'>上传时间</td><td height='22' bgcolor='#FFFFFF'style='width:20%;'><input type='text' id='txtUpDateTime" + addi + "' name='txtUpDateTime' class='tdinput' value = '" + item.UpDateTime + "' style='width:98%' /></td><td height='22' bgcolor='#FFFFFF'><a href='#' onclick='javascript:DelFileInput(this)' style=\"cursor: pointer;\">删除附件</a><input type='hidden' id='txtaddr" + addi + "' name='txtaddr' value='" + item.AnnAddr + "'  /></td></tr>");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.data, function(i, item) {
                ...
                $("#txtUnitCost" + rowid).val(Number(item.StandardBuy).toFixed(2)); //单价（含税进价）
            });
        },
        complete: function () { } //接收数据完毕
    });
...
function AfterUploadFile(url, docName) {
    if (url != "") {
        ...
        $("#Tb_05").append("<tr class='newrow'><td height='22'  class='td_list_fields' align='right' style='width:10%;'>附件名称</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><a href='#' name='aFileName' onclick=\"DealAttachment('download'," + addi + ");\" style=\"cursor: pointer;width: 98%;\"> " + docName + "</a></td><td height='22'  class='td_list_fields' align='right' style='width:10%;'>附件说明</td><td height='22' bgcolor='#FFFFFF'style='width:20%;'><input type='text' id='txtAnnRemark" + addi + "' class='tdinput' name='txtAnnRemark' style='width:98%' /></td><td height='22'  class='td_list_fields' align='right' style='width:10%;'>上传时间</td><td height='22' bgcolor='#FFFFFF' style='width:20%;'><input type='text' id='txtUpDateTime" + addi + "' name='txtUpDateTime' class='tdinput' value = '" + upDateTime + "' style='width:98%' /></td><td height='22' bgcolor='#FFFFFF'><a href='#' onclick='javascript:DelFileInput(this)' style=\"cursor: pointer;\">删除附件</a><input type='hidden' id='txtaddr" + addi + "' name='txtaddr' value='" + url + "'  /></td></tr>");
...
function deleteAllRow() {
    var signFrame = findObj("dg_Log", document);
    for (i = signFrame.rows.length - 1; i > 0; i--) {
        $(signFrame.rows[i]).remove();
...
function SaveSellOrder() {
    ...
    /////////////////
    if ($("#ddlContractID_txtCode").val() == "" && $("#ddlContractID_ddlCodeRule").val() == "") {
        isFlag = false;
        fieldText = fieldText + "单据编号|";
        msgText = msgText + "合同编号不能为空|"; 
    }

    if (document.getElementById("txtProviderID").value == "") {
        isFlag = false;
        fieldText = fieldText + "单据主体|";
        msgText = msgText + "供应商信息不能为空|";    
    }
    ...
    {   
      ...
      $.ajax({
        ...
        data: strInfo + '&strfitinfo=' + escape(strfitinfo) + '&action=insert&billtype=2&headid=' + escape(headid) + '&isconfirm=' + escape(isconfirm),
...
//    if ($("#ddlContractID_txtCode").val() != "") {
//        ContractID = $("#ddlContractID_txtCode").val();
//    }
    //var ContractID=$("#txtContractID").val();//合同号
    var ProviderID = $("#txtProviderID").val(); //供应商
    ...
    strInfo = 'ContractID=' + escape(ContractID) + '&ContractIDType=' + ContractIDType + '&ProviderID=' + escape(ProviderID) + '&DeliveryAddress=' + escape(DeliveryAddress)
