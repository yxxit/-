...
{   
    ...
    if(treeview_selnode != null)
    {
        ...
        if(selmode == 1)
        {
            ...
            {
                 var obj2= document.getElementById("treeview_checkbox"+treeview_selnodeindex);
                 obj2.checked = false;
             }
         }
    }
    ...
    obj.checked = !obj.checked ;        
      ...
      if(selmode != 1)
      {
           ...
           treeview_allchks2(idx);//checkbox 连动
            ...
            document.getElementById(lastid2).value = treenodes[idx].text;
            document.getElementById(lastid1).value = treenodes[idx].value;
            return;
      }
    ...
    document.getElementById(lastid2).value = userlist;
    document.getElementById(lastid1).value = useridlist
...
function treeview_unsel()
{
   ...
   for(var i=0;i<treenodes.length;i++)
   {
        var chk2 = document.getElementById("treeview_checkbox"+i);
        chk2.checked = false;
   }
...
function treeview_selall()
{
   ...
   function showInfo(msg)
        ...
        function LoadUserList(action,callback)
        {
            $.ajax({ 
                    type: "POST",
                    url: "../../../Handler/Personal/WorkFlow/ReportSendInfo.ashx?action=" + action,
                    dataType:'string',
                    data: '',
                    cache:false,
...
{    
 ...
 function   divposition(o)   
    {   
           if( event.clientX < document.all.selUserBox.offsetWidth ) {
                 document.all.selUserBox.style.left=event.clientX; 
           }else{
                 document.all.selUserBox.style.left=event.clientX -document.all.selUserBox.offsetWidth; 
                    
         }
      document.all.selUserBox.style.top=event.clientY;   
  }