function DelMessage()
{
    if(!confirm("确认删除吗"))
    {
        ...
        //删除
         $.ajax({ 
              type: "POST",
              url: "../../../Handler/Personal/MessageBox/AdviceInput.ashx",
...
//      }
    ...
    function TurnToPage(pageIndex)
    {
           ...
           var selUsers=document.getElementById("txtSenderHidden").value;
...
//            condition += " AND a.Title like '%"+txtTitle+"%'";
              condition += " AND a.DoUserID In ("+selUsers+") AND DisplayName='0'";
           ...
           var fields="a.[ID],a.FromUserID,a.CreateDate,a.Title,a.Content,DisplayName,AdviceType,b.EmployeeName";
          ...
          var prams = "Fields="+fields;
           ...
           url:  '../../../Handler/Personal/MessageBox/AdviceInput.ashx',//目标地址
           cache:false,
           ...
           //return;//"[ID],SendUserID,CreateDate,ReceiveUserID,Title,Content,Status
                    ...
                    $.each(result.data.list,function(i,item){
                        ...
                        $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+"<input id='Checkbox1' name='Checkbox1'  value="+item.ID+"  type='checkbox'/>"+"</td>"+
                         "<td height='22' align='center'>"+(item.AdviceType == "1"?"建议":"意见")+"</td>"+
                       "<td height='22' align='center'><a href=\"#\" onclick=\"viewMsg("+item.ID+")\" title=\"点击查看详细\">"+item.Title+"</a></td>"+          
                        "<td height='22' align='center'><div style='width:350px;height:30px;border:0px;overflow:hidden;text-overflow:ellipsis'>"+item.Content +"</div></td>"+
                                  
                        "<td height='22' align='center'>"+date2str(str2date(item.CreateDate),"yyyy-MM-dd HH:mm:ss")+"</td>"+        
                        "<td height='22' align='center'>"+(item.DisplayName == "0"?"否":"是")+"</td>"+
                        "<td height='22' align='center'>"+ (item.DisplayName == "1" ?"&nbsp;":item.EmployeeName)+"</td>").appendTo($("#pageDataList1 tbody"));
                        }
                   });
                    //页码
