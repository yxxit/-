...
objFlow.Fun_Show = function(isSure,billID)
{
    ...
    if(isSure)
    {
        ...
        document.getElementById('divApplayToWait').style.display='none';//提交审批时，显示“选择下一步接收人“,审批时隐藏 2014-04-13 bsd new Add 
        //2014-04-15 审批界面，清空下一步接收人
        document.getElementById("FlowApply1_txtNextPersonID").value="";
        document.getElementById("FlowApply1_txtNextPersonName").value="";
            
        document.getElementById('trApprovalStep').style.display='none';//审批步骤隐藏, 2014-04-15 bsd new add
    ...
    {
        document.getElementById('divApplayToWait').style.display='';//提交审批时，显示“选择下一步接收人”,审批时隐藏 2014-04-13 bsd new Add  
        document.getElementById('trApprovalStep').style.display='none';//审批步骤隐藏, 2014-04-15 bsd new add
...
function ShowFlowObject(isSure)
{
   ...
   //暂取消 document.getElementById('trApprovalStep').style.display = isSure?'':'none';
...
objFlow.Fun_Hidden =function()
{
   ...
   //暂取消  document.getElementById('trApprovalStep').style.display = objFlow.IsSure?'':'none';
...
objFlow.Fun_Save_FlowApply =function()
{
        ...
        //获取下一步骤接收人ID
        var nextperson=document.getElementById("FlowApply1_txtNextPersonID").value;
        ...
        var UrlParam = "Action="+Action+"&BillTypeFlag="+objFlow.BillTypeFlag+"&BillTypeCode="+objFlow.BillTypeCode+"&BillID="+objFlow.BillID+"&BillNo=" + objFlow.BillNo
         + "&PageUrl=" + escape(objFlow.PageUrl) + "&MsgRemind=" + MsgRemind + "&FlowNo="+escape(objFlow.FlowNo)+"&nextperson="+escape(nextperson)+"";
        $.ajax({ 
                  type: "POST",
                  url: "../../../Handler/Common/Flow.ashx?"+UrlParam,
                  dataType:'json',//返回json格式数据
...
objFlow.Fun_Save_FlowApproval = function() {
    ...
    var UrlParam = "Action=" + Action + "&BillTypeFlag=" + objFlow.BillTypeFlag + "&BillTypeCode=" + objFlow.BillTypeCode + "&BillID=" + objFlow.BillID + "&BillNo=" + objFlow.BillNo + "&FlowNo=" + objFlow.FlowNo + "&State=" + State + "&MsgRemind=" + MsgRemind + "&Note=" + escape(objFlow.Note) +"&nextperson="+escape(nextperson)+ "";
    $.ajax({
        type: "POST",
        url: "../../../Handler/Common/Flow.ashx?" + UrlParam,
        dataType: 'json', //返回json格式数据
