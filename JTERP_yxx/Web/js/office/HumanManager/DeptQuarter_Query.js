...
function GetInfoByDeptID(DeptID)
{
   //判断组织机构ID输入时，判断样式
//    if (quarterID != "")
//    {
...
function DoSaveInfo()
{
    ...
    $.ajax({ 
        ...
        //    hidePopup();
            ...
            { 
                hidePopup();
                showPopup("../../../Images/Pic/Close.gif", "../../../Images/Pic/note.gif", "保存失败,岗位编码流水号长度不够，请增加流水号长度！");
            }
        } 
    });  
}
...
//        document.getElementById("hidEditFlag").value = "UPDATE";
  ...
  //  ShowReclickDeptDiv();
    ...
    document.getElementById("divEditDeptQuarter").style.display = "block";
             if(document.getElementById("codeRule_ddlCodeRule")[document.getElementById("codeRule_ddlCodeRule").selectedIndex].text == "手工输入")
         {
            document.getElementById("codeRule_txtCode").style.display = "block";
            document.getElementById("codeRule_txtCode").value = "";
            document.getElementById("codeRule_txtCode").disabled = false;     
         }
    return true;
}
