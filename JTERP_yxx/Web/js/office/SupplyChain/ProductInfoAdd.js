...
//            document.getElementById('product_btnback').style.display = 'block';
            ...
            //
            document.getElementById("CodingRuleControl1_txtCode").style.display = 'none';
            document.getElementById("CodingRuleControl1_txtCode").value =$.trim($("#divNo").val());
            OldNo= $.trim($("#divNo").val());
        ...
        $("#txt_StandardCost").val("1.00");
        $("#txt_Size").val("**");
        document.getElementById('chk_sale').checked=true;
        document.getElementById('chk_purchase').checked=true;
        document.getElementById('chk_consume').checked=true;
        document.getElementById('chk_self').checked=true;
...
function DealEmployeePhoto(flag) {
    ...
    //清除相片
        ...
        document.getElementById("imgPhoto").width = 200;
        document.getElementById("imgPhoto").height = 140;
    }
}
...
function AfterUploadFile(url) {
    ...
    if ("PHOTO" == kind) {
        if (url != "") {
            ...
            setTimeout('DrawImage(220, 165)', 0.001);
        }
    }
}
function OpenImage() {
    window.open(document.getElementById("imgPhoto").src);
}
function Fun_Save_ProductInfo() {
    ...
    var Manufacturer=$("#txtManufacturer").val();      //厂家
   ...
   var bpurchase=0;
   if(document.getElementById('chk_purchase').checked)
   {
       bpurchase=1;
   }
   var bconsume=0;
   if(document.getElementById('chk_consume').checked)
   {
       bconsume=1;
   }
   var baccessary=0;
    if(document.getElementById('chk_accessary').checked)
   {
       baccessary=1;
   }
   var bself=0;
     if(document.getElementById('chk_self').checked)
   {
       bself=1;
   }
   var bproducing=0;
     if(document.getElementById('chk_producing').checked)
   {
       bproducing=1;
   }
   var bservice=0;
     if(document.getElementById('chk_service').checked)
   {
       bservice=1;
   }
...
//    }
    ...
    /*长宽高全部输入或者全部不输入*/
    var txtLength=$("#txt_Length").val();
    var txtWidth=$("#txt_Width").val();
    var txtHeight=$("#txt_Height").val();
    if(!((txtLength=="" && txtWidth=="" && txtHeight=="") || (txtLength!="" && txtWidth!="" && txtHeight!=""))){
        isFlag = false;
        fieldText += "尺寸|";
        msgText += "请按规范输入长宽高或者全部不输入!|";
    }
    ...
    if (StayStandard != "") {
        if (!IsNumeric(StayStandard)) {
            isFlag = false;
            fieldText += "呆滞积压标准|";
            msgText += "呆滞积压标准格式不对|";
        }
        else {
            if (StayStandard.indexOf('.') > -1) {
                if (!IsNumeric(StayStandard, 8, glb_SelPoint)) {
                    isFlag = false;
                    fieldText += "呆滞积压标准|";
                    msgText += "呆滞积压标准格式不对|";
                }
            }
            else if (StayStandard.indexOf('.') == -1) {
                if (strlen(StayStandard) > 8) {
                    isFlag = false;
                    fieldText += "呆滞积压标准|";
                    msgText += "呆滞积压标准整数部分不能超过8位|";
                }
            }
        }
    }
    ...
    if (StandardCost != "") {
        if (!IsNumeric(StandardCost)) {
            ...
            if (StandardCost.indexOf('.') > -1) {
                ...
                else if(parseFloat(StandardCost)<=0.00)
                {
                    isFlag = false;
                    fieldText += "标准成本|";
                    msgText += "标准成本大于0.00|";
    ...
    $.ajax({
        ...
        data:'OldNo='+escape(OldNo)+
         ...
         '&ProductUnit='+escape(ProductUnit)+'&IsBatchNo='+escape(IsBatchNo)+'&Action='+Action+GetExtAttrValue(),
        dataType: 'json', //返回json格式数据
        cache: false,
        beforeSend: function() {
        },
...
function ChangeSizeValue() {
    var valCombined = $("#txt_Length").val() + "*" +$("#txt_Width").val() + "*" + $("#txt_Height").val();
    $("#txt_Size").val(valCombined);
}

//选择供应商
function fnSelectProvider() {
        popProviderObj.ShowList('all');
}

//选择纸号、耐磨纸、平衡纸、基材
function fnSelectP(id)
{
    lmIfDel = "0";
