...
function fnproductinfo(id) {
    ...
    var flag = "0"; //默认为未点击查询的时候
    if (searchCondition != "") flag = "1"; //设置了查询条件时
    window.parent.addTab(null, ModuleID, '物品档案', 'Pages/Office/SupplyChain/ProductInfoAdd.aspx?ModuleID=' + ModuleID + '&intOtherCorpInfoID=' + escape(id) + "&" + searchCondition + "&Flag=" + flag);
}
...
//           var BarCode=document.getElementById("txt_BarCode").value;
           ...
           //-----------------------医药行业专用属性 2012-09-26-------------------------------// 
//           var a=document.getElementById("txt_FileNo");
//           var b=document.getElementById("Qualitystandard")
//           var c=document.getElementById("MedCheckNo");
...
//            var MedCheckNo=document.getElementById("MedCheckNo").value;
          ...
          //-----------------------医药行业专属字段 2012-09-26 --------------------------//
          search+="&ValidityBegin="+escape(ValidityBegin);
          search+="&ValidityEnd="+escape(ValidityEnd);
          search+="&MedFileDateBegin="+escape(MedFileDateBegin);
          search+="&MedFileDateEnd="+escape(MedFileDateEnd);
          search+="&MedCheckDateBegin="+escape(MedCheckDateBegin);
          search+="&MedCheckDateEnd="+escape(MedCheckDateEnd);
          search+="&FileNo="+escape(FileNo);
          search+="&Qualitystandard="+escape(Qualitystandard);
          search+="&MedCheckNo="+escape(MedCheckNo);
          //-------------------------------------------------------------//
...
function ScanBarCodeSearch()
{
    ...
    //检测是否以选择并生成拼音码
    function CreatePYShort()
    {
        var c = window.confirm("是否生成拼音缩写？");
        if(c==true)
        {
            var ck = document.getElementsByName("Checkbox1");
            var x=Array(); 
            ...
            x=ck2;
            var str2 = ck2.substring(0,ck2.length-1);
            if(x.length-1<1)
                showPopup("../../../Images/Pic/Close.gif","../../../Images/Pic/note.gif","至少要选择一项！");
            else
            {
                var UrlParam ='';
                var UrlParam = str2+"\
                ...
                $.ajax({ 
                    type: "POST",
                    url:  '../../../Handler/Office/SupplyChain/ProductInfoAdd.ashx?str2='+UrlParam,//目标地址
                    dataType:'json',//返回json格式数据
                    cache:false,
                    beforeSend:function()
                    { 
