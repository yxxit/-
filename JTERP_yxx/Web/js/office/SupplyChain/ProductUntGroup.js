...
function TurnToPage(pageIndex) {
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
                $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + " <input type=\"radio\" name=\"radioTech\" id=\"radioTech_" + item.ID + "\" value=\"" + item.GroupUnitNo + "\" onclick=\"Fun_FillParent_Content('" + item.GroupUnitName + "','" + item.GroupUnitNo + "');closeProductUnitdiv();\" />" + "</td>" +
                    ...
                    "<td height='22' align='center'>" + item.Remark + "</td>").appendTo($("#pageDataList2 tbody"));
            });
            //页码
            ShowPageBar("pageDataList2_Pager", //[containerId]提供装载页码栏的容器标签的客户端ID
            "<%= Request.Url.AbsolutePath %>", //[url]
            {style: pagerStyle, mark: "pageDataList2Mark",
        ...
        complete: function() {
            hidePopup();
            $("#pageDataList2_Pager").show();
            Ifshow(document.getElementById('Text4').value);
            pageDataList2("pageDataList2", "#E7E7E7", "#FFFFFF", "#cfc", "cfc");
...
//table行颜色
function pageDataList2(o, a, b, c, d) {
...
function closeProductUnitdiv() {
    $("#DivUnitGroup").hide();
     closeRotoscopingDiv(false,"divBackShadow")//关闭遮罩层
}
/*********************************显示组层**************************************/
function ShowUnitGroup() {
    openRotoscopingDiv(false,"divBackShadow","BackShadowIframe")//弹遮罩层
    $("#DivUnitGroup").show();
    SearchData();
}