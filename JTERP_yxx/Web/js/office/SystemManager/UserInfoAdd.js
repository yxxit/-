function CheckInput()
{
    ...
    if(txtUserID!="")
    {
       if(txtUserID.length>20)
    {
        isFlag = false;
        fieldText = fieldText + "用户名|";
   		msgText = msgText + "用户名长度必须在6-20位之间|";
    }
    if(txtUserID.length<6)
    {
      isFlag = false;
        fieldText = fieldText + "用户名|";
   		msgText = msgText + "用户名长度必须在6-10位之间|";
    }
 // if(!txtUserID.match(/^[a-zA-Z]+[\d]+([a-zA-Z0-9])*$|^[\d]+[a-zA-Z]+([a-zA-Z0-9])*$/))
  if(!txtUserID.match(/^[0-9a-zA-Z]+$/))
   {
   isFlag = false;
    fieldText = fieldText + "用户名|";
	msgText = msgText + "用户名必须是字母或者数字的组合|";
     }
    }
