...
function BuildSubNodes(node,tb)
{
    ...
    {
       tr.insertCell(-1).innerHTML = "<a "+expx+" onmouseover=\"treeview_onmove("+(treenodes.length-1)+",1)\" onmouseout=\"treeview_onmove("+(treenodes.length-1)+",0)\" id=\"treeview_node"+(treenodes.length-1)+"\" style=\"color:black;font-size:14px;\" href=\"javascript:treeview_onselnode("+(treenodes.length-1)+");\">◇"+node.CodeName+"</a>";
    }
    curLevel++;
   if(node.SubNodes.length > 0)
    {
