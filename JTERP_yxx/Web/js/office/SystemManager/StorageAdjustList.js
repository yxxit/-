...
//      document.getElementById('ToPage').value=currentPageIndex;
    ...
    var MyData = "pageIndex="+pageIndex+"&pageCount="+pageCount+"&action="+action+"&orderBy="+orderBy+
                     "&txtAdjustNo="+escape(txtAdjustNo)+"&txtTitle="+escape(txtTitle)+"&StorageID="+escape(StorageID)+"&Executor="+Executor+"&Reason="+Reason+"&ProductName="+escape(document.getElementById("txtProductName").value)+
    ...
    $.ajax({
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        if(testApplyNo!=null)
                        {
                            if(testApplyNo.length>20)
                            {
                                testApplyNo=testApplyNo.substring(0,20)+'...';
                            }
                        }
                        if(parseFloat( item.AdjustCount)!=0&&parseFloat( item.AdjustCount).toString()!="NaN")
                        {
                            total1+=parseFloat( item.AdjustCount);
                        }
                        if(parseFloat( item.CostPriceTotal)!=0&&parseFloat( item.CostPriceTotal).toString()!="NaN")
                        {
                            total2+=parseFloat( item.CostPriceTotal);
                        }
                        ...
                        "<td height='22' align='center'>" + parseFloat(item.AdjustCount).toFixed(2) + "</td>"+
                        "<td height='22' align='center'>" + parseFloat(item.CostPriceTotal).toFixed(2) + "</td>"+
...
//                        "<td height='22' align='center'><span title=\""+item.BatchNo+"\">" + BatchNo + "</td>"+
                     ...
                     $("<tr class='newrow'></tr>").append("<td height='22' align='center'>合计</td>"+
                        ...
                        "<td height='22' align='center'></td>"+
                        "<td height='22' align='center'>" + parseFloat(total1).toFixed(2) + "</td>"+
                        "<td height='22' align='center'>" + parseFloat(total2).toFixed(2) + "</td>"+
                        "<td height='22' align='center'></td>"+
//                        "<td height='22' align='center'><span title=\""+item.BatchNo+"\">" + BatchNo + "</td>"+
                      "<td height='22' align='center'></td>"+
                        "<td height='22' align='center'></td>"+
