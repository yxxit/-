...
{
    var storage=document.getElementById("ddlInStorage").value;
    if(parseInt(storage)>0)
    {
        popTechObj.ShowListCheckSpecial12('SubStorageIn"+rowID+"', 'Check',storage);
    }else
    {
         popMsgObj.ShowMsg('请选择调整仓库！');
    }
}
function GetValue() {
            ...
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                if (!IsExist(item.ProdNo)) {
                    //AddRow(id,ProNo,ProdName,UnitName,Specification,UnitPrice)
                     ...
                     Fun_FillParent_Content1(item.ID,item.ProdNo,item.ProductName,item.StandardSell,item.UnitID,item.CodeName,item.TaxRate,item.SellTax,item.Discount,item.Specification,item.CodeTypeName,item.TypeID,item.StandardCost,"",item.GroupUnitNo,item.SaleUnitID,item.SaleUnitName,item.InUnitID,item.InUnitName,item.StockUnitID,item.StockUnitName,item.MakeUnitID,item.MakeUnitName,item.IsBatchNo,item.BatchNo,rowid);
...
//        colNum.innerHTML = "<input type=\"text\"  style='width:60%' id=\"TD_Num_"+rowID+"\" value=\""+rowID+"\"  class=\"tdinput\"  readonly />";
        ...
        colProductNo.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" readOnly onfocus=\"popTechObj.ShowListSpecial('"+rowID+"','d');\" id='TD_ProNo_"+rowID+"' name='TD_ProNo_"+rowID+"' class=\"tdinput\" /></td></tr></table>";
        
        var colProductName=newTR.insertCell(2);//添加列:物品名称
        colProductName.className="cell";
        colProductName.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\"   readOnly  id=\"TD_Pro_"+rowID+"\" class=\"tdinput\"    /><input id=\"TD_ProID_"+rowID+"\" type=\"hidden\" ></td></tr></table>";
...
/*选择物品控件物品 赋值*/
                            ...
                            ,Source,GroupUnitNo,SaleUnitID,SaleUnitName,InUnitID,InUnitName, StockUnitID,StockUnitName,MakeUnitID,MakeUnitName,IsBatchNo,BatchNo)
 {
  ...
  function Fun_FillParent_Content1(id,prodNo,prodName,standardSell,unitID,codeName,taxRate,sellTax,discount,specification,codeTypeName,typeID,StandardCost
                            ,Source,GroupUnitNo,SaleUnitID,SaleUnitName,InUnitID,InUnitName, StockUnitID,StockUnitName,MakeUnitID,MakeUnitName,IsBatchNo,BatchNo,rowid)
 {
    if(taxRate!="")taxRate=parseFloat(taxRate).toFixed($("#HiddenPoint").val());
    if(sellTax!="")sellTax=parseFloat(sellTax).toFixed($("#HiddenPoint").val());
    if(discount!="")discount=parseFloat(discount).toFixed($("#HiddenPoint").val());
    ...
    if(rowid!=null)
    {
       document.getElementById('TD_ProNo_'+rowid).value=prodNo;
       document.getElementById('TD_Pro_'+rowid).value=prodName;
       document.getElementById('TD_ProID_'+rowid).value=id;
        //绑定批次
        var ListControlID="BatchNo_SignItem_TD_Text_"+rowid;
        var StorageControlID="ddlInStorage";
        GetBatchList(id,StorageControlID,ListControlID,IsBatchNo,BatchNo);
        ...
        if($("#HiddenMoreUnit").val()=="True")
        {
           ...
           $("#" + BaseUnitControl).attr("title", unitID);
           $("#"+BasePriceControl).val(StandardCost);
           GetUnitGroupSelectEx(id,"StockUnit", "TD_UnitID_" + rowid,"ChangeUnit(this.id,"+rowid+","+StandardCost+")","unitdiv" + rowid,'',"FillContent("+rowid+","+StandardCost+")");
        }//
        else
        {
           document.getElementById('TD_Unit_'+rowid).value=codeName;
           document.getElementById('TD_UnitID_'+rowid).value=unitID;
           document.getElementById('TD_CostPrice_'+rowid).value=StandardCost;
           document.getElementById('divStorageProduct').style.display='none';
        }
   }
...
function Fun_ConfirmOperate()
{
    ...
    var UrlParam = "myAction=Confirm&ID="+hiddeniqreportid.toString()+"";
    ...
    $.ajax({ 
                  ...
                  { 
                   ...
                   complete: function() { LoadAdjustInfo(hiddeniqreportid); } //接收数据完毕 
               });
...
function Fun_UnConfirmOperate()
{
    ...
    var hiddeniqreportid = document.getElementById('hiddenAdjustID').value;
     var UrlParam = "myAction=UnConfirm&ID="+hiddeniqreportid.toString()+"";
    ...
    $.ajax({ 
                  ...
                  complete: function() {  location.reload(true); } //接收数据完毕 
               });
}
