...
$(document).ready(function() {
    ...
    get_Table_Cell_add(true,false,'officedba.StorageOutOtherDetail',null,'dg_Log','Custom_div');
});
//是否显示单价金额
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
             ...
             get_Table_Cell_add(false,true,'officedba.StorageOutOtherDetail',msg.data,'dg_Log');
...
function Fun_Save_StorageOutOther() {
    ...
    var txtOtherCorpName=$("#txtOtherCorp").val();//20121122 添加 往来单位
    ...
    if (!isFlag) {
        ...
        var UrlParam = "&txtOutNo="+escape(txtOutNo)+"&txtTitle="+ escape(txtTitle)+"&ddlFromType="+escape(ddlFromType)+"&txtFromBillID="+escape(txtFromBillID)+"&sltCorpBigType="+escape(sltCorpBigType)+"&txtOtherCorpID="+escape(txtOtherCorpID)+"&ddlReason="+ escape(ddlReason)+"&txtSendAddress="+escape(txtSendAddress)+"&txtArrAddress="+escape(txtArrAddress)+"&txtSender="+escape(txtSender)+"&txtTotalPrice="+escape(txtTotalPrice)+"&txtTotalCount="+escape(txtTotalCount);
         ...
         UrlParam+="&HiddenProjectID="+escape(HiddenProjectID);
                     ...
                     UrlParam+="&txtOtherCorpName="+escape(txtOtherCorpName);//20121122 添加 往来单位
...
//通过销售退货单带出明细
//  AddSignRow(i,'',item.ID, item.ProdNo, item.ProductName,item.Specification,item.CodeName,item.UnitID/*单位*/, item.StandardCost/*基本单价*/,'','','','','','',item.IsBatchNo/*是否启用批次*/,'','',batcharray[i],item.ColorName,item.StorageID);
               
function AddSignRow(i, RejectNo, ProductID, ProductNo
        , ProductName, Specification, UnitID,HiddenUnitID,UnitPrice
        , BackCount, NotOutCount, FromLineNo, Remark, OutedTotal,StorageID
        ,IsBatchNo,UsedUnitID,UsedPrice,BatchNo,ColorName) 
    ...
    newFitNametd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"ProductNo_SignItem_TD_Text_" + rowID + "\" value=\"" + ProductNo + "\" title=\"" + ProductID + "\" class=\"tdinput\"  readonly=\"readonly\" style='width:90%'  /></td></tr></table>"; //添加列内容
    ...
    newBatchNotd.innerHTML ="<input  type='text' style=' width:90%;' value=\"\" class=\"tdinput\" readonly=\"readonly\"   id='BatchNo_SignItem_TD_Text_"+rowID+"' onclick=\"popBatchObj.ShowListCheckSpecial('BatchNo_SignItem_TD_Text_"+rowID+"','"+ProductNo+"')\"  />";//添加列内容
    ...
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='Specification_SignItem_TD_Text_" + rowID + "' value=\"" + Specification + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:90%' /></td></tr></table>";//添加列内容
    ...
    newFitNametd.innerHTML = "<select  id=\"StorageID_SignItem_TD_Text_" + rowID + "\"   style=' width:90%;'>" + document.getElementById("ddlStorageInfo").innerHTML + "</select>";
    document.getElementById("StorageID_SignItem_TD_Text_" + rowID).value = StorageID; //赋值当前的的仓库
    
    /*绑定批次*/
  // GetBatchList(ProductID,StorageControlID,ListControlID,IsBatchNo);onchange=\"GetBatchList('"+ProductID+"','"+StorageControlID+"','"+ListControlID+"','"+IsBatchNo+"')\"
...
//        GetBatchList(ProductID,StorageControlID,ListControlID,IsBatchNo);
     ...
     if($("#HiddenMoreUnit").val()=="False")
        {
            ...
            newFitNametd.className = "tdinput";
            newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
            newFitNametd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NotOutCount + "\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice("+rowID+");CountNum();\"/>"; //添加列内容
        }
        else
        {
            if(RejectNo=="")
            {
                ...
                newFitNametd.className="tdinput";
                newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
                newFitNametd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_"+rowID+"\" value=\"1.00\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice_MoreUnit(this.id,"+rowID+");CountNum();\"/>";//添加列内容
            }
            else
            {
                var newFitNametd=newTR.insertCell(14);//添加列:出库数量
                newFitNametd.className="tdinput";
                newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
                newFitNametd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_"+rowID+"\"  value=\"" + NotOutCount + "\" class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice_MoreUnit(this.id,"+rowID+");CountNum();\"/>";//添加列内容
            }
        }
        ...
        if($("#HiddenMoreUnit").val()=="False")
        {
            if(RejectNo=="")
            {
                ...
                newFitDesctd.innerHTML = "<input name='chk' id='UnitPrice_SignItem_TD_Text_" + rowID + "' value=\"" + UnitPrice + "\" type='text' class=\"tdinput\" onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice("+rowID+");CountNum();\"  style='width:90%' />";
            }//添加列内容
            else
            {
                var newFitDesctd = newTR.insertCell(15); //添加列:单价
                newFitDesctd.className = "cell";
                newFitDesctd.id = 'SignItem_TD_UnitPrice_' + rowID;
                newFitDesctd.innerHTML = "<input name='chk' id='UnitPrice_SignItem_TD_Text_" + rowID + "' value=\"" + UnitPrice + "\" type='text' class=\"tdinput\"  style='width:90%' readonly=\"readonly\" />";
            }
        }
        else
        {
            if(RejectNo=="")
            {
                ...
                newFitDesctd.className="cell";
                newFitDesctd.id = 'SignItem_TD_UnitPrice_'+rowID;
                newFitDesctd.innerHTML = "<input type='hidden' value=\""+UnitPrice+"\" id='baseprice_td"+rowID+"'><input type='hidden' id='BaseExRate"+rowID+"'><input name='chk' id='UnitPrice_SignItem_TD_Text_"+rowID+"'  type='text' class=\"tdinput\"  onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice_MoreUnitByPrice(this.id,"+rowID+");CountNum();\" style='width:90%' />";//添加列内容
            }
            else
            {
                var newFitDesctd=newTR.insertCell(15);//添加列:出库单价(根据基本单价计算,隐藏基本单价，比率)
    ...
    get_Table_Cell_add_1(false,false,'officedba.StorageOutOtherDetail',null,'dg_Log');
}
...
function FillSignRow(i, RejectNo, ProductID, ProductNo
        ...
        ,BatchNo,UsedUnitID,UsedUnitCount,UsedPrice,ExRate,IsBatchNo,ColorName) 
        { //读取最后一行的行号，存放在txtTRLastIndex文本框中 
        if(UsedPrice=="")UsedPrice="0.00";
        ...
        newBatchNotd.className="cell";
        newBatchNotd.id = 'SignItem_TD_BatchNo_'+rowID;
        newBatchNotd.innerHTML ="<input  type='text' style=' width:90%;' value=\""+BatchNo+"\" class=\"tdinput\" readonly=\"readonly\"   id='BatchNo_SignItem_TD_Text_"+rowID+"' onclick=\"popBatchObj.ShowListCheckSpecial('BatchNo_SignItem_TD_Text_"+rowID+"','"+ProductNo+"')\"  />";//添加列内容
       // newBatchNotd.innerHTML ="<select style=' width:90%;' id='BatchNo_SignItem_TD_Text_"+rowID+"' />";//添加列内容
    ...
    document.getElementById("StorageID_SignItem_TD_Text_" + rowID).value = StorageID; //赋值当前的的仓库
     ...
     /*绑定批次*/
    //GetBatchList(ProductID,StorageControlID,ListControlID,IsBatchNo,BatchNo);onchange=\"GetBatchList('"+ProductID+"','"+StorageControlID+"','"+ListControlID+"','"+IsBatchNo+"')\"
...
function Fun_FillParent_Content(id, ProNo, ProdName, a, UnitID, UnitName, b, c, d, Specification, a1, a2, a3, a4, a5, UnitPrice,j,k,m,n,o, p,q,r,s, IsBatchNo,ColorName,StorageID) {
    ...
    var oStorageID = 'oStorageID' + idstr;
    document.getElementById(ProductNo).value = ProNo;
    document.getElementById(oStorageID).value = StorageID;
...
function UnConfirmBill() {
    ...
    if (confirm('确认要进行取消确认操作吗？')) {
        var txtIndentityID = $("#txtIndentityID").val();
        var UrlParam = "&Act=unconfirm&ID="+txtIndentityID.toString()+"";
        $.ajax({
            type: "POST",
            url: "../../../Handler/Office/StorageManager/StorageOutOtherInfo.ashx",
            data:UrlParam,
            dataType: 'json', //返回json格式数据
            cache: false,
            beforeSend: function() {
...
function CancelCloseBill() {
    ...
    if (confirm('确认要进行取消结单操作吗？')) {
        var txtIndentityID = $("#txtIndentityID").val();
        var UrlParam = "&Act=CancelClose&ID="+txtIndentityID.toString()+"";
        $.ajax({
            type: "POST",
            url: "../../../Handler/Office/StorageManager/StorageOutOtherInfo.ashx",
            data:UrlParam,
...
function DoChange() {
    ...
    if (InType != "0") {
        ...
        $("#txtFromBillID").attr("title", "");
        fnDelRow(); //删除明细中的行
        document.getElementById('AddNewRow').onclick = ShowProdInfo; //增加明细行按钮事件
        document.getElementById('txtOtherCorp').onclick = alerTreediv;
        //closePRdiv();
        $("#txtOtherCorp").val("");
        $("#txtOtherCorpID").val("");
        ...
        success: function(msg) {
            ...
            if (msg.PRList != null) {
                ...
                $.each(msg.PRList, function(i, item) {
                    AddSignRow(i, item.RejectNo, item.ProductID, item.ProdNo,
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
                ...
                {
                var td="";
                var strBackCount = parseFloat(item.BackCount).toFixed($("#HiddenPoint").val());
                var strOutedTotal = parseFloat(item.OutedTotal).toFixed($("#HiddenPoint").val());
                if($("#HiddenMoreUnit").val()=="True")
                {
                    var strProductCount1 = parseFloat(item.BackCount).toFixed($("#HiddenPoint").val());
...
//                if(parseFloat(strBackCount)>parseFloat(strOutedTotal))
//                {
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + " <input type=\"Checkbox\" name=\"ck\" id=\"ckPR_" + index + "\" value=\"" + item.ID + "\" />" + "</td>" +
                            ...
                            td+
                            "<td height='22' align='center'>" + strBackCount + "</td>" + //发货数量
                            "<td height='22' align='center'>" + strOutedTotal + "</td>" + //已出库数量
   ...
   var ID = $("#txtIndentityID").val();
    if (ID == "" || ID == "0") {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageOutOtherPrint.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
...
function GetGoodsDataByBarCode(id, ProNo, ProdName,
    ...
    if (!IsExist(ProNo))//如果重复记录，就不增加
    {
        ...
        AddSignRow('','',id,ProNo,ProdName,Specification,UnitName,UnitID,UnitPrice,'','','','','','',IsBatchNo);
    }
    else {
...
//            for(var j=0;j<batcharray.length-1;j++){
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                ...
                /*次方法已统一到添加行：2010.4.12 */
                    AddSignRow(i,'',item.ID, item.ProdNo, item.ProductName,item.Specification,item.CodeName,item.UnitID/*单位*/, item.StandardCost/*基本单价*/,'','','','','',item.StorageID,item.IsBatchNo/*是否启用批次*/,'','',batcharray[i],item.ColorName,item.StorageID);
                //}
                
            });
            //get_Table_Cell_add(false,false,'officedba.StorageOutOtherDetail',msg.data,'dg_Log');
//            }
