...
$(document).ready(function()
{
    ...
    GetField("Tb_01", "2051301", "0");
    GetField("dg_Log", "2051301", "1");
    get_Table_Cell_add(true,false,'officedba.StorageOutSellDetail',null,'dg_Log','Custom_div');
...
//                        {
                            ...
                            ,item.PProductName,item.Specification,item.Package,item.UnitID,item.HiddenUnitID,item.UnitPrice,item.B_TotalPrice
                            ,item.FromBillCount,item.NotOutCount,item.ProductCount,item.FromBillNo
                            ,item.FromLineNo,item.DetaiRemark,item.StorageID,item.MinusIs,item.UseCount,item.OutCount,item.BatchNo,item.UsedUnitID,item.UsedUnitCount,item.UsedPrice,item.ExRate,item.IsBatchNo,CustID,item.packages);
//                        }
               });
               get_Table_Cell_add(false,true,'officedba.StorageOutSellDetail',msg.data,'dg_Log');
...
//    }
    ...
    else
    {
        ...
        var UrlParam = "&txtOutNo="+escape(txtOutNo)+
                        ...
                        "&Act=edit&bmgz="+bmgz+
                        "&ID="+txtIndentityID.toString()+GetExtAttrValue()+GetTableCustom("dg_Log");
                        //alert(UrlParam);   
...
/*带出基本数量，比率，基本单价，算出数量，单价，数量手工修改时，改变基本数量*/
 function AddSignRow(i,SendNo,ProductID,ProductNo
        ...
        ,IsBatchNo,UsedUnitID,UsedPrice,CustID,pack)
 { //读取最后一行的行号，存放在txtTRLastIndex文本框中 
        
        //edit 20110426 lcb 物品别名不要
//         //------钱锋锋 2010-11-01 物品别名
//        var str='';
        var ProdAlias=' ';
//        var serch="CustID="+CustID+"&ProdNo="+ProductNo;
//        var ActionFlag="GetProd"
//        $.ajax({
//        type: "POST",//用POST方式传输
//        dataType:"json",//数据格式:JSON
//        url:  '../../../Handler/Office/SupplyChain/CustProduct.ashx?str='+str,//目标地址
//        cache:false,
//        data: "ActionFlag="+escape(ActionFlag)+"&" + serch,//数据
//        beforeSend:function(){},//发送数据之前         
//        success: function(msg){  
//        $.each(msg.data,function(i,item){
//        if(item.ProdAlias!=undefined)
//        {
//            ProdAlias=item.ProdAlias;
//        }
//                //---end 
//        //alert(UsedPrice);
        ...
        var newFitDesctd=newTR.insertCell(3);//添加列:物品别名
        newFitDesctd.className="cell";
        //newFitDesctd.style.display="none"
        newFitDesctd.id = 'SignItem_TD_ProductName_'+rowID;
        newFitDesctd.innerHTML ="<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ProductName_SignItem_TD_Text_"+rowID+"' value=\""+ProductName+"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:90%' /></td></tr></table>";;//添加列内容
        ...
        newFitDesctd.className="cell";
        newFitDesctd.style.display="none"
        newFitDesctd.id = 'SignItem_TD_ProductAlias_'+rowID;
        newFitDesctd.innerHTML ="<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ProductAlias_SignItem_TD_Text_"+rowID+"' value=\""+ProdAlias+"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:95%' /></td></tr></table>";;//添加列内容
        //-----end
        
        var newBatchNotd=newTR.insertCell(5);//添加列:批次
        newBatchNotd.className="cell";
        newBatchNotd.id = 'SignItem_TD_BatchNo_'+rowID;
        newBatchNotd.innerHTML ="<table border='0' cellspacing='0' cellpadding='0' width='100%'><input  type='text' class=\"tdinput\" readonly=\"readonly\"  style='width:80%;' id='BatchNo_SignItem_TD_Text_"+rowID+"' onclick=\"popBatchObj.ShowListCheckSpecial('BatchNo_SignItem_TD_Text_"+rowID+"','"+ProductNo+"')\"  /></td></tr></table>";//添加列内容
        
        var newFitDesctd=newTR.insertCell(6);//添加列:物品规格
        newFitDesctd.className="cell";
        ...
        if($("#HiddenMoreUnit").val()=="False")
        {
            var newFitNametd=newTR.insertCell(7);//添加列:出库数量
        ...
        {
            var newFitNametd=newTR.insertCell(7);//添加列:出库数量(根据基本数量计算)
            newFitNametd.className="tdinput";
            newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
        ...
        var newPackage=newTR.insertCell(8);//添加列包装要求
        newPackage.className="cell";
        newPackage.id = 'SignItem_TD_Package_'+rowID;
        newPackage.innerHTML ="<input id='Package_SignItem_TD_Text_"+rowID+"' value=\""+pack+"\"  specialworkcheck='包装要求' type='text' class=\"tdinput\"  style='width:90%' />";;//添加列内容
        
        var newBaseUnit=newTR.insertCell(9);//添加列:基本单位
        ...
        var newBaseCount=newTR.insertCell(10);//添加列：基本数量
        ...
        if($("#HiddenMoreUnit").val()=="False")
        {
            ...
            var newFitNametd=newTR.insertCell(11);//添加列:单位
            newFitNametd.className="cell";
        ...
        var newFitNametd=newTR.insertCell(12);//添加列:仓库ID
        newFitNametd.className="tdinput";
        newFitNametd.id = 'SignItem_TD_StorageID_'+rowID;
        newFitNametd.innerHTML = "<select  disabled='disabled'   id=\"StorageID_SignItem_TD_Text_" + rowID + "\"   style=' width:90%;' >"+ document.getElementById("ddlStorageInfo").innerHTML + "</select>";
        ...
        var newFitNametd=newTR.insertCell(13);//添加列:原单数量
        newFitNametd.className="tdinput";
        newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
        ...
        var newFitNametd=newTR.insertCell(14);//添加列:已出库数量
        newFitNametd.className="tdinput";
        ...
        var newFitNametd=newTR.insertCell(15);//添加列:未出库数量
        newFitNametd.className="tdinput";
        newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
        ...
        if($("#HiddenMoreUnit").val()=="False")
        {
            var newFitDesctd=newTR.insertCell(16);//添加列:单价
            newFitDesctd.className="cell";
        ...
        {
            var newFitDesctd=newTR.insertCell(16);//添加列:出库单价(根据基本单价计算,隐藏基本单价，比率)
            newFitDesctd.className="cell";
            newFitDesctd.id = 'SignItem_TD_UnitPrice_'+rowID;
       ...
       // alert(document.getElementById("UnitID_SignItem_TD_Text_"+rowID).value);
        ...
        var newFitNametd=newTR.insertCell(17);//添加列:物品总价（金额）
        newFitNametd.className="cell";
        ...
        var newFitNametd=newTR.insertCell(18);//添加列:原单编号
        newFitNametd.className="cell";
        newFitNametd.id = 'SignItem_TD_TotalPrice_'+rowID;
        ...
        var newFitDesctd=newTR.insertCell(19);//添加列:原单行号
        newFitDesctd.className="cell";
        ...
        var newFitDesctd=newTR.insertCell(20);//添加列:备注
        newFitDesctd.className="cell";
        newFitDesctd.id = 'SignItem_TD_Remark_'+rowID;
        ...
        var newFitDesctd=newTR.insertCell(21);//添加列:可否为负库存（0不允许，1允许）
        newFitDesctd.className="cell";
        newFitDesctd.style.display="none"
        ...
        var newFitDesctd=newTR.insertCell(22);//添加列:可用存量
        newFitDesctd.className="cell";
        newFitDesctd.style.display="none"
        newFitDesctd.id = 'SignItem_TD_Remark_'+rowID;
...
//        });
        //        //-----end
        //        get_Table_Cell_add_1(false,false,'officedba.StorageOutSellDetail',null,'dg_Log');
...
function FillSignRow(i,OutNo,ProductID,ProductNo
        ...
        ,BatchNo,UsedUnitID,UsedUnitCount,UsedPrice,ExRate,IsBatchNo,CustID,pack)
 { //读取最后一行的行号，存放在txtTRLastIndex文本框中 
        //------钱锋锋 2010-11-01 物品别名
        var str='';
        var serch="CustID="+CustID+"&ProdNo="+ProductNo;
        ...
        */
        ...
        //------钱锋锋 2010-11-01 物品别名
        var newFitDesctd=newTR.insertCell(4);//添加列:物品别名
        newFitDesctd.className="cell";
        newFitDesctd.style.display = "none";
        newFitDesctd.id = 'SignItem_TD_ProductAlias_'+rowID;
        newFitDesctd.innerHTML ="<input id='ProductAlias_SignItem_TD_Text_"+rowID+"' value=\""+item.ProdAlias+"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:90%' />";;//添加列内容
       //-----end
        
        var newBatchNotd=newTR.insertCell(5);//添加列:批次
        newBatchNotd.className="cell";
        newBatchNotd.id = 'SignItem_TD_BatchNo_'+rowID;
         newBatchNotd.innerHTML ="<input  type='text' value=\""+BatchNo+"\" class=\"tdinput\" readonly=\"readonly\"  style='width:80%'  id='BatchNo_SignItem_TD_Text_"+rowID+"' onclick=\"popBatchObj.ShowListCheckSpecial('BatchNo_SignItem_TD_Text_"+rowID+"','"+ProductNo+"')\"  />";//添加列内容
        ...
        else
        {
            ...
            newFitNametd.className="tdinput";
            newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
            newFitNametd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_"+rowID+"\" value=\""+ProductCount+"\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice_MoreUnit(this.id,"+rowID+");CountNum();\"/>";//添加列内容
       ...
       if($("#HiddenMoreUnit").val()=="False")
        ...
        /*绑定批次*/
        //onchange=\"GetBatchList('"+ProductID+"','"+StorageControlID+"','"+ListControlID+"','"+IsBatchNo+"')\"
...
//选择单位时计算
 function ChangeUnit(ObjID/*下拉列表控件（单位）*/,RowID/*行号*/,UsedPrice/*实际单价*/)
 {
    ...
    if (EXRate != '')
    {
        ...
        $("#BaseCount_SignItem_TD_Text_"+RowID).val(OutCount);/*基本数量根据出库数量和比率算出*/
        $("#OutCount_SignItem_TD_Text_"+RowID).val(tempcount);/*基本数量根据出库数量和比率算出*/
        $("#UnitPrice_SignItem_TD_Text_"+RowID).val(UsedPrice.toFixed($("#HiddenPoint").val()));/*单价根据基本单价和比率算出*/
        //$("#TotalPrice_SignItem_TD_Text_"+RowID).val(parseFloat(OutCount*tempprice).toFixed($("#HiddenPoint").val()));/*金额*/
        $("#TotalPrice_SignItem_TD_Text_"+RowID).val(parseFloat(tempcount*UsedPrice).toFixed($("#HiddenPoint").val()));/*金额*/
        ...
        $("#FromBillCount_SignItem_TD_Text_"+RowID).val((FromBillCount*1).toFixed($("#HiddenPoint").val()));
        $("#OutedCount_SignItem_TD_Text_"+RowID).val((OutedCount*1).toFixed($("#HiddenPoint").val()));
        $("#NotOutCount_SignItem_TD_Text_"+RowID).val((NotOutCount*1).toFixed($("#HiddenPoint").val()));
    }
     CountNum();
 }
 // 
    ...
    $.ajax({
        ...
        success: function(msg) {
            ...
            if (msg.SSList != null) {
                ...
                var CustID = msg.SSList[0].CustID;
                //循环加载明细信息
                $.each(msg.SSList, function(i, item) {
                    AddSignRow(i, item.SendNo, item.ProductID, item.ProdNo,
                                ...
                                item.ProductCount, item.NotOutCount, item.SortNo, item.Remark, item.OutCount, item.StorageID, item.IsBatchNo, item.UsedUnitID, item.UsedPrice, CustID, item.packages);
                    get_Table_Cell_add(false, false, 'officedba.StorageOutSellDetail' + '*' + document.getElementById("Custom_Index").value, msg.SSList, 'dg_Log');
                });
    ...
    function TurnToOffPage(pageIndex)
    {
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item)
                    {
                        ...
                        {
                            var td="";
                            var strProductCount = parseFloat(item.ProductCount).toFixed($("#HiddenPoint").val());
                            var strOutCount = parseFloat(item.OutCount).toFixed($("#HiddenPoint").val());
                            if($("#HiddenMoreUnit").val()=="True")
                            {
                                var strProductCount1 = parseFloat(item.ProductCount).toFixed($("#HiddenPoint").val());
                                var strUnitPrice = parseFloat(item.UnitPrice).toFixed($("#HiddenPoint").val());                                
                                td="<td height='22' align='center'>"+item.CodeName+"</td>"+
                                    "<td height='22' align='center'>"+strProductCount1+"</td>"+
                                    "<td height='22' align='center'>"+strUnitPrice+"</td>";
                                    $("#frombaseunit").show();
                                    $("#frombasecount").show();
                                    $("#frombaseprice").show();
                            }
                            ...
                            td+
                            "<td height='22' align='center'>"+strProductCount+"</td>"+//发货数量
                            "<td height='22' align='center'>"+strOutCount+"</td>"+//已出库数量
   ...
   var ID=$("#txtIndentityID").val();
    if(ID=="" || ID=="0")
    {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageOutSellPrint.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
...
//    }
//    //if(confirm("请确认您的单据已经保存？"))
//       window.open("../../../Pages/PrinttingModel/StorageManager/StorageOutSellPrintP.aspx?ID="+ID);
//}
//库存快照
...
function Fun_FillBatch(id,batch,storageid,storagename)
{
    ...
    if(id!=""||id!=undefined)
    {
        var str=id.substring(25,id.length);
        document.getElementById(id).value=batch;
        document.getElementById("StorageID_SignItem_TD_Text_" + str ).value=storageid;
         document.getElementById("StorageID_SignItem_TD_Text_" + str ).disabled="disabled";
    }
}