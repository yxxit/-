            ...
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                if (!IsExist(item.ProdNo)) {
                    //AddRow(id,ProNo,ProdName,UnitName,Specification,UnitPrice)
                      var rowid=AddRow();
                      Fun_FillParent_Content(item.ID,item.ProdNo,item.ProductName,item.StandardSell,item.CodeName,item.UnitID,item.Specification,rowid,item.IsBatchNo,item.BatchNo);
...
function AddRow()//增加一个空白行
{
    ...
    newFitNametd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"ProductNo_SignItem_TD_Text_" + rowID + "\" value=\"\"  class=\"tdinput\"  readonly=\"readonly\" onclick=\"popProductInfoObj.ShowList(this.id,'"+rowID+"');\" style='width:100%'  /></td></tr></table>"; //添加列内容

    var newFitDesctd = newTR.insertCell(3); //添加列:物品名称
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_ProductName_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ProductName_SignItem_TD_Text_" + rowID + "' value=\"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:100%' /></td></tr></table>"; ; //添加列内容
    ...
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='Specification_SignItem_TD_Text_" + rowID + "' value=\"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:100%' /></td></tr></table>"; ; //添加列内容
   ...
   var ID = $("#txtIndentityID").val();
    if (ID == "" || ID == "0") {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageLossPrint.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
