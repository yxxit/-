...
//通过销售出库和其他出库带出明细信息
 ...
 { //读取最后一行的行号，存放在txtTRLastIndex文本框中 
        ...
        if($("#HiddenMoreUnit").val()=="False")
        {
            ...
            newFitNametd.className="tdinput";
            newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
            newFitNametd.innerHTML = "<input type=\"text\" id=\"ProductCount_SignItem_TD_Text_"+rowID+"\" value=\"1.00\"  class=\"tdinput\"  style='width:90%;color:red' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice(this.id);CountNum();\"/>";//添加列内容
        }
        else
        {
            ...
            newFitNametd.className="tdinput";
            newFitNametd.id = 'SignItem_TD_ProductCount_'+rowID;
            newFitNametd.innerHTML = "<input type=\"text\" id=\"ProductCount_SignItem_TD_Text_"+rowID+"\" value=\"1.00\" class=\"tdinput\"  style='width:90%;color:red' onblur=\"check(this.id);Number_round(this,'"+point+"');TotalPrice_MoreUnit(this.id,"+rowID+");CountNum();\"/>";//添加列内容
        }
        if($("#HiddenMoreUnit").val()=="False")
        {
    ...
    function TurnToOffPage(pageIndex)
    {
           ...
           success: function(msg){
                    ...
                    $.each(msg.data,function(i,item){
                        ...
                        {
                           var strCountTotal = parseFloat(item.CountTotal).toFixed($("#HiddenPoint").val());
                            $("<tr class='newrow'></tr>").append("<td height='22' align='center'>"+" <input type=\"radio\" name=\"rd\" id=\"rdOutType_"+index+"\" value=\""+item.ID+"\" onclick=\"fnSelectInfo('"+item.OutNo+"')\" />"+"</td>"+
                             ...
                             "<td height='22' align='center'>"+strCountTotal+"</td>").appendTo($("#offerDataList tbody"));
                             index=index+1;
                        }
                   });
                    //页码
