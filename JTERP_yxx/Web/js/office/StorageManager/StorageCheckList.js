...
function TurnToPage(pageIndex)
{
        ...
        var diffCountStart=document.getElementById("txtDiffCountStart").value;
        var diffCountEnd=document.getElementById("txtDiffCountEnd").value;
       if(diffCountStart!="" && !IsNumOrFloat(diffCountStart))
       {
            popMsgObj.Show("盘点单列表|","差异起始金额必须为有效的数值|");
            return;
       }
       
        if(diffCountEnd!="" && !IsNumOrFloat(diffCountEnd))
       {
            popMsgObj.Show("盘点单列表|","差异结束金额必须为有效的数值|");
            return;
       }
   ...
   if(!isFlag)
   {
    ...
    document.getElementById("txtPageSize").value=pageCount;
       var Para="pageIndex="+pageIndex+"&pageCount="+pageCount+"&action=GET&orderby="+orderBy+
                      "&CheckNo="+escape(document.getElementById("txtCheckNo").value)+
       ...
       $.ajax({
           ...
           {    
                    ...
                    $.each(msg.data,function(i,item){
                        if(item!=null && item!="")
                        {
                         if(parseFloat( item.D_DiffCount)!=0&&parseFloat( item.D_DiffCount).toString()!="NaN")
                        {
                            total1+=parseFloat( item.D_DiffCount);
                        }
                        $("<tr class='newrow'></tr>").append(
                            "<td align=\"center\" >"+checkStatus(item)+
                            "<td align=\"center\" ><a href=\"StorageCheckSave.aspx?ModuleID="+document.getElementById("MoudleID").value+"&action=EDIT&CheckID="+item.ID+"&CheckNo="+item.TransferNo+getFlowStatus(item.FlowStatus)+"\">"+item.CheckNo+"</a></td>"+
...
//                             "<td align=\"center\" title=\""+item.BatchNo+"\">"+fnjiequ(item.BatchNo,12)+"</td>"+
                             ...
                             "<td align=\"center\" >"+parseFloat(item.D_DiffCount).toFixed(point)  +"</td>"+
...
//                             "<td align=\"center\" title=\""+item.BatchNo+"\">"+fnjiequ(item.BatchNo,12)+"</td>"+
                            ...
                            "<td align=\"center\" ></td>"+
                            "<td align=\"center\" ></td>"+
                             "<td align=\"center\" >"+parseFloat(total1).toFixed(point)  +"</td>"+
                             "<td align=\"center\" ></td>"+
                             "<td align=\"center\" ></td>" ).appendTo($("#tblStoragelist tbody")); 
                     //页码
...
//        return "<input type=\"checkbox\" id=\"chk_list_"+item.ID+"\" name=\"chk\" value=\""+item.ID+"\"  disabled=\"true\"  onclick=\"unSelectAll('"+item.ID+"');\"/></td>"
 ...
 function setStatusAction(item)
 {
    if(item.BillStatus!="制单")
    {
        ...
        {
            return "<a href=\"StorageTransferOut.aspx?TransferID="+item.ID+"\">"+item.BusiStatus+"</a>";
        }
        else if(item.BusiStatus=="调拨入库")
        {
            return "<a href=\"StorageTransferIn.aspx?TransferID="+item.ID+"\">"+item.BusiStatus+"</a>";
        }
        else
