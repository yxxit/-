    ...
    for (var i = 0; i < chk.length; i++) {
        ...
        if (document.getElementById("tr_list_" + chk[i].value).style.display != "none") {
            ...
            if ($("#txtIsMoreUnit").val() == "1") {
                ...
                price += parseFloat(document.getElementById("txtReturnCount_" + chk[i].value).value) * parseFloat(document.getElementById("txtUnitPrice_" + chk[i].value).value);
...
function checkc(index) {
 var objPrice ="";
 if ($("#txtIsMoreUnit").val() == "1") {
     objPrice = document.getElementById("hidUnitPrice_" + index);
  }
  else
  {
    objPrice = document.getElementById("txtUnitPrice_" + index);
  }
...
function cCheckReturnCount(rowid, MustReturnCount) {
    ...
    objValue=document.getElementById("txtReturnCount_" + rowid).value;
    }
...
if ($("#txtIsMoreUnit").val() == "1") {
    ...
    {
       var objValue = document.getElementById("txtUnitPrice_" + rowid).value;
    if (!IsNumOrFloat(objValue)) {
        popMsgObj.Show("借货返还单明细|", "返还单价必须是有效的数值|");
        document.getElementById("txtUnitPrice_" + rowid).value = Price.toFixed(2);
        return;
    }
...
function Add() {
    ...
    for (var i = 0; i < ck.length; i++) {
        if (document.getElementById("tr_list_" + ck[i].value).style.display != "none") {
            ...
            {
             UnitPrice.push(document.getElementById("txtUnitPrice_" + ck[i].value).value);
            }
            ...
            $.each(msg.data, function(i, item) {
                if (item.ID != null && item.ID != "") {
                    ...
                    if ($("#txtIsMoreUnit").val() == "1") {
                        ...
                        html += "<td  class=\"cell\"><input type=\"text\"   class=\"tdinput tboxsize textAlign\" id=\"txtUnitPrice_" + item.SortNo + "\"  value=\"" + parseFloat(item.UnitPrice).toFixed(2) + "\"  onblur=\"_cCheckPrice(" + item.SortNo + "," + item.UnitPrice + ")\" onchange=\"Number_round(this,"+selPoint+")\"/></td>";
                    }
...
function _cCheckCountReturnCount(rowid, MustReturnCount) {
    ...
    {
        document.getElementById("txtReturnTotalPrice_" + rowid).value = parseFloat(parseFloat(objValue) * parseFloat(document.getElementById("txtUnitPrice_" + rowid).value)).toFixed(2);
        CalCulateNum('ddlUsedUnit_' + rowid, "txtUsedReturnCount_" + rowid, "txtReturnCount_" + rowid, "txtUsedUnitPrice_" + rowid, "txtUnitPrice_" + rowid, $("#hidSelPoint").val());
...
function _cCheckPrice(rowid, Price) {
    var objValue ="";
    if ($("#txtIsMoreUnit").val() == "1") {
    objValue = document.getElementById("hidUnitPrice_" + rowid).value;
    }
    else
    {
     objValue = document.getElementById("txtUnitPrice_" + rowid).value;
    }
    if (!IsNumOrFloat(objValue)) {
        popMsgObj.Show("借货返还单明细|", "返还单价必须是有效的数值|");
        if ($("#txtIsMoreUnit").val() == "1") {
        ...
        success: function(msg) {
            ...
            if (str[0] == "1") {
                ...
                }else if(type==4)
                {
                    ...
                    document.getElementById("tbxoModifiedUser").value = "";
                    document.getElementById("tboxModifiedDate").value = "";
                }
                ...
                if(type==4)
                    popMsgObj.Show(info + "|", info + "失败,"+str[1]+"，库存不足|");
                else
...
function cSaveButton() {
    ...
    var objqx1=document.getElementById("img1");
    var objqx2=document.getElementById("img2");
    ...
    try {
        if (document.getElementById("ddlBillStatus").value == "1") {
            ...
            objqx1.style.display = "block";
            objqx2.style.display = "none";
            ...
            objqx1.style.display = "none";
            objqx2.style.display = "block";
        }
        ...
        if (document.getElementById("tr_list_" + ck[i].value).style.display != "none") {
            try{
                if (document.getElementById("chk_list_" + ck[i].value).checked) {
                    detailRows++;
                    intProductID = document.getElementById('hidProductID_' + ck[i].value).value;
                    snapProductName = document.getElementById('txtProductNo_' + ck[i].value).value;
                    snapProductNo = document.getElementById('txtProcutName_' + ck[i].value).value;
                }
            }catch(err)
            {
                 if (document.getElementById("chkchkDetail_" + ck[i].value).checked) {
                    detailRows++;
                    intProductID = document.getElementById('hidProductID_' + ck[i].value).value;
                    snapProductName = document.getElementById('txtProductNo_' + ck[i].value).value;
                    snapProductNo = document.getElementById('txtProcutName_' + ck[i].value).value;
