...
$(document).ready(function() {
    ...
    get_Table_Cell_add(true,false,'officedba.StorageInOtherDetail',null,'dg_Log','Custom_div');
});
                ...
                if (item.ProductID != null && item.ProductID != '')
                 {
                    ...
                    FillSignRow(i, item.InNo, item.ProductID, item.ProductNo, item.ProductName,item.ColorName,  item.Specification, item.UnitID,item.UnitPrice,
                     item.B_TotalPrice, item.FromBillCount, item.InCount, item.NotInCount, item.BackNo,item.FromLineNo, item.DetaiRemark,
                      item.StorageID, item.FromType, item.InNumber,item.UsedUnitID,item.UsedUnitCount,item.UsedPrice,item.ExRate,item.IsBatchNo,item.Pnumber,item.Pnumberid,item.AbrasionResist,item.AbrasionResistid,item.BalancePaper,item.BalancePaperid,item.BaseMaterial,item.BaseMaterialid,item.SurfaceTreatment);
                }
            });
            get_Table_Cell_add(false,true,'officedba.StorageInOtherDetail',msg.data,'dg_Log');
...
//                    else 
                    if(BratchNo !="")
                 {
                    if (!CodeCheck($("#BatchRuleControl1_txtBatch").val()))
                    {
                        isFlag = false;
...
//            {
//               
//            	var AbrasionResist= $("#AbrasionResist" + i).val();
//		        if (AbrasionResist== "") {
//                isFlag = false;
//                fieldText = fieldText + "订单明细（行号：" + i + "）|";
//                msgText = msgText + "请选择耐磨度|";
//            	}
//            	
//            	var BalancePaper= $("#BalancePaper" + i).val();
//		        if (BalancePaper== "") {
//                isFlag = false;
//                fieldText = fieldText + "订单明细（行号：" + i + "）|";
//                msgText = msgText + "请选择平衡纸|";
//            	}
//            	
//            	var Pnumber= $("#Pnumber" + i).val();
//		        if (Pnumber== "") {
//                isFlag = false;
//                fieldText = fieldText + "订单明细（行号：" + i + "）|";
//                msgText = msgText + "请选择纸号|";
//            	}
//            	
//            	var BaseMaterial= $("#BaseMaterial" + i).val();
//		        if (BaseMaterial== "") {
//                isFlag = false;
//                fieldText = fieldText + "订单明细（行号：" + i + "）|";
//                msgText = msgText + "请选择基材板|";
//            	}
//            	
//            	var SurfaceTreatment= $("#SurfaceTreatment" + i).val();
//		        if (SurfaceTreatment== "") {
//                isFlag = false;
//                fieldText = fieldText + "订单明细（行号：" + i + "）|";
//                msgText = msgText + "请选择表面处理工艺|";
//            	}
//            	
//            	
//            }
...
//            }
    ...
    if (!isFlag) {
        ...
        var Pnumber = new Array();         /*纸号*/
        var Pnumberid = new Array();         /*纸号id*/
        var AbrasionResist = new Array();         /*耐磨度*/
        var AbrasionResistid = new Array();         /*耐磨度id*/
        var BalancePaper = new Array();         /*平衡纸*/
        var BalancePaperid = new Array();         /*平衡纸id*/
        var BaseMaterial = new Array();         /*基材*/
        var BaseMaterialid = new Array();         /*基材id*/
        var SurfaceTreatment = new Array();         /*表面处理*/
        ...
        for (var i = 1; i < count; i++) {
            if (signFrame.rows[i].style.display != "none") {
                ...
                var Pnumber1="";//纸号
                var Pnumberid1="";//纸号id
                var AbrasionResist1="";//耐磨度
                var AbrasionResistid1="";//耐磨度id
                var BalancePaper1="";//平衡纸
                var BalancePaperid1="";//平衡纸id
                var BaseMaterial1="";//基材
                var BaseMaterialid1="";//基材id
                var SurfaceTreatment1="";//表面工艺

                 if($("#Vervalue").val()=="floor")
                {
                     Pnumber1= $("#Pnumber" + i).val();
                     Pnumberid1= $("#Pnumberid" + i).val();
                     AbrasionResist1= $("#AbrasionResist" + i).val();
                     AbrasionResistid1= $("#AbrasionResistid" + i).val();
                     BalancePaper1= $("#BalancePaper" + i).val();
                     BalancePaperid1= $("#BalancePaperid" + i).val();
                     BaseMaterial1= $("#BaseMaterial" + i).val();
                     BaseMaterialid1= $("#BaseMaterialid" + i).val();
                     SurfaceTreatment1= $("#SurfaceTreatment" + i).val();
                    ...
                    if($("#Vervalue").val()=="floor")
                {
                     ...
                     Pnumber.push(Pnumber1);
                     Pnumberid.push(Pnumberid1);
                     AbrasionResist.push(AbrasionResist1);
                     AbrasionResistid.push(AbrasionResistid1);
                     BalancePaper.push(BalancePaper1);
                     BalancePaperid.push(BalancePaperid1);
                     BaseMaterial.push(BaseMaterial1);
                     BaseMaterialid.push(BaseMaterialid1);
                     SurfaceTreatment.push(SurfaceTreatment1);
                }
                if(document.getElementById("hidMoreUnit").value == "true")
                {
         ...
         var UrlParam = "&txtInNo="+escape(txtInNo)+
                        ...
                        "&Pnumber=" + escape(Pnumber.toString()) +
                        "&Pnumberid=" + Pnumberid.toString() +
                        "&AbrasionResist=" + escape(AbrasionResist.toString()) +
                        "&AbrasionResistid=" + AbrasionResistid.toString() +
                        "&BalancePaper=" + escape(BalancePaper.toString()) +
                        "&BalancePaperid=" + BalancePaperid.toString() +
                        "&BaseMaterial=" + escape(BaseMaterial.toString()) +
                        "&BaseMaterialid=" + BaseMaterialid.toString() +
                        "&SurfaceTreatment=" + escape(SurfaceTreatment.toString()) +
                        "&ReStorageID="+ReStorageID.toString()+GetExtAttrValue()+GetTableCustom("dg_Log");
...
function AddSignRow_uc(i, InNo, ProductID, ProductNo, ProductName,ColorName, Specification, UnitID, UnitPrice, TotalPrice, FromBillCount,
                 NotInCount, FromLineNo, Remark, InNumber, StorageID,DetailUnitID,IsBatchNo,UsedUnitID,UsedUnitCount,Pnumber,Pnumberid,AbrasionResist,AbrasionResistid,BalancePaper,BalancePaperid,BaseMaterial,BaseMaterialid,SurfaceTreatment) 
{ //读取最后一行的行号，存放在txtTRLastIndex文本框中 
    ...
    x++;
    var newNameTD = newTR.insertCell(x); //添加列:序号
    ...
    newFitNametd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"ProductNo_SignItem_TD_Text_" + rowID + "\" value=\"" + ProductNo + "\" title=\"" + ProductID + "\" class=\"tdinput\"  readonly=\"readonly\" style='width:100%'  /></td></tr></table>"; //添加列内容
    x++;

    var newFitDesctd = newTR.insertCell(x); //添加列:物品名称
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_ProductName_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ProductName_SignItem_TD_Text_" + rowID + "' value=\"" + ProductName + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:100%' /></td></tr></table>"; ; //添加列内容
    x++;
    
    var newFitDesctd = newTR.insertCell(x); //添加列:颜色
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_ColorName_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ColorName_SignItem_TD_Text_" + rowID + "' value=\"" + ColorName + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:90%' /></td></tr></table>"; ; //添加列内容
    x++;

    var newFitDesctd = newTR.insertCell(x); //添加列:物品规格
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_Specification_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='Specification_SignItem_TD_Text_" + rowID + "' value=\"" + Specification + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:100%' /></td></tr></table>"; ; //添加列内容
     x++;
     if( $("#Vervalue").val()=="floor")
    {
        var colSpecification = newTR.insertCell(x); //添加列:耐磨度
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='AbrasionResist" + rowID + "' onclick=\"getFloor(2,'AbrasionResist" + rowID + "','AbrasionResistid" + rowID + "')\" value=\"" + AbrasionResist + "\"   style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + AbrasionResistid + "\" id='AbrasionResistid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:平衡纸
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='BalancePaper" + rowID + "'  value=\"" + BalancePaper + "\" onclick=\"getFloor(3,'BalancePaper" + rowID + "','BalancePaperid" + rowID + "')\"  style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + BalancePaperid + "\" id='BalancePaperid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:纸号
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='Pnumber" + rowID + "'  value=\"" + Pnumber + "\" onclick=\"getFloor(1,'Pnumber" + rowID + "','Pnumberid" + rowID + "')\"  style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + Pnumberid + "\" id='Pnumberid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:基材
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='BaseMaterial" + rowID + "'   value=\"" + BaseMaterial + "\" onclick=\"getFloor(4,'BaseMaterial" + rowID + "','BaseMaterialid" + rowID + "')\" style=' width:80%;'   type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + BaseMaterialid + "\" id='BaseMaterialid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:表面工艺
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='SurfaceTreatment" + rowID + "' value=\"" + SurfaceTreatment + "\"   style=' width:80%;'  type='text'  class='tdinput' />"; //添加列内容
        x++;
    ...
    x++;
    var newDanWeitd = newTR.insertCell(x); //添加列:单位组
    ...
    x++;
    var newFitNametd = newTR.insertCell(x); //添加列:原单数量
    newFitNametd.className = "tdinput";
    ...
    x++;
    var newFitNametd = newTR.insertCell(x); //添加列:已入库数量
    newFitNametd.className = "tdinput";
    ...
    var newFitNametd = newTR.insertCell(x); //添加列:未入库数量
    newFitNametd.className = "tdinput";
    newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
    ...
    x++;
    var newFitNametd = newTR.insertCell(x); //添加列:jiben数量
    newFitNametd.className = "tdinput";
    newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
...
var sss = (productcount * unitprice).toFixed($("#hidSelPoint").val());   
    ...
    var newFitNametd = newTR.insertCell(x); //添加列:原单编号
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_TotalPrice_' + rowID;
    ...
    x++;
    var newFitDesctd = newTR.insertCell(x); //添加列:原单行号
    newFitDesctd.className = "cell";
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    var newFitDesctd = newTR.insertCell(x); //添加列:备注
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_Remark_' + rowID;
    ...
    x++;
    var newBatchNo = newTR.insertCell(x); //添加列:pici
    ...
    newBatchNo.style.display="none";
       x++; 
    var newJiBen_DanJia = newTR.insertCell(x); //添加列:基本单价
...
function FillSignRow(i, InNo, ProductID, ProductNo, ProductName,ColorName, Specification, UnitID, UnitPrice, TotalPrice, FromBillCount, InCount, 
                    NotInCount, SBNo, FromLineNo, Remark, StorageID, ISNOFromType, InNumber,UsedUnitID,UsedUnitCount,UsedPrice,ExRate,IsBatchNo,Pnumber,Pnumberid,AbrasionResist,AbrasionResistid,BalancePaper,BalancePaperid,BaseMaterial,BaseMaterialid,SurfaceTreatment) 
{ //读取最后一行的行号，存放在txtTRLastIndex文本框中 
    var x=0;
    var signFrame = findObj("dg_Log", document);
    var rowID = signFrame.rows.length;
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    if( $("#Vervalue").val()=="floor")
    {
        ...
        var colSpecification = newTR.insertCell(x); //添加列:平衡纸
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='BalancePaper" + rowID + "' onclick=\"getFloor(3,'BalancePaper" + rowID + "','BalancePaperid" + rowID + "')\"  value=\"" + BalancePaper + "\"  style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + BalancePaperid + "\" id='BalancePaperid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:纸号
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='Pnumber" + rowID + "' onclick=\"getFloor(1,'Pnumber" + rowID + "','Pnumberid" + rowID + "')\"   value=\"" + Pnumber + "\"    style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + Pnumberid + "\" id='Pnumberid" + rowID + "'/>"; //添加列内容
        x++;
        
        var colSpecification = newTR.insertCell(x); //添加列:基材
        colSpecification.className = "cell";
        colSpecification.innerHTML = "<input  id='BaseMaterial" + rowID + "' onclick=\"getFloor(4,'BaseMaterial" + rowID + "','BaseMaterialid" + rowID + "')\"  value=\"" + BaseMaterial + "\"  style=' width:80%;'  type='text'  class='tdinput' /><input type=\"hidden\" value=\"" + BaseMaterialid + "\" id='BaseMaterialid" + rowID + "'/>"; //添加列内容
        x++;
    ...
    newFitNametd.innerHTML = "<input type=\"text\" id=\"NotInCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(NotInCount) + "\"  class=\"tdinput\"  style='width:90%' readonly=\"readonly\"/>"; //添加列内容
    x++;
    var newFitNametd = newTR.insertCell(x); //添加列:入库数量
    newFitNametd.className = "tdinput";
    newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"InCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(InCount) + "\"  class=\"tdinput\"  style='width:90%'  onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice(this.id);CountNum();\"    onkeydown=\"SetEventFocus('InCount_SignItem_TD_Text_','ProductNo_SignItem_TD_Text_'," + rowID + ",true);\"    />"; //添加列内容
    x++;
if(UsedUnitCount == "")
{
    UsedUnitCount = "0";
...
else
{
    ...
    var tdhtml4 = "<input name='chk' id='UnitPrice_SignItem_TD_Text_" + rowID + "' value=\"" + NumberSetPoint(danjia) + "\" type='text' class=\"tdinput\"  style='width:90%'  onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice_a(this.id);CountNum();\" />";
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    //get_Table_Cell_add_1(false,false,'officedba.StorageInOtherDetail',null,'dg_Log');
}
...
function AddRow(id, ProNo, ProdName,ColorName, UnitName, Specification, UnitPrice,UnitID,IsBatchNo,StorageID,Pnumber,Pnumberid,AbrasionResist,AbrasionResistid,BalancePaper,BalancePaperid,BaseMaterial,BaseMaterialid,SurfaceTreatment)//增加一个空白行
{
    //读取最后一行的行号，存放在txtTRLastIndex文本框中 
    var signFrame = findObj("dg_Log", document);
    var rowID = signFrame.rows.length;
    ...
    var newFitNametd = newTR.insertCell(x); //添加列:基本单位
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_UnitID_' + rowID;
    ...
    newFitNametd.id = 'SignItem_TD_StorageID_' + rowID;
    newFitNametd.innerHTML = "<select class='tdinput' id=\"StorageID_SignItem_TD_Text_" + rowID + "\"   style=' width:90%;'>" + document.getElementById("ddlStorageInfo").innerHTML + "</select>";
    document.getElementById("StorageID_SignItem_TD_Text_"+rowID).value = StorageID;
    x++;
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_ColorName_' + rowID;
    newFitDesctd.innerHTML = "<input id='ColorName_SignItem_TD_Text_" + rowID + "' value=\"\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:90%' />"; ; //添加列内容
...
newDanWeitd.innerHTML = "<div id=\"unitdiv"+rowID+"\"></div>";//添加列内容
    ...
    newFitNametd.className = "tdinput";
    newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"InCount_SignItem_TD_Text_" + rowID + "\" value=\"0\"  class=\"tdinput\"  style='width:90%'  onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice(this.id);CountNum();\"   onkeydown=\"SetEventFocus('InCount_SignItem_TD_Text_','ProductNo_SignItem_TD_Text_'," + rowID + ",true);\"    />"; //添加列内容

 var newShuLiangtd = newTR.insertCell(13); //添加列:数量?
newShuLiangtd.className = "tdinput";
newShuLiangtd.id = 'SignItem_TD_ProductCount_' + rowID;
newShuLiangtd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_" + rowID + "\" value=\"0\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice_z(this.id);CountNum();\"    onkeydown=\"SetEventFocus('OutCount_SignItem_TD_Text_','ProductNo_SignItem_TD_Text_'," + rowID + ",true);\"    />"; //添加列内容
...
else
{
    ...
    newFitDesctd.style.display = IsDisplayPrice;
    newFitDesctd.id = 'SignItem_TD_UnitPrice_' + rowID;
    newFitDesctd.innerHTML = "<input name='chk' id='UnitPrice_SignItem_TD_Text_" + rowID + "' value=\"\" type='text' class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice_a(this.id);CountNum();\"/>"; //添加列内容
...
//选择单位时计算
 ...
 { 
    if($("#OutCount_SignItem_TD_Text_"+rowid).val() != "")
    {
        ...
        var EXRate = $("#DetailUnitID_SignItem_TD_Text_"+ rowid).val().split('|')[1].toString(); /*换算率*/
...
function Fun_FillParent_Content(id, ProNo,ProdName,a,UnitID,UnitName,b,c,d,Specification,e,f,UnitPrice,g,h,i,j,k,m,n,o, p,q,r,s, IsBatchNo,ColorName,StorageID) 
{
...
if(UnitPrice == "")
{
   ...
   var aStorageID = 'StorageID_' +  idstr;
   document.getElementById(aStorageID).value = StorageID;
...
function alerTreediv() {
   ...
   popSellCustObj.ShowList('protion','txtOtherCorpID','txtOtherCorp','','','sltCorpBigType');
}
   ...
   var ID = $("#txtIndentityID").val();
    if (ID == "" || ID == "0") {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageInOtherPrint1.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
            ...
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                if (!IsExist(item.ProdNo)) {
                    //AddRow(id,ProNo,ProdName,UnitName,Specification,UnitPrice)
                      if( $("#Vervalue").val()=="floor")
                      {
                            AddRow(item.ID, item.ProdNo, item.ProductName,item.ColorName, item.CodeName, item.Specification, item.StandardCost,item.UnitID,item.IsBatchNo,item.StorageID,item.Pnumber,item.Pnumberid,item.AbrasionResist,item.AbrasionResistid,item.BalancePaper,item.BalancePaperid,item.BaseMaterial,item.BaseMaterialid,item.SurfaceTreatment);
                      }
                      else
                      {
                            AddRow(item.ID, item.ProdNo, item.ProductName,item.ColorName, item.CodeName, item.Specification, item.StandardCost,item.UnitID,item.IsBatchNo,item.StorageID);
                      }
                      get_Table_Cell_add_1(false,false,'officedba.StorageInOtherDetail',msg.data,'dg_Log');
                }
            });
