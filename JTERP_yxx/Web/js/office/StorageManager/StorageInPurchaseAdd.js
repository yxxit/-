...
$(document).ready(function() {
    ...
    GetField("Tb_01", "2051201", "0");
    GetField("dg_Log", "2051201", "1");
    get_Table_Cell_add(true,false,'officedba.StorageInPurchaseDetail',null,'dg_Log','Custom_div');
});
function LoadDetailInfo(InNoID) {
    $.ajax({
        ...
        success: function(msg) {
            ...
            $.each(msg.data, function(i, item) {
                ...
                if (item.ProductID != null && item.ProductID != '') 
                {
                    if(item.BatchNo != "")
                     {
                            document.getElementById("divBatchNo").style.display = "none";
                            document.getElementById('divBatchNoShow').value = item.BatchNo;
            ...
            get_Table_Cell_add(false,true,'officedba.StorageInPurchaseDetail',msg.data,'dg_Log');
...
function AddSignRow(i, InNo, ProductID, ProductNo , ProductName,ColorName, UnitID,DetailUnitID, UnitPrice, TotalPrice, DetailID , ProductCount, 
                    Remark, FromBillID, FromBillNo, FromLineNo, FromTypeName, Specification, FromBillCount, InCount, TaxRate, StorageID,IsBatchNo,UsedUnitID,counts) 
{ //读取最后一行的行号，存放在txtTRLastIndex文本框中 
    ...
    var newFitDesctd = newTR.insertCell(4); //添加列:物品颜色
    newFitDesctd.className = "cell";
    ...
    newFitNametd.innerHTML = "<select id=\"StorageID_SignItem_TD_Text_" + rowID + "\"  disabled=\"disabled\"  style=' width:90%;'>" + document.getElementById("ddlStorageInfo").innerHTML + "</select>";
    ...
    var newShuLiangtd = newTR.insertCell(13); //添加列:数量 注：启用多记量单位时，此数量为 实际单位数量
   ...
   //newShuLiangtd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(counts) + "\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice_z(this.id);CountNum();\"/>"; //添加列内容
    //默认值为源单数量-已入库数量=应收数量 edit 20110504 lcb

    if(document.getElementById("hidMoreUnit").value == "true") // 启用多计量单位
    {
...
//    get_Table_Cell_add_1(false,false,'officedba.StorageInPurchaseDetail',null,'dg_Log');
}
...
//    newFitDesctd.innerHTML = "<input id='ColorName_SignItem_TD_Text_" + rowID + "' value=\"" + ColorName + "\" type='text' class=\"tdinput\" readonly=\"readonly\"  style='width:90%' />"; ; //添加列内容
    ...
    var newFitNametd = newTR.insertCell(7); //添加列:源单数量
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_FromBillCount_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"FromBillCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(FromBillCount) + "\" class=\"tdinput\" readonly=\"readonly\"  style='width:90%' />"; //添加列内容

    var newFitNametd = newTR.insertCell(8); //添加列:已入库数量
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_InCount_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"InCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(InCount) + "\" class=\"tdinput\" readonly=\"readonly\"  style='width:90%' />"; //添加列内容

    var newFitNametd = newTR.insertCell(9); //添加列:应收数量
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_ProductCountjh_' + rowID;
...
else
{
    ...
    var newFitDesctd = newTR.insertCell(14); //添加列:税率
    newFitDesctd.className = "cell";
    newFitDesctd.style.display = IsDisplayPrice;
    newFitDesctd.id = 'SignItem_TD_TaxRate_' + rowID;
    newFitDesctd.innerHTML = "<input name='chk' id='TaxRate_SignItem_TD_Text_" + rowID + "' value=\"" + NumberSetPoint(TaxRate) + "\" type='text' class=\"tdinput\" readonly=\"readonly\"  style='width:90%' />"; //添加列内容
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    var newFitDesctd = newTR.insertCell(19); //添加列:退货数量
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_BackCount_' + rowID;
    ...
    var newBatchNo = newTR.insertCell(20); //添加列:pici
    newBatchNo.className = "cell";
    newBatchNo.id = 'SignItem_TD_BatchNo_' + rowID;
    newBatchNo.innerHTML = "<input name='chk' id='BatchNo_SignItem_TD_Text_" + rowID + "' value=\"" + IsBatchNo + "\" type='text' class=\"tdinput\"  style='width:90%'  />"; //添加列内容
    newBatchNo.style.display="none";
    
    var newJiBen_DanJia = newTR.insertCell(21); //添加列:基本单价
    newJiBen_DanJia.className = "cell";
    newJiBen_DanJia.id = 'TD_JiBen_DanJia_' + rowID;
    newJiBen_DanJia.innerHTML = "<input name='chk' id='JiBen_DanJia_" + rowID + "' value=\"" + UnitPrice + "\" type='text'  />"; //添加列内容
    newJiBen_DanJia.style.display="none";
   ...
   var ID = $("#txtIndentityID").val();
    if (ID == "" || ID == "0") {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageInpurchasePrint.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
            {
                ...
                {
                    window.open("../../../Pages/PrinttingModel/StorageManager/StorageInpurchasePrint.aspx?ID=" + ID);
                }
            }
        }else
        {
...
//    window.open("../../../Pages/PrinttingModel/StorageManager/StorageInpurchasePrintP.aspx?ID=" + ID);
//}
