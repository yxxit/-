...
function DoSearch(currPage) {
   ...
   "&EFDesc=" + escape(EFDesc) + "&time1=" + escape(time1) + "&time2=" + escape(time2) +
   "&Flag=1"+'&prodid='+escape( $("#prodid").val())+'&ReasonType='+escape(ddlReasonType);
            ...
            $.each(msg.data, function(i, item) {
                if (item.ID != null && item.ID != ""){
                    ...
                    $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + "<input id='OptionCheck_" + item.ID + "' name='Checkbox1'  value=" + item.ID + " onclick=IfSelectAll('Checkbox1','checkall')  type='checkbox'/>" + "</td>" +
                        "<td height='22' align='center' title=\"" + item.InNo + "\"><a href='#' onclick= GetLinkParam("+item.ID+");>" + fnjiequ(item.InNo, 16) + "</a></td>" +
                        "<td height='22' align='center' title=\"" + item.productname + "\">" + fnjiequ(item.productname, 10) + "</td>" +
                        "<td height='22' align='center' >" + parseFloat(item.unitprice).toFixed($("#hidSelPoint").val()) + "</td>" +
                        "<td height='22' align='center' >" + parseFloat(item.productcount).toFixed($("#hidSelPoint").val()) + "</td>" +
                        "<td height='22' align='center' >" + parseFloat(item.totalprice1).toFixed($("#hidSelPoint").val()) + "</td>" +
...
//    window.location.href = GetLinkParam();
    parent.addTab(null, '2051205', '新建其他入库单', 'Pages/Office/StorageManager/StorageInOtherAdd.aspx?ModuleID=2051205')
}

/*
* 获取链接的参数
*/
function GetLinkParam(id) {
//    //获取模块功能ID
...
//    //返回链接的字符串
    //    return linkParam;
    parent.addTab(null, '2051205', '新建其他入库单', 'Pages/Office/StorageManager/StorageInOtherAdd.aspx?ModuleID=2051205&InNoID=' + id);
}
/*------------------------------------新建-----------------------------------*/
