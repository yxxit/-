...
function ValidateInfo() {
    ...
    if (document.getElementById("txtOutDeptID").value == "") {
        ...
        if (document.getElementById("txtApplyDeptID").value == document.getElementById("txtOutDeptID").value && document.getElementById("txtOutDeptID").value != "") {
        msgtitle = msgtitle + "要货部门,调货部门|";
        msgtext = msgtext + "不能为同一个部门|";
        flag = false;
    }
            ...
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                if (!IsExist(item.ProdNo)) {
                    //AddRow(id,ProNo,ProdName,UnitName,Specification,UnitPrice)
                      var rowid=AddRow();
                     fnSelectProductInfo(item.ID,item.ProdNo,item.ProductName,item.Specification,0,item.CodeName,item.UnitID,item.StandardCost,rowid,item.BatchNo);
...
function AddRow() {
    ...
    cellProductNo.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"txtProductNo_" + RowID + "\" class=\"tdinput tboxsize textAlign\" value=\"\"  readonly onclick=\"getProdcutList(" + RowID + ");\"  /></td></tr></table>";
    i++;

    //物品名称
    var cellProductName = row.insertCell(i);
    cellProductName.className = "cell";
    cellProductName.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"txtProductName_" + RowID + "\" class=\"tdinput tboxsize textAlign\" value=\"\"  disabled=\"true\"/></td></tr></table>";
    i++;
    ...
    cellProdcutSpec.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"txtProductSpec_" + RowID + "\" class=\"tdinput tboxsize textAlign\" value=\"\"  disabled=\"true\" /></td></tr></table>";
    i++;
...
function ShowSnapshot() {
    ...
    for (var i = 0; i < signFrame.rows.length; i++) {
        if (signFrame.rows[i].style.display != "none") {
            var rowid = signFrame.rows[i].id;
            if(document.getElementById('chk_list_' + i) != null)
            {
 ...
 //   if (detailRows == 1) {
  //      try {
      if(detailRows == 0)
      {
                 popMsgObj.ShowMsg('请添加物品并选择，然后查看');
      }
      if(intProductID != undefined)
      {
