...
$(document).ready(function() {
   var varItem = new Option("等同销售单号","等同销售单号");
   
    document.getElementById("BatchRuleControl1_ddlBatchRule").options.add(varItem);
    if (InNoID > 0) {
        ...
        try {
            ...
            $("#UnConfirm_disable").show();
            

        }
        catch (e) { }
    }
    get_Table_Cell_add(true,false,'officedba.StorageInProcessDetail',null,'dg_Log','Custom_div');
});
...
//            {
            ...
            document.getElementById('txtUserman').value = msg.data[0].UserMans;
            document.getElementById('txtUsermanid').value = msg.data[0].UserManIds;
            ...
            $.each(msg.data, function(i, item) {
                if (item.ProductID != null && item.ProductID != '') {
                    if(msg.data[i].BatchNo != "")
                     {
                            document.getElementById("divBatchNo").style.display = "none";
                            document.getElementById('divBatchNoShow').value = msg.data[i].BatchNo;
                    ...
                    FillSignRow(i, item.SortNo, item.InNo, item.ProductID, item.ProductNo, item.ProductName, item.Specification,item.UnitID,
                     item.UnitPrice, item.B_TotalPrice, item.ProductCount, item.DetaiRemark, item.TaskNo,item.FromLineNo, item.StorageID,
                      item.NotInCount, item.FromBillCount, item.InCount,item.UsedUnitID,item.UsedUnitCount,item.UsedPrice,item.ExRate,item.IsBatchNo,item.Size,item.TotalNumber,item.PieceCount,item.pakeageid,item.pakeages,item.NoPassAnswer,item.NoPassNum);
                }

            });
            get_Table_Cell_add(false,true,'officedba.StorageInProcessDetail',msg.data,'dg_Log');
...
//        {
                ...
                else if($("#BatchRuleControl1_ddlBatchRule").val() == "等同销售单号")//取原单单号
                {
                    BratchNo = $("#hiddenfromno1").val();
                    pcgz = "xd";
                }
                 else //自动生成批次
                {
...
//        {}
        ...
        BratchNo = document.getElementById("divBatchNoShow").value;
    }
    ...
    var userids=$("#txtUsermanid").val();
    var usernames=$("#txtUserman").val();
    ...
    for (i = 1; i < signFrame.rows.length; i++) {
        ...
        var NotInCount = "NotInCount_SignItem_TD_Text_" + i;
        if(document.getElementById("IsStorageOver").value=="False")
        {
    ...
    if (!isFlag) {
        ...
        var DetailBatchNo = new Array();//批次
         var PieceCount= new Array();
            ...
            var PieceNum= new Array();
            var pakeid= new Array();
            var NoPassAnswer=new Array();
            var NoPassNum=new Array();
        ...
        for (var i = 1; i < count; i++) {
            if (signFrame.rows[i].style.display != "none") {
                ...
                {
                    PieceCount.push($("#PieceCount"+i).val());
                    TotalNumber.push($("#TotalNumber"+i).val());
                    PieceNum.push($("#PieceNum"+i).val());
                    pakeid.push($("#pakeage"+i).val());
                    NoPassNum.push($("#NoPassNum"+i).val());
                    NoPassAnswer.push($("#NoPassAnswer"+i).val());
                }
        ...
        var UrlParam ="&txtInNo="+escape(txtInNo)+"&txtTitle="+escape(txtTitle)+"&ddlFromType="+escape(ddlFromType)+"&txtFromBillID="+escape(txtFromBillID);
                 UrlParam+="&txtTotalPrice="+escape(txtTotalPrice);
                           ...
                           UrlParam+="&PieceCount="+escape(PieceCount);
                           UrlParam+="&TotalNumber="+escape(TotalNumber);
                           UrlParam+="&PieceNum="+escape(PieceNum);
                           UrlParam+="&pakeid="+escape(pakeid);
                           UrlParam+="&NoPassNum="+escape(NoPassNum);
                           UrlParam+="&NoPassAnswer="+escape(NoPassAnswer);
                           UrlParam+="&userids="+escape(userids);
                           UrlParam+="&usernames="+escape(usernames);
                           ...
                           UrlParam+="&ID="+txtIndentityID.toString()+GetExtAttrValue()+GetTableCustom("dg_Log");
        $.ajax({
            type: "POST",
                ...
                if (data.sta == 1) {
                    ...
                    if (reInfo.length > 1) {                        
                        ...
                        if (parseInt(txtIndentityID) <= 0) {
                            ...
                            if(reInfo[4] != "NULL")
                            {
                                document.getElementById("divBatchNoShow").value = reInfo[4];
                                document.getElementById("divBatchNoShow").style.display = "block";
                                document.getElementById("divBatchNo").style.display = "none";
                            }
                        }
...
function AddSignRow(i, FromBillNo, ProductID, ProductNo, ProductName, Specification, UnitID, UnitPrice, ProductCount, FromLineNo,
                 FromBillCount, InCount, StorageID,IsBatchNo,UsedUnitID,jibencount,Fromno,size,TotalNumber,PieceCount,pakeageid,pakeages,NoPassAnswer,NoPassNum) 
{ //读取最后一行的行号，存放在txtTRLastIndex文本框中 
    ...
    var k=0;
    var newNameXH = newTR.insertCell(k); //添加列:选择
    newNameXH.className = "cell";
    newNameXH.id = 'SignItem_TD_Check_' + rowID;
    newNameXH.innerHTML = "<input id='hiddenfromno"+rowID+"'type='hidden' value=\""+Fromno+"\"><input name='chk1' id='chk_Option_" + rowID + "' value=\"\" type='checkbox' onclick=\"IfSelectAll('chk1','checkall');\"/>";
    k++;
    
    var newNameTD = newTR.insertCell(k); //添加列:序号
    newNameTD.className = "cell";
    newNameTD.id = 'SignItem_TD_Index_' + rowID;
    newNameTD.innerHTML = newTR.rowIndex.toString();
    k++;

    var newFitNametd = newTR.insertCell(k); //添加列:物品编号
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_ProductNo_' + rowID;
    newFitNametd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" id=\"ProductNo_SignItem_TD_Text_" + rowID + "\" value=\"" + ProductNo + "\" title=\"" + ProductID + "\" class=\"tdinput\"  readonly=\"readonly\" style='width:95%''  /></td></tr></table>"; //添加列内容
    k++;
    
    var newFitDesctd = newTR.insertCell(k); //添加列:物品名称
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_ProductName_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='ProductName_SignItem_TD_Text_" + rowID + "' value=\"" + ProductName + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:95%'' /></td></tr></table>"; ; //添加列内容
    k++;
    
    var newFitDesctd = newTR.insertCell(k); //添加列:物品规格
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_Specification_' + rowID;
    newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input id='Specification_SignItem_TD_Text_" + rowID + "' value=\"" + Specification + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:95%' /></td></tr></table>"; ; //添加列内容
    k++;
    
     if( $("#vervalue").val()=="floor")
     {
        var newFitDesctd = newTR.insertCell(k); //添加列:尺寸
        newFitDesctd.className = "cell";
        newFitDesctd.id = 'Size_' + rowID;
        newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='98%'><td><input id='Size" + rowID + "' value=\"" + size + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:99%' /></td></tr></table>"; ; //添加列内容
        k++;
        
        var newFitDesctd = newTR.insertCell(k); //添加列:包装
        newFitDesctd.className = "cell";
        newFitDesctd.id = 'pakeage_' + rowID;
        newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='98%'><td><input id='pakeage" + rowID + "' value=\"" + pakeages + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:99%' /> <input type='hidden' id=\"pakeageid"+rowID+"\" value=\""+pakeageid+"\" /></td></tr></table>"; ; //添加列内容
        k++;
     }
    var newFitNametd = newTR.insertCell(k); //添加列:基本单位
    newFitNametd.className = "cell";
    ...
    var newDanWeitd = newTR.insertCell(k); //添加列:单位组
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:仓库ID
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:源单数量
    newFitNametd.className = "cell";
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:已入库数量
    newFitNametd.className = "cell";
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:未入库数量
    newFitNametd.className = "cell";
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:基本数量
    newFitNametd.className = "tdinput";
    newFitNametd.id = 'SignItem_TD_ProductCount_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"ProductCount_SignItem_TD_Text_" + rowID + "\" value=\"" + NumberSetPoint(jibencount) + "\"  class=\"tdinput\"  style='width:90%'  onblur=\"check(this.id);Number_roundBegin(this,2);TotalPrice(this.id);CountNum();calculation('ProductCount_SignItem_TD_Text_" + rowID + "','','','"+rowID+"');\"/>"; //添加列内容
    k++;
    
     var newShuLiangtd = newTR.insertCell(k); //添加列:数量?
    newShuLiangtd.className = "tdinput";
    newShuLiangtd.id = 'SignItem_TD_ProductCount_' + rowID;
    newShuLiangtd.innerHTML = "<input type=\"text\" id=\"OutCount_SignItem_TD_Text_" + rowID + "\" value=\"0\"  class=\"tdinput\"  style='width:90%' onblur=\"check(this.id);Number_roundBegin(this,2);CountNum();TotalPrice2(this.id);\"/>"; //添加列内容
    k++;
    
     if( $("#vervalue").val()=="floor")
     {
        var newFitNametd = newTR.insertCell(k); //添加列:每件片数
        newFitNametd.className = "tdinput";
        newFitNametd.id = 'PieceCount_' + rowID;
        newFitNametd.innerHTML = "<input type=\"text\" id=\"PieceCount" + rowID + "\" value=\"" + NumberSetPoint(PieceCount) + "\" readonly=\"readonly\"    class=\"tdinput\"  style='width:90%'  />"; //添加列内容
        k++;
        
        var newFitNametd = newTR.insertCell(k); //添加列:件数
        newFitNametd.className = "tdinput";
        newFitNametd.id = 'TotalNumber_' + rowID;
        newFitNametd.innerHTML = "<input type=\"text\" id=\"TotalNumber" + rowID + "\" value=\"" + NumberSetPoint(TotalNumber) + "\"  class=\"tdinput\"  style='width:90%'  onblur=\"calculation('','','TotalNumber" + rowID + "','"+rowID+"');\" />"; //添加列内容
        k++;
        
        var newFitNametd = newTR.insertCell(k); //添加列:片数
        newFitNametd.className = "tdinput";
        newFitNametd.id = 'PieceNum_' + rowID;
        newFitNametd.innerHTML = "<input type=\"text\" id=\"PieceNum" + rowID + "\" value=\""+NumberSetPoint(parseFloat(PieceCount)*parseFloat(TotalNumber))+"\"  class=\"tdinput\"  style='width:90%'  onblur=\"calculation('','PieceNum" + rowID + "','','"+rowID+"');\"/>"; //添加列内容
        k++;
//        if(parseFloat(TotalNumber).toString()!="NaN"&&parseFloat(PieceCount).toString()!="NaN"&&parseFloat(PieceCount)*parseFloat(TotalNumber)>0)
//        {
//            document.getElementById("PieceNum"+rowID).value=parseFloat(parseFloat(PieceCount)*parseFloat(TotalNumber)).toFixed($("#hidSelPoint").val());
//        }else
//        {
//            document.getElementById("PieceNum"+rowID).value=parseFloat(0).toFixed($("#hidSelPoint").val());
//        }
        var newFitNametd = newTR.insertCell(k); //添加列:次品数量
        newFitNametd.className = "tdinput";
        newFitNametd.id = 'NoPassNum_' + rowID;
        newFitNametd.innerHTML = "<input type=\"text\" id=\"NoPassNum" + rowID + "\" value=\"" + NumberSetPoint(NoPassNum) + "\"  class=\"tdinput\"  style='width:90%'  onblur=\"getnopass(this);\"/>"; //添加列内容
        k++;
        
        var newFitNametd = newTR.insertCell(k); //添加列:次品原因
        newFitNametd.className = "tdinput";
        newFitNametd.id = 'NoPassAnswer_' + rowID;
        newFitNametd.innerHTML = "<input type=\"text\" id=\"NoPassAnswer" + rowID + "\" value=\""+NoPassAnswer+"\"  class=\"tdinput\"  style='width:90%'  />"; //添加列内容
        k++;
     }
...
else
{
    ...
    var newFitDesctd = newTR.insertCell(k); //添加列:单价
    newFitDesctd.className = "cell";
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:物品总价（金额）
    newFitNametd.className = "cell";
    ...
    var newFitNametd = newTR.insertCell(k); //添加列:原单编号
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_TotalPrice_' + rowID;
    ...
    var newFitDesctd = newTR.insertCell(k); //添加列:原单行号
    newFitDesctd.className = "cell";
...
if(document.getElementById("hidMoreUnit").value == "true")
{
    ...
    var newFitDesctd = newTR.insertCell(k); //添加列:备注
    newFitDesctd.className = "cell";
    newFitDesctd.id = 'SignItem_TD_Remark_' + rowID;
    ...
    var newBatchNo = newTR.insertCell(k); //添加列:pici
    ...
    var newJiBen_DanJia = newTR.insertCell(k); //添加列:基本单价
    ...
    k++;
     ...
     get_Table_Cell_add_1(false,false,'officedba.StorageInProcessDetail',null,'dg_Log');
}
...
//选择单位时计算
 ...
 { 
    if($("#OutCount_SignItem_TD_Text_"+rowid).val() != "")
    {
        ...
        EXRate=$("#DetailUnitID_SignItem_TD_Text_"+rowid).val().split('|')[1].toString();
        else
         EXRate=$(""+ObjID).val().split('|')[1].toString();
...
function FillSignRow(i, SortNo, InNo, ProductID, ProductNo, ProductName, Specification,UnitID, UnitPrice, TotalPrice, ProductCount, Remark,
                    FromBillNo,FromLineNo, StorageID, NotInCount, FromBillCount, InCount,UsedUnitID,UsedUnitCount,UsedPrice,ExRate,IsBatchNo,size,TotalNumber,PieceCount,pakeageid,pakeages,NoPassAnswer,NoPassNum)
{
    var k=0;
    var signFrame = findObj("dg_Log", document);
    var rowID = signFrame.rows.length;
 ...
 if( $("#vervalue").val()=="floor")
     {
        ...
        newFitDesctd.className = "cell";
        newFitDesctd.id = 'Size_' + rowID;
        newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='99%'><input id='Size" + rowID + "' value=\"" + size + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:99%' /></td></tr></table>"; ; //添加列内容
        k++;
        
        var newFitDesctd = newTR.insertCell(k); //添加列:包装
        newFitDesctd.className = "cell";
        newFitDesctd.id = 'pakeage_' + rowID;
        newFitDesctd.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='99%'><input id='pakeage" + rowID + "' value=\"" + pakeages + "\" type='text' class=\"tdinput\" readonly=\"readonly\"   style='width:99%' /> <input type='hidden' id=\"pakeageid"+rowID+"\" value=\""+pakeageid+"\" /></td></tr></table>"; ; //添加列内容
        k++;
     }
     
    var newFitNametd = newTR.insertCell(k); //添加列:单位
    newFitNametd.className = "cell";
    newFitNametd.id = 'SignItem_TD_UnitID_' + rowID;
    newFitNametd.innerHTML = "<input type=\"text\" id=\"UnitID_SignItem_TD_Text_" + rowID + "\" value=\"" + UnitID + "\" class=\"tdinput\" readonly=\"readonly\"  style='width:90%''  />"; //添加列内容
k++;
   ...
   var ID = $("#txtIndentityID").val();
    if (ID == "" || ID == "0") {
        ...
        if(document.getElementById("IsPrint").value=="true")
        {
            ...
            var url="../../../Pages/PrinttingModel/StorageManager/"+document.getElementById("PrintNo").value +"_StorageInProcess.aspx?ID=" + ID;
            if(testExist(url))
            {
                window.open(url);
            }else
            {
                if(confirm("不存在此个性打印页面，是否跳转标准打印？"))
                {
                    window.open("../../../Pages/PrinttingModel/StorageManager/StorageInProcess.aspx?ID=" + ID);
                }
            }
        }else
        {
...
//        return;
//    }
//    window.open("../../../Pages/PrinttingModel/StorageManager/StorageInProcessP.aspx?ID=" + ID);
//}
...
function NumberSetPoint(num)
{
    ...
    if(SetPoint.toString()=="NaN")
    SetPoint= parseFloat(0).toFixed($("#hidSelPoint").val());
    return SetPoint;
}
...
//总平米，件数，片数换算
function calculation(squaremeters,PieceCount,TotalNumber,rowid)
{
    var re = /^[0-9]+.?[0-9]*$/;
    var precisionLength=$("#hidSelPoint").val();
    if(document.getElementById("vervalue").value!="floor")
    {
        var meter=document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value;
        if(meter==""||!re.test(meter))
        {
            document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value="0.00";
            return;
        }
        else
        {
            document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=parseFloat(meter).toFixed(precisionLength);
        }
        return;
    }
    ...
    var meter=document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value;
    var pcount=document.getElementById("PieceCount"+rowid).value;
    var tnum=document.getElementById("TotalNumber"+rowid).value;
    var size=document.getElementById("Size"+rowid).value;
    var pnum=document.getElementById("PieceNum"+rowid).value;
    ...
    else
    {
     ...
     if(size==""||size==null||size.length==2)
    {
        
        
        tnum=document.getElementById("TotalNumber"+rowid).value="0.00";
        return;
    }
     
    if(meter!=""&&pcount!=""&&size!="")
    {
            var str1= new Array();
            str1=size.split('*');
            var area1=parseFloat(str1[0])*parseFloat(str1[1])/1000/1000;
            if(parseFloat(area1)!=0&&parseFloat(pcount)!=0)
            {
            var total=parseFloat(meter)/parseFloat(area1)/parseFloat(pcount);
            document.getElementById("TotalNumber"+rowid).value=total.toFixed(precisionLength);
           
            }else
            {
                document.getElementById("TotalNumber"+rowid).value="0.00";
            }
    }
        if(squaremeters!="")
        {
            ...
            var str= new Array();
            str=size.split('*');
            var area=parseFloat(str[0])*parseFloat(str[1])/1000/1000;
            ...
            {
               var total=parseFloat(meter)/parseFloat(area);
                document.getElementById("PieceNum"+rowid).value=total.toFixed(precisionLength);
                //document.getElementById("TotalSquare"+rowid).value=parseFloat(parseFloat(meter)*parseFloat(area)).toFixed(precisionLength);
            }
            else
            {
//                if(parseFloat(pnum)!=0)
//                {
//                    var total=parseFloat(meter)/parseFloat(area);
//                    document.getElementById("TotalNumber"+rowid).value=total.toFixed(precisionLength);
//                }
//                else if(parseFloat(tnum)!=0)
//                {
                    var total=parseFloat(meter)/parseFloat(area);
                    document.getElementById("PieceNum"+rowid).value=total.toFixed(precisionLength);
                    document.getElementById("TotalNumber"+rowid).value=parseFloat(parseFloat(total).toFixed(precisionLength)/parseFloat(pcount)).toFixed(precisionLength);
//                }
        ...
        if(PieceCount!="")
        {
             if(pcount==""||!re.test(pcount))
             {
              document.getElementById("PieceNum"+rowid).value="0.00";
                return;
              
             }
              document.getElementById("PieceNum"+rowid).value=parseFloat(pnum).toFixed(precisionLength);
             if(size==""||size==null||size.length==2)
             {
                return;
             }
            ...
            {
                var total=parseFloat(pnum)*parseFloat(area);
                document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=total.toFixed(precisionLength);
                //document.getElementById("TotalSquare"+rowid).value=parseFloat(parseFloat(pcount)*parseFloat(tnum)*parseFloat(area)).toFixed(precisionLength);
            }
            else
            {
...
//                {
                    var total=parseFloat(pnum)*parseFloat(area);
                    document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=total.toFixed(precisionLength);
                    document.getElementById("TotalNumber"+rowid).value=parseFloat(parseFloat(pnum)/parseFloat(pcount)).toFixed(precisionLength);
//                }else if(parseFloat(tnum)!=0)
//                {
//                    var total=parseFloat(area)*parseFloat(pnum);
//                     document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=total.toFixed(precisionLength);
//                }
                 //document.getElementById("TotalSquare"+rowid).value=parseFloat(parseFloat(meter)*parseFloat(area)).toFixed(precisionLength);
     ...
     if(TotalNumber!="")
    {
        if(tnum==""||!re.test(tnum))
        {
            document.getElementById("TotalNumber"+rowid).value="0.00";
            return;
        }
         document.getElementById("TotalNumber"+rowid).value=parseFloat(tnum).toFixed(precisionLength);
        if(size==""||size==null||size.length==2)
        return;
        ...
        return;
         ...
         if(meter==""&&pnum=="")
            ...
            {
             var total=parseFloat(tnum)*parseFloat(pcount)*parseFloat(area);
                document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=total.toFixed(precisionLength);
               // document.getElementById("TotalSquare"+rowid).value=parseFloat(parseFloat(pcount)*parseFloat(tnum)*parseFloat(area)).toFixed(precisionLength);
            ...
            else
            {
               var total=parseFloat(tnum)*parseFloat(pcount)*parseFloat(area);
                document.getElementById("ProductCount_SignItem_TD_Text_"+rowid).value=total.toFixed(precisionLength);
                document.getElementById("PieceNum"+rowid).value=(parseFloat(total)/parseFloat(area)).toFixed(precisionLength);
                 //document.getElementById("TotalSquare"+rowid).value=parseFloat(parseFloat(pcount)*parseFloat(tnum)*parseFloat(area)).toFixed(precisionLength);
            }
    }
...
//次品数量
function getnopass(obj)
{
    var str=parseFloat(obj.value).toFixed($("#hidSelPoint").val());
    if(str.toString()=="NaN")
    {
        obj.value=parseFloat(0).toFixed($("#hidSelPoint").val());
    }else
    {
        obj.value=str;
    }
