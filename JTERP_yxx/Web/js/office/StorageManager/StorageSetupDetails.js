$(document).ready(function() {
    ...
    var tmpDisplayPrice = document.getElementById('hidDisplayPrice').value;
    document.getElementById('tdTitlePrice').style.display = tmpDisplayPrice==0?'none':'';
    TurnToPage(1);
});    
    ...
    function TurnToPage(pageIndex) {
        ...
        +document.getElementById("hidendTime").value+"&productid="+document.getElementById("HidProductID").value+"&timeType="
        +document.getElementById("HidTimeType").value+"&storageid="+document.getElementById("HidStorageID").value+"&ByTimeType="+document.getElementById("HiddenField1").value+"&timestr="+escape(document.getElementById("hidvalue").value);

         $.ajax({
             type: "POST", //用POST方式传输
             dataType: "json", //数据格式:JSON
             url: '../../../Handler/OperatingModel/StorageManager/StorageSetUpDetails.ashx', //目标地址
             ...
             success: function(msg) {
                 ...
                 if (item.id != null && item.id != "") {
                         if (tmpDisplayPrice == 0) {
                             $("<tr class='newrow'></tr>").append("<td height='22' align='center'>" + item.OutNo
                                 + "</td>" +
