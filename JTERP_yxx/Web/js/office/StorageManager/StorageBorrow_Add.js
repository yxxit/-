...
*添加行
****************************************************************/
function Show_ProductInfo()
{
    var storage=document.getElementById("ddlDepot").value;
    if(parseInt(storage)>0)
    {
        popTechObj.ShowListCheckSpecial('SubStorageIn"+rowID+"', 'Check',storage);
    }else
    {
         popMsgObj.ShowMsg('请选择借出仓库！');
    }
}
            ...
            $.each(msg.data, function(i, item) {//填充物品ID，物品编号，物品名称，规格，单位ID，单位名称，入库数量(默认为0)
                if (!IsExist(item.ProdNo)) {
                    //AddRow(id,ProNo,ProdName,UnitName,Specification,UnitPrice)
                      var rowid=addProductList();
                      SetRowValue1(item, rowid);
...
function addProductList() {
    ...
    cellRerurnDate.innerHTML = "<input type=\"text\"  class=\"tdinput tboxsize textAlign\" id=\"tboxReturnDate_list_" + rowid + "\" onclick=\"WdatePicker({dateFmt:'yyyy-MM-dd',el:$dp.$('tboxReturnDate_list_" + rowid + "')})\" readOnly  />";
    i++;
...
function GetUsedTotalPrice(rowid) {
    ...
    if(parseFloat(objcount.value).toString()!="NaN")
    {
        document.getElementById("tboxReturnCount_list_"+rowid).value=objcount.value;
    }
}
...
function SetRowValue1(data, rowid) {
    $("#tboxProductID_" + rowid).val(data.ID);
    ...
    $("#tboxQuantity_list_" + rowid).val(parseFloat("0").toFixed(2));
    $("#tboxStorageQuantity_list_" + rowid).val(parseFloat("0").toFixed(2));
    $("#tboxPrice_list_" + rowid).val(parseFloat(data.StandardCost).toFixed(2));
    $("#tboxTotalPrice_list_" + rowid).val(parseFloat("0").toFixed(2));
    $("#tboxReturnDate_list_" + rowid).val(getSysDate());
    $("#tboxReturnCount_list_" + rowid).val(parseFloat("1").toFixed(2));
    $("#tboxRealReturnCount_list_" + rowid).val(parseFloat("0").toFixed(2));
    $("#tboxRemark_list_" + rowid).val(data.Remark);

    //批次相关
    GetBatchList(data.ID, "ddlDepot", "BatchNo_SignItem_TD_Text_" + rowid, document.getElementById("hidIsBatchNo").value == "1" ? true : false, data.BatchNo);
    ...
    if ($("#txtIsMoreUnit").val() == "1") {
        ...
        if (data.ExRate != null && data.ExRate != "") {
            $("#tboxReturnCount_list_" + rowid).val(parseFloat("0").toFixed(2));
            $("#tboxUsedPrice_list_" + rowid).val(parseFloat("0").toFixed(2));
            $("#tboxUsedQuantity_list_" + rowid).val(parseFloat("0").toFixed(2));
            GetUnitGroupSelect(data.ID, 'StockUnit', 'tboxUsedUnit_list_' + rowid, 'ChangeUnit(this,' + rowid + ')', "tboxUsedUnit_listtd_" + rowid, data.UsedUnitID);

        }
        else {
            GetUnitGroupSelectEx(data.ID, 'StockUnit', 'tboxUsedUnit_list_' + rowid, 'ChangeUnit(this,' + rowid + ')', "tboxUsedUnit_listtd_" + rowid, data.UsedUnitID, "ReCalByMoreUnit('" + rowid + "','" + data.StandardCost + "')");
...
function AddRowByBarCode(ID, ProdNo, ProductName, Specification, ProductCount, CodeName, UnitID, StandardCost) {
    ...
    cellProductNo.className = "cell";
    cellProductNo.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" onclick=\"getProductInfo(" + rowid + ");\" value=\"" + ProdNo + "\" class=\"tdinput tboxsize textAlign\"  id=\"tboxProductNo_list_" + rowid + "\" /></td></tr></table>";

    //物品名称
    var cellProductName = row.insertCell(3);
    cellProductName.className = "cell";
    cellProductName.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\"   onclick=\"getProductInfo(" + rowid + ");\" value=\"" + ProductName + "\" class=\"tdinput tboxsize textAlign\" id=\"tboxProcutName_list_" + rowid + "\" disabled=\"true\"  /></td></tr></table>";

    //规格
    var cellStandard = row.insertCell(4);
    cellStandard.className = "cell";
    cellStandard.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' width='100%'><input type=\"text\" readonly class=\"tdinput tboxsize textAlign\" id=\"tboxStandard_list_" + rowid + "\" value=\"" + Specification + "\" disabled=\"true\" /></td></tr></table>";
