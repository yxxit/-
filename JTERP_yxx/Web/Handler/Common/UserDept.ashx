...
public class UserDept : BaseHandler
{
    ...
    {
        string CustType = GetParam("CustType");
        string Important = GetParam("Important");
        DataTable linkman = XBase.Business.Office.CustManager.LinkManBus.GetLinkManListEx(UserInfo.CompanyCD,CustType,Important);
        ...
        foreach (DataRow row in linkman.Rows) 
        {
            ...
            {
                custNos.Add(row["CustNo"].ToString());//根据CustNo筛选重复客户，并添加到数组custNos
                custs.Add(row);                                                 //同时添加行数组custs
            }
        }

        StringBuilder sb = new StringBuilder();
        sb.Append("[");
        foreach (DataRow cust in custs)//循环 筛选有重复CustNo后的记录
        {
            if (cust["CustNam"].ToString().Trim() == string.Empty)
            {
                continue;//筛选CustNam为空的记录
            }
            
            DataRow[] rows = linkman.Select("CustNo='" + cust["CustNo"].ToString() + "' AND Handset<> ' '"); //筛选联系人手机号为" "的记录，赋值于[]rows
