...
public class Flow : IHttpHandler, System.Web.SessionState.IRequiresSessionState{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (Action == ActionUtil.Add.ToString())//提交审批
        {
            ...
            string nextperson = context.Request.QueryString["nextperson"].ToString().Trim();
            int MsgRemind = 0;
            ...
            string strRetVal = FlowBus.FlowApplyAdd(companyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID, BillNo, PageUrl, ApplyUserID, ModifiedUserID, MsgRemind,nextperson);
        ...
        else if (Action == ActionUtil.Edit.ToString())//审批
        {
            ...
            string nextperson = context.Request.QueryString["nextperson"].ToString().Trim();//下一步接收人
            int MsgRemind = 0;
            if (!string.IsNullOrEmpty(context.Request.QueryString["MsgRemind"].ToString().Trim()))
            {
                MsgRemind = int.Parse(context.Request.QueryString["MsgRemind"].ToString().Trim());
            ...
            string strRetVal = FlowBus.FlowApproveAdd(companyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID,BillNo, State, Note, ModifiedUserID,loginEmployeeID,MsgRemind,nextperson);
            if (!String.IsNullOrEmpty(strRetVal))
            {
                string[] retArray = strRetVal.Split('|');
                jc = new JsonClass(retArray[1].ToString(), "", int.Parse(retArray[0].ToString()));
        ...
        else if(Action == "GetSet")//是否设置了流程
        {
            ...
            UsedStatus = 2;//bao 2014-04-01 默认为2,开启流程审批功能
