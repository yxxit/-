...
public class Notice :  BaseHandler
{
    ...
    /// <summary>
    /// 完整截取HTML字符，保持HTML标记完整性
    /// </summary>
    /// <param name="str"></param>
    /// <returns></returns>
    private string SubstringToHTML(string str)
    {
        string tt=str.Substring(0,16);
        int index=tt.LastIndexOf('&');
        int hindex=tt.LastIndexOf('<');
        bool ishtml = false;
        bool iscode = false;
        char temp;
        char[] pchar = tt.ToCharArray();
