    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            if (action == "Get")
            {
                ...
                string isbatch = context.Request.Params["isbatch"].ToString().Trim();
                model.Prostatus = context.Request.Params["prostatus"].ToString().Trim();
                model.Storagestatus = context.Request.Params["storagestatus"].ToString().Trim();
                ...
                int TotalCount = 0;
                DataTable dt = StorageSearchBus.GetProductStorageTableBycondition(model, pdtModel, StorageCount1, EFIndex, EFDesc, pageIndex, pageCount, ord,BatchNo, ref TotalCount,isbatch);
            ...
            else if (action == "getfloorbatch")//获取产品配置
            {
                string proid = context.Request.QueryString["proid"].Trim();
                string batchno = context.Request.QueryString["batchno"].Trim();
                DataTable dt = StorageSearchBus.getfloorbatch(proid,batchno);
