<%@ WebHandler Language="C#" Class="StorageOutOtherAdd" %>
...
public class StorageOutOtherAdd : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if ("Del".Equals(action.Trim()))
        {
            ...
            {
                if (!StorageCommonBus.IsDelete("officedba.StorageOutOther", IDArray[i].ToString(), companyCD))
                {
                    ifdele = false;
                    break;
                }
            ...
            if (ifdele == true)
            {
                for (int i = 0; i < IDArray.Length; i++)
                {
                    ...
                    if (StorageOutOtherBus.DeleteStorageOutOther(IDArray[i].ToString(), companyCD))
                    {
                        delcout += 1;
                    }
                }
        ...
        else
        {
            ...
            if (context.Request.Params["Act"].ToString().Trim() == "Confirm")
            {
                if (ID > 0)
                {
                    ...
                    if (!StorageOutOtherBus.ISConfirmBill(model))
                    {
                        jc = new JsonClass("出库数量不能大于源单未出库数量", "", 0);
                        context.Response.Write(jc);
                        return;
                    }
                    else if (!string.IsNullOrEmpty(StorageOutOtherBus.ifExist(model)))
                    {
                        jc = new JsonClass("第" + StorageOutOtherBus.ifExist(model) + "行物品在该仓库中不存在，请核对！", "", 0);
                        context.Response.Write(jc);
                        return;
                    }
                    else if (!((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsStorageZero)
                    {
                        if(StorageOutOtherBus.ISBigUseCountWhenCant(model) != "" || StorageOutOtherBus.ISBigUseCountWhenCant(model) != string.Empty)
                        {
                            string[] Info = StorageOutOtherBus.ISBigUseCountWhenCant(model).Split('|');
                    ...
                    {
                        if (StorageOutOtherBus.ISBigUseCountWhenCan(model) != "" || StorageOutOtherBus.ISBigUseCountWhenCan(model) != string.Empty)
                        {
                            string Info = StorageOutOtherBus.ISBigUseCountWhenCan(model);
                    ...
                    string retstrval = "";
                    if (StorageOutOtherBus.ConfirmBill(model, out retstrval))
                    {
                        jc = new JsonClass(retstrval, LoginUserName + "|" + Date, 1);
                    }
                    else
                    {
            ...
            else
            {
                if (ID > 0)
                {
                    ...
                    string HiddenProjectID = context.Request.Params["HiddenProjectID"].ToString().Trim();//所属项目
                    ...
                    model.ProjectID = HiddenProjectID;//所属项目
                    ...
                    model.OtherCorpID = context.Request.Params["txtOtherCorpID"].ToString();//往来单位ID
                    model.CorpBigType = context.Request.Params["sltCorpBigType"].ToString();//往来单位类型
                    model.OtherCorpName = context.Request.Params["txtOtherCorpName"].ToString();//往来单位名称 20121122 添加 

                    model.ReasonType = context.Request.Params["ddlReason"].ToString().Trim();
                    model.SendAddr = context.Request.Params["txtSendAddress"].ToString().Trim();
                    model.ReceiveAddr = context.Request.Params["txtArrAddress"].ToString().Trim();
                    ...
                    if (productID.Length >= 1)
                    {
                        ...
                        {
                            StorageOutOtherDetailModel model1 = new StorageOutOtherDetailModel();
                    ...
                    //获取明细扩展项值
                    string[] strCustomdetail = new string[2];
                    strCustomdetail[0] = context.Request.Params["RowNum"].ToString();
                    strCustomdetail[1] = context.Request.Params["strCustom"].ToString();
                    if (StorageOutOtherBus.UpdateStorageOutOther(model, ht, modellist, strCustomdetail))
                    {
                ...
                else
                {
                    ...
                    {
                        model.OutNo = ItemCodingRuleBus.GetCodeValue(context.Request.Params["txtOutNo"].ToString().Trim(), "StorageOutOther", "OutNo");
                    ...
                    bool isExsist = PrimekeyVerifyBus.CheckCodeUniq("StorageOutOther", "OutNo", model.OutNo);
                    if (!isExsist)
                    {
                    ...
                    else
                    {
                        ...
                        if (productID.Length >= 1)
                        {
                            for (int i = 0; i < productID.Length; i++)
                            {
                                StorageOutOtherDetailModel model2 = new StorageOutOtherDetailModel();
                                ...
                                try
                                {model2.UsedPrice = unitPrice[i];}catch(Exception){}//实际单价
                                try
                                { model2.UsedUnitCount = productCount[i]; }catch(Exception){}//实际数量
                                try
                                { model2.ExRate = ExtRate[i]; }catch(Exception){}//比率
                        ...
                        strCustomdetail[0] = context.Request.Params["RowNum"].ToString();
                        strCustomdetail[1] = context.Request.Params["strCustom"].ToString();
                        if (StorageOutOtherBus.InsertStorageOutOther(model, ht, modellist, out IndexIDentity, strCustomdetail))
                        {
