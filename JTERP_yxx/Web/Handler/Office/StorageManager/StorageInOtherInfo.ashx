<%@ WebHandler Language="C#" Class="StorageInOtherInfo" %>
...
public class StorageInOtherInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            else if ("unconfirm".Equals(action.Trim()))
            {
                unconfirm(context);
            }
            else
            {
                ...
                model.ID = ID;
                DataTable DT = StorageInOtherBus.GetStorageInOtherDetailInfo(model);
    ...
    private void unconfirm(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            if (StorageInOtherBus.CheckIsUnfirm(model))
            {
                if (StorageInOtherBus.Unconfirm(model))
                {
    ...
    private void CloseBill(HttpContext context)
    {
        ...
        if (context.Request.QueryString["Act"].ToString().Trim() == "Close")
            if (ID > 0)
            {
                ...
                model.Closer = loginUser_id.ToString();
                model.ModifiedUserID = LoginUserID;
                if (StorageInOtherBus.CloseBill(model))
                {
    ...
    private void CancelCloseBill(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            model.Closer = loginUser_id.ToString();
            model.ModifiedUserID = LoginUserID;
            if (StorageInOtherBus.CancelCloseBill(model))
            {
