<%@ WebHandler Language="C#" Class="StorageInRedInfo" %>
...
public class StorageInRedInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            else
            {
                ...
                model.ID = ID;
                DataTable DT = StorageInRedBus.GetStorageInRedDetailInfo(model);
    ...
    private void unconfirm(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            if (StorageInRedBus.unconfirm(model))
            {
    ...
    private void CloseBill(HttpContext context)
    {
        ...
        if (context.Request.QueryString["Act"].ToString().Trim() == "Close")
            if (ID > 0)
            {
                ...
                model.Closer = loginUser_id.ToString();
                model.ModifiedUserID = LoginUserID;
                if (StorageInRedBus.CloseBill(model))
                {
    ...
    private void CancelCloseBill(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            model.Closer = loginUser_id.ToString();
            model.ModifiedUserID = LoginUserID;
            if (StorageInRedBus.CancelCloseBill(model))
            {
