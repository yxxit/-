    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (action == "Confirm" || action == "UnConfirm")  //确认
        {
            if (ID > 0)
            {
                ...
                if (action == "Confirm")
                {
                    if (StorageAdjustBus.checkcount(ID.ToString(), true, ref msg))
                    {
                        if (StorageAdjustBus.ConfirmBill(ID.ToString()))
                        {
                            jc = new JsonClass("确认成功", EmployeeName + "|" + DateTime.Now.ToString("yyyy-MM-dd") + "|" + LoginUserID, 1);
                        }
                        else
                        {
                    ...
                    {
                        jc = new JsonClass("物品："+msg+"库存不足", "", 0);
                    }
                }
                if (action == "UnConfirm")
                {
                    //if (StorageAdjustBus.checkcount(ID.ToString(), false, ref msg))
                    //{
                        if (StorageAdjustBus.UnConfirmBill(ID.ToString()))
                        {
                            jc = new JsonClass("取消确认成功", LoginUserID + "|" + DateTime.Now.ToString("yyyy-MM-dd"), 1);
                        }
                        else
                        {
