...
public class StorageInOtherList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.RequestType == "POST")
        {
            ...
            if (context.Request.QueryString["ReasonType"].Trim() != "undefined")
            {
                model.ReasonType = context.Request.QueryString["ReasonType"].ToString();
            }
            string ord = orderBy + " " + order;
            int TotalCount = 0;
            DataTable dt = StorageInOtherBus.GetStorageInOtherTableBycondition(BatchNo,model, StorageID, EnterDateStart, EnterDateEnd, pageIndex, pageCount, ord, ref TotalCount,time1,time2,prodid);
