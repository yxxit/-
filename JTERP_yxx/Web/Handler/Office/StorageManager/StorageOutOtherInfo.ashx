...
public class StorageOutOtherInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private void GetDetailList(HttpContext context)
    {
        ...
        strDetailIDList = (context.Request.Params["DetailIDList"].ToString());//明细信息的ID组合字符串
    ...
    {
        string orderString = (context.Request.Params["orderByOff"].ToString());//排序
        ...
        int pageCount = int.Parse(context.Request.Params["pageOffCount"].ToString());//每页显示记录数
        int pageIndex = int.Parse(context.Request.Params["pageIndex"].ToString());//当前页
        string txtNo = context.Request.Params["txtNo"].ToString();
        string txtTitle = context.Request.Params["txtTitle"].ToString();
    ...
    private void CloseBill(HttpContext context)
    {
        ...
        int ID = int.Parse((context.Request.Params["ID"].ToString()));//明细信息的ID组合字符串
        if (context.Request.Params["Act"].ToString().Trim() == "Close")
            if (ID > 0)
            {
                StorageOutOtherModel model = new StorageOutOtherModel();
    ...
    private void unconfirm(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            model.Closer = loginUser_id.ToString();
            model.ModifiedUserID = LoginUserID;
            if (StorageOutOtherBus.CheckIsUnfirm(model))
            {
                if (StorageOutOtherBus.unconfirm(model))
                {
