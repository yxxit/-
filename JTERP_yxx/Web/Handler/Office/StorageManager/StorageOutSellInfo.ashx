...
public class StorageOutSellInfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void GetSellSendList(HttpContext context)
    {
        ...
        string CustID = "";
        if (context.Request.QueryString["CustID"]!= null)
        {
            CustID = context.Request.QueryString["CustID"].ToString();
        }

        DataTable dt = StorageOutSellBus.GetSSDetailInfo(companyCD, txtNo, txtTitle,CustID);
        ...
        var dsLinq =
            ...
            (from x in dsXML.Descendants("Data")
             ...
             select new DataSourceModel()
             {
                 ...
                 Custom1=x.Element("Custom1").Value
             })
                      :
            (from x in dsXML.Descendants("Data")
    ...
    private void unconfirm(HttpContext context)
    {
        ...
        if (ID > 0)
        {
            ...
            model.Closer = loginUser_id.ToString();
            model.ModifiedUserID = LoginUserID;
            if (StorageOutSellBus.CheckIsUnfirm(model))
            {
                if (StorageOutSellBus.unconfirm(model))
                {
