        ...
        if (context.Request.RequestType == "POST")
        {
            ...
            else if (context.Request.QueryString["action"].Trim() == "Load")
            {
                try
                {
                    ...
                    //string ValidityBegin = context.Request.Params["ValidityBegin"].ToString().Trim();
                    //string ValidityEnd = context.Request.Params["ValidityEnd"].ToString().Trim();
                    //string MedFileDateBegin = context.Request.Params["MedFileDateBegin"].ToString().Trim();
                    //string MedFileDateEnd = context.Request.Params["MedFileDateEnd"].ToString().Trim();
                    //string MedCheckDateBegin = context.Request.Params["MedCheckDateBegin"].ToString().Trim();
                    //string MedCheckDateEnd = context.Request.Params["MedCheckDateEnd"].ToString().Trim();//药品检验
                    //string FileNo = context.Request.Params["FileNo"].ToString().Trim();//批准文号
                    //string Qualitystandard = context.Request.Params["Qualitystandard"].ToString().Trim();//质量标准
                    //string MedCheckNo = context.Request.Params["MedCheckNo"].ToString().Trim();//药品检验报告编号 
                    
                    //---------------------------------------------------------//
                    
                    model.CompanyCD = companyCD;
                   // DataTable dt = ProductInfoBus.GetProductInfo(model, EFIndex, EFDesc, pageIndex, pageCount, ord, ref totalCount);
                    DataTable dt = ProductInfoBus.GetProductInfo(model, pageIndex, pageCount, ord, ref totalCount);
                    System.Text.StringBuilder sb = new System.Text.StringBuilder();
                    sb.Append("{");
