<%@ WebHandler Language="C#" Class="TableCellsInfo" %>
...
public class TableCellsInfo: IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (action == "AddExtAttr")
        {
            ...
            string pCustomValues = context.Request.Params["pCustomValues"].ToString().Trim();//对应物品特性自定义项索引
            ...
            {
                DataTable dt1 = TableCellsBus.IsHaveRepeatNo(tableExtFields.CompanyCD, tableExtFields.BranchID, tableExtFields.ModelNo);
                if (dt1.Rows.Count > 0)
                {
                ...
                DataTable dt2 = TableCellsBus.IsHaveAttr(tableExtFields.CompanyCD, tableExtFields.BranchID, tableExtFields.TabName);
                if (dt2.Rows.Count > 0)
                {
                ...
                isSucc = TableCellsBus.Add(tableExtFields, out strMsg, CustomValues,pCustomValues);
            }
            else
                isSucc = TableCellsBus.Update(tableExtFields, out strMsg, CustomValues,pCustomValues);
        }
        else if (action == "update")//保存后修改
        {
            string CustomValues = context.Request.Params["CustomValues"].ToString().Trim();	//各项自定义的值
            string pCustomValues = context.Request.Params["pCustomValues"].ToString().Trim();//对应物品特性自定义项索引
            isSucc = TableCellsBus.Update(GetModel(context), out strMsg, CustomValues, pCustomValues);
        }
        ...
        else if (action == "GetDataByNo")//根据NO获取数据
        {
            ...
            DataTable dt = TableCellsBus.GetDataByNo(CompanyCD, BranchID, ModelNo);
        ...
        else if (action == "all")//获取列表
        {
            ...
            DataTable dt = TableCellsBus.GetAllList(CompanyCD, BranchID, TableName);
    ...
    private void DelOrder(HttpContext context)
    {
        ...
        string BranchID = "";
        JsonClass JC;
        if (TableCellsBus.Delete(CompanyCD, orderNos, BranchID, out strMsg, out strFieldText))
    ...
    private void GetLsit(HttpContext context)
    {
        ...
        DataTable dt = TableCellsBus.Getlist(FunctionModule, FormType, CompanyCD, BranchID, EFDesc, ModelNo, pageIndex, pageCount, ord, ref totalCount);
