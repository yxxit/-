...
public class ApprovalFlowSetAdd : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        else 
        {
            ...
            {
                //2013-10-09  bsd  修正部门名称强制换换位int型异常
                string strCompanyCD = string.Empty;//单位编号
                strCompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                string strsel = "select id,deptno from officedba.deptinfo where CompanyCD='" + strCompanyCD + "' and  deptname = '" + DeptName + "'";
                DataTable dtDepNo = XBase.Data.DBHelper.SqlHelper.ExecuteSql(strsel);
                model.DeptID =int.Parse( dtDepNo.Rows[0][0].ToString());
                //2013-10-09  bsd
            }
        }
        model.FlowNo = FlowID;
