    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (model != null)
        {
            if (ConstUtil.EDIT_FLAG_UPDATE.Equals(model.EditFlag))
            {
                ...
                if (string.IsNullOrEmpty(model.SuperDeptID))
                {
                    int rootCount = DeptInfoDBHelper.ISHaveRoot();
                    if (rootCount > 0)
                    {
                        string isd = DeptInfoDBHelper.GETDetptNo();
                        if (model.DeptNO != isd)
                        {
                            jc = new JsonClass("", "", 4);
                            sisi = true;
            ...
            else if (ConstUtil.EDIT_FLAG_INSERT.Equals(model.EditFlag))
            {
