...
public class SellModuleSelectCustUC: IHttpHandler, System.Web.SessionState.IRequiresSessionState 
{
    ...
    private void GetCustInfo(string strID, HttpContext context)
    {
        ...
        DataTable dt = SalaryPersonalRoyaltySetBus.GetCustInfo(strID);
        if (dt.Rows.Count == 1)
        {
    ...
    private void GetLsit(HttpContext context)
    {
        ...
        string BigType = context.Request.Form["CustType"].ToString().Trim().Length == 0 ? null : context.Request.Form["CustType"].ToString().Trim();
        
        int totalCount = 0;
        string ord = orderBy + " " + order;
        DataTable dt= SalaryPersonalRoyaltySetBus.GetCustListByType(OrderNo, BigType, Title, model, pageIndex, pageCount, ord, ref totalCount);
