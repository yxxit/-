<%@ WebHandler Language="C#" Class="UFCustName" %>
...
public class UFCustName : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private DataTable dtGetUFCust(string UFCustId, string UFCustName)
    {
        ...
        string selUFCust = "SELECT a.cCusCode id,a.cCusCode,a.cCusName,a.cCusAbbName,isnull(a.cCusAddress,'') cCusAddress,isnull(a.ccusperson,'') ccusperson," +
        "    isnull(a.cCusPPerson,'') cCusPPerson,isnull(b.cpersonname,'') cpersonname FROM " + Hc_Database + ".dbo.customer a" +
        "    left join  "+Hc_Database+".dbo.person b on a.cCusPPerson=b.cpersoncode"+
        "    inner join " + Hc_Database + ".dbo.customerclass  as cusclass on a.ccccode=cusclass.ccccode " +
        " where 1=1  and cusclass.cccname not like '%虚拟%'  ";
        if (UFCustId != "")
        {
