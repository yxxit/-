<%@ WebHandler Language="C#" Class="PayBill_List" %>
using System;
using System.Web;
using XBase.Common;
using System.Data;
...
public class PayBill_List : IHttpHandler, IRequiresSessionState
{
       ...
       //if (context.Request.ContentType == "POST")
        if(true)
        {
            ...
            if (Action == "Get")//查询所有
            {
                ...
                string PayNo = context.Request.Params["IncomeNo"].ToString().Trim();  //单据编号                
                ...
                string sql = @"select distinct a.ID,a.PayNo,a.CustName,a.BillingID
                            , REPLACE(CONVERT(varchar(100), a.PayDate, 23), '1900-01-01', '') AS PayDate,a.PayAmount
                            , (CASE a.AcceWay WHEN 0 THEN '现金' WHEN 1 THEN '银行存款' ELSE '其他' END) AcceWay
                            ,b.EmployeeName as Executor,c.EmployeeName as Confirmor
                            , REPLACE(CONVERT(varchar(100), a.ConfirmDate, 23), '1900-01-01', '') AS ConfirmDate
                            from Officedba.PayBill a
                            left join officedba.EmployeeInfo b on a.Executor=b.id
                            left join officedba.EmployeeInfo c on a.Confirmor=c.id" +
                               " where a.CompanyCD = '" + CompanyCD + "'";


                if (PayNo != "")
                    sql += " and a.PayNo like '%" + PayNo + "%'";
                if (Executor != "")
                    sql += " and b.Executor like '%" + Executor + "%'";
                if (ConfirmStatus != "")
                    sql += " and a.ConfirmStatus like '%" + ConfirmStatus + "%'";
                if (StartDate != "")
                    sql += " and PayDate>='" + StartDate + "'";
                if (EndDate != "")
                    sql += " and PayDate<='" + EndDate + "'";
                if (CustName != "")
    ...
    private bool IsSure(string[] ALLID)
    {
        ...
        {
            string sql = "select ConfirmStatus from Officedba.PayBill where id=" + ExpIDs[0];
    ...
    private bool DeleteDataByIds(string[] ALLID)
    {
        ...
        string sql = "delete from Officedba.PayBill where id in (" + allExpID + ")";
