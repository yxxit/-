<%@ WebHandler Language="C#" Class="IncomeBill_Add" %>
...
using System.Text;
public class IncomeBill_Add : IHttpHandler, IRequiresSessionState
{
    ...
    private void LoadIncomBill(HttpContext context)
    {
        ...
        IncomeBillModels ibm = new IncomeBillModels();
        ...
        string strQuery = @"SELECT     a.ConfirmStatus, a.id, a.IncomeNo, REPLACE(CONVERT(varchar(12), a.AcceDate, 23), '1900-01-01', '') AS AcceDate, a.CustName, a.FromBillType, 
                      a.BillingID, a.TotalPrice, a.PaymentTypes, a.AcceWay, b.EmployeeName AS Executor, REPLACE(CONVERT(varchar(12), a.AccountDate, 23), 
                      '1900-01-01', '') AS AccountDate, d.EmployeeName AS Accountor, REPLACE(CONVERT(varchar(12), a.ConfirmDate, 23), '1900-01-01', '') AS ConfirmDate, 
...
where a.id=" +headid+"";
    ...
    private void EditDate(HttpContext context)
    {
        ...
        ibm.InComeNo = request.Params["IncomeNo"].ToString().Trim();//单据编号
        ibm.AcceDate = request.Params["AcceDate"].ToString().Trim();//收款日期
        ...
        ibm.TotalPrice = Convert.ToDouble(request.Params["TotalPrice"].ToString().Trim());//收款金额
        ibm.PaymentTypes = Convert.ToInt32(request.Params["PaymentTypes"].ToString().Trim());//款项类型
        ...
        int isSucc = IncomeBillDeal.IncomeBillEdit(ibm);
    ...
    private void InsertData(HttpContext context)
    {
        ...
        //    }
        //}
        bool ishave = XBase.Business.Common.PrimekeyVerifyBus.CheckCodeUniq_jt("Officedba.IncomeBill", "InComeNo", request.Params["IncomeNo"].ToString().Trim());
        if (!ishave)
        {
            jc = new JsonClass("faile", "该编号已被使用，请输入未使用的编号！", -1);
            context.Response.Write(jc);
            context.Response.End();
        }
        ...
        ibm.Accountor = EmployeeID;//--------------------------------------------------------------------------------------------------------修改

        ibm.CompanyCD = CompanyCD;
        int isSucc=IncomeBillDeal.IncomeBillAdd(ibm);
    ...
    private void ConfirmDate(HttpContext context)
    {
        ...
        HttpRequest request = context.Request;//获取request
        ibm.Id = Convert.ToInt32(request.Params["ID"].ToString().Trim());//----------------------------->update 2014-7-14
        ibm.InComeNo = request.Params["IncomeNo"].ToString().Trim();
        ibm.AcceDate = request.Params["AcceDate"].ToString().Trim(); ;//收款日期
        ibm.CustName = request.Params["CustName"].ToString().Trim();//往来顾客
        ibm.FromBillType = request.Params["FromBillType"].ToString().Trim() == "" ? "0" : request.Params["FromBillType"].ToString().Trim();//业务单类型
        ibm.BillingID = Convert.ToInt32(request.Params["BillingID"].ToString().Trim());//来源单编号,现在不要去去做
        ...
        ibm.CompanyCD = CompanyCD;
        bool isSucc = IncomeBillDeal.ConfirmIncomeBill(ibm);
        if (isSucc)
        {
    ...
    private void UpdateDate(HttpContext context)
    {
        ...
        ibm.CompanyCD = CompanyCD;
        bool isSucc = IncomeBillDeal.CloseConfirmIncomeBill(ibm);
        if (isSucc)
        {
