    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.RequestType == "POST")
        {
            ...
            else if (Action == "SearchQTestList")  //检索质检单列表
            {
                ...
                string strQuery = " "+
               "     select a.id,a.reportno,a.othercorpid as providerid,d.custname as providername,"+
               "     isnull(b.CoalType,'') as coalid,isnull(e.productname,'') as coalname,"+
               "     isnull(b.checknum,0) checknum,isnull(b.checkresult,'') checkresult ,f.ship_place,f.to_place "+
               "     from jt_cgzj a inner join jt_cgzj_mx b on a.id=b.reportno "+
               "     left join jt_cgdh c on a.FromReportNo=c.id "+
               "     left join jt_HuocheDiaoyun f on c.TransPortNo=f.id_at "+
               "     left join officedba.providerinfo d on a.othercorpid=d.id "+
               "     left join officedba.ProductInfo e on b.CoalType=e.id "+
                ...
                if (ProductName != "")
                    strQuery += " and e.productname like '%" + ProductName + "%'";
                if (StartPlace != "")
                    strQuery += " and f.ship_place like '%" + StartPlace + "%'";
                // dt = SqlHelper.ExecuteSql(strQuery);
                // TotalCount = dt.Rows.Count;
                System.Data.SqlClient.SqlCommand comm = new System.Data.SqlClient.SqlCommand();
                comm.CommandText = strQuery;
            ...
            else if (Action == "SearchQTestInfo") //查询质检单详细信息
            {
                ...
                string strQuery = @"select a.id ,a.ReportNo,a.FromReportNo as SourceBillID, b.ArriveNo as SourceBillNo,
                                    b.ProviderID,c.CustName as ProviderName,CONVERT(varchar(100),a.CheckDate, 23) as CheckDate,a.CheckMode,
                                    e.id_at as TranSportID,e.DiaoyunNO,g.id as CoalID ,g.productname as CoalName,
                                    ...
                                    from dbo.jt_cgzj a
                                    left join dbo.jt_cgdh b on b.id=a.FromReportNo
                                    left join officedba.ProviderInfo c on c.id=b.ProviderID
                                    left join dbo.jt_HuocheDiaoyun e on e.id_at=b.TransPortNo
