    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.RequestType == "POST")
        {
            ...
            if (Action == "Delete")
            {
                ...
                {
                    string CustID = context.Request.Params["CustID" + j];
                    ids += CustID + ",";
                    if (CustID != null && CustID != "")
                    {
                        //DataTable dtt = PrimekeyVerifyBus.CheckQuote("ContractHead_Pur", "ProviderID", CustID);
                        //if (dtt.Rows.Count > 0)
                        //{
                        //    for (int k = 0; k < dtt.Rows.Count; k++)
                        //    {
                        //        StrAlert += dtt.Rows[k]["OrderName"] + "，";
                        //    }
                        //}
                    }
                }
                if (StrAlert != "")
                {
                    string StrInfo = "被" + StrAlert + "引用，无法删除";
                    jc = new JsonClass(StrInfo, "", 0);
            ...
            else
            {
                ...
                //--------------------------------医药行业专用信息 2012-09-24-----------------------------------------------//
                //string Usebegin = context.Request.Params["Usebegin"].ToString().Trim();
                //string Useend = context.Request.Params["Useend"].ToString().Trim();
                //string Certibegin = context.Request.Params["Certibegin"].ToString().Trim();
                //string Certiend = context.Request.Params["Certiend"].ToString().Trim();
                //string Warbegin = context.Request.Params["Warbegin"].ToString().Trim();
                //string Warend = context.Request.Params["Warend"].ToString().Trim();
                //string Powerbegin = context.Request.Params["Powerbegin"].ToString().Trim();
                //string Powerend = context.Request.Params["Powerend"].ToString().Trim();
                //string Gmspbegin = context.Request.Params["Gmspbegin"].ToString().Trim();
                //string Gmspend = context.Request.Params["Gmspend"].ToString().Trim();
                //-------------------------------------------------------------------------//Usebegin,Useend,Certibegin,Certiend,Warbegin,Warend,Powerbegin,Powerend,Gmspbegin,Gmspend
                int TotalCount = 0;
                ...
                //---------------------------------医药行业专用信息 2012-09-24-------------------------------------------------//
              DataTable dt = ProviderInfoBus.SelectProviderInfo(pageIndex, pageCount, orderBy, ref TotalCount, CustNo, CustName,  Manager);
