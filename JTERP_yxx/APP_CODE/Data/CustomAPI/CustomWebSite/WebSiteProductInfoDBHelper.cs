...
namespace XBase.Data.CustomAPI.CustomWebSite
{
    ...
    public class WebSiteProductInfoDBHelper
    {
        ...
        private const string C_SELECT_ID =
                                " SELECT wspi.ID,wspi.ProductID,cut.CodeName AS UnitName,wspi.Description,wspi.Price,wspi.ImgDIr,wspi.Status,wspi.DiscountStatus,pi1.ProductName" +
                                " FROM websitedba.WebSiteProductInfo wspi" +
                                " LEFT JOIN officedba.ProductInfo pi1 ON ProductID=pi1.ID" +
                                " LEFT JOIN officedba.CodeUnitType AS cut ON cut.ID=pi1.UnitID "+
                                " WHERE wspi.ID=@ID";
