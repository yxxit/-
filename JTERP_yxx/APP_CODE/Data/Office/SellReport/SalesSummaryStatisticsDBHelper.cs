...
namespace XBase.Data.Office.SellReport
{
    ...
    {
        public static DataTable GetSalesSummary(string strCompanyCD, string startTime, string endTime, string sumType, int pageIndex, int pageSize, string orderBy, ref int totalCount)
        {
            ...
            switch (sumType)
            {
                case "byDept":
                    ...
                    sbSql.AppendLine("AND CONVERT(DATETIME, CONVERT(VARCHAR(11),createdate))<=@EndDate  AND  CompanyCD = @Company  ");
                    sbSql.AppendLine("GROUP BY SellDept) AS a LEFT JOIN officedba.DeptInfo AS b ON ");
                    sbSql.AppendLine("a.SellDept=b.ID and b.CompanyCD = @Company");
                    break;
                case "bySeller":
                    ...
                    sbSql.AppendLine(" FROM ( ");
                    sbSql.AppendLine(" SELECT a.CompanyCD,b.CompanyCD as CompanyCD1,a.sellerID,b.sellNum,b.sellPrice,b.CreateDate  ");
                    sbSql.AppendLine(" FROM officedba.sellerRate AS a ");
                    sbSql.AppendLine(" LEFT JOIN officedba.SellReport AS b ON  ");
                    sbSql.AppendLine(" a.sellreportID=b.ID and b.CompanyCD = @Company and a.CompanyCD = b.CompanyCD ) AS a  ");
                    sbSql.AppendLine(" WHERE  CONVERT(DATETIME,  CONVERT(VARCHAR(11),a.createdate))>=@StartDate ");
                    sbSql.AppendLine(" AND CONVERT(DATETIME, CONVERT(VARCHAR(11),a.createdate))<=@EndDate and a.CompanyCD = @Company and a.CompanyCD1 = @Company   ");
                    sbSql.AppendLine(" GROUP BY SellerID ) AS a LEFT JOIN officedba.EmployeeInfo AS b ");
                    sbSql.AppendLine(" ON a.sellerID =b.ID  and b.CompanyCD = @Company  ");
                    break;
                case "byProduct":
            ...
            sqlParams[index++] = SqlHelper.GetParameter("@Company", strCompanyCD);
