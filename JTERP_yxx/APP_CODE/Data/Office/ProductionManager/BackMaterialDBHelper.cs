...
namespace XBase.Data.Office.ProductionManager
{
    public class BackMaterialDBHelper
    {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static bool InsertBackMaterial(BackMaterialModel model, Hashtable htExtAttr, string loginUserID, out string ID, string[] strCustomdetail)
        {
        ...
        private static void SaveDetail(BackMaterialModel model, ArrayList list, string loginUserID, string[] strCustomdetail)
        {
        ...
        /// <param name="loginUserID"></param>
        /// <returns></returns>
        public static bool UpdateBackMaterialInfo(BackMaterialModel model, Hashtable htExtAttr, string loginUserID, string[] strCustomdetail)
        {
            //获取登陆用户ID
            ArrayList listADD = new ArrayList();
        ...
        public static DataTable GetBackDetailInfo(BackMaterialModel model)
        {
            ...
            detSql.AppendLine(@"				select	isnull(A.Custom1,'')Custom1,isnull(A.Custom2,'')Custom2,isnull(A.Custom3,'')Custom3,isnull(A.Custom4,'')Custom4,isnull(A.Custom5,'')Custom5,isnull(A.Custom6,'')Custom6,isnull(A.Custom7,'')Custom7,isnull(A.Custom8,'')Custom8,isnull(A.Custom9,'')Custom9,isnull(A.Custom10,'')Custom10,
                                                    a.CompanyCD,a.ID as DetailID,isnull(b.Specification,'')Specification,a.BackNo,a.SortNo,a.ProductID,a.FromBillID,a.FromBillNo,b.ProdNo,b.ProductName,a.StorageID,e.StorageName,");
        ...
        /// <returns></returns>
        public static DataTable GetBackMaterialListBycondition(BackMaterialModel model, string ReceiveDateStart, string ReceiveDateEnd, int BillTypeFlag, int BillTypeCode, int FlowStatus, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount,string time1,string time2,string prodid,string storageid)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            ...
            searchSql.AppendLine("				select	a.CompanyCD,a.ID,p.productname,de.backcount,de.price,de.totalprice,isnull(dbo.getStorageName(de.storageid),'')storagename,a.BackNo,a.Subject,isnull( CONVERT(CHAR(10), a.ReceiveDate, 23),'') as ReceiveDate,");
            ...
            searchSql.AppendLine("				from officedba.BackMaterial a left join officedba.BackMaterialdetail de on a.companycd=de.companycd and a.BackNo=de.BackNo left join officedba.productinfo p on p.id=de.productid");
            ...
            searchSql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Taker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  OR (a.Receiver IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  OR (a.SaleID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) )");
            ...
            if (model.TakeID > 0)
            {
                searchSql.AppendLine(" and a.TakeID=@TakeID ");
            ...
            if (model.Receiver > 0)
            {
                searchSql.AppendLine(" and a.Receiver=@Receiver ");
            ...
            {
                searchSql.AppendLine(" and a.ReceiveDate>=@ReceiveDateStart ");
            ...
            {
                searchSql.AppendLine(" and a.ReceiveDate<=@ReceiveDateEnd ");
            ...
            if (prodid != "")
            {
                searchSql.AppendLine(" and p.productname like'%" + prodid + "%'");
            }
            ...
            if (FlowStatus > -1)
            {
                searchSql.AppendLine("and f.FlowStatus=@FlowStatus ");
        ...
        public static bool ConfirmOrCompleteBackMaterial(BackMaterialModel model, string loginUserID, int OperateType)
        {
            ...
            if (OperateType == 1)
            {
                ...
                commTake.CommandText = sql.ToString();
                ...
                string sqldt = @"select productid,storageid,isnull(batchno,'')batchno,isnull(b.BackCount,0)backcount from officedba.BackMaterial a left join officedba.BackMaterialdetail b on 
a.backno=b.backno and a.companycd=b.companycd where a.id=" + model.ID + " and a.companycd='" + model.CompanyCD + "'";
                SqlCommand comm = new SqlCommand();
                comm.CommandText = sqldt;
                ...
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        sql1 += "update  officedba.StorageProduct set productcount=isnull(productcount,0)+" + dt.Rows[i]["backcount"].ToString() + " where  storageid=" + dt.Rows[i]["storageid"].ToString() + " and productid=" + dt.Rows[i]["productid"].ToString() + " and " + batch + " and companycd='" + model.CompanyCD + "' ;";
        ...
        public static bool CancelConfirmOperate(BackMaterialModel model, int BillTypeFlag, int BillTypeCode, string loginUserID)
        {
            ...
            try
            {
                ...
                SqlCommand comm = new SqlCommand();
                string sql = @"select productid,storageid,isnull(batchno,'')batchno,isnull(b.BackCount,0)backcount from officedba.BackMaterial a left join officedba.BackMaterialdetail b on 
a.backno=b.backno and a.companycd=b.companycd where a.id=" + model.ID + " and a.companycd='" + model.CompanyCD + "'";
        ...
        private static void GetExtAttrCmd(BackMaterialModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@BackNo", model.BackNo);
