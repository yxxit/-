...
namespace XBase.Data.Office.ProductionManager
{
    public class TakeMaterialDBHelper
    {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static bool InsertTakeMaterial(TakeMaterialModel model, Hashtable htExtAttr, string loginUserID, out string ID, string[] strCustomdetail)
        {
            ...
            try
            {
                ...
                sqlTake.AppendLine("           ,ModifiedUserID,Handout					");
                if (model.TakeDate > DateTime.Parse("1753-01-01"))
                {
                    sqlTake.AppendLine("           ,TakeDate					");
                }
                ...
                sqlTake.AppendLine("           ,'" + loginUserID + "',@Handout        ");
                if (model.TakeDate > DateTime.Parse("1753-01-01"))
                {
                    sqlTake.AppendLine("           ,@TakeDate					");
                }
                ...
                comm.Parameters.Add(SqlHelper.GetParameter("@Handout", model.Handout));
                if (model.TakeDate >DateTime.Parse("1753-01-01"))
                {
                    comm.Parameters.Add(SqlHelper.GetParameter("@TakeDate", model.TakeDate));
                }
                ...
                //领料单明细添加SQL语句
                SaveDetail(model, listADD, loginUserID, strCustomdetail);
        ...
        private static void SaveDetail(TakeMaterialModel model, ArrayList list, string loginUserID, string[] strCustomdetail)
        {
            ...
            for (int i = 0; i < detSortNo.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(detBatchNo[i].Trim()))//liu
                {//批次
                ...
                //添加扩展项
                sqlDet.Append(GetBillTableCellsDBHelper.insertCustomFilder(strCustomdetail, (i + 1)));
                //扩展项值
                sqlValues.Append(GetBillTableCellsDBHelper.insertCustomvalue(strCustomdetail, (i + 1)));
                sqlDet.Append(")");
                sqlValues.Append(")");
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static bool UpdateTakeMaterialInfo(TakeMaterialModel model, Hashtable htExtAttr, string loginUserID, string[] strCustomdetail)
        {
            //获取登陆用户ID
            ArrayList listADD = new ArrayList();
            ...
            sqlEdit.AppendLine("      ,ProjectID = @ProjectID,Handout=@Handout            ");
            if (model.TakeDate > DateTime.Parse("1753-01-01"))
            {
                sqlEdit.AppendLine("      ,TakeDate=@TakeDate             ");
            }
        ...
        public static bool isCanTakeMaterial(TakeMaterialModel model, out string reason)
        {
            ...
            if (model.DetProductID.Length > 0)
            {
                ...
                for (int i = 0; i < detProductID.Length; i++)
                {
                    ...
                    string strStock = isStockIs(model.CompanyCD, ProductID);
                    if (TakeCount > productCount && strStock == "0")
                    {
                        isCan = false;
        ...
        public static DataTable GetTakeDetailInfo(TakeMaterialModel model)
        {
            ...
            detSql.AppendLine(@"				select	isnull(A.Custom1,'')Custom1,isnull(A.Custom2,'')Custom2,isnull(A.Custom3,'')Custom3,isnull(A.Custom4,'')Custom4,isnull(A.Custom5,'')Custom5,isnull(A.Custom6,'')Custom6,isnull(A.Custom7,'')Custom7,isnull(A.Custom8,'')Custom8,isnull(A.Custom9,'')Custom9,isnull(A.Custom10,'')Custom10,
                                                a.CompanyCD,a.ID as DetailID,a.TakeNo,a.SortNo,a.ProductID,b.ProductName,b.Specification,b.ProdNo,a.BatchNo,isnull(b.IsBatchNo,0) as IsBatchNo,");
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public static DataTable GetTakeDetailInfo1(string ids)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            #region 查询语句
            //查询SQL拼写
            ...
            detSql.AppendLine("             left join officedba.CodeUnitType e on a.UsedUnitID=e.ID");
            detSql.AppendLine("				left join officedba.StorageInfo d on a.StorageID=d.ID");
            detSql.AppendLine("				where a.id in ( "+ids+" )");
            detSql.AppendLine(") as info");
            detSql.AppendLine("where CompanyCD=@CompanyCD ");
            #endregion
            ...
            //comm.Parameters.Add(SqlHelper.GetParameterFromString("@ID", ids));
        ...
        /// <returns></returns>
        public static DataTable GetTakeMaterialListBycondition(TakeMaterialModel model, string TakeDateStart, string TakeDateEnd, int BillTypeFlag, int BillTypeCode, int FlowStatus, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount,string time1,string time2,string prodid,string storageid)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            ...
            searchSql.AppendLine("select	a.CompanyCD,a.ID,de.id deid,pro.productname,de.Remark detailremark,Convert(numeric(12," + point + "),isnull(de.takecount,0)) takecount,Convert(numeric(12," + point + "),isnull(de.price,0)) price,Convert(numeric(12," + point + "),isnull(de.totalprice,0)) totalprice,isnull(dbo.getStorageName(de.storageid),'')storagename, a.TakeNo,a.Subject,a.FromType,a.TaskID,isnull( f.TaskNo,'')as TaskNo,a.ModifiedDate,");
            ...
            searchSql.AppendLine("from officedba.TakeMaterial a left join officedba.TakeMaterialdetail de on a.companycd=de.companycd and a.TakeNo=de.TakeNo left join officedba.productinfo pro on pro.id=de.productid");
            ...
            searchSql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Taker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Handout IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.SaleID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) )");
            ...
            if (time1 != "" && time2 != "")
            {
                searchSql.AppendLine(" and a.CreateDate between '" + time1 + "' and '" + time2 + "' ");
            }
            else if (time1 == "" && time2 != "")
            {
                searchSql.AppendLine(" and a.CreateDate<='" + time2 + "' ");
            }
            else if (time1 != "" && time2 == "")
            {
                searchSql.AppendLine(" and a.CreateDate>='" + time1 + "' ");
            }
            if (prodid != "")
            {
                searchSql.AppendLine(" and pro.productname like'%" + prodid + "%'");
            }
            if (storageid != "" && storageid != "0")
            {
                searchSql.AppendLine(" and de.storageid =" + storageid + "");
            }
            //审批状态
            if (FlowStatus > -1)
            {
        ...
        public static DataTable GetTaskDetailBom_ByTaskID(string intTaskID, string CompanyCD)
        {
            ...
            StringBuilder infoSql = new StringBuilder();
            infoSql.AppendLine("select CompanyCD,ProductID,Convert(numeric(10,2),ProductCount) as ProductCount from officedba.ManufactureTaskDetail ");
            infoSql.AppendLine("where id in("+intTaskID+")");
            infoSql.AppendLine("and BomID is not null");
            infoSql.AppendLine("and CompanyCD=@CompanyCD");
                    ...
                    else
                    {
                        ...
                        if (dtDetail.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dtDetail.Rows.Count; i++)
                            {
                                ...
                                if (TakeCount <= productCount || strStock == "1")
                                {
                                    #region 更新分仓存量表
        ...
        public static string isStockIs(string CompanyCD, int intProductID)
        {
            string sql = "select StockIs  from officedba.ProductInfo where  ID=@ProductID and CompanyCD=@CompanyCD ";
            SqlParameter[] parms = new SqlParameter[2];
            parms[0] = SqlHelper.GetParameter("@CompanyCD", CompanyCD);
            parms[1] = SqlHelper.GetParameter("@ProductID", intProductID);
            object obj = SqlHelper.ExecuteScalar(sql, parms);
            if (obj == null)
            {
        ...
        public static bool ConfirmOrCompleteTakeMaterial(TakeMaterialModel model, string loginUserID, int OperateType)
        {
            ...
            if (OperateType == 1)
            {
                ...
                sqlTake.AppendLine(" ConfirmDate        = getdate(),");
                ...
                SqlCommand comm=new SqlCommand();
                string sql = @"select productid,storageid,isnull(batchno,'')batchno,isnull(takecount,0)takecount from officedba.TakeMaterial a left join officedba.TakeMaterialDetail b on 
a.takeno=b.takeno and a.companycd=b.companycd where a.id=" + model.ID + " and a.companycd='" + model.CompanyCD+"'";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {   string batch="";
                        ...
                        {
                            batch = "batchno is null ";
                        }else
                        {
                            batch = "batchno='" + dt.Rows[i]["batchno"].ToString() + "' ";
                        }
                        sql1+="update  officedba.StorageProduct set productcount=isnull(productcount,0)-" + dt.Rows[i]["takecount"].ToString() + " where  storageid=" + dt.Rows[i]["storageid"].ToString() + " and productid=" + dt.Rows[i]["productid"].ToString() + " and " + batch + " and companycd='" + model.CompanyCD + "' ;";
        ...
        private static void GetExtAttrCmd(TakeMaterialModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@TakeNo", model.TakeNo);
        ...
        /// <param name="BillTypeCode"></param>
        /// <returns></returns>
        public static DataTable GetTakeMaterialListBycondition_Operating(string CompanyCD, int ProcessDeptID, string TaskNo, string ProdNo, string ProductName, string ConfirmDateStart, string ConfirmDateEnd, int pageIndex, int pageCount, string OrderBy, ref int totalCount, string ProjectID, string sltManufactureType)
        {
            ...
            searchSql.AppendLine("	select  a.ProjectID,p.ProjectName, isnull( dbo.getCodeublic(a.ManufactureType),'') as strManufactureType, a.ManufactureType,b.ID,a.CompanyCD,a.ProcessDeptID,c.DeptName,a.TaskID,d.TaskNo,a.TakeNo,");
            ...
            searchSql.AppendLine("	from officedba.TakeMaterial a LEFT JOIN officedba.ProjectInfo p on a.ProjectID=p.ID ");
            ...
            {
                searchSql.AppendLine(" and ProjectID="+ProjectID+" ");
            }
            if (sltManufactureType != "")
            {
                searchSql.AppendLine(" and ManufactureType=" + sltManufactureType + " ");
            }
        ...
        public static string ISBigUseCountWhenCant(TakeMaterialModel model)
        {
            string batchsql = "SELECT A.BatchNo FROM officedba.TakeMaterialDetail A LEFT OUTER JOIN officedba.TakeMaterial B on A.TakeNo=B.TakeNo AND A.CompanyCD=B.CompanyCD where A.CompanyCD='" + model.CompanyCD + "' and B.ID=" + model.ID + "";
            ...
            if (dtbatch.Rows.Count > 0)
            {
                for (int i = 0; i < dtbatch.Rows.Count; i++)
                {
                    ...
                    sql.AppendLine("select b.ID,a.ExRate,a.ProductID,a.StorageID,a.TakeCount                                                                          ");
                    sql.AppendLine(",ISNULL(d.MinusIs,0) as MinusIs                                                                                             ");
                    sql.AppendLine(",Convert(numeric(22," + point + "),ISNULL(c.ProductCount,0))+isnull(c.roadcount,0)+isnull(c.incount,0)-isnull(c.ordercount,0)-isnull(c.outcount,0) as UseCount ");
                    sql.AppendLine(" from officedba.TakeMaterialDetail a                                                                                      ");
                    sql.AppendLine("left outer join officedba.TakeMaterial b on a.TakeNo=b.TakeNo                                                                     ");
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        if (dt.Rows.Count == 1)
                        {
                            if (decimal.Parse(dt.Rows[0]["TakeCount"].ToString()) > decimal.Parse(dt.Rows[0]["UseCount"].ToString()))
                            {
                        ...
                        {
                            if (decimal.Parse(dt.Rows[i]["TakeCount"].ToString()) > decimal.Parse(dt.Rows[i]["UseCount"].ToString()))
                            {
                                if (RowNumList == "" || RowNumList == string.Empty)
                                {
                                    RowNumList = (i + 1).ToString();
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable GetTakeMaterialSum_Operating(string CompanyCD, string ProdNo, string ProductName, string ConfirmDateStart, string ConfirmDateEnd, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            string point = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).SelPoint;
            //定义查询的命令
            SqlCommand comm = new SqlCommand();
            ...
            //查询SQL拼写
            StringBuilder searchSql = new StringBuilder();
            searchSql.AppendLine("select temptb.TakeCount,temptb.BackCount,p.ID AS ProductID,p.ProdNo,p.ProductName ");
            searchSql.AppendLine(" ,isnull(p.Specification,'') Specification,p.UnitID,f.CodeName as UnitName from (");
            searchSql.AppendLine("	select a.CompanyCD,a.ProductID,Convert(numeric(10," + point + "),sum(isnull(a.TakeCount,0))) as TakeCount,");
            searchSql.AppendLine("			Convert(numeric(10,"+point+"),sum(isnull(a.BackCount,0))) as  BackCount ");
            searchSql.AppendLine("	from officedba.takematerialdetail a  ");
            searchSql.AppendLine("	left join officedba.takematerial  b on b.takeno=a.takeno and b.companycd=a.companycd");
            searchSql.AppendLine("	where a.companycd=@CompanyCD and b.billstatus=2 ");
            //起始日期
            if (!string.IsNullOrEmpty(ConfirmDateStart))
            {
                searchSql.AppendLine(" and b.ConfirmDate>=@ConfirmDateStart ");
            }
            //截止日期
            if (!string.IsNullOrEmpty(ConfirmDateEnd))
            {
                searchSql.AppendLine(" and b.ConfirmDate<=@ConfirmDateEnd ");
            }

            searchSql.AppendLine("	group  by a.companycd,a.productid ");
            searchSql.AppendLine(") as temptb");
            searchSql.AppendLine(" left join officedba.Productinfo p on p.id=temptb.ProductID ");
            searchSql.AppendLine(" left join officedba.CodeUnitType f on p.UnitID=f.ID ");
            ...
            if (!string.IsNullOrEmpty(ProdNo))
            {
                searchSql.AppendLine(" and p.ProdNo like @ProdNo");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProdNo", "%" + ProdNo + "%"));
            }
            //--物料名称
            if (!string.IsNullOrEmpty(ProductName))
            {
                searchSql.AppendLine(" and p.ProductName like @ProductName");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProductName", "%" + ProductName + "%"));
            }
