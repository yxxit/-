...
namespace XBase.Data.Office.ProductionManager
{
    public class MRPDBHelper
    {
        ...
        public static DataTable GetMRPDetailInfo(MRPModel model)
        {
            ...
            detSql.AppendLine("			a.ProductID,b.ProdNo,b.Manufacturer,b.ProductName,a.UnitID,c.CodeName as UnitName,a.UsedUnitID,e.CodeName as UsedUnitName,Convert(numeric(14," + userInfo.SelPoint + "),a.UsedUnitCount) as UsedUnitCount,");
        ...
        //add 20110506 lcb
        #region 采购订单直接来源于物料需求计划
        /// <summary>
        /// MRP源单明细信息
        /// </summary>
        /// <param name="model"></param>
        /// <returns>DataTable</returns>
        public static DataTable GetPurchaseMRPDetail(string CompanyCD, string ProductNo, string ProductName, string StartDate, string EndDate
            , int pageIndex, int pageCount, string OrderBy, string OrderByType, ref int totalCount)
        {
            ...
            StringBuilder detSql = new StringBuilder();
            detSql.AppendLine("select * from (");
            //detSql.AppendLine("	select  a.CompanyCD,a.ID,a.ID as FromBillID,a.MRPNo as FromBillNo,");
            //detSql.AppendLine("			a.ProductID,b.ProdNo,b.Manufacturer,b.ProductName,a.UnitID,c.CodeName as UnitName,a.UsedUnitID,e.CodeName as UsedUnitName,Convert(numeric(14," + userInfo.SelPoint + "),a.UsedUnitCount) as UsedUnitCount,");
            //detSql.AppendLine("			Convert(numeric(14," + userInfo.SelPoint + "),a.GrossCount) as GrossCount,Convert(numeric(14," + userInfo.SelPoint + "),a.PlanCount) as PlanCount,isnull( CONVERT(CHAR(10), a.PlanDate, 23),'') as PlanDate,a.MaterialSource,");
            //detSql.AppendLine("			a.FromBillID,a.FromBillNo,a.FromLineNo,a.Remark,Convert(numeric(14," + userInfo.SelPoint + "),a.ProcessedCount) as ProcessedCount,");
            //detSql.AppendLine("         case when a.MaterialSource=0 then '采购' when a.MaterialSource=1 then '生产' when a.MaterialSource=2 then '库存' end as strMaterialSource,");
            ...
            //detSql.AppendLine("					select top 1 MRPNo from officedba.MRP where ID=@ID");
            //detSql.AppendLine("			     )");
            detSql.AppendLine("  SELECT  ISNULL(A.ID, 0) AS ID, ISNULL(D.ID, 0) AS FromBillID, ISNULL(A.MRPNo, 0) AS FromBillNo, ISNULL(A.CompanyCD, 0) AS CompanyCD, ISNULL(A.SortNo, 0) AS FromLineNo,");
            detSql.AppendLine("     ISNULL(A.ProductID, 0) AS ProductID, ISNULL(C.ProdNo, 0) AS ProductNo, ISNULL(C.ProductName, 0) AS ProductName, ISNULL(A.UnitID, 0) AS UnitID,");
            detSql.AppendLine("     ISNULL(B.CodeName, '') AS UnitName, ISNULL(C.Specification, '') AS Specification, ISNULL(C.standardsell, 0) AS UnitPrice,");
            detSql.AppendLine("     ISNULL(C.selltax, 0) AS TaxPrice, ISNULL(A.PlanCount, 0) AS ProductCount, A.PlanCount AS RequireCount, ISNULL(CONVERT(varchar(23), A.PlanDate, 23), '') AS RequireDate,");
            detSql.AppendLine("     ISNULL(C.taxrate, 0) AS InTaxRate, ISNULL(C.TypeID, 0) AS ProdTypeID,");
            detSql.AppendLine("     ISNULL(D.DeptID, 0) AS PlanDeptID, ISNULL(F.DeptName, '') AS PlanDeptName, A.UsedUnitID, ISNULL(A.UsedUnitCount, 0) AS UsedUnitCount, ISNULL(H.CodeName, '') AS UsedUnitName,");
            detSql.AppendLine("     K.TypeName AS ColorName, D.Creator");
            detSql.AppendLine("  FROM  officedba.MRPDetail AS A LEFT OUTER JOIN");
            detSql.AppendLine("     officedba.CodeUnitType AS B ON A.UnitID = B.ID AND A.CompanyCD = B.CompanyCD LEFT OUTER JOIN");
            detSql.AppendLine("     officedba.ProductInfo AS C ON A.ProductID = C.ID AND A.CompanyCD = C.CompanyCD INNER JOIN");
            detSql.AppendLine("     officedba.MRP AS D ON A.MRPNo = D.MRPNo AND A.CompanyCD = D.CompanyCD AND D.BillStatus = '2' AND A.MaterialSource='0' LEFT OUTER JOIN");
            detSql.AppendLine("     officedba.DeptInfo AS F ON D.DeptID = F.ID LEFT OUTER JOIN");
            detSql.AppendLine("     officedba.CodeUnitType AS H ON A.UsedUnitID = H.ID LEFT OUTER JOIN");
            detSql.AppendLine("     officedba.CodePublicType AS K ON K.CompanyCD = C.CompanyCD AND C.ColorID = K.ID");
            detSql.AppendLine(") as info");
            detSql.AppendLine("where CompanyCD=@CompanyCD AND ProductNo like '%" + ProductNo + "%' AND ProductName like '%" + ProductName + "%'");
            
            if (StartDate != null && StartDate != "")
            {
                detSql.AppendLine(" AND RequireDate = convert(varchar(23),@StartDate1,23)");
            }
            ...
            if (StartDate != null && StartDate != "")
            {
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@StartDate1", StartDate));
            }            
            //comm.Parameters.Add(SqlHelper.GetParameterFromString("@EndDate1", EndDate));

            //指定命令的SQL文
            comm.CommandText = detSql.ToString();
            //执行查询
            return SqlHelper.PagerWithCommand(comm, pageIndex, pageCount, OrderBy + " " + OrderByType, ref totalCount);

            //PagerWithCommand(comm, pageIndex, pageCount, OrderBy, ref totalCount);
        }
        #endregion
        ...
        public static Decimal GetStorage(string companycd, string proid)
        {
            //string sql = "select isnull(sum(productcount),0)productcount from officedba.StorageProduct where companycd='"+companycd+"' and productid="+proid+" group by productid";
            string sql=string.Format(@"select isnull(sum(a.productcount),0)-isnull(sum(b.SafeStockNum),0)productcount from officedba.StorageProduct a left join 
                                        officedba.ProductInfo b on a.companycd=b.companycd and a.ProductID=b.id 
                                        where a.companycd='{0}' and a.ProductID={1} group by a.productid",companycd,proid);
            DataTable dt = SqlHelper.ExecuteSql(sql);
            Decimal procount = 0;
            if (dt.Rows.Count>0)
            {
                procount=decimal.Parse(dt.Rows[0][0].ToString());
            }
            return procount;
        }
        ...
        /// <param name="ProduceCount"></param>
        /// <returns></returns>
        public static bool checkis(string CompanyCD, int ProductID)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            string sql=@"select count(1) from officedba.Bom a left join officedba.BomDetail b on a.bomno=b.bomno and a.companycd=b.companycd  
                        where a.companycd='"+CompanyCD+@"' and b.productid="+ProductID+@" and a.usedstatus=1 and sourcetype=0";
            SqlCommand comm=new SqlCommand();
        ...
        public static DataTable GetProductSnapshot(int ProductID, string CompanyCD)
        {
            ...
            //增加在途量、受定量(待发货量)、借出量、借入量 add 20110429 lcb
            infoSql.AppendLine("select	a.ID as ProductID,a.ProductName,a.ProdNo as ProductNo,");
            infoSql.AppendLine("		Convert(numeric(22,"+userInfo.SelPoint+"),isnull(a.SafeStockNum,0)) as SafeCount,");
            ...
            infoSql.AppendLine("        isnull(c.OrderCount,0) as OrderCount,isnull(c.RoadCount,0) as RoadCount,");//受定量(待发货量)、在途量
            infoSql.AppendLine("        isnull(c.OutCount,0) as OutCount,isnull(c.InCount,0) as InCount,");//借出量、借入量
            infoSql.AppendLine("        isnull(c.BatchNo,'') as BatchNo ");//批次 add 20110504 lcb
            infoSql.AppendLine("from officedba.ProductInfo a");
            infoSql.AppendLine("left join officedba.CodeUnitType b on a.UnitID=b.ID");
        ...
        private static void GetExtAttrCmd(MRPModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@MRPNo", model.MRPNo);
