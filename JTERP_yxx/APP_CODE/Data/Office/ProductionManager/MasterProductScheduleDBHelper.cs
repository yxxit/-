...
namespace XBase.Data.Office.ProductionManager
{
    public class MasterProductScheduleDBHelper
    {
        ...
        /// <param name="model">查询条件</param>
        /// <returns></returns>
        public static DataTable getsellorder(int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            ...
            //查询SQL拼写
            StringBuilder searchSql = new StringBuilder();
//            searchSql.AppendLine(@"select a.ModifiedDate,a.id ID,orderno PlanNo,isnull(title,'')Subject,j.ProductName,isnull(EmployeesName,'')PrincipalReal,isnull(b.deptname,'')DeptName,productcount TotalCounts from 
//(select dsell.productid,sell.orderno,sell.confirmor,sell.companycd,sell.ModifiedDate,dsell.id,sell.title,sell.selldeptid,sell.seller,dsell.productcount from officedba.sellorder sell left join officedba.sellorderdetail dsell on sell.orderno=dsell.orderno where sell.companycd='" + userInfo.CompanyCD + @"' and dsell.companycd='" + userInfo.CompanyCD + @"')
//a left join officedba.DeptInfo b on a.selldeptid=b.id left join ((select a.ID,ISNULL(a.EmployeeName,'') AS EmployeesName,a.Flag,b.SuperDeptID,
//isnull(cast (a.DeptID as varchar),'')  as  DeptID   from officedba.EmployeeInfo  as a left join officedba.deptinfo  as b on a.DeptID=b.ID left join officedba.ProductInfo j on j.id=a.productid where a.CompanyCD='" + userInfo.CompanyCD + @"' and a.Flag!='2' and a.DeptID Is not null and a.Flag<>'3'))c on c.id=a.seller where a.companycd='" + userInfo.CompanyCD + @"' and confirmor is not null");
            searchSql.AppendLine(@"select a.pakeageid,a.pakeage,a.ModifiedDate,a.id ID,orderno PlanNo,isnull(title,'')Subject,j.ProductName,isnull(EmployeesName,'')PrincipalReal,isnull(b.deptname,'')DeptName,productcount TotalCounts from 
(select isnull(dsell.package,0)pakeageid,isnull(dbo.getCodeublic(dsell.package),'')pakeage,dsell.productid,sell.orderno,sell.confirmor,sell.companycd,sell.ModifiedDate,dsell.id,sell.title,sell.selldeptid,sell.seller,dsell.productcount from officedba.sellorder sell left join officedba.sellorderdetail dsell on sell.orderno=dsell.orderno where sell.companycd='" + userInfo.CompanyCD + @"' and dsell.companycd='" + userInfo.CompanyCD + @"')
 a left join officedba.DeptInfo b on a.selldeptid=b.id left join ((select a.ID,ISNULL(a.EmployeeName,'') AS EmployeesName,a.Flag,b.SuperDeptID,
isnull(cast (a.DeptID as varchar),'')  as  DeptID   from officedba.EmployeeInfo  as a left join officedba.deptinfo  as b on a.DeptID=b.ID where a.CompanyCD='" + userInfo.CompanyCD + @"' and a.Flag!='2' and a.DeptID Is not null and a.Flag<>'3'))c on c.id=a.seller
left join officedba.ProductInfo j on j.id=a.productid where a.companycd='" + userInfo.CompanyCD + @"' and confirmor is not null
");
            #endregion
            SqlCommand comm = new SqlCommand();
            //指定命令的SQL文
