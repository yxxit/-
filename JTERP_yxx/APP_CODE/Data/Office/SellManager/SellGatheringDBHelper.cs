...
namespace XBase.Data.Office.SellManager
{
    public class SellGatheringDBHelper
    {
        ...
        public static DataTable GetSellGathering(SellGatheringModel sellGatheringModel, string PlanPrice0,string EFIndex,string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            //strSql.Append(" OR  (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = s.ID AND BillTypeFlag = 5 AND BillTypeCode = 2) AND operateUserId = '" + userInfo.UserID + "')>0 ");

            strSql.Append(" (g.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (g.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
