...
namespace XBase.Data.Office.SellManager
{
    public class SellBackDBHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static bool SaveSellBack(Hashtable ht, SellBackModel sellBackModel, List<SellBackDetailModel> sellBackDetailModellList, out string strMsg, string[] strCustomdetail)
        {
            ...
            if (NoIsExist(sellBackModel.BackNo))
            {
                ...
                try
                {
                    ...
                    InsertSellBackDetail(sellBackDetailModellList, tran, strCustomdetail);
                    tran.Commit();
                    isSucc = true;
                    strMsg = "保存成功！";
                }
        ...
        /// </summary>
        /// <returns></returns>
        public static bool UpdateSellBack(Hashtable ht, SellBackModel sellBackModel, List<SellBackDetailModel> sellBackDetailModellList, out string strMsg, string[] strCustomdetail)
        {
            bool isSucc = false;//是否添加成功
            strMsg = "";
        ...
        /// <param name="tran"></param>
        private static void InsertSellBackDetail(List<SellBackDetailModel> sellBackDetailModellList, TransactionManager tran, string[] strCustomdetail)
        {
            int j = 1;
        ...
        /// <returns></returns>
        public static DataTable GetOrderList(SellBackModel sellBackModel, DateTime? dt, int? Reason, int? FlowStatus, string EFIndex, string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount, string time1, string time2, string prodid, string EFIndexP, string EFDescP)
        {
            ...
            strSql = "select * from (SELECT s.ID, s.BackNo,p.productname,Convert(decimal(22," + sellpoint + "), isnull(de.productcount,0)) productcount,";
            if (isMoreUnit)
            {
            ...
            strSql += "  ,Convert(decimal(22," + sellpoint + "), isnull(de.totalprice,0)) totalprice1,Convert(decimal(22," + sellpoint + "), isnull(de.backnumber,0)) backnumber,";
            strSql += "  Convert(decimal(22," + sellpoint + "), isnull(de.innumber,0)) innumber, s.Title,s.ModifiedDate, CONVERT(varchar(100), s.BackDate, 23) AS BackDate, Convert(decimal(22," + sellpoint + "), isnull(s.TotalPrice,0)) TotalPrice, c.CustName, e.EmployeeName, ";
            ...
            strSql += "FROM officedba.SellBack AS s left join officedba.SellBackdetail de on s.companycd=de.companycd and s.backno=de.backno left join officedba.productinfo p on p.id=de.productid LEFT JOIN ";
            ...
            strSql += " (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = s.ID AND BillTypeFlag = 5 AND BillTypeCode = 5) AND BillID = s.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ";
        ...
        public static DataTable GetOrderDetail(string orderNo)
        {
            ...
            string strSql = @" SELECT isnull(s.Custom1,'')Custom1,isnull(s.Custom2,'')Custom2,isnull(s.Custom3,'')Custom3,isnull(s.Custom4,'')Custom4,isnull(s.Custom5,'')Custom5,isnull(s.Custom6,'')Custom6,isnull(s.Custom7,'')Custom7,isnull(s.Custom8,'')Custom8,isnull(s.Custom9,'')Custom9,isnull(s.Custom10,'')Custom10,
            s.ProductID, s.ProductCount, s.UnitID, s.UnitPrice, s.TaxPrice, s.Discount,       ";
        ...
        /// <returns></returns>
        public static bool ConfirmOrder(string OrderNO, int SellOrderID, out string strMsg, out string strFieldText)
        {
            ...
            if (isHandle(OrderNO, "1"))
            {
                if (IsConfirm(OrderNO, out strMsg, out strFieldText))
                {
                    ...
                    try
                    {
                        ...
                        //20121121添加对于销售订单已发货数量的控制
                        UpdateSellOrderDetail(1, SellOrderID, OrderNO, tran);
                        tran.Commit();
                        isSuc = true;
                        strMsg = "确认成功！";
                    }
        ...
        /// <param name="BackNo">退货单编号</param>
        /// <param name="SellOrderID">与之相对应的销售订单的编号</param>
        private static void UpdateSellOrderDetail(int flag, int sellOrderID, string BackNo, TransactionManager tran)
        {
            string strCompanyCD = string.Empty;//单位编号
            string strSql = " SELECT  isnull(BackNumber,0) as BackNumber,isnull(UsedUnitCount,0) as UsedUnitCount FROM officedba.SellBackDetail ";
            strSql += " WHERE(BackNo = @BackNo) AND (CompanyCD = @CompanyCD) AND (FromType = '1')";
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                strSql += ") where OrderNo =( SELECT OrderNo  FROM officedba.SellOrder where  ID =@ID )  and (CompanyCD = @CompanyCD) ";
                SqlParameter[] param = { new SqlParameter("@ID",sellOrderID),
        ...
        /// <param name="strMsg">操作返回的信息</param>
        /// <returns></returns>
        //20121121添加sellorderID参数
        public static bool UnConfirmOrder(string OrderNO, int SellOrderID, out string strMsg)
        {
            string strSq = string.Empty;
            bool isSuc = false;
            strMsg = "";
            ...
            if (isHandle(OrderNO, "2"))
            {
                ...
                if (IsRef(OrderNO))
                {
                    ...
                    try
                    {
                        ...
                        //20121121添加sellorder参数
                        UpdateSellOrderDetail(2, SellOrderID, OrderNO, tran);
                        tran.Commit();
                        isSuc = true;
                        strMsg = "取消确认成功！";
                    }
        ...
        //20121121 添加  
        //函数功能：在退货单页面根据发货单号获取相对应的订单号
        //函数返回值：dt
        //返回值类型：datatable
        public static DataTable GetOrderIDbySendNo(string SendNo)
        {
            string strCompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
            string strSql = "select so.ID,ss.SendNo from officedba.SellOrder as so";
            strSql += " left join officedba.SellSend ss on ss.FromBillID=so.ID";
            strSql += " where so.companycd=@CompanyCD and ss.SendNo=@SendNo";
            SqlParameter[] paras = new SqlParameter[2];
            ArrayList arr = new ArrayList();
            paras[0] = new SqlParameter("@SendNo", SendNo);
            arr.Add(paras[0]);
            paras[1] = new SqlParameter("@CompanyCD", strCompanyCD);
            arr.Add(paras[1]);
            DataTable dt = SqlHelper.ExecuteSql(strSql,arr);
            return dt;
        }
