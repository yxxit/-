...
namespace XBase.Data.Office.SellManager
{
    ...
    public class SellChanceDBHelper
    {
            ...
            DateTime? FindDate, DateTime? FindDate1, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
          ...
          //  strSql +=" OR  (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F.FlowNo AND operateUserId = '" + userInfo.UserID + "')>0 ";

            strSql += " (s.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (sp.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))";

            strSql += " or (CHARINDEX('," + eid + ",',','+s.CanViewUser+',')>0  )) " ;
