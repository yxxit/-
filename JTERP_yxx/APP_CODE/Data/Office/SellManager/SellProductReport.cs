...
namespace XBase.Data.Office.SellManager //Method---1列表2报表
{
    public class SellProductReport
    {
        ...
        public static DataTable GetReceivables(string CompanyCD, string CustNo, string CustName, string BeginDate, string EndDate, int pageIndex, int pageSize, string OrderBy, ref int totalCount)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" select b.ID,isnull(b.TotalPrice,0) as TotalPrice,isnull(a.NAccounts,0) as NAccounts,isnull(e.CustName,'') as CustName ");
            ...
            sql.AppendLine(" from   officedba.BlendingDetails as a right join officedba.SellOrder as b on a.BillCD=b.OrderNo  and a.CompanyCD=b.CompanyCD ");
            ...
            sql.AppendLine(" where (a.TotalPrice>a.YAccounts or a.totalprice is null) and b.CompanyCD=@CompanyCD and b.BillStatus!='1' and b.Status!='3'   ");
            ...
            sql.AppendLine(" select d.ID,isnull(d.TotalPrice,0)as TotalPrice,isnull(c.NAccounts,0) as NAccounts,isnull(f.CustName,'') as CustName                ");
            ...
            sql.AppendLine(" from  officedba.BlendingDetails as c right join officedba.SellBack as d on c.BillCD=d.BackNo  and c.CompanyCD=d.CompanyCD                                           ");
            ...
            sql.AppendLine(" where (c.NAccounts>0 or c.NAccounts is null)  and d.CompanyCD=@CompanyCD  and d.BillStatus!='1'  ");
            if (!string.IsNullOrEmpty(CustNo) && CustNo != "0")
            {
        ...
        public static DataTable GetReceivablesReport(string CompanyCD, string CustNo, string CustName, string BeginDate, string EndDate, int pageIndex, int pageSize, string OrderBy, ref int totalCount)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" select b.ID,isnull(b.TotalPrice,0) as TotalPrice,isnull(a.NAccounts,0) as TotalFee,isnull(e.CustName,'') as Title ");
            ...
            sql.AppendLine(" union                                                                                                                           ");
            sql.AppendLine(" select d.ID,isnull(d.TotalPrice,0)as TotalPrice,isnull(c.NAccounts,0) as TotalFee,isnull(f.CustName,'') as Title                ");
            ...
            sql.AppendLine("       left join officedba.CustInfo as f on d.CustID=f.ID                                                                        ");
            sql.AppendLine("       left join officedba.EmployeeInfo as h on d.Seller=f.ID  ");
            sql.AppendLine(" where (c.NAccounts>0 or c.NAccounts is null)  and c.CompanyCD=@CompanyCD and d.BillStatus!='1' ");
            if (!string.IsNullOrEmpty(CustNo) && CustNo != "0")
            {
