...
namespace XBase.Data.Office.SellManager
{
    public class SellPlanDBHelper
    {
        ...
        public static DataTable GetOrderList(string EFIndex,string EFDesc,SellPlanModel sellPlanModel, int? FlowStatus, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            strSql.Append(" FROM officedba.SellPlan as so where 1=1");

            strSql.Append(" and so.CompanyCD=@CompanyCD                          ");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    strSql.AppendLine(" (so.Creator IN  ");
            ...
            strSql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = so.ID AND BillTypeFlag = 5 AND BillTypeCode = 8) AND BillID = so.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            strSql.AppendLine("or (so.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) ");

            strSql.Append( " or (CHARINDEX('," + eid + ",',','+so.CanViewUser+',')>0  )) ");
            string strCompanyCD = string.Empty;//单位编号
