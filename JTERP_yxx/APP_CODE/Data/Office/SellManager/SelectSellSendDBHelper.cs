    ...
    public class SelectSellSendDBHelper
    {
        ...
        public static DataTable GetSellSendList(string busType, string OrderNo, string Title, string CustName, int? CurrencyType, string model, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            string strCompanyCD = string.Empty;//单位编号
                ...
                strSql = "SELECT s.ID,s.FromBillID, s.Title, e.EmployeeName, s.Rate, cts.CurrencyName, c.CustName, s.SendNo, CONVERT(varchar(100), ISNULL(s.IntendSendDate, GETDATE()), 23)   AS IntendSendDate " +
                     ...
                     " WHERE  s.CompanyCD=@CompanyCD and s.ID IN (SELECT FromBillID FROM officedba.StorageOutSell WHERE CompanyCD=@CompanyCD )";
            strSql += " and (  ";
        ...
        public static DataTable GetSellSendDetail(string strOrderID, string strOrderDetailID, string CustID,string busType)
        {
            ...
            strSql += ",isnull(p.ExtField1,'') as ExtField1,isnull(p.ExtField2,'') as ExtField2,isnull(p.ExtField3,'') as ExtField3 ";
            strSql += ",isnull(p.ExtField4,'') as ExtField4,isnull(p.ExtField5,'') as ExtField5,isnull(p.ExtField6,'') as ExtField6 ";
            strSql += ",isnull(p.ExtField7,'') as ExtField7,isnull(p.ExtField8,'') as ExtField8,isnull(p.ExtField9,'') as ExtField9, isnull(p.ExtField10,'') as ExtField10 ";
