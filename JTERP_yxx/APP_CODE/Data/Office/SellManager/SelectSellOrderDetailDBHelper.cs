...
namespace XBase.Data.Office.SellManager
{
    public class SelectSellOrderDetailDBHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable GetSellOrderList(string OrderNo, string Title, string CustName, int? CurrencyType, string model, int pageIndex, int pageCount, string ord, ref int TotalCount,string custid,string Prodname,string Prodno)
        {
            ...
            bool isMoreUnit = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit;//多计量单位
                ...
                if (custid != "")
                {
                    strSql = "select * from(SELECT s.ID,dt.id DetailID,isnull(dt.sendcount,0)SendCount,dt.ProductCount,pr.productname,pr.prodno,pr.Specification,pr.Size,dt.Pnumber,dt.AbrasionResist,dt.BalancePaper,dt.BaseMaterial,dt.SurfaceTreatment,dt.BackBottomPlate,dt.BuckleType, s.Title, e.EmployeeName, s.Rate, cts.CurrencyName, c.id custid,c.CustName, CONVERT(varchar(100), s.OrderDate, 23) AS OrderDate, s.OrderNo ";
                }
                else
                {
                    strSql = "select * from(SELECT s.ID,dt.id DetailID,isnull(dt.sendcount,0)SendCount,dt.ProductCount,pr.productname,pr.prodno,pr.Specification,pr.Size,dt.Pnumber,dt.AbrasionResist,dt.BalancePaper,dt.BaseMaterial,dt.SurfaceTreatment,dt.BackBottomPlate,dt.BuckleType, s.Title, e.EmployeeName, s.Rate, cts.CurrencyName, c.CustName, CONVERT(varchar(100), s.OrderDate, 23) AS OrderDate, s.OrderNo ";
                }
                ...
                strSql += " FROM   officedba.SellOrderDetail dt left join  officedba.productinfo pr on pr.id=dt.productid  left join officedba.SellOrder AS s on dt.orderno=s.orderno and dt.companycd=s.companycd LEFT OUTER JOIN ";
                ...
                if (custid == "")
                {
                    strSql += " and (dt.UsedUnitCount - ISNULL(dt.SendCount, 0.0000))>0 ";
                }
                strSql += " and (  ";
            ...
            {
                strSql += " and pr.productname like '%" + Prodname + "%'";

            }
            if (Prodno != null && Prodno!="")
            {
                strSql += " and pr.prodno like '%" + Prodno + "%'";
            ...
            if (custid != "")
            {
                strSql += ") as total where custid=" + custid + "  group by  ID,Pnumber,AbrasionResist,BalancePaper,BaseMaterial,SurfaceTreatment,BackBottomPlate,BuckleType,DetailID,sendcount,ProductCount, productname,prodno,Specification,Size,custid, Title,EmployeeName, Rate, CurrencyName, CustName, OrderDate,OrderNo,isSendText ";
            }
            else
            {
                strSql += ") as total group by  ID,Pnumber,AbrasionResist,BalancePaper,BaseMaterial,SurfaceTreatment,BackBottomPlate,BuckleType,DetailID,sendcount,ProductCount, productname,prodno,Specification,Size, Title,EmployeeName, Rate, CurrencyName, CustName, OrderDate,OrderNo,isSendText ";
            }
