...
namespace XBase.Data.Office.SellManager
{
    public class SellContractDBHelper
    {
        ...
        public static bool InsertOrder(Hashtable ht,SellContractModel sellContractModel,
            List<SellContractDetailModel> SellContractDetailModelList, out string strMsg,string[] strCustomdetail)
        {
            ...
            if (NoIsExist(sellContractModel.ContractNo))
            {
                ...
                try
                {
                    ...
                    InsertSellContractDetail(SellContractDetailModelList, sellContractModel, tran,strCustomdetail);
        ...
        /// </summary>
        /// <param name="sellContractModel"></param>
        private static void InsertSellContractDetail(List<SellContractDetailModel> SellContractDetailModelList, SellContractModel sellContractModel, TransactionManager tran, string[] strCustomdetail)
        {
        ...
        /// <returns></returns>
        public static DataTable GetOrderList(string EFIndex,string EFDesc,SellContractModel sellContractModel, int? FlowStatus, int pageIndex, int pageCount, string ord, ref int TotalCount,string prodid,string state,string EFIndexP,string EFDescP)
        {
            ...
            strSql = "select * from ( SELECT s.ID, s.Title,s.ModifiedDate, s.ContractNo,Convert(decimal(22," + sellpoint + "), isnull( s.TotalFee,0)) TotalFee,case s.FromType when  ";
            strSql += "  '1' then  ISNULL(so.OfferNo, '') when '2' then isnull(sc.ChanceNo,'') end as OfferNo, ";
            strSql += " p.productname,Convert(decimal(22," + sellpoint + "), isnull(de.productcount,0)) productcount, ";
            if (isMoreUnit)
            {
            ...
            strSql += "THEN '已执行' END AS stateText, isnull(CASE                                                             ";
            ...
            strSql += " officedba.sellcontractdetail as de on de.ContractNo=s.ContractNo and s.companycd=de.companycd ";
            strSql += "   left outer join officedba.productinfo p on p.id=de.productid  LEFT OUTER JOIN ";
            ...
            strSql += "or (s.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s.OurDelegate IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))";

            strSql += " or ( charindex('," + sellContractModel.Creator + ",' , ','+s.CanViewUser+',')>0 )) ";
        ...
        public static DataTable GetTotal(DateTime startDate, DateTime endDate, bool isCon, bool isOrd)
        {
            ...
            if (isCon)
            {
                ...
                strSql += "THEN '已执行（合同）' END AS Remark                                                   ";
             ...
             int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            if (isCon)
            {
                ...
                strSql += "THEN '已执行（合同）' END AS StateName                                              ";
                strSql += "From officedba. SellContract as sc left join                                        ";
                strSql += "officedba.EmployeeInfo AS  e ON sc.Seller = e.ID                                    ";
        ...
        public static DataTable GetContractByStateNum(string GroupType, string DeptOrEmployeeId, string BeginDate, string EndDate)
        {
            ...
            strCompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
            StringBuilder sb = new StringBuilder();
            sb.Append(" select CASE State WHEN '1' THEN '执行中' WHEN '2' THEN '意外终止' WHEN '3'  THEN '已执行' END as Name,count(1) as Counts,State ");
        ...
        public static DataTable GetContractByStatePrice(string GroupType, string DeptOrEmployeeId, string BeginDate, string EndDate)
        {
            ...
            strCompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
            StringBuilder sb = new StringBuilder();
            sb.Append(" select CASE State WHEN '1' THEN '执行中' WHEN '2' THEN '意外终止' WHEN '3'  THEN '已执行' END as Name,sum(isnull(RealTotal,0)*isnull(rate,1)) as Counts,State ");
            sb.Append(" from officedba. SellContract ");
            sb.Append(" where companyCD='");
            sb.Append(strCompanyCD);
            sb.Append("' ");
        ...
        public static DataTable GetSellContractDetail(int Status, int Type, string Name, string DeptAndPerson, string BeginDate, string EndDate, string DataType, string GroupType, string DeptOrEmployeeId, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            sb.Append(" CASE s.State WHEN '1' THEN '执行中' WHEN '2' THEN '意外终止' WHEN '3'  THEN '已执行' END AS stateText, ");/*合同状态*/
