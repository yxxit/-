    ...
    public class SellSendDetailsListDBHelper
    {
        ...
        public static DataTable GetSellSendDetailListData(SellSendDetailsListModel model, int pageIndex, int pageCount, string ord, ref int totalCount)
        {
            ...
            if (dtt != null && dtt.Rows.Count > 0)
            {
                ...
                {
                    strSql.Append(" (s2.Creator IN  ");
                    strSql.Append("( SELECT ID FROM  officedba.EmployeeInfo ");
                    strSql.Append("  WHERE DeptID IN (SELECT a.ID  ");
                    strSql.Append(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    strSql.Append("  WHERE a.ID=b.ID) AND DeptID<>" + userInfo.DeptID.ToString() + " )) OR ");
                }
                if (dtt.Rows[0]["RoleRange"].ToString() == "2")
                {
                    ...
                    strSql.Append("  WHERE DeptID IN (SELECT a.ID  ");
                    strSql.Append(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    strSql.Append("  WHERE a.ID=b.ID) ))  OR ");
                }
                if (dtt.Rows[0]["RoleRange"].ToString() == "3")
                {
                    ...
                    strSql.Append("( SELECT ID FROM  officedba.EmployeeInfo ");
                    strSql.Append("  WHERE DeptID IN (SELECT a.ID  ");
                    strSql.Append(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.BranchID.ToString() + ") b  ");
                    strSql.Append("  WHERE a.ID=b.ID)))  OR ");
                }
            }
            strSql.Append(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = s2.ID AND BillTypeFlag = 5 AND BillTypeCode = 4) AND BillID = s2.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            strSql.Append("or (s2.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s2.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s2.Sender IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            strSql.Append(" or ( charindex(',"+userInfo.EmployeeID+",' , ','+s2.CanViewUser+',')>0  )) ");
