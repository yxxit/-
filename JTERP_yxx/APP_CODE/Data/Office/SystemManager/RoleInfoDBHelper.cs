...
namespace XBase.Data.Office.SystemManager
{
    public class RoleInfoDBHelper
    {
        ...
        {
            string sql = "select RoleID,CompanyCD,RoleName,ModifiedDate,remark,RoleRange from officedba.RoleInfo where RoleID=@ID";
            SqlParameter[] param = new SqlParameter[1];
            param[0] = SqlHelper.GetParameter("@ID", ID);
            return SqlHelper.ExecuteSql(sql,param);
        }
        public static DataTable GetRoleInfoByUserID(string UserID)
        {
            string sql = "select top(1) RoleID,CompanyCD,RoleName,ModifiedDate,remark,RoleRange from officedba.RoleInfo where RoleID in (select RoleID From officedba.UserRole where UserID=@UserID) order by RoleRange desc";
            SqlParameter[] param = new SqlParameter[1];
        ...
        {
            string sql = "select RoleID,CompanyCD,RoleName,isnull( CONVERT(CHAR(19), ModifiedDate, 120),'') as ModifiedDate,ModifiedUserID,remark,RoleRange,case RoleRange when '0' then '本人' when '1' then '本人及下属' when '2' then '本部门' when '3' then '全部' end RoleRangeName from officedba.RoleInfo where RoleID>0 and (IsRoot!='1'or IsRoot is null) ";
            //企业代码不为空
        ...
        public static bool InsertRoleInfo(RoleInfoModel model, string loginUserID, out string ID)
        {
            ...
            param[2] = SqlHelper.GetParameter("@RoleRange", model.RoleRange);
            param[3] = SqlHelper.GetParameter("@RoleName", model.RoleName);
            param[4] = SqlHelper.GetOutputParameter("@ID", SqlDbType.Int);
            SqlHelper.ExecuteTransSql(sql.ToString(), param);
            ID = param[4].Value.ToString();
            return SqlHelper.Result.OprateCount > 0 ? true : false;
        ...
        public static bool UpdateRoleInfo(RoleInfoModel model, string loginUserID, int RoleID)
        {
            ...
            sql.AppendLine("		RoleRange = @RoleRange,             ");
            ...
            param[1] = SqlHelper.GetParameter("@RoleRange", model.RoleRange);
            param[2] = SqlHelper.GetParameter("@remark", model.Remark);
            param[3] = SqlHelper.GetParameter("@CompanyCD",model.CompanyCD);
            param[4] = SqlHelper.GetParameter("@ModifiedDate", System.DateTime.Now);
