...
namespace XBase.Data.Office.SystemManager
{
    public class ParameterSettingDBHelper
    {
           ...
           //验证是否存在指定配置项
            if (ValidateKey(CompanyCD, FuntionType))
            {
                if (FuntionType == "12")
                {
                    ...
                    sbSql.AppendLine(" (CompanyCD,FunctionType,PrintNo) ");
                    sbSql.AppendLine(" VALUES ");
                    sbSql.AppendLine(" ('"+CompanyCD+"',"+FuntionType+",'"+Status+"' );");
                    if (ValidateKey(CompanyCD, "11"))
                    {
                        sbSql.AppendLine(" INSERT INTO officedba.ParameterSetting ");
                        sbSql.AppendLine(" (CompanyCD,FunctionType,UsedStatus) ");
                        sbSql.AppendLine(" VALUES ");
                        sbSql.AppendLine(" ('" + CompanyCD + "',11,1 )");
                    }
                    else
                    {
                        sbSql.AppendLine(" UPDATE officedba.ParameterSetting SET ");
                        sbSql.AppendLine(" UsedStatus=1 ");
                        sbSql.AppendLine(" WHERE CompanyCD='" + CompanyCD + "' AND FunctionType=11 ");
                    }
                }
                else
                {
            ...
            else
            {
                ...
                {
                    sbSql.AppendLine(" UPDATE officedba.ParameterSetting SET ");
                    sbSql.AppendLine(" PrintNo='"+Status+"' ");
                    sbSql.AppendLine(" WHERE CompanyCD='"+CompanyCD+"' AND FunctionType="+FuntionType+" ;");
                    if (ValidateKey(CompanyCD, "11"))
                    {
                        sbSql.AppendLine(" INSERT INTO officedba.ParameterSetting ");
                        sbSql.AppendLine(" (CompanyCD,FunctionType,UsedStatus) ");
        ...
        #region 设置定制打印模板宽度
        public static bool SetPrintWidth(string CompanyCD, string FuntionType, string Status)
        {
            StringBuilder sbSql = new StringBuilder();
            //验证是否存在指定配置项
            if (ValidateKey(CompanyCD, FuntionType))
            ...
            else
            {
                    ...
                    //已经存在 则更新指定项的状态
                    sbSql.AppendLine(" UPDATE officedba.ParameterSetting SET ");
                    sbSql.AppendLine(" PrintNo=@UsedStatus ");
                    sbSql.AppendLine(" WHERE CompanyCD=@CompanyCD AND FunctionType=@FunctionType ");
