...
namespace XBase.Data.Office.AdminManager
{
   ...
   public class EquipMentInfoDBHelper
   {
       ...
       {
           EpLength = EpLength+1;
           string sql = "select sum(TotalCount)TotalCount,Status,CodeName,sum(FreeCount)FreeCount,sum(WarningLimit)WarningLimit,Type "
                        + "from (select CompanyCD,isnull(m.TotalCount,'') TotalCount,isnull(m.Type,'') Type,isnull(m.status,'') Status,"
                         ...
                         + "isnull(n.FreeCount,'') FreeCount"
                         + " from (select a.*,b.CodeName,b.WarningLimit from (select CompanyCD,count(*)  as TotalCount ,[Type],"
       ...
       /// <returns></returns>
       public static int GetEquipInfoByIndex(string EquipCode, string TableName, string ColName)
       {
           //查询语句
           string sql = "SELECT COUNT(*) AS IndexCount FROM " + TableName + " WHERE  " + ColName + " = " + EquipCode + " and companycd='" + ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD + "'";
