    ...
    public class ServiceDBHelper
    {
        ...
        public static int CustServiceAdd(CustServiceModel CustServiceM)
        {
            try
            {
                ...
                param[22] = paramid;
                param[23] = SqlHelper.GetParameter("@NextLinkDate",CustServiceM.NextLinkDate==null?SqlDateTime.Null:SqlDateTime.Parse(CustServiceM.NextLinkDate.ToString()));
                param[24] = SqlHelper.GetParameter("@JoinUser", CustServiceM.JoinUser);
                param[25] = SqlHelper.GetParameter("@JoinUserName", CustServiceM.JoinUserName);
                param[26] = SqlHelper.GetParameter("@CustLinkAddress",CustServiceM.CustLinkAddress);
                param[27] = SqlHelper.GetParameter("@OutNo", CustServiceM.OutNo);
                param[28] = SqlHelper.GetParameter("@ProductNo", CustServiceM.ProductNo);
                param[29] = SqlHelper.GetParameter("@Specification", CustServiceM.Specification);
                param[30] = SqlHelper.GetParameter("@Faults", CustServiceM.Faults);
                param[31] = SqlHelper.GetParameter("@Results", CustServiceM.Result);
                param[32] = SqlHelper.GetParameter("@SellDate", CustServiceM.SellDate == null ? SqlDateTime.Null : SqlDateTime.Parse(CustServiceM.SellDate.ToString()));
                param[33] = SqlHelper.GetParameter("@RepairDate", CustServiceM.RepairDate == null ? SqlDateTime.Null : SqlDateTime.Parse(CustServiceM.RepairDate.ToString()));
                #endregion
        ...
        public static bool UpdateService(CustServiceModel CustServiceM)
        {
            try
            {
                ...
                sqlcontact.AppendLine("NextLinkDate = @NextLinkDate, ");
                sqlcontact.AppendLine("JoinUser=@JoinUser,");
                sqlcontact.AppendLine("JoinUserName=@JoinUserName");
                sqlcontact.AppendLine(",CustLinkAddress = @CustLinkAddress,    ");
                sqlcontact.AppendLine(" OutNo = @OutNo, ");
                sqlcontact.AppendLine("ProductNo = @ProductNo, ");
                sqlcontact.AppendLine(" Specification = @Specification, ");
                sqlcontact.AppendLine("Faults = @Faults, ");
                sqlcontact.AppendLine("Results=@Results, ");
                sqlcontact.AppendLine(" SellDate=@SellDate,");
                sqlcontact.AppendLine(" RepairDate=@RepairDate");
                ...
                param[20] = SqlHelper.GetParameter("@CanViewUser", CustServiceM.CanViewUser);
                param[21] = SqlHelper.GetParameter("@CanViewUserName", CustServiceM.CanViewUserName);
                param[22] = SqlHelper.GetParameter("@NextLinkDate",CustServiceM.NextLinkDate==null?SqlDateTime.Null:SqlDateTime.Parse(CustServiceM.NextLinkDate.ToString()));
                param[23] = SqlHelper.GetParameter("@JoinUser",CustServiceM.JoinUser);
                param[24] = SqlHelper.GetParameter("@JoinUserName",CustServiceM.JoinUserName);
                param[25] = SqlHelper.GetParameter("@CustLinkAddress", CustServiceM.CustLinkAddress);
                param[26] = SqlHelper.GetParameter("@OutNo", CustServiceM.OutNo);
                param[27] = SqlHelper.GetParameter("@ProductNo", CustServiceM.ProductNo);
                param[28] = SqlHelper.GetParameter("@Specification", CustServiceM.Specification);
                param[29] = SqlHelper.GetParameter("@Faults", CustServiceM.Faults);
                param[30] = SqlHelper.GetParameter("@Results", CustServiceM.Result);
                param[31] = SqlHelper.GetParameter("@SellDate", CustServiceM.SellDate==null?SqlDateTime.Null:SqlDateTime.Parse(CustServiceM.SellDate.ToString()));
                param[32] = SqlHelper.GetParameter("@RepairDate", CustServiceM.RepairDate==null?SqlDateTime.Null:SqlDateTime.Parse(CustServiceM.RepairDate.ToString()));
                #endregion
        ...
        public static DataTable GetServiceInfoServerType(string ServerType, string CustName, CustServiceModel CustServiceM, string ServiceDateBegin, string ServiceDateEnd, string Executant, string CustLinkMan, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                #region sql语句
                string sql = "select " +
                                   " s.id,s.ServeNo,s.title,s.Contents,s.Feedback,CONVERT(varchar(100), s.BeginDate, 20) BeginDate,s.custid,c.custname custnam," +
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        sql += " (s.ModifiedUserID IN  ";
                        sql += " (  SELECT UserID FROM  officedba.UserInfo ";
                ...
                sql += "(CHARINDEX('," + userInfo.EmployeeID + ",',','+s.CanViewUser+',')>0 ) or (s.ModifiedUserID IN (SELECT UserID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) ";
                sql += " or (s.Executant IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (s.OurLinkMan IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
        ...
        public static DataTable GetServiceInfoBycondition(string CanUserID, string CustName, CustServiceModel CustServiceM, string ServiceDateBegin, string ServiceDateEnd, string Executant, string CustLinkMan, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                #region sql语句
                string sql = "select s.ModifiedDate, " +
                               " s.id,s.ServeNo,s.title,s.Contents,s.Feedback,CONVERT(varchar(100), s.BeginDate, 20) BeginDate,s.custid,c.custname custnam," +
                               " p.TypeName ServeType,c.CustNo,c.CustBig, c.CanViewUser,c.Manager,c.Creator," +
                               " (case s.Fashion when '1' then '远程支持' when '2' then '现场服务' when '3' then '综合服务' end) Fashion," +
                               " e.EmployeeName,l.LinkManName,isnull(CONVERT(varchar(100), s.NextLinkDate, 23),'')NextLinkDate  " +
                               " from " +
                                   " officedba.CustService s" +
                               " left join officedba.custinfo c on  c.id = s.custid " +
                               " left join  officedba.CodePublicType p on p.id = s.ServeType" +
        ...
        public static DataTable GetCustServiceByCustNo(string CompanyCD, string CustNo, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                string sql="select cs.id,cs.serveno,cs.title,cs.servetype,cs.fashion,cs.state,cs.Contents, "+
                            "cs.begindate,cs.executant,cl.id custlinkid,cl.linkmanname,ci.custno,ci.custname,ei.employeename "+
                            " from "+
                            " officedba.custservice cs "+
                            "left join officedba.custlinkman cl on cl.id=cs.custlinkman "+
                            "left join officedba.CustInfo ci on ci.id=cs.custid " +
                            "left join officedba.EmployeeInfo ei on ei.id=cs.executant " +
                            " where "+
                            " cs.CompanyCD = '" + CompanyCD + "'" +
                         "and  " +
                             "ci.CustNo = '" + CustNo + "'";
        ...
        public static DataTable GetSellAnnal(string CompanyCD, string CustID, string ProductID, string DateBegin, string DateEnd)
        {
            ...
            try
            {
                ...
                sql += " (CHARINDEX('," + userInfo.EmployeeID + ",',','+c.CanViewUser+',')>0 )";
                sql += " or (c.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (c.Manager IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
        ...
        public static DataTable GetServiceInfoByID(string CompanyCD, int serviceid)
        {
            try
            {
                string sql = "select " +
                                   ...
                                   " ,s.CanViewUser,s.CanViewUserName,isnull(CONVERT(varchar(100), s.NextLinkDate, 23),'') NextLinkDate,s.JoinUser,s.JoinUserName " +
                                   ",isnull(s.CustLinkAddress,'') CustLinkAddress,isnull(s.OutNo,'') OutNo,isnull(s.ProductNo,'')ProductNo,"+
                                   " isnull(s.Specification,'')Specification,isnull(s.Faults,'')Faults,isnull(s.Results,'') Results,"+
                                   " isnull(CONVERT(varchar(100), s.SellDate, 23),'') SellDate,isnull(CONVERT(varchar(100), s.RepairDate, 23),'') RepairDate "+
                               " from " +
                                   " officedba.CustService s " +
                                   " left join officedba.custinfo c on c.id = s.custid " +
                                   " left join officedba.custlinkman l on l.id = s.CustLinkMan " +
        ...
        public static DataTable GetServiceList(string CustName, string ServeType, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                ...
                sql.Append(" select a.id as ServeID,b.CustNO,b.id,b.CustBig,b.CustName,c.EmployeeName,a.ServeNO,a.Title,substring(a.Contents,0,10) Contents , ");
        ...
        public static DataTable GetCustServiceCount(string CustName, string ServeType, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" select a.CustNO,a.id,a.CustBig,a.CustName, isnull(c.TypeName,'') TypeName,isnull(b.ServeCount,'0') ServeCount ");
        ...
        public static DataTable GetServiceByMan(string Executant, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" select c.CustNO,c.CustName,c.id,c.CustBig,b.EmployeeName,a.ServeCount from ");
            ...
            try
            {
                string sql = "select " +
                                   ...
                                   " CONVERT(varchar(100), s.ModifiedDate, 23) ModifiedDate,s.ModifiedUserID,s.CanViewUserName " +
                                     " ,CONVERT(varchar(100), s.SellDate, 23) SellDate,isnull(s.CustLinkAddress,'') CustLinkAddress,isnull(s.ProductNo,'') ProductNo,"+
                                     " ISNULL(s.Specification,'') Specification " +
                               " from " +
                                   " officedba.CustService s " +
                                   " left join officedba.custinfo c on c.id = s.custid " +
        ...
        #region  服务次数分析报表（new 水晶报表做）
        public static DataTable GetCustServiceCountNew(string CustName, string ServeType, string CompanyCD, string LinkDateBegin, string LinkDateEnd, string Executant, string Fashion, string State, string DateType, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                string SearchField = "";
                if (DateType == "1")
                {
                    SearchField = "dateName(year,BeginDate)+'年'";
                }
                else if (DateType == "2")
                {
                    SearchField = "dateName(year,BeginDate)+'年-'+dateName(month,BeginDate)+'月'";
                }
                else
                {
                    SearchField = "dateName(year,BeginDate)+'年-'+dateName(week,BeginDate)+'周'";
                }
                StringBuilder sql = new StringBuilder();
                sql.Append(" select a.CustNO,a.id,a.CustBig,a.CustName, isnull(c.TypeName,'') TypeName,isnull(b.ServeCount,'0') ServeCount,b.DateType  ");
                sql.Append(" from officedba.CustInfo a inner join  ");
                sql.Append(" (select ");
                sql.Append(SearchField);
                sql.Append(" as DateType,count(1) ServeCount,CustId,ServeType from officedba.CustService where 1=1 ");
                ...
                if (!string.IsNullOrEmpty(Executant))
                {
                    sql.Append(" and Executant=");
                    sql.Append(Convert.ToInt32(Executant));
                }
                if (!string.IsNullOrEmpty(Fashion))
                {
                    sql.Append(" and Fashion=");
                    sql.Append(Convert.ToInt32(Fashion));
                }
                ...
                sql.Append(" group by CustId,ServeType,");
                sql.Append(SearchField);
                sql.Append(") b on a.Id=b.CustId left join ");
                sql.Append(" officedba.CodePublicType c on b.ServeType=c.Id ");
