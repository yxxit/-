    ...
    public class ComplainDBHelper
    {
        ...
        public static DataTable GetComplainInfoBycondition(string CanUserID,string CustName, CustComplainModel CustComplainM, string ComplainBegin, string ComplainEnd, string CustLinkMan, string DestClerk, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                ...
                sql += "(CHARINDEX('," + userInfo.EmployeeID + ",',','+cc.CanViewUser+',')>0 ) or (cc.ModifiedUserID IN (SELECT UserID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) ";
                sql += " or (cc.ComplainedMan IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (cc.DestClerk IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
        ...
        public static DataTable GetComplainInfoByCustNo(string CompanyCD, string CustNo, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                string sql = "select cc.id,cc.complainno,cc.title,cc.state,cc.complaindate,cc.Contents, "+
                    "cl.id custlinkid,cl.linkmanname,ci.custno,ci.custname,ei.employeename "+
                    " from "+
                    " officedba.custcomplain cc "+
                    " left join officedba.custlinkman cl on cl.id=cc.custlinkman "+
                    " left join officedba.custinfo ci on ci.id=cc.custid "+
                    " left join officedba.employeeinfo ei on ei.id=cc.destclerk "+
                       " where "+
                       " cc.CompanyCD = '"+CompanyCD+"'"+
                       " and "+
                       " ci.CustNo = '"+CustNo+"'";
        ...
        public static DataTable GetComplainList(string CustName, string TypeId, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" SELECT    a.id as ComplainID,b.CustNo,b.id,b.CustBig, b.CustName, d.EmployeeName, a.ComplainNo, a.Title, ");
        ...
        public static DataTable GetComplainCount(string CustName, string TypeId, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append("SELECT   b.CustNo,b.id,b.CustBig, b.CustName, c.TypeName, a.ComplainNum ");
        ...
        public static DataTable GetComplainByMan(string ComplainedMan, string CompanyCD, string LinkDateBegin, string LinkDateEnd, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                ...
                if (ComplainedMan != "")
                {
                    sql.Append(" and c.EmployeeName like '%");
            ...
            try
            {
              ...
              //   sql.Append(" INNER JOIN officedba.EmployeeInfo AS c ON a.ComplainedMan = c.ID ");
                ...
                sql.Append(@" FROM #temptable as a INNER JOIN officedba.CustInfo AS b ON a.CustId=b.Id
                                INNER JOIN officedba.EmployeeInfo AS c ON a.ComplainedMan = c.ID
        ...
        public static DataTable GetComplainByDays(string Days, string CompanyCD,int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" select a.id,a.CustBig,a.CustNo,a.CustName,b.EmployeeName from officedba.CustInfo a,officedba.EmployeeInfo b where a.Manager=b.Id ");
                sql.Append(" and b.CompanyCD='");
                sql.Append(CompanyCD);
                sql.Append("' ");
