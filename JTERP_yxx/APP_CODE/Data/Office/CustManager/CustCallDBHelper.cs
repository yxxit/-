    ...
    public class CustCallDBHelper
    {
        ...
        public static DataTable GetCustCallByTel(CustCallModel CustCallM, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            try
            {
                ...
                strSql.Append(" left join officedba.CustInfo ci on a.CustCall = ci.ID ");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        strSql.AppendLine(" (ci.Creator IN  ");
                ...
                strSql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+ci.CanViewUser+',')>0 ) or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
                strSql.AppendLine("or (ci.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (ci.Manager IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
