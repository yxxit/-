...
namespace XBase.Data.Office.CustManager
{
    public class LinkManDBHelper
    {
        ...
        /// <returns></returns>
        public static DataTable GetLinkManListEx(string companyCD,string CustType,string Important)
        {
            string sql = "select ";
            ...
            if (CustType != "0")
            {
                sql += " and b.CustType='"+CustType+"'"; 
            }
            if (Important != "0")
            {
                sql += " and a.Important='"+Important+"'";
            }
            sql += " AND  (  ";
            ...
            sql += "(CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )";
            sql += " or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (b.Manager IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
        ...
        public static int LinkManAdd(LinkManModel LinkManModel)
        {
            try
            {
                ...
                param[56] = SqlHelper.GetParameter("@IsDefault", LinkManModel.IsDefault);
                param[57] = SqlHelper.GetParameter("@ProfessionalDes", LinkManModel.ProfessionalDes);
                param[58] = SqlHelper.GetParameter("@Skype",LinkManModel.Skype);
        ...
        public static DataTable GetLinkInfoByID(string CompanyCD, int linkid)
        {
            try
            {
                string sql = "SELECT cl.id    ," +
                                ...
                                "cl.PaperNum     ," +
                                "cl.Photo,cl.ProfessionalDes,cl.Skype       " +
                          " from " +
        ...
        public static bool UpdateLinkMan(LinkManModel LinkManM)
        {
            try
            {
                ...
                sqllink.AppendLine(" isDefault=@IsDefault, ");
                sqllink.AppendLine(" Skype=@Skype ");
                ...
                param[55] = SqlHelper.GetParameter("@IsDefault", LinkManM.IsDefault);
                param[56] = SqlHelper.GetParameter("@Skype", LinkManM.Skype);
                #endregion
        ...
        public static DataTable GetLinkManInfoBycondition(string CustNam, LinkManModel LinkManM, string BeginDate, string EndDate, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            {
                string sql = "select " +
                                   "link.id,isnull(cust.id,'-1') custid,isnull(link.linkmanname,'') linkmanname,isnull(link.Appellation,'') Appellation,isnull(link.HomeAddress,'') linkManAdd, " +
                ...
                sql += " (CHARINDEX('," + userInfo.EmployeeID + ",',','+link.CanViewUser+',')>0 )";
                sql += " or (link.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (cust.Manager IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
                ...
                if (LinkManM.HomeAddress != "")
                    sql += " and HomeAddress like '%" + LinkManM.HomeAddress + "%'";
                if (LinkManM.Handset != "")
        ...
        public static DataTable GetLinkManByCustNo(string LinkManName, string Appellation, string Department, string CompanyCD, string CustNo)
        {
            ...
            {
                string sql = "select " +
                                   "id,isnull(LinkManName,'') LinkManName,isnull(Appellation,'') Appellation,isnull(Department,'') Department ,isnull(WorkTel,'') WorkTel,"+
                                   " isnull(HandSet,'') HandSet,isnull(homeAddress,'') HomeAddress,isnull(MSN,'') MSN,isnull(MailAddress,'') MailAddress " +
                               "from " +
        ...
        /// <param name="CustNo">客户编号</param>
        /// <returns>联系人列表</returns>
        public static DataTable GetLinkManByCustNo1(string CompanyCD, string CustNo)
        {
            try
            {
                string sql = "select " +
                                   "ID, CustNo, CompanyCD, LinkManName,WorkTel,HandSet,HomeAddress,IsDefault " +
                               "from " +
                                   "officedba.CustLinkMan " +
                               "where " +
                                   "CustNo = @CustNo " +
        ...
        /// <param name="CustNo">客户编号</param>
        /// <returns>联系人列表</returns>
        public static DataTable GetLinkManByCustNo2(string CompanyCD, string CustNo,  int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                string sql = "select " +
                                   "id, CustNo, CompanyCD, LinkManName,WorkTel,Handset,QQ,CreatedDate " +
                               "from " +
                                   "officedba.CustLinkMan " +
                               "where " +
                                   "CustNo = '"+CustNo+"' " +
        ...
        /// <returns></returns>
        public static bool GetLinkManByID(string CompanyCD,string[] IDList)
        {
            if (IDList.Length == 0)
            {
                return false;
            ...
            try
            {
                ...
                string sql = "select id from officedba.CustComplain where CustLinkMan in (" + allCustID + ") and CompanyCD='" + CompanyCD + "'";
                if (IsHave(sql))
                    return true;
                else
                {
                    string sql1 = "select id from officedba.CustTalk where CustLinkMan in (" + allCustID + ") and CompanyCD='" + CompanyCD + "'";
                    if (IsHave(sql1))
                        return true;
                    else
                    {
                        string sql2 = "select id from officedba.CustLove where CustLinkMan in (" + allCustID + ") and CompanyCD='" + CompanyCD + "'";
                        if (IsHave(sql2))
                            return true;
                        else
                        {
                            string sql3 = "select id from officedba.CustAdvice where CustLinkMan in (" + allCustID + ") and CompanyCD='" + CompanyCD + "'";
                            if (IsHave(sql3))
                                return true;
                            else
                            { 
                                string sql4="select id from officedba.CustService where CustLinkMan in (" + allCustID + ") and CompanyCD='" + CompanyCD + "'";
                                if (IsHave(sql4))
                                    return true;
                                else
                                    return false;
        ...
        public static DataTable PrintLink(string CompanyCD, string linkid)
        {
            try
            {
                string sql = "SELECT cl.id    ," +
                                ...
                                "cl.[AboutFamily]     ," +
                                "cl.[Car]             ," +
                                "cl.[LiveHouse],cl.ProfessionalDes,cl.Skype     " +
        ...
        #region insertcustlinkman
        public static int LinkManAddReg(LinkManModel linkmanM, TransactionManager tran)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into officedba.CustLinkMan (");
            strSql.Append("CompanyCD,CustNo,LinkManName,HomeAddress,WorkTel,HandSet,MailAddress,Sex,CreatedDate )");
            strSql.Append(" values (");
            strSql.Append("@CompanyCD,@CustNo,@LinkManName,@HomeAddress,@WorkTel,@HandSet,@MailAddress,@Sex,@CreateDate )");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@HandSet",SqlDbType.VarChar,20),
                     new SqlParameter("@MailAddress",SqlDbType.VarChar,30),
                     new SqlParameter("@Sex",SqlDbType.Char,1),
                     new SqlParameter("@CreateDate",SqlDbType.DateTime),
                      new SqlParameter("@ID", SqlDbType.Int,0)
					};
            parameters[0].Value = linkmanM.CompanyCD;
            parameters[1].Value = linkmanM.CustNo;
            parameters[2].Value = linkmanM.LinkManName;
            parameters[3].Value = linkmanM.HomeAddress;
            parameters[4].Value = linkmanM.WorkTel;
            parameters[5].Value = linkmanM.Handset;
            parameters[6].Value = linkmanM.MailAddress;
            parameters[7].Value = linkmanM.Sex;
        ...
        //Updatelinkman creator
        public static void UpdateLinkMan(int linkid, int empID, string CompanyCD, TransactionManager tran)
        {
            StringBuilder sqlstr = new StringBuilder();
            sqlstr.AppendLine("update officedba.CustLinkMan set Creator=@empID where ID=@ID and CompanyCD=@CompanyCD");
            SqlParameter[] parameters = {
					new SqlParameter("@CompanyCD", SqlDbType.VarChar,8),
