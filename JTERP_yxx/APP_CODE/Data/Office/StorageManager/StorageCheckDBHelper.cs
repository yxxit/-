...
namespace XBase.Data.Office.StorageManager
{
    public class StorageCheckDBHelper
    {
        ...
        public static string StorageCheckAdd(StorageCheck sc, Hashtable htExtAttr, List<StorageCheckDetail> scdList)
        {
            ...
            foreach (StorageCheckDetail scd in scdList)
            {
                ...
                //strSubSql.Append("@CompanyCD,@CheckNo,@SortNo,@ProductID,@UnitID,@NowCount,@CheckCount,@DiffCount,@DiffType,@Remark,@ModifiedDate,@ModifiedUserID,@BatchNo,@UsedUnitID,@UsedUnitCount,@UsedPrice,@ExRate)");
                strSubSql.Append("CompanyCD,CheckNo,SortNo,ProductID,UnitID,NowCount,CheckCount,DiffCount,DiffType,Remark,ModifiedDate,ModifiedUserID)");
                strSubSql.Append(" values (");
                strSubSql.Append("@CompanyCD,@CheckNo,@SortNo,@ProductID,@UnitID,@NowCount,@CheckCount,@DiffCount,@DiffType,@Remark,@ModifiedDate,@ModifiedUserID)");
        ...
        private static void GetExtAttrCmd(StorageCheck sc, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@CompanyCD", sc.CompanyCD);
                cmd.Parameters.Add("@CheckNo", sc.CheckNo);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT sc.ID,sc.CompanyCD,sc.CheckNo,sc.Title,Convert(varchar(10),sc.CheckStartDate,120)CheckStartDate,Convert(varchar(10),sc.CheckEndDate,120)CheckEndDate,");
            strSql.Append("sc.StorageID,   ");
            ...
            strSql.Append(" Convert(varchar(10),sc.CreateDate,120)CreateDate,sc.BillStatus,");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select scd.SortNo,scd.ProductID,ISNULL(p.IsBatchNo,'')IsBatchNo,");
        ...
        #region 读取盘点列表 无botchNO
        public static DataTable GetStorageCheckList(Hashtable htPara, string EFIndex, string EFDesc, int PageIndex, int PageSize, string OrderBy, ref int TotalCount, string productname)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select distinct TMP.ID,TMP.ProductName,TMP.D_DiffCount,TMP.CheckNo,TMP.Title,TMP.DeptID,TMP.DeptName,TMP.Transactor,");
            ...
            strSql.Append(" select sc.ID,p.ProductName,de.DiffCount D_DiffCount,sc.CheckNo,sc.Title,sc.DeptID,sc.Transactor,");
            strSql.Append(" ei.EmployeeName as TransactorName,");
            strSql.Append("di.DeptName  AS DeptName,");
            strSql.Append("sc.StorageID,");
            strSql.Append(" si.StorageName AS StorageName,");
            ...
            strSql.Append("sc.DiffCount, (CASE sC.BillStatus WHEN '1' THEN '制单' when '2' THEN '执行' when '3' THEN '变更' when '4' THEN '手工结单' when '5' then '自动结单' end) as BillStatus,");
            ...
            strSql.Append(" from officedba.StorageCheck as sc left join officedba.StorageCheckdetail de on de.companycd=sc.companycd and de.CheckNo=sc.CheckNo left join officedba.productinfo p on p.id=de.productid ");
            strSql.Append(@" left join (select ei.ID,ei.EmployeeName FROM officedba.EmployeeInfo as ei  )ei on ei.ID=sc.Transactor
              left join (select di.ID,di.DeptName from officedba.DeptInfo di )di on di.ID=sc.DeptID
             left join (SELECT si.ID,si.StorageName from officedba.StorageInfo si ) si on si.ID=sc.StorageID 
             left join (SELECT x.CheckNo,x.CompanyCD from officedba.StorageCheckDetail as x   ) x on  x.CheckNo=sc.CheckNo and x.CompanyCD=sc.CompanyCD 
            WHERE 1=1 AND sc.CompanyCD=@CompanyCD ");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    strSql.AppendLine("(sc.Creator IN  ");
            ...
            strSql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where Sc.ID=FI.BillID AND fi.CompanyCD=Sc.CompanyCD AND fi.BillTypeCode=6 AND fi.BillTypeFlag=8 order by FI.ModifiedDate DESC) AND BillID=SC.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");

            strSql.AppendLine("or (sc.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (sc.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            {
                strSql.Append(" AND p.productname LIKE '%" + productname + "%' ");
            }
        ...
        public static DataTable GetStorageCheckList(Hashtable htPara, string EFIndex, string EFDesc,string BatchNo, int PageIndex, int PageSize, string OrderBy, ref int TotalCount,string productname)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select distinct TMP.ID,TMP.ProductName,TMP.D_DiffCount,TMP.CheckNo,TMP.Title,TMP.DeptID,TMP.DeptName,TMP.Transactor,");
            strSql.Append(" TMP.TransactorName,TMP.StorageID,TMP.StorageName,TMP.CheckStartDate,TMP.CheckType,TMP.DiffCount,TMP.BillStatus,TMP.FlowStatus from ( ");
            ...
            strSql.Append(@" left join (select ei.ID,ei.EmployeeName FROM officedba.EmployeeInfo as ei  )ei on ei.ID=sc.Transactor
              left join (select di.ID,di.DeptName from officedba.DeptInfo di )di on di.ID=sc.DeptID
             left join (SELECT si.ID,si.StorageName from officedba.StorageInfo si ) si on si.ID=sc.StorageID 
             left join (SELECT x.CheckNo,x.CompanyCD,x.BatchNo from officedba.StorageCheckDetail as x   ) x on  x.CheckNo=sc.CheckNo and x.CompanyCD=sc.CompanyCD 
            WHERE 1=1 AND sc.CompanyCD=@CompanyCD ");
            strSql.AppendLine("AND  (  ");
            XBase.Common.UserInfoUtil userInfo = (XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"];
            DataTable dt = XBase.Data.Office.SystemManager.RoleInfoDBHelper.GetRoleInfoByUserID(userInfo.UserID);
        ...
        /*不分页*/
        public static DataTable GetStorageCheckList(Hashtable htPara, string IndexValue, string TxtValue,string BatchNo, string OrderBy,string productname)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select distinct TMP.ID,TMP.ProductName,TMP.D_DiffCount,TMP.CheckNo,TMP.Title,TMP.DeptID,TMP.DeptName,TMP.Transactor,");
            strSql.Append(" TMP.TransactorName,TMP.StorageID,TMP.StorageName,TMP.CheckStartDate,TMP.CheckType,TMP.DiffCount,TMP.BillStatus,TMP.FlowStatus from ( ");
  ...
  left join (select di.ID,di.DeptName from officedba.DeptInfo di )di on di.ID=sc.DeptID
 left join (SELECT si.ID,si.StorageName from officedba.StorageInfo si ) si on si.ID=sc.StorageID 
 left join (SELECT x.CheckNo,x.CompanyCD,x.BatchNo from officedba.StorageCheckDetail as x   ) x on  x.CheckNo=sc.CheckNo and x.CompanyCD=sc.CompanyCD WHERE 1=1 AND sc.CompanyCD=@CompanyCD ");
            strSql.AppendLine("AND  ( ");
            XBase.Common.UserInfoUtil userInfo = (XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"];
            DataTable dt = XBase.Data.Office.SystemManager.RoleInfoDBHelper.GetRoleInfoByUserID(userInfo.UserID);
            if (dt != null && dt.Rows.Count > 0)
            ...
            strSql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where Sc.ID=FI.BillID AND fi.CompanyCD=Sc.CompanyCD AND fi.BillTypeCode=6AND fi.BillTypeFlag=8  order by FI.ModifiedDate DESC) AND BillID=SC.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");
