    ...
    public class StorageReturnDBHelper
    {
        ...
        public static DataTable GetStorageBorrow(Hashtable htPara, int PageIndex, int PageSize, string OrderBy, ref int TotalCount)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    sbSql.AppendLine("  (Sb.Creator IN  ");
            ...
            sbSql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where Sb.ID=FI.BillID AND fi.CompanyCD=Sb.CompanyCD AND fi.BillTypeCode=" + XBase.Common.ConstUtil.BILL_TYPECODE_STORAGE_BORROW.ToString() + " AND fi.BillTypeFlag=" + XBase.Common.ConstUtil.BILL_TYPEFLAG_STORAGE.ToString() + " order by FI.ModifiedDate DESC) AND BillID=SB.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");
            sbSql.AppendLine("or (Sb.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            sbSql.AppendLine("or (Sb.Borrower IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (Sb.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        #region 读取借货返还单列表
        public static DataTable GetStorageReturnList(string EFIndex, string EFDesc, Hashtable htPara, string CompanyCD, int PageSize, int PageIndex, string OrderBy, ref int TotalCount,string productnme)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("SELECT sr.ID,p.ProductName,de.ReturnCount,de.UnitPrice,de.TotalPrice, sr.ReturnNo,sr.Title,(SELECT si.StorageName from officedba.StorageInfo as si where si.id=sr.StorageID) AS StorageName,(SELECT ei.EmployeeName from officedba.EmployeeInfo as ei where ei.id=sr.ReturnPerson ) as ReturnName,sr.ReturnDate,(CASE sr.BillStatus WHEN '1' THEN '制单' when '2' THEN '执行' when '3' THEN '变更' when '4' THEN '手工结单' when '5' then '自动结单' end) as BillStatus,sb.BorrowNo,(select di.DeptName from officedba.DeptInfo as di where sb.DeptID=di.ID ) AS BorrowDept  from officedba.StorageReturn as sr left join officedba.StorageBorrow as sb on sr.FromBillID=sb.ID  ");
            strSql.AppendLine(" left join officedba.StorageReturnDetail de on sr.companycd=de.companycd and sr.ReturnNo=de.ReturnNo left join officedba.productinfo p on p.id=de.productid where 1=1  AND sr.CompanyCD=@CompanyCD");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["RoleRange"].ToString() == "1")
                {
                    strSql.AppendLine("(Sr.Creator IN  ");
            ...
            strSql.AppendLine("(select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where Sr.ID=FI.BillID AND fi.CompanyCD=SR.CompanyCD AND fi.BillTypeCode=8 AND fi.BillTypeFlag=10 order by FI.ModifiedDate DESC) AND BillID=SR.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");
            strSql.AppendLine("or (Sr.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            strSql.AppendLine("or (Sr.ReturnPerson IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (Sr.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (!string.IsNullOrEmpty(productnme))
            {
                strSql.AppendLine(" and p.productname  like '%" + productnme + "%' ");
            }
            #region 构造参数
            if (htPara.Count > 0)
            {
        ...
        /*不分页*/
        public static DataTable GetStorageReturnList(Hashtable htPara, string CompanyCD, string OrderBy, string productnme)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public static bool UpdateStatus(StorageReturn sr, string stype,ref string msg)
        {
            /***********************************
            ...
            if (stype == "1") //确认
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        SubSql.Append(" WHERE CompanyCD=@CompanyCD AND StorageID=@StorageID AND ProductID=@ProductID " + (isBatchNo ? "AND BatchNo=@BatchNo" : " and BatchNo is null "));
            ...
            else if (stype == "4")
            {
                ...
                if (checkcount(sr.ID.ToString(), ref msg))
                {
                    #region 构造更新状态SQL
                    sbUpdate.Append(" BillStatus=@BillStatus ,Confirmor=null,ConfirmDate=null ");
                    sbUpdate.Append(", ModifiedDate=@ModifiedDate,ModifiedUserID=@ModifiedUserID ");
                    sbUpdate.Append(" WHERE ID=@ID");
                    ...
                    Paras[0].Value = sr.BillStatus;
                    Paras[1].Value = sr.ID;
                    Paras[2].Value = sr.ModifiedDate;
                    Paras[3].Value = sr.ModifiedUserID;
        ...
        #endregion
        public static bool checkcount(string id, ref string msg)
        {
            string sql = @"select p.productname,isnull(s.productcount,0)+isnull(s.roadcount,0)+isnull(s.incount,0)-isnull(s.ordercount,0)-isnull(s.outcount,0)-b.ReturnCount num from officedba.StorageReturn a left join officedba.StorageReturnDetail b on a.companycd=b.companycd and a.ReturnNo=b.ReturnNo 
left join officedba.StorageProduct s on a.storageid=s.storageid and a.companycd=s.companycd and b.productid=s.productid and isnull(b.batchno,'')=isnull(s.batchno,'')  left join officedba.productinfo p on p.id=b.productid where a.id=" + id;
        ...
        private static void GetExtAttrCmd(StorageReturn model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@ReturnNo", model.ReturnNo);
