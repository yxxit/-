...
namespace XBase.Data.Office.StorageManager
{
    public class StockSearchDBHelper
    {
        ...
        public static DataTable GetLineInfo(string Confirmor, string type, StockAccountModel model, int pageIndex, int pageCount, string ord, ref int TotalCount, bool iflist)
        {
            ...
            {
                sql.AppendLine(" and a.ProdNo='" + model.ProductNo + "'                                  ");
        ...
        #region 收发货统计
        public static DataTable GetInvertoryGoods(string time1, string time2, string prodno1, string prodno2, string storageid, string prodtypeid, string showid, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            string sql="";
            string sqlb = "";
            ...
            if (showid == "0")
            {
                if (time1 != "" && time2 != "")
                {
                    sqlb = @"select b.productid,b.storageid,a.initcount,b.incount,b.outcount,isnull(a.initcount,0)+isnull(incount,0)-isnull(outcount,0) nowcount from 
                        (select productid,storageid,sum(pcount) initcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate<'" + time1 + @" 00:00:00'
                        group by productid,storageid )a right join
                        (select productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate between '" + time1 + @" 00:00:00' and '" + time2 + @" 23:59:59'
                        group by productid,storageid)b on a.productid=b.productid and a.storageid=b.storageid ";
                }
                else if (time1 == "" && time2 != "")
                {
                    sqlb = @"select b.productid,b.storageid,0 initcount,b.incount,b.outcount,0+isnull(incount,0)-isnull(outcount,0) nowcount from 
                    (select productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate <='" + time2 + @" 23:59:59'
                    group by productid,storageid)b ";
                }
                else if (time1 != "" && time2 == "")
                {
                    sqlb = @"select b.productid,b.storageid,a.initcount,b.incount,b.outcount,isnull(a.initcount,0)+isnull(incount,0)-isnull(outcount,0) nowcount from 
                        (select productid,storageid,sum(pcount) initcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate<'" + time1 + @" 00:00:00'
                        group by productid,storageid )a right join
                        (select productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate>='" + time1 + @" 00:00:00'
                        group by productid,storageid)b on a.productid=b.productid and a.storageid=b.storageid ";
                }
                else
                {
                    sqlb = @"select b.productid,b.storageid,0 initcount,b.incount,b.outcount,0+isnull(incount,0)-isnull(outcount,0) nowcount from 
                    (select productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' 
                    group by productid,storageid)b ";
                }
                sql += @"select p.productname,p.prodno,p.Specification,p.unitid,u.codename unitname,b.productid,p.typeid,t.codename,b.storageid,s.storagename,b.initcount,b.incount,b.outcount,b.nowcount 
                        from officedba.productinfo p right join  
                        (" + sqlb + @" )b on b.productid=p.id and p.companycd='" + CompanyCD + @"'
                        left join officedba.storageinfo s on s.id=b.storageid left join officedba.CodeProductType t on t.companycd=p.companycd and p.typeid=t.id
                        left join officedba.CodeUnitType u on u.id=p.unitid";
            }
            else
            {
                if (time1 != "" && time2 != "")
                {
                    sqlb = @"select billtype,productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate between '" + time1 + @" 00:00:00' and '" + time2 + @" 23:59:59'
                        group by productid,storageid,billtype ";
                }
                else if (time1 == "" && time2 != "")
                {
                    sqlb = @"select billtype,productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate <='" + time2 + @" 23:59:59'
                    group by productid,storageid,billtype ";
                }
                else if (time1 != "" && time2 == "")
                {
                    sqlb = @"select billtype,productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"' and confirmdate>='" + time1 + @" 00:00:00'
                        group by productid,storageid,billtype ";
                }
                else
                {
                    sqlb = @"select billtype,productid,storageid,sum(incount)  incount,-sum(outcount)  outcount from officedba.storageallbill where companycd='" + CompanyCD + @"'
                        group by productid,storageid,billtype ";
                }
                sql += @"select b.billtype,p.productname,p.prodno,p.Specification,p.unitid,u.codename unitname,b.productid,p.typeid,t.codename,b.storageid,s.storagename,b.incount,b.outcount
                        from officedba.productinfo p right join  
                        (" + sqlb + @" )b on b.productid=p.id and p.companycd='" + CompanyCD + @"'
                        left join officedba.storageinfo s on s.id=b.storageid left join officedba.CodeProductType t on t.companycd=p.companycd and p.typeid=t.id
                        left join officedba.CodeUnitType u on u.id=p.unitid";
            ...
            if (prodno1 != "" && prodno2 != "")
            {
                sql += " and p.prodno between '" + prodno1 + "' and '" + prodno2 + "'";
            }
            else if (prodno1 == "" && prodno2 != "")
            {
                sql += "  and p.prodno<= '" + prodno2 + "'";
            }
            else if (prodno1 != "" && prodno2 == "")
            {
                sql += "  and p.prodno>= '" + prodno1 + "'";
            }
            if (storageid != "")
            {
                sql+=" and b.storageid="+storageid+"";
            }
            if (prodtypeid != "")
            {
                sql += " and p.typeid=" + prodtypeid + "";
            }
            com.CommandText = sql;
            return SqlHelper.PagerWithCommand(com, pageIndex, pageCount, ord, ref TotalCount);
        }
        #endregion
