...
namespace XBase.Data.Office.StorageManager
{
    public class StorageOutOtherDBHelper
    {
        ...
        public static DataTable GetStorageOutOtherTableBycondition(StorageOutOtherModel model, string timeStart, string timeEnd,string EFIndex,string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount,string time1,string time2,string prodid)
        {
            ...
            sql.AppendLine(",a.OutNo ,p.productname                                                                              ");
            ...
            //------------------------------20121116---------------------------------//
           // sql.AppendLine(",case a.FromType when 0 then ISNULL(o.CodeName,'') else ISNULL(p1.CustName,'') end as OtherCorpName");        


            sql.AppendLine(" ,ISNULL(a.OtherCorpName,'') as OtherCorpName");
            ...
            sql.AppendLine(",de.TotalPrice                                                                          ");
            sql.AppendLine(",de.productcount CountTotal                                                                          ");
            ...
            sql.AppendLine("FROM officedba.StorageOutOther a  left join officedba.StorageOutOtherdetail de on a.outno=de.outno and a.companycd=de.companycd left join officedba.productinfo p on p.id=de.productid                                                ");
          ...
          //  sql.AppendLine(" left join officedba.ProviderInfo p1 on a.OtherCorpID=p1.ID ");

            sql.AppendLine("where a.CompanyCD=@CompanyCD");
            //---------------------20121116-----------------//
            if (model.OtherCorpName != string.Empty && model.OtherCorpName !=null)
            {
                sql.AppendLine(" and(a.OtherCorpName  like '%"+model.OtherCorpName+"%')");
            } 
        ...
        public static DataTable GetStorageOutOtherTableBycondition(StorageOutOtherModel model, string IndexValue, string TxtValue, string timeStart, string timeEnd,string BatchNo, string orderby,string prodid,string time1,string time2)
        {
        ...
        public static DataTable GetStorageOutOtherDetailInfo(StorageOutOtherModel model)
        {
            ...
            sql.AppendLine(",a.OtherCorpName                                                                                                            ");//往来单位名称 20121122 添加 
        ...
        /// <returns></returns>
        public static bool InsertStorageOutOther(StorageOutOtherModel model, Hashtable htExtAttr, List<StorageOutOtherDetailModel> modelList, out int IndexIDentity, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into officedba.StorageOutOther(");
            strSql.Append("CompanyCD,OutNo,Title,FromType,FromBillID,ReasonType,CorpBigType,OtherCorpID,OtherCorpName,SendAddr,ReceiveAddr,Sender,DeptID,TotalPrice,CountTotal,Summary,OutDate,Transactor,Remark,Creator,CreateDate,BillStatus,ModifiedDate,ModifiedUserID,ProjectID,CanViewUser,CanViewUserName)");
            strSql.Append(" values (");
            strSql.Append("@CompanyCD,@OutNo,@Title,@FromType,@FromBillID,@ReasonType,@CorpBigType,@OtherCorpID,@OtherCorpName,@SendAddr,@ReceiveAddr,@Sender,@DeptID,@TotalPrice,@CountTotal,@Summary,@OutDate,@Transactor,@Remark,@Creator,getdate(),@BillStatus,getdate(),@ModifiedUserID,@ProjectID,@CanViewUser,@CanViewUserName)");
        ...
        /// <param name="modelList"></param>
        /// <returns></returns>
        public static bool UpdateStorageOutOther(StorageOutOtherModel model, Hashtable htExtAttr, List<StorageOutOtherDetailModel> modelList, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.AppendLine("OtherCorpName=@OtherCorpName,");//20121122 添加 往来单位名称
        ...
        private static void SetSaveParameter(SqlCommand comm, StorageOutOtherModel model)
        {
            ...
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@OtherCorpID ", model.OtherCorpID));//往来单位ID
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@OtherCorpName ", model.OtherCorpName));//往来单位名称  20121122 添加
        ...
        #endregion
        #region 判断单据是否可反确认
        public static bool CheckIsUnfirm(StorageOutOtherModel model)
        {
            string sql = "select count(1) from officedba.StorageOutRed where companycd='" + model.CompanyCD + "' and fromtype=2 and frombillid=" + model.ID;
        ...
        #endregion
        #region 取消确认
        public static bool unconfirm(StorageOutOtherModel model)
        {
            ArrayList lstConfirm = new ArrayList();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" UPDATE officedba.StorageOutOther SET");
            ...
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=8 and billno='" + dt.Rows[0]["OutNo"].ToString() + "'";
        ...
        public static DataTable GetPRDetailInfo(string CompanyCD, string RejectNo, string Title)
        {
            ...
            sql.AppendLine(" where a.CompanyCD='" + CompanyCD + "' and a.BillStatus=2 and b.fromtype<>1 and ISNULL(b.BackCount,0)>ISNULL(b.OutedTotal,0) ");
            SqlCommand comm = new SqlCommand();
            //添加公司代码参数
