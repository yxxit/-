        ...
        public static DataTable GetInTypeInfo(string CompanyCD, string InType, string InNo, string Title)
        {
            ...
            switch (InType)
            {
                ...
                case "1":
                    ...
                    strSql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+CanViewUser+',')>0 )");
                    strSql.AppendLine("or (Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
                    strSql.AppendLine("or (Taker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
                ...
                case "2":
                    ...
                    strSql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+CanViewUser+',')>0 )");
                    strSql.AppendLine("or (Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) ");
                    strSql.AppendLine("or (Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
