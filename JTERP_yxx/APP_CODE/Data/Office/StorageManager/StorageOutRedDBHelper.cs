...
namespace XBase.Data.Office.StorageManager
{
    public class StorageOutRedDBHelper
    {
        ...
        public static DataTable GetStorageOutRedTableBycondition(StorageOutRedModel model, string timeStart, string timeEnd, string EFIndex, string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount,string prodid)
        {
            ...
            sql.AppendLine(",ISNULL(de.productcount,0) CountTotal                                                     ");
            ...
            sql.AppendLine("FROM officedba.StorageOutRed a left join officedba.StorageOutReddetail de on a.outno=de.outno and a.companycd=de.companycd left join officedba.productinfo p on p.id=de.productid                                                  ");
        ...
        public static DataTable GetStorageOutRedTableBycondition(StorageOutRedModel model, string IndexValue, string TxtValue, string timeStart, string timeEnd,string BatchNo, string orderby,string prodid)
        {
        ...
        public static DataTable GetStorageOutRedDetailInfo(StorageOutRedModel model)
        {
            ...
            sql.AppendLine("                                                                                                                                                                                                  ");
            sql.AppendLine(",case a.FromType                                                                                                                                                                                  ");
            sql.AppendLine("when '1' then (select  bb.ProductCount from officedba.StorageOutSellDetail bb left join officedba.StorageOutSell ee on bb.OutNo=ee.OutNo and bb.SortNo=b.FromLineNo where ee.id=a.FrombillID)     ");
            sql.AppendLine("when '2' then (select  bb.ProductCount from officedba.StorageOutOtherDetail  bb left join officedba.StorageOutOther  ee on bb.OutNo=ee.OutNo and bb.SortNo=b.FromLineNo where ee.ID=a.FrombillID) ");
        ...
        public static bool ConfirmBill(StorageOutRedModel model)
        {
            ...
            foreach (SqlCommand cmd in GetOutFromBillInfo(model.CompanyCD, model.ID,true))
            {
                lstConfirm.Add(cmd);
            }
        ...
        #region 取消确认
        public static bool unconfirm(StorageOutRedModel model)
        {
            ArrayList lstConfirm = new ArrayList();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" UPDATE officedba.StorageOutRed SET");
            ...
            foreach (SqlCommand cmd in GetOutFromBillInfo(model.CompanyCD, model.ID,false))
            {
                lstConfirm.Add(cmd);
            }
            //删除流水账
            SqlCommand comm1 = new SqlCommand();
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=9 and billno='" + dt.Rows[0]["OutNo"].ToString() + "'";
        ...
        public static DataTable GetStorageOutList(StorageOutRedModel model, string OutType)
        {
            ...
            switch (OutType)
            {
                case "1":
                    {
                        sql.AppendLine( "select a.ID,'销售出库单' as FromType,a.OutNo,a.Title,a.CountTotal,ISNULL(CONVERT(VARCHAR(10),a.CreateDate,21),'') AS CreateDate from officedba.StorageOutSell a where a.CompanyCD='" + model.CompanyCD + "' and a.BillStatus=2");
                        sql.AppendLine("AND  (  ");
                        if (dt != null && dt.Rows.Count > 0)
                        {
                ...
                case "2":
                    {
                        sql.AppendLine( "select a.ID,'其他出库单' as FromType,a.OutNo,a.Title,a.CountTotal,ISNULL(CONVERT(VARCHAR(10),a.CreateDate,21),'') AS CreateDate from officedba.StorageOutOther a where a.CompanyCD='" + model.CompanyCD + "' and a.BillStatus=2");
                        sql.AppendLine("AND  (  ");
                        if (dt != null && dt.Rows.Count > 0)
                        {
            ...
            if (!string.IsNullOrEmpty(model.OutNo))
            {
                sql.AppendLine( " and OutNo like '%'+ @OutNo +'%'");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@OutNo", model.OutNo));
            }
            if (!string.IsNullOrEmpty(model.Title))
            {
        ...
        public static DataTable GetOutTypeInfo(string CompanyCD, string OutType, string OutNo, string Title)
        {
            ...
            switch (OutType)
            {
                case "1":
                    ...
                    strSql.AppendLine("ISNULL(a.OutNo,'') as OutNo,'销售出库单' as OutType,");
                    strSql.AppendLine("ISNULL(a.Title,'') as Title,");
                    strSql.AppendLine("ISNULL(a.CountTotal,0) as CountTotal");
                    strSql.AppendLine("from officedba.StorageOutSell a where a.CompanyCD='" + CompanyCD + "' and a.BillStatus=2");
                    strSql.AppendLine(" and a.ConfirmDate>='" + StartDate + "'");
                    strSql.AppendLine(" and a.ConfirmDate<='" + EndDate + "'");
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (dt.Rows[0]["RoleRange"].ToString() == "1")
                        {
                            strSql.AppendLine("(a.Creator IN  ");
                    ...
                    strSql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");
                    strSql.AppendLine("or (a.Sender IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            
                    if (!string.IsNullOrEmpty(OutNo))
                    {
                        strSql.AppendLine(" and a.OutNo like '%'+ @OutNo +'%'");
                        comm.Parameters.Add(SqlHelper.GetParameterFromString("@OutNo", OutNo));
                    }
                    if (!string.IsNullOrEmpty(Title))
                    {
                        strSql.AppendLine(" and a.Title like '%'+ @Title +'%'");
                ...
                case "2":
                    strSql.AppendLine("select a.ID,");
                    strSql.AppendLine("ISNULL(a.OutNo,'') as OutNo,'其他出库单' as OutType,");
                    strSql.AppendLine("ISNULL(a.Title,'') as Title,");
                    strSql.AppendLine("ISNULL(a.CountTotal,0) as CountTotal");
                    strSql.AppendLine("from officedba.StorageOutOther a where a.CompanyCD='" + CompanyCD + "' and a.BillStatus=2");
                    strSql.AppendLine(" and a.ConfirmDate>='" + StartDate + "'");
                    strSql.AppendLine(" and a.ConfirmDate<='" + EndDate + "'");
                    strSql.AppendLine("AND  (  ");
                    if (dt != null && dt.Rows.Count > 0)
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static ArrayList GetOutFromBillInfo(string CompanyCD, string ID,bool isadd)
        {
                    ...
                    switch (dr["FromType"].ToString())
                    {
                        case "1":
                            ...
                            if (dt1.Rows.Count > 0)
                            {
                                if (dr["UsedUnitCount"].ToString() == "")
                                    listcmmd.Add(CmdComfirmReturn(dr["ProductCount"].ToString(), TableName, column, BillNoName, dt1.Rows[0]["SendNo"].ToString(), dt1.Rows[0]["SortNo"].ToString(), CompanyCD,isadd));
                                else
                                    listcmmd.Add(CmdComfirmReturn(dr["UsedUnitCount"].ToString(), TableName, column, BillNoName, dt1.Rows[0]["SendNo"].ToString(), dt1.Rows[0]["SortNo"].ToString(), CompanyCD, isadd));
                            }
                        ...
                        case "2":
                            if (dr["FromFlag"].ToString() != "0")//其他出库单的时候，要判断有来源才回写
                            {
                                ...
                                {
                                    if (dr["UsedUnitCount"].ToString() == "")
                                        listcmmd.Add(CmdComfirmReturn(dr["ProductCount"].ToString(), TableName, column, BillNoName, dt3.Rows[0]["RejectNo"].ToString(), dt3.Rows[0]["SortNo"].ToString(), CompanyCD, isadd));
                                    else
                                        listcmmd.Add(CmdComfirmReturn(dr["UsedUnitCount"].ToString(), TableName, column, BillNoName, dt3.Rows[0]["RejectNo"].ToString(), dt3.Rows[0]["SortNo"].ToString(), CompanyCD, isadd));
