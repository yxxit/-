...
namespace XBase.Data.Office.StorageManager
{
    public class StorageBorrowDBHelper
    {
        ...
        public static DataTable GetProductInfo(string CompanyCD, int BorrowDeptID, int StorageID, string ProductName, string ProdNo, string EFIndex, string EFDesc)
        {
            ...
            sbSql.Append("WHERE   A.CompanyCD=@CompanyCD AND C.StorageID=@StorageID  and A.CheckStatus='1'");
        ...
        #endregion
        public static bool  checkcount(StorageBorrow borrw,ref string msg)
        {
            string sql = @"select p.productname,isnull(s.productcount,0)+isnull(s.roadcount,0)+isnull(s.incount,0)-isnull(s.ordercount,0)-isnull(s.outcount,0)-b.productcount num from officedba.StorageBorrow a left join officedba.StorageBorrowDetail b on a.companycd=b.companycd and a.borrowno=b.borrowno 
left join officedba.StorageProduct s on a.storageid=s.storageid and a.companycd=s.companycd and b.productid=s.productid and isnull(b.batchno,'')=isnull(s.batchno,'')  left join officedba.productinfo p on p.id=b.productid where a.borrowno='" + borrw.BorrowNo + "' and a.companycd='" + borrw.CompanyCD + "'";
            DataTable dt = SqlHelper.ExecuteSql(sql);
            bool result = true;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (double.Parse(dt.Rows[i]["num"].ToString()) < 0)
                {
                    result = false;
                    msg = dt.Rows[i]["productname"].ToString();
                    break;
                }
            }
            return result;
        ...
        public static void SetStorageInfo(StorageBorrow borrow)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    //验证 明细中是否存入批次
                    bool IsBatch = false;
                    if (row["BatchNo"] != null && row["BatchNo"] != "" && row["BatchNo"].ToString().ToLower() != "nobatch" && row["BatchNo"].ToString().Trim() != "")
                        IsBatch = true;
                    ...
                    //借出量（增加）
                    sbSql.Append(" WHERE CompanyCD=@CompanyCD AND ProductID=@ProductID  AND StorageID=@StorageID " + (IsBatch ? "AND  BatchNo=@BatchNo" : " and BatchNo is null "));
        ...
        #region 读取借货单列表
        public static DataTable GetStorageList(string EFIndex, string EFDesc, int PageIndex, int PageSzie, string OrderBy, ref int TotalCount, StorageBorrow borrow, DateTime StartTime, DateTime EndTime, int SubmitStatus,string productname)
        {
            StringBuilder sbSql = new StringBuilder();
            ...
            sbSql.AppendLine("(SELECT S.ID, S.CompanyCD,p.ProductName,p.ProdNo,de.ProductCount,de.TotalPrice as DTotalPrice, S.BorrowNo, S.Title, ");
            sbSql.AppendLine("(select e.EmployeeName from officedba.EmployeeInfo as e where e.ID=S.Borrower  ) as Borrower, ");
            sbSql.AppendLine("(SELECT D.DeptName FROM officedba.DeptInfo as d where d.ID=S.DeptID) AS DeptID , S.BorrowDate, S.ReasonType,");
            sbSql.AppendLine("(SELECT OD.DeptName FROM officedba.DeptInfo as OD where OD.ID=S.OutDeptID) AS OutDeptID, ");
            sbSql.AppendLine(" (select si.StorageName from officedba.StorageInfo as si where si.ID= S.StorageID) AS StorageID, S.OutDate,");
            sbSql.AppendLine("(select te.EmployeeName from officedba.EmployeeInfo as te where te.ID=S.Transactor ) AS Transactor,S.Creator, S.CreateDate,");
            sbSql.AppendLine("(CASE S.BillStatus WHEN '1' THEN '制单' when '2' THEN '执行' when '3' THEN '变更' when '4' THEN '手工结单' when '5' then '自动结单' end) as BillStatus,");
            sbSql.AppendLine("( select top 1 fi.FlowStatus  from  officedba.FlowInstance AS FI  where S.ID=FI.BillID AND fi.CompanyCD=S.CompanyCD AND fi.BillTypeCode=" + XBase.Common.ConstUtil.BILL_TYPECODE_STORAGE_BORROW.ToString() + " AND fi.BillTypeFlag=" + XBase.Common.ConstUtil.BILL_TYPEFLAG_STORAGE.ToString() + " order by FI.ModifiedDate DESC) as FlowStatus,");
            sbSql.AppendLine("S.Confirmor, S.ConfirmDate, S.Closer, S.CloseDate, S.ModifiedDate, S.ModifiedUserID,S.TotalPrice,S.CountTotal");
            sbSql.AppendLine(" FROM officedba.StorageBorrow AS S left join  officedba.StorageBorrowdetail de on s.BorrowNo=de.BorrowNo and s.companycd=de.companycd left join officedba.productinfo p on productid=p.id  WHERE s.CompanyCD=@CompanyCD  ");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    sbSql.AppendLine("(s.Creator IN  ");
                    sbSql.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sbSql.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sbSql.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    sbSql.AppendLine("  WHERE a.ID=b.ID) AND DeptID<>" + userInfo.DeptID.ToString() + " )) OR  ");
                }
                if (dt.Rows[0]["RoleRange"].ToString() == "2")
                {
                    ...
                    sbSql.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sbSql.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    sbSql.AppendLine("  WHERE a.ID=b.ID) ))  OR  ");
                }
                if (dt.Rows[0]["RoleRange"].ToString() == "3")
                {
                    ...
                    sbSql.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sbSql.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sbSql.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.BranchID.ToString() + ") b  ");
                    sbSql.AppendLine("  WHERE a.ID=b.ID))) OR   ");
                }
            }
            sbSql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where S.ID=FI.BillID AND fi.CompanyCD=S.CompanyCD AND fi.BillTypeCode=" + XBase.Common.ConstUtil.BILL_TYPECODE_STORAGE_BORROW.ToString() + " AND fi.BillTypeFlag=" + XBase.Common.ConstUtil.BILL_TYPEFLAG_STORAGE.ToString() + " order by FI.ModifiedDate DESC) AND BillID=S.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");
            sbSql.AppendLine("or (S.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            sbSql.AppendLine("or (s.Borrower IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (s.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (!string.IsNullOrEmpty(productname))
            {
                htPara.Add("ProductName", productname);
            }
            ...
            foreach (string key in htPara.Keys)
            {
                ...
                {
                    case "BorrowNo":
                        sbSql.AppendLine(" AND S.BorrowNo like @" + key);
                        break;
                    case "Title":
                        sbSql.AppendLine(" AND S.Title like @" + key);
                        break;
                    case "Borrower":
                        sbSql.AppendLine(" AND  S.Borrower =@" + key);
                        break;
                    case "DeptID":
                        sbSql.AppendLine(" AND S.DeptID=@" + key);
                        break;
                    case "OutDeptID":
                        sbSql.AppendLine(" AND  S.OutDeptID=@" + key);
                        break;
                    case "StorageID":
                        sbSql.AppendLine(" AND  S.StorageID=@" + key);
                        break;
                    case "Transactor":
                        sbSql.AppendLine(" AND S.Transactor=@" + key);
                        break;
                    case "StartTime":
                        sbSql.AppendLine(" AND S.BorrowDate>=@" + key);
                        break;
                    case "EndTime":
                        sbSql.AppendLine(" AND  S.BorrowDate<=@" + key);
                        break;
                    case "ExtField":
                        sbSql.AppendLine(" AND ExtField" + EFIndex + " like '" + EFDesc + "' ");
                        break;
                    case "BillStatus":
                        sbSql.AppendLine(" AND S.BillStatus=@" + key);
                        break;
                    case "ProductName":
                        sbSql.AppendLine(" and p.Productname like '%" + productname + "%'");
                        break;
                    case "CompanyCD":
                        break;
                }
        ...
        /*不分页*/
        public static DataTable GetStorageList(string EFIndex, string EFDesc, StorageBorrow borrow, string OrderBy, DateTime StartTime, DateTime EndTime, int SubmitStatus,string productname)
        {
            StringBuilder sbSql = new StringBuilder();
            string Submit = string.Empty;
            if (SubmitStatus > 0)
            ...
            foreach (string key in htPara.Keys)
            {
                switch (key)
                {
                    ...
                    case "BillStatus":
                        ...
                        break;
                    case "ExtField":
                        sbSql.AppendLine(" AND s.ExtField" + EFIndex + " like @" + EFDesc);
                        break;
                    case "ProductName":
                        sbSql.AppendLine(" and p.Productname like '%" + productname + "%'");
                        break;
        ...
        public static bool CancelConfirm(StorageBorrow borrow)
        {
            ...
            foreach (DataRow row in dtDetail.Rows)
            {
                ...
                bool isBatch = false;
                if (row["BatchNo"] != null && row["BatchNo"].ToString().ToLower() != "nobatch" && row["BatchNo"].ToString().Trim() != "")
                    isBatch = true;
                StringBuilder sbSql = new StringBuilder();
                sbSql.Append("UPDATE officedba.StorageProduct SET  ");
        ...
        private static void GetExtAttrCmd(StorageBorrow model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@BorrowNo", model.BorrowNo);
