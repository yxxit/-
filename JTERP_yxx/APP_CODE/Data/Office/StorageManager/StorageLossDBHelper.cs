...
namespace XBase.Data.Office.StorageManager
{
    public class StorageLossDBHelper
    {
        ...
        public static DataTable GetStorageLossTableBycondition(StorageLossModel model, string timeStart, string timeEnd, string TotalPriceStart, string TotalPriceEnd, string FlowStatus, string EFIndex, string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount, string productname)
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT distinct a.ID                                                                        ");
            sql.AppendLine(",a.LossNo ,p.ProductName,de.ProductCount,de.UnitPrice,de.CostPrice                          ");
            ...
            sql.AppendLine("FROM officedba.StorageLoss a   left join officedba.StorageLossdetail de on de.lossno=a.lossno and de.companycd=a.companycd left join officedba.productinfo p on p.id=de.productid      ");
            ...
            sql.AppendLine("(select COUNT(*) from officedba.FlowTaskHistory where FlowNo=i.FlowNo  AND BillID=A.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");
        ...
        public static DataTable GetStorageLossTableBycondition(StorageLossModel model, string IndexValue, string TxtValue, string timeStart, string timeEnd, string TotalPriceStart, string TotalPriceEnd, string FlowStatus,string BatchNo, string orderby,string productname)
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT distinct a.ID                                                                        ");
            sql.AppendLine(",a.LossNo,p.ProductName,de.ProductCount,de.UnitPrice,de.CostPrice                                                                          ");
        ...
        private static void GetExtAttrCmd(StorageLossModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@LossNo", model.LossNo);
        ...
        #endregion
        public static bool CheckCount(StorageLossModel model, ref string msg)
        {
            string sql = @"select p.productname,isnull(s.productcount,0)-b.productcount num from officedba.StorageLossDetail b left join officedba.StorageLoss a 
on a.lossno=b.lossno and a.companycd=b.companycd left join officedba.StorageProduct s on a.storageid=s.storageid and a.companycd=s.companycd and b.productid=s.productid and 
isnull(b.batchno,'')=isnull(s.batchno,'') left join officedba.productinfo p on p.id=b.productid  where a.id= " + model.ID;
            bool result=true;
        ...
        public static bool ConfirmBill(StorageLossModel model)
        {
            ...
            string sqlSele = "select ProductID,isnull(BatchNo,'')BatchNo,LossNo,UnitPrice,CompanyCD,UsedUnitCount,ProductCount from officedba.StorageLossDetail where CompanyCD='" + model.CompanyCD + "'"
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    SqlCommand commPD = updateStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, dt1.Rows[0]["StorageID"].ToString(), modelList[i].ProductCount, model, false);
        ...
        public static bool CancelConfirmBill(StorageLossModel model)
        {
            ...
            List<StorageLossDetailModel> modelList = new List<StorageLossDetailModel>();
            string sqlSele = "select LossNo,ProductID,isnull(BatchNo,'')BatchNo,UsedUnitCount,ProductCount from officedba.StorageLossDetail where CompanyCD='" + model.CompanyCD + "'"
                            + "and LossNo=(select LossNo from officedba.StorageLoss where ID=" + model.ID + ")";
            DataTable dt = SqlHelper.ExecuteSql(sqlSele);
            string sqlpri = "select StorageID from officedba.StorageLoss where ID=" + model.ID + "";
            DataTable dt1 = SqlHelper.ExecuteSql(sqlpri);
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    SqlCommand commPD = updateStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, dt1.Rows[0]["StorageID"].ToString(), modelList[i].ProductCount, model, true);
                    lstConfirm.Add(commPD);
                }
            }
            ...
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD + "' and billtype=16 and billno='" + dt.Rows[0]["LossNo"].ToString() + "'";
