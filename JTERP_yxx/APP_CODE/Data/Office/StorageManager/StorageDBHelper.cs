...
namespace XBase.Data.Office.StorageManager
{
    public class StorageDBHelper
    {
        ...
        public static bool ChargeProductInfo(string codename, string compid, string prodNo)
        {
            ...
            //string pno=prodNo.Trim();
            string goodsno = "";
            for (int i = 0; i < prodNo.Length; i++)
            {
                char str=char.Parse(prodNo.Substring(i, 1));
                if (str != '')
                {
                    goodsno += str.ToString();
                }
            }
            string sql = "select count(*) from officedba.ProductInfo where ProdNo='"+goodsno+"' AND ProductName='" + codename.Trim() + "' and CompanyCD='" + compid.Trim() + "'";
        ...
        /// <param name="enddate"></param>
        /// <returns></returns>
        public static DataTable GetStorageCompare(string companycd, int storageID, string begindate, string enddate,int ByTimeType,int custid,string proname)
        {
        ...
        /// <param name="recordCount"></param>
        /// <returns></returns>
        public static DataTable GetStorageCompareDetails(string companycd, int storageID, int ProductID, string begindate, string enddate, int ByTimeType, int pageindex, int pagesize, ref int recordCount, int custid, string proname)
        {
            SqlParameter[] param = 
            {
                new SqlParameter("@companyCD",companycd),
