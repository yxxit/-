    ...
    public class StorageQualityCheckProDBHelper
    {
        public static DataTable GetQualityCheckPro(string Method, string QuaStr, string CompanyCD)
        {
            ...
            else if (Method == "2")  //生产类型 
            {
                ...
                string ProductNo = str[0];
                string ProductName = str[1];
                sql.AppendLine("SELECT isnull(b.ProductName,'') as ProductName,isnull(a.ModifiedDate,'') as ModifiedDate,isnull(a.UsedUnitCount,0) as InCount, isnull(b.ProdNo,'') as ProdNo,isnull(a.TaskNo,'') as FromBillNo,isnull(a.ApplyCheckCount,0) as ApplyCheckCount,ISNULL(a.UsedUnitID,0) AS UsedUnitID,ISNULL(CU.CodeName,'') AS UsedUnitName,ISNULL(a.UsedUnitCount,0) AS UsedUnitCount, ");
                ...
                sql.AppendLine(" where d.BillStatus = 2 and d.CompanyCD=@CompanyCD   and a.CompanyCD=@CompanyCD");
                if (ProductNo != "")
                {
        ...
        public static DataTable GetQualityCheckList(StorageQualityCheckApplay model, DateTime EndCheckDate, string FlowStatus, string EFIndex, string EFDesc, ref int TotalCount)
        {
            ...
            sql.AppendLine("  (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = a.ID AND BillTypeFlag =" + BillTypeFlag + " AND BillTypeCode = " + BillTypeCode + " ) AND BillID = a.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  OR (a.Principal IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  )");
