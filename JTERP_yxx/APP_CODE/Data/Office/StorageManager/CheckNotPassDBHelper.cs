...
namespace XBase.Data.Office.StorageManager
{
    public class CheckNotPassDBHelper
    {
        ...
        public static DataTable GetReportInfo(string CompanyCD, string method, string NoPassStr)
        {
            ...
            if (dtt != null && dtt.Rows.Count > 0)
            {
                ...
                if (dtt.Rows[0]["RoleRange"].ToString() == "1")
                {
                    ...
                    sql += "  WHERE DeptID IN (SELECT x.ID  ";
                    sql += " FROM officedba.DeptInfo x,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") y  ";
                    sql += "  WHERE x.ID=y.ID) AND DeptID<>" + userInfo.DeptID.ToString() + " )) OR ";
                }
        ...
        public static DataTable GetAllNoPass(CheckNotPassModel model, string ProductID, string BetinTime, string EndTime, string FlowStatus, string EFIndex, string EFDesc, ref int TotalCount)
        {
            ...
            sql += "  (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = c.ID AND BillTypeFlag = " + BillTypeFlag + " AND BillTypeCode = " + BillTypeCode + " ) AND BillID = c.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ";

            sql += "or (c.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (c.Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) )";
