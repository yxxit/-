    ...
    public class StorageTransferDBHelper
    {
        ...
        public static DataTable GetStorageTransferList(string EFIndex, string EFDesc, Hashtable htPara, int PageIndex, int PageSize, string OrderBy, ref int TotalCount)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    sbSql.AppendLine("  (St.Creator IN  ");
            ...
            sbSql.AppendLine(" (St.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            sbSql.AppendLine(" OR  (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=( select top 1 fi.FlowNo  from  officedba.FlowInstance AS FI  where St.ID=FI.BillID AND fi.CompanyCD=St.CompanyCD AND fi.BillTypeCode=4 AND fi.BillTypeFlag=8 order by FI.ModifiedDate DESC) AND BillID=ST.ID   AND operateUserId = '" + userInfo.UserID + "')>0 ");
            sbSql.AppendLine("or (St.InUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (St.OutDeptID=" + userInfo.DeptID + ") or (St.ApplyDeptID=" + userInfo.DeptID + ")");
            sbSql.AppendLine("or (St.ApplyUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (St.OutUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        public static string StorageTransferOut(Model.Office.StorageManager.StorageTransfer st, DataTable dtDetail, bool isBatchNo)
        {
            ...
            for (int i = 0; i < dtDetail.Rows.Count; i++)
            {
                ...
                accountSql.AppendLine(" (CompanyCD,BillType,ProductID,StorageID," + (isBatchNo ? "BatchNo," : string.Empty) + "BillNo,HappenDate,HappenCount,ProductCount,Creator,Price,PageUrl,ReMark) ");
                ...
                accountSql.AppendLine(" ,@PageUrl,@ReMark ");
                ...
                SqlParameter[] acctParams = new SqlParameter[7];
        ...
        private static void GetExtAttrCmd(StorageTransfer model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@TransferNo", model.TransferNo);
