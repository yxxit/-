...
namespace XBase.Data.Office.StorageManager
{
    public class StorageInRedDBHelper
    {
        ...
        public static DataTable GetStorageInRedTableBycondition(string BatchNo,StorageInRedModel model, string timeStart, string timeEnd, int pageIndex, int pageCount, string ord, ref int TotalCount,string prodid)
        {
            ...
            sql.AppendLine("(select a.ID,p.productname                                                                                                                                                                   ");
            sql.AppendLine(",ISNULL(c.CodeName,'') as CodeName");
            sql.AppendLine(",ISNULL(a.InNo,'') AS InNo                                                                                                                                                      ");
            sql.AppendLine(",ISNULL(a.Title,'') AS Title                                                                                                                                                    ");
            sql.AppendLine(",ISNULL(de.productcount,'0') AS CountTotal                                                                                                                                         ");
            sql.AppendLine(",ISNULL(de.TotalPrice,'0') AS TotalPrice                                                                                                                                         ");
            ...
            sql.AppendLine(" from officedba.StorageInRed a  left join officedba.StorageInReddetail de on a.inno=de.inno and a.companycd=de.companycd  left join officedba.productinfo p on p.id=de.productid ");
            ...
            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (a.Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        public static DataTable GetStorageInRedTableBycondition(string BatchNo,StorageInRedModel model, string timeStart, string timeEnd, string orderby,string prodid)
        {
            //列表：入库单编号、入库单主题、源单类型、原始入库单、入库部门、人库人、入库时间、红冲数量、红冲金额、摘要、单据状态。
            string CanUser = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID.ToString();
            StringBuilder sql = new StringBuilder();
        ...
        public static DataTable GetStorageInRedDetailInfo(StorageInRedModel model)
        {
            ...
            sql.AppendLine(",case a.FromType                                                                                                           ");
            sql.AppendLine("when '1' then (select  bb.ProductCount from officedba.StorageInPurchaseDetail bb left join officedba.StorageInPurchase ee on bb.InNo=ee.InNo and bb.companycd=ee.companycd  and bb.SortNo=b.FromLineNo where ee.id=a.FrombillID)");
            sql.AppendLine("when '2' then (select  bb.ProductCount from officedba.StorageInProcessDetail  bb left join officedba.StorageInProcess  ee on bb.InNo=ee.InNo and bb.companycd=ee.companycd   and bb.SortNo=b.FromLineNo where ee.ID=a.FrombillID)");
            sql.AppendLine("when '3' then (select  bb.ProductCount from officedba.StorageInOtherDetail bb left join officedba.StorageInOther ee on ee.InNo=bb.InNo and bb.companycd=ee.companycd  and bb.SortNo=b.FromLineNo where ee.id=a.FrombillID)      ");
        ...
        public static bool ConfirmBill(StorageInRedModel model)
        {
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    SqlCommand commPD = updateStorageProduct(modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model.CompanyCD, modelList[i].BatchNo,true);
                    lstConfirm.Add(commPD);
                }
            }
            foreach (SqlCommand cmd in GetInFromBillInfo(model.CompanyCD, model.ID,true))
            {
                lstConfirm.Add(cmd);
            }
        ...
        #region 取消确认
        public static bool unconfirm(StorageInRedModel model)
        {
            ArrayList lstConfirm = new ArrayList();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" UPDATE officedba.StorageInRed SET");
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    SqlCommand commPD = updateStorageProduct(modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model.CompanyCD, modelList[i].BatchNo,false);
                    lstConfirm.Add(commPD);
                }
            }
            foreach (SqlCommand cmd in GetInFromBillInfo(model.CompanyCD, model.ID,false))
            {
                lstConfirm.Add(cmd);
            }
            //删除流水账
            SqlCommand comm1 = new SqlCommand();
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=6 and billno='" + dt.Rows[0]["InNo"].ToString() + "'";
        ...
        public static DataTable GetStorageInList(StorageInRedModel model, string InType)
        {
            ...
            switch (InType)
            {
                case "1":
                    {
                        sql.AppendLine( "select a.ID,'采购入库单' as FromType,a.InNo,a.Title,a.CountTotal,ISNULL(CONVERT(VARCHAR(10),a.CreateDate,21),'') AS CreateDate from officedba.StorageInPurchase a where a.CompanyCD='" + model.CompanyCD + "' and a.BillStatus=2");
                        sql.AppendLine("AND  (  ");
                        if (dt != null && dt.Rows.Count > 0)
                        {
                        ...
                        sql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");
                        sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
                        sql.AppendLine("or (a.Taker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (a.Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            
                    }
                    break;
                case "2":
                    {
                        sql.AppendLine( "select a.ID,'生产完工入库单' as FromType,a.InNo,a.Title,a.CountTotal,ISNULL(CONVERT(VARCHAR(10),a.CreateDate,21),'') AS CreateDate from officedba.StorageInProcess a where a.CompanyCD='" + model.CompanyCD + "' and a.BillStatus=2");
                        sql.AppendLine("AND  (  ");
                        if (dt != null && dt.Rows.Count > 0)
                        {
                ...
                case "3":
                    {
                       sql.AppendLine("select a.ID,'其他入库单' as FromType,a.InNo,a.Title,a.CountTotal,ISNULL(CONVERT(VARCHAR(10),a.CreateDate,21),'') AS CreateDate from officedba.StorageInOther a where a.CompanyCD='" + model.CompanyCD + "' and a.BillStatus=2");
                       sql.AppendLine("AND  (  ");
                       if (dt != null && dt.Rows.Count > 0)
                       {
        ...
        /// <param name="CompanyCD"></param>
        /// <returns></returns>
        public static SqlCommand updateStorageProduct(string ProductID, string StorageID, string ProductNum, string CompanyCD, string BatchNo,bool isadd)
        {
            StringBuilder strSql = new StringBuilder();
            if (isadd)
            {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public static ArrayList GetInFromBillInfo(string CompanyCD, string ID,bool isadd)
        {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        switch (dr["FromType"].ToString())
                        {
                            case "1":
                                ...
                                if (dt1.Rows.Count > 0)
                                {
                                    listcmmd.Add(CmdComfirmReturn(dr["ProductCount"].ToString(), TableName, column, BillNoName, dt1.Rows[0]["ArriveNo"].ToString(), dt1.Rows[0]["SortNo"].ToString(), CompanyCD,isadd));
                                }
                            ...
                            case "2":
                                ...
                                if (dt2.Rows.Count > 0)
                                {
                                    listcmmd.Add(CmdComfirmReturn(dr["ProductCount"].ToString(), TableName, column, BillNoName, dt2.Rows[0]["TaskNo"].ToString(), dt2.Rows[0]["SortNo"].ToString(), CompanyCD,isadd));
                                }
                                break;
                            case "3":
                                if (dr["FromFlag"].ToString() != "0")//其他入库单的时候，要判断有来源才回写
                                {
                                    ...
                                    if (dt3.Rows.Count > 0)
                                    {
                                        listcmmd.Add(CmdComfirmReturn(dr["ProductCount"].ToString(), TableName, column, BillNoName, dt3.Rows[0]["BackNo"].ToString(), dt3.Rows[0]["SortNo"].ToString(), CompanyCD,isadd));
                                    }
                                }
                                break;
                            default:
        ...
        /// <returns></returns>
        public static SqlCommand CmdComfirmReturn(string ProductCount, string TableName, string column, string BillNoName, string BillNo, string sortNo, string CompanyCD,bool isadd)
        {
            StringBuilder StrSql = new StringBuilder();
            if (isadd)
            {
