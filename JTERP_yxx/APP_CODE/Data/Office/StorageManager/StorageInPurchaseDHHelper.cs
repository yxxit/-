...
namespace XBase.Data.Office.StorageManager
{
    public class StorageInPurchaseDHHelper
    {
        ...
        public static DataTable GetStorageInPurchaseTableBycondition(string BatchNo, StorageInPurchaseModel model, string timeStart, string timeEnd, string FromBillNo, string StorageID, int pageIndex, int pageCount, string ord, ref int TotalCount,string time1,string time2,string prodid)
        {
            ...
            sql.AppendLine("SELECT DISTINCT a.ID,p.productname,de.productcount,isnull(dbo.getStorageName(de.storageid),'')storagename ");
            ...
            sql.AppendLine(" FROM officedba.StorageInPurchase a left join officedba.StorageInPurchasedetail de on a.companycd=de.companycd and a.InNo=de.InNo left join officedba.productinfo p on p.id=de.productid ");
            ...
            sql.AppendLine(" (f.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");
            ...
            if (!string.IsNullOrEmpty(model.EFIndexP) && !string.IsNullOrEmpty(model.EFDescP))
            {
                sql.AppendLine(" and p.ExtField" + model.EFIndexP + " LIKE @EFDescP");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@EFDescP", "%" + model.EFDescP + "%"));
            }
            if (time1 != "" && time2 != "")
            {
        ...
        public static DataTable GetStorageInPurchaseTableBycondition(string BatchNo, StorageInPurchaseModel model, string timeStart, string timeEnd, string FromBillNo, string StorageID, string orderby,string time1,string time2,string prodid)
        {
            //入库单编号、入库单主题、采购到货单、交货人、验收人、
            //入库部门、人库人、入库时间、入库数量、入库金额、摘要、单据状态
            string CanUser = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID.ToString();
        ...
        public static DataTable GetStorageInPurchaseDetailInfo(StorageInPurchaseModel model)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("select isnull(A.Custom1,'')Custom1,isnull(A.Custom2,'')Custom2,isnull(A.Custom3,'')Custom3,isnull(A.Custom4,'')Custom4,isnull(A.Custom5,'')Custom5,isnull(A.Custom6,'')Custom6,isnull(A.Custom7,'')Custom7,isnull(A.Custom8,'')Custom8,isnull(A.Custom9,'')Custom9,isnull(A.Custom10,'')Custom10,");
        ...
        /// </summary>
        /// <returns>DataTable</returns>
        public static bool InsertStorageInPurchase(StorageInPurchaseModel model, List<StorageInPurchaseDetailModel> modelList, Hashtable htExtAttr, out int IndexIDentity, string[] strCustomdetail)
        {
        ...
        public static bool UpdateStorageInPurchase(StorageInPurchaseModel model, List<StorageInPurchaseDetailModel> modelList, Hashtable htExtAttr, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public static bool ConfirmBill(StorageInPurchaseModel model, out string Msg)
        {
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                ...
                for (int i = 0; i < modelList.Count; i++)
                {
                    ...
                    //}
                    SqlCommand commRoad = new SqlCommand();
                    commRoad = updateRoadCount(modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model,true);
                    lstConfirm.Add(commRoad);
                }
            }
        ...
        #region 取消确认
        public static bool unconfirm(StorageInPurchaseModel model)
        {
            //判断源单是无来源还是有来源，无来源则不需要更新在途量
            string sqlFromType = "select a.FromType,a.ProviderID from officedba.PurchaseArrive a"
                        + " inner join officedba.StorageInPurchase b on b.FromBillID=a.ID and b.ID=" + model.ID;
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                ...
                for (int i = 0; i < modelList.Count; i++)
                {
                    ...
                    //}
                    SqlCommand commRoad = new SqlCommand();
                    commRoad = updateRoadCount(modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model,false);
                    lstConfirm.Add(commRoad);
                }
            }
            //删除流水账
            SqlCommand comm1 = new SqlCommand();
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=3 and billno='" + dt.Rows[0]["BillNo"].ToString() + "'";
        ...
        private static SqlCommand updateRoadCount(string ProductID, string DefaultStorageID, string ProductNum, StorageInPurchaseModel model,bool isadd)
        {
            StringBuilder strSql = new StringBuilder();
            if (isadd)
            {
        ...
        #region 判断单据是否可反确认
        public static bool CheckIsUnfirm(StorageInPurchaseModel model)
        {
            string sql = "select count(1) from officedba.StorageInRed where companycd='" + model.CompanyCD + "' and fromtype=1 and frombillid=" + model.ID;
        ...
        public static bool ISConfirmBill(StorageInPurchaseModel model)
        {
            ...
            //2011-11-12 add by shjp( and a.companycd=b.companycd   )
            sql.AppendLine("left join officedba.StorageInPurchase a on a.InNo=b.InNo         and a.companycd=b.companycd                                ");
