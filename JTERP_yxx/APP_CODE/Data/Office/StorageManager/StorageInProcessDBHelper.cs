...
namespace XBase.Data.Office.StorageManager
{
    public class StorageInProcessDBHelper
    {
        ...
        public static DataTable GetStorageInProcessTableBycondition(string BatchNo,StorageInProcessModel model, string timeStart, string timeEnd, string StorageID, string FromBillNo, int pageIndex, int pageCount, string ord, ref int TotalCount,string proname,string time1,string time2)
        {
            //这个是列表查询
            ...
            sql.AppendLine("SELECT DISTINCT jj.productname,isnull(dbo.getStorageName(de.storageid),'')storagename,de.productcount,jj.specification,dbo.getCodename(jj.unitid)codename,a.ID,ISNULL(a.InNo,'') AS InNo                                                                ");
            ...
            sql.AppendLine("	FROM officedba.StorageInProcessdetail de left join  officedba.StorageInProcess a on a.companycd=de.companycd and a.inno=de.inno  left join officedba.productinfo jj on jj.id=de.productid    ");
            ...
            sql.AppendLine(" (j.Principal IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");
            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) ");
            sql.AppendLine("or (a.Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (proname != "")
            {
                sql.AppendLine(" and jj.productname like '%"+proname+"%' ");
            }
            ...
            if (time1 != "" && time2 != "")
            {
                sql.AppendLine(" and s.CreateDate between '" + time1 + "' and '" + time2 + "' ");
            }
            else if (time1 == "" && time2 != "")
            {
                sql.AppendLine(" and s.CreateDate<='" + time2 + "' ");
            }
            else if (time1 != "" && time2 == "")
            {
                sql.AppendLine(" and s.CreateDate>='" + time1 + "' ");
            }
            comm.CommandText = sql.ToString();
            return SqlHelper.PagerWithCommand(comm, pageIndex, pageCount, ord, ref TotalCount);
        }

        public static DataTable GetStorageInProcessTableBycondition(string BatchNo,StorageInProcessModel model, string timeStart, string timeEnd, string StorageID, string FromBillNo, string orderby,string proname,string time1,string time2)
        {
            //这个是列表查询
            //入库单编号、入库单主题、生产任务单、加工类别、加工单位、生产负责人、
            //部门、人库人、入库时间、入库数量、入库金额、摘要、单据状态
            string CanUser = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID.ToString();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT isnull(de.NoPassAnswer,'')NoPassAnswer,isnull(de.Remark,'')Remark,isnull(dbo.getStorageName(de.storageid),'')storagename,jj.productname,jj.specification,dbo.getCodename(jj.unitid)codename,a.ID,ISNULL(a.InNo,'') AS InNo                                                                ");
            sql.AppendLine(",ISNULL(a.Title,'') AS Title                                                                         ");
            sql.AppendLine(",ISNULL(j.TaskNo,'') AS TaskNo                                                                       ");
            sql.AppendLine(",ISNULL(b.DeptName,'') as ProcessDeptName                                                            ");
            sql.AppendLine(",case j.ManufactureType when '0' then '普通' when '1' then '返修' when '2' then '拆件'  else '' end as ProcessType");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(@"SELECT isnull(b.Custom1,'')Custom1,isnull(b.Custom2,'')Custom2,isnull(b.Custom3,'')Custom3,isnull(b.Custom4,'')Custom4,isnull(b.Custom5,'')Custom5,isnull(b.Custom6,'')Custom6,isnull(b.Custom7,'')Custom7,isnull(b.Custom8,'')Custom8,isnull(b.Custom9,'')Custom9,isnull(b.Custom10,'')Custom10,
                                    a.ID ,isnull(c.Size,'')[Size],isnull(b.TotalNumber,0)TotalNumber,isnull(b.PieceCount,0)PieceCount,isnull(b.PieceNum,0)PieceNum,isnull(b.NoPassNum,0)NoPassNum,isnull(b.NoPassAnswer,'')NoPassAnswer,isnull(b.pakeages,'')pakeages,isnull(b.pakeages,0)pakeageid ,isnull(a.UserManIds,'')UserManIds,isnull(a.UserMans,'')UserMans,a.CanViewUser,a.CanViewUserName,a.ExtField1,a.ExtField2,a.ExtField3,a.ExtField4,a.ExtField5,a.ExtField6,a.ExtField6,a.ExtField7,a.ExtField8,a.ExtField9,a.ExtField10 ");
            ...
            sql.AppendLine(",ISNULL(MD.ProductCount,0) as FromBillCount");
            sql.AppendLine(",ISNULL(MD.InCount,0) as InCount");
            //sql.AppendLine("  ,ISNULL(MD.ProductedCount,0)-ISNULL(MD.InCount,0) as NotInCount      
            sql.AppendLine("  ,ISNULL(MD.ProductCount,0)-ISNULL(MD.InCount,0) as NotInCount  ");
        ...
        /// </summary>
        /// <returns>DataTable</returns>
        public static bool InsertStorageInProcess(StorageInProcessModel model, List<StorageInProcessDetailModel> modelList, Hashtable htExtAttr, out int IndexIDentity, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into officedba.StorageInProcess(");
            strSql.Append("CompanyCD,InNo,FromType,FromBillID,Title,Executor,EnterDate,Remark,Creator,CreateDate,BillStatus,ModifiedDate,ModifiedUserID,DeptID,TotalPrice,CountTotal,Summary,CanViewUser,CanViewUserName,UserManIds,UserMans)");
            strSql.Append(" values (");
            strSql.Append("@CompanyCD,@InNo,@FromType,@FromBillID,@Title,@Executor,@EnterDate,@Remark,@Creator,getdate(),@BillStatus,getdate(),@ModifiedUserID,@DeptID,@TotalPrice,@CountTotal,@Summary,@CanViewUser,@CanViewUserName,@UserManIds,@UserMans)");
            ...
            if (modelList != null && modelList.Count > 0)//明细不为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    ...
                    {
                        strSqlDetail.Append(",TotalNumber,PieceCount,PieceNum,pakeages,NoPassNum,NoPassAnswer");
                    }
                    //添加扩展项
                    strSqlDetail.Append(GetBillTableCellsDBHelper.insertCustomFilder(strCustomdetail, (i + 1)));
                    ...
                    //扩展项值
                    strSqlDetail.Append(GetBillTableCellsDBHelper.insertCustomvalue(strCustomdetail, (i + 1)));
        ...
        public static bool UpdateStorageInProcess(StorageInProcessModel model, List<StorageInProcessDetailModel> modelList, Hashtable htExtAttr, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            //shjp add
            strSql.Append(",UserManIds='"+model.Userids+"'");
            strSql.Append(",UserMans='"+model.Usernames+"'");
            //
                ...
                for (int i = 0; i < modelList.Count; i++)
                {
                    ...
                    if (Exists(modelList[i].BatchNo, modelList[i].StorageID, modelList[i].ProductID, model.CompanyCD))
                    {
                        if(((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit)
                            commPD = updateStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].StorageID, modelList[i].UsedUnitCount, model, true);
                        else
                    ...
                    {
                        if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit)
                            commPD = InsertStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].StorageID, modelList[i].UsedUnitCount, model.CompanyCD);
                        else
                    ...
                    if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit)
                        commRoad = updateRoadCount(modelList[i].BatchNo,modelList[i].ProductID, modelList[i].DefaultStorageID, modelList[i].UsedUnitCount, model,true);
                    else
                        commRoad = updateRoadCount(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].DefaultStorageID, modelList[i].ProductCount, model,true);
        ...
        #region 取消确认
        public static bool unconfirm(StorageInProcessModel model)
        {
            ArrayList lstConfirm = new ArrayList();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" UPDATE officedba.StorageInProcess SET");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if (modelList != null && modelList.Count > 0)//明细不为空的时候
                {
                    ...
                    for (int i = 0; i < modelList.Count; i++)
                    {
                        ...
                        SqlCommand commPD = new SqlCommand();
                        if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit)
                            commPD = updateStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].StorageID, modelList[i].UsedUnitCount, model, false);
                        else
                            commPD = updateStorageProduct(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model, false);

                        lstConfirm.Add(commPD);
                        SqlCommand commRoad = new SqlCommand();
                        if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).IsMoreUnit)
                            commRoad = updateRoadCount(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].DefaultStorageID, modelList[i].UsedUnitCount, model,false);
                        else
                            commRoad = updateRoadCount(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].DefaultStorageID, modelList[i].ProductCount, model,false);
                        lstConfirm.Add(commRoad);
                ...
                comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=4 and billno='" + dt.Rows[0]["InNo"].ToString() + "'";
                lstConfirm.Add(comm1);
                return SqlHelper.ExecuteTransWithArrayList(lstConfirm);
        ...
        private static void SetSaveParameter(SqlCommand comm, StorageInProcessModel model)
        {
            ...
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@UserManIds ", model.Userids));//操作人ID
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@UserMans ", model.Usernames));//操作人Name
        }
        #endregion
        ...
        private static void EditInitailDetailInfo(SqlCommand comm, StorageInProcessDetailModel model)
        {
            ...
            if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).Version == "floor")
            {
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@TotalNumber ", model.TotalNumber));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@PieceCount ", model.PieceCount));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@PieceNum ", model.PieceNum));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@pakeages ", model.pakeid));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@NoPassNum ", model.NoPassNum));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@NoPassAnswer ", model.NoPassAnswer));
            }
        }
        #endregion
        ...
        public static SqlCommand updateStorageProduct(string BatchNo, string ProductID, string StorageID, string ProductNum, StorageInProcessModel model, bool flag)
        {
            ...
            if (flag == true)
            {
                ...
                //strSql.AppendLine(",RoadCount=isnull(RoadCount,0)-@ProductNum");//减少在途量
        ...
        private static SqlCommand updateRoadCount(string batchno,string ProductID, string DefaultStorageID, string ProductNum, StorageInProcessModel model,bool isadd)
        {
            StringBuilder strSql = new StringBuilder();
            if (isadd)
            {
                ...
                //else
                //{
                //    strSql.AppendLine(" and batchno='" + batchno + "' ");
                //}
            }
            else
            {
        ...
        #region 判断单据是否可反确认
        public static bool CheckIsUnfirm(StorageInProcessModel model)
        {
            string sql = "select count(1) from officedba.StorageInRed where companycd='" + model.CompanyCD + "' and fromtype=2 and frombillid=" + model.ID;
        ...
        public static bool ISConfirmBill(StorageInProcessModel model)
        {
            ...
            //sql.AppendLine(",ISNULL(l.usedunitcount,0)-ISNULL(l.InCount,0) as NotInCount ");
        ...
        private static void GetExtAttrCmd(StorageInProcessModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@InNo", model.InNo);
