...
namespace XBase.Data.Office.StorageManager
{
    public class StorageOutSellDBHelper
    {
        ...
        public static DataTable GetStorageOutSellTableBycondition(StorageOutSellModel model, string timeStart, string timeEnd, string SendNo,string EFIndex,string EFDesc, int pageIndex, int pageCount, string ord, ref int TotalCount,string time1,string time2,string prodid,string EFIndexP,string EFDescP)
        {
            ...
            sql.AppendLine("SELECT DISTINCT a.ID ,p.productname,de.unitprice,de.productcount,de.totalprice totalprice1,isnull(dbo.getStorageName(de.storageid),'')storagename        ");
            ...
            sql.AppendLine("FROM officedba.StorageOutSell a left join officedba.StorageOutSelldetail de on a.companycd=de.companycd and a.OutNo=de.OutNo left join officedba.productinfo p on p.id=de.productid   ");
            ...
            sql.AppendLine(" (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");
            sql.AppendLine("or (a.Sender IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Transactor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (time1 != "" && time2 != "")
            {
                sql.AppendLine(" and a.CreateDate between '" + time1 + " 0:00:00' and '" + time2 + " 23:59:59' ");
            }
            else if (time1 == "" && time2 != "")
            {
                sql.AppendLine(" and a.CreateDate<='" + time2 + " 23:59:59' ");
            }
            else if (time1 != "" && time2 == "")
            {
                sql.AppendLine(" and a.CreateDate>='" + time1 + " 0:00:00' ");
            }
            if (prodid != "")
            {
                sql.AppendLine(" and p.productname like'%" + prodid + "%'");
            }
        ...
        public static DataTable GetStorageOutSellTableBycondition(StorageOutSellModel model,string IndexValue,string TxtValue, string timeStart, string timeEnd, string SendNo,string BatchNo, string orderby,string time1,string time2,string prodid,string EFIndexP,string EFDescP)
        {
        ...
        public static DataTable GetStorageOutSellDetailInfo(StorageOutSellModel model)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT DISTINCT	 isnull(b.Custom1,'')Custom1,isnull(b.Custom2,'')Custom2,isnull(b.Custom3,'')Custom3,isnull(b.Custom4,'')Custom4,isnull(b.Custom5,'')Custom5,isnull(b.Custom6,'')Custom6,isnull(b.Custom7,'')Custom7,isnull(b.Custom8,'')Custom8,isnull(b.Custom9,'')Custom9,isnull(b.Custom10,'')Custom10,																																																			  	");
            ...
            sql.AppendLine(",a.CompanyCD,c.IsBatchNo,isnull(b.package,'')packages,b.UnitID as HiddenUnitID,b.BatchNo,b.UsedUnitID,b.UsedUnitCount,b.UsedPrice,b.ExRate,ii.CodeName as UsedUnitName                                                                                                              ");
            ...
            sql.AppendLine(",a.Title                                                                                                                  ");
            sql.AppendLine(",ISNULL(y.CustName,'') as CustName  ,k.CustID                                                                                                  ");
            ...
            sql.AppendLine(",c.ProductName AS PProductName,  CASE WHEN CP.ProdAlias IS NULL THEN c.ProductName ELSE CP.ProdAlias END AS ProductName                                                                                                           ");
            ...
            sql.AppendLine(" left join          officedba.CustProdDetails AS CP ON CP.ProdNo = c.ProdNo AND CP.CustID = k.CustID ");
        ...
        #region 获取：销售出库单信息及其详细信息
        /// <summary>
        /// 获取销售出库详细信息
        /// </summary>
        /// <returns>DataTable</returns>
        public static DataTable GetStorageOutSellDetailInfo1(StorageOutSellModel model)
        {
            ...
            sql.AppendLine(",c.ProdNo as ProductNo                                                                                                   ");
            sql.AppendLine(",c.ProductName AS PProductName,  CASE WHEN CP.ProdAlias IS NULL THEN c.ProductName ELSE CP.ProdAlias END AS ProductName                                                                                                           ");
            sql.AppendLine(",c.Specification,c.CASRegi,c.EngName                                                                                                       ");
            sql.AppendLine(",cbt.TypeName as ColorName                                                                                                        ");
            sql.AppendLine(",ISNULL(c.MinusIs,0) as MinusIs");
            sql.AppendLine(",q.CodeName as UnitID                                                                                                     ");
            sql.AppendLine(",b.UnitPrice as UnitPrice                                                                                                 ");
            sql.AppendLine(",l.TaxPrice as TaxPrice                                                                                                 ");
            sql.AppendLine(",l.TaxRate as TaxRate                                                                                                 ");
            sql.AppendLine(",l.TotalFee as TotalFee                                                                                                 ");
            sql.AppendLine(",l.TotalTax as TotalTax                                                                                                 ");
            sql.AppendLine(",b.StorageID                                                                                                              ");
            sql.AppendLine(",ISNULL(l.ProductCount,0)-ISNULL(l.BackCount,0) as FromBillCount                                                                                          ");
            sql.AppendLine(",ISNULL(l.OutCount,0) as OutCount");
            sql.AppendLine(",ISNULL(l.ProductCount,0)-ISNULL(l.BackCount,0)-ISNULL(l.OutCount,0) as NotOutCount");
            ...
            sql.AppendLine(",ISNULL(b.Remark,'') as DetaiRemark                                                                                       ");
            sql.AppendLine(" ,ISNULL(s.ProductCount,0)+ISNULL(s.RoadCount,0)+ISNULL(s.InCount,0)-ISNULL(s.OrderCount,0)-ISNULL(s.OutCount,0) as UseCount ");
            sql.AppendLine(" ,case sl.fromtype when '2' then zz.contractno else '' end as contractno ");
            sql.AppendLine("FROM officedba.StorageOutSell a                                                                                           ");
            sql.AppendLine("left join officedba.StorageOutSellDetail b                                                                                ");
            sql.AppendLine("on a.OutNo=b.OutNo and a.CompanyCD=b.CompanyCD                                                                 ");
            sql.AppendLine("left join officedba.ProductInfo c on c.ID=b.ProductID                                                                     ");
            sql.AppendLine("left join officedba.CodePublicType cbt on c.ColorID=cbt.ID                                                              ");
            sql.AppendLine("left join officedba.EmployeeInfo f on a.Transactor=f.ID                                                                   ");
            sql.AppendLine("left join officedba.EmployeeInfo g on a.Creator=g.ID     																												          ");
            sql.AppendLine("left join officedba.EmployeeInfo h on a.Confirmor=h.ID                                                                    ");
            sql.AppendLine("left join officedba.EmployeeInfo i on a.Closer=i.ID                                                                       ");
            ...
            sql.AppendLine("left join officedba.CodeUnitType q on q.ID=b.UnitID                                                                       ");
            sql.AppendLine("left join officedba.SellSend k on k.ID=a.FromBillID                                                                       ");
            sql.AppendLine(" left join officedba.[SellOrder] sl on sl.id=k.frombillid  ");
            sql.AppendLine("left join officedba.CodeUnitType ii on ii.ID=b.UsedUnitid                                                                       ");
            sql.AppendLine("left join officedba.SellSendDetail l on l.SendNo=k.SendNo and l.SortNo=b.FromLineNo and l.CompanyCD=k.CompanyCD                                     ");
            sql.AppendLine("left join (select w.UserID,x.EmployeeName from officedba.UserInfo w ,officedba.EmployeeInfo x where w.EmployeeID =x.ID) m ");
            sql.AppendLine("on a.ModifiedUserID=m.UserID                                                                                              ");
            ...
            sql.AppendLine(" left join          officedba.CustProdDetails AS CP ON CP.ProdNo = c.ProdNo AND CP.CustID = k.CustID ");
            sql.AppendLine("left join officedba.StorageProduct s on s.CompanyCD=a.CompanyCD and s.StorageID=b.StorageID and b.ProductID=s.ProductID AND b.BatchNo=s.BatchNo ");
            sql.AppendLine(" left join (select b.id outid,b.outno,e.contractno from floor_erp_1.officedba.storageoutselldetail a ");
            sql.AppendLine("   left join floor_erp_1.officedba.storageoutsell b on a.outno=b.outno left join floor_erp_1.officedba.sellsend c on  c.id=b.frombillid");
            sql.AppendLine("  left join floor_erp_1.officedba.[SellOrder] d on d.id=c.frombillid left join [floor_erp_1].[officedba].[Sellcontract] e on e.id=d.frombillid");
            sql.AppendLine("  where d.fromtype='2' and b.fromtype='1' and c.fromtype='1' ) zz on zz.outid=a.id");
            sql.AppendLine("   where a.CompanyCD='" + model.CompanyCD + "' and a.id=" + model.ID);
            sql.AppendLine(" order by b.SortNo asc  ");
            return SqlHelper.ExecuteSql(sql.ToString());
        }
        #endregion
        ...
        /// <returns></returns>
        public static bool InsertStorageOutSell(StorageOutSellModel model, List<StorageOutSellDetailModel> modelList, Hashtable htExtAttr, out int IndexIDentity, string[] strCustomdetail)
        {
        ...
        private static void GetExtAttrCmd(StorageOutSellModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@OutNo", model.OutNo);
        ...
        /// <param name="modelList"></param>
        /// <returns></returns>
        public static bool UpdateStorageOutSell(StorageOutSellModel model, Hashtable htExtAttr, List<StorageOutSellDetailModel> modelList, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public static SqlCommand updateStorageProduct(string BatchNo,string ProductID, string StorageID, string ProductNum, StorageOutSellModel model, bool flag)
        {
            ...
            else
            {
                ...
                if (BatchNo != "")
                    ...
                    strSql.AppendLine(" and BatchNo is null ;");
                strSql.AppendLine("update officedba.StorageProduct set ");
                strSql.AppendLine("ordercount=isnull(ordercount,0)-@ProductNum  ");
                strSql.AppendLine(" where StorageID=@StorageID and ProductID=@ProductID and CompanyCD=@CompanyCD and BatchNo is null ");
            }
        ...
        #region 更新分仓存量数据
        public static SqlCommand updateStorageProduct1(string BatchNo, string ProductID, string StorageID, string ProductNum, StorageOutSellModel model, bool flag)
        {
            //true 表示入库增加分仓存量数据
            StringBuilder strSql = new StringBuilder();
            if (flag == true)
        ...
        #region 取消确认
        public static bool unconfirm(StorageOutSellModel model)
        {
                ...
                if (modelList != null && modelList.Count > 0)//明细不为空的时候
                {
                    ...
                    for (int i = 0; i < modelList.Count; i++)
                    {
                        ...
                        SqlCommand commPD = updateStorageProduct1(modelList[i].BatchNo, modelList[i].ProductID, modelList[i].StorageID, modelList[i].ProductCount, model, false);
                        lstConfirm.Add(commPD);
                ...
                comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=7 and billno='" + dt.Rows[0]["OutNo"].ToString() + "'";
        ...
        #region 判断单据是否可反确认
        public static bool CheckIsUnfirm(StorageOutSellModel model)
        {
            string sql = "select count(1) from officedba.StorageOutRed where companycd='"+model.CompanyCD+"' and fromtype=1 and frombillid="+model.ID;
        ...
        /// <param name="CompanyCD"></param>
        /// <returns></returns>
        public static DataTable GetSSDetailInfo(string CompanyCD, string SendNo, string Title,string CustID)
        {
            StringBuilder sql = new StringBuilder();
            ...
            sql.AppendLine(",ISNULL(i.Specification,'') as Specification                                                              ");
            sql.AppendLine(",ISNULL(b.Custom1,'') as Custom1, ISNULL(b.Custom2,'') as Custom2                                          ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("select b.ID,isnull([dbo].[getCodeublic](a.package),'')packages,a.SendNo,a.UsedUnitID,a.UsedUnitCount,a.UsedPrice,a.ExRate,a.UnitID as HiddenUnitID,p.IsBatchNo ");
            ...
            sql.AppendLine(",isnull(p.ExtField1,'') as ExtField1,isnull(p.ExtField2,'') as ExtField2,isnull(p.ExtField3,'') as ExtField3 ");
            sql.AppendLine(",isnull(p.ExtField4,'') as ExtField4,isnull(p.ExtField5,'') as ExtField5,isnull(p.ExtField6,'') as ExtField6 ");
            sql.AppendLine(",isnull(p.ExtField7,'') as ExtField7,isnull(p.ExtField8,'') as ExtField8,isnull(p.ExtField9,'') as ExtField9, isnull(p.ExtField10,'') as ExtField10 ");
            sql.AppendLine(",q.CodeName as UnitID                                                                             ");
            sql.AppendLine(",ISNULL(p.Specification,'') as Specification,a.StorageID                             ");
        ...
        public static bool ISConfirmBill(StorageOutSellModel model)
        {
           ...
           // liuch 20110703 add  left join 需要针对每个表进行 companycd 限制
            sql.AppendLine(" where a.CompanyCD='" + model.CompanyCD + "' and b.CompanyCD='" + model.CompanyCD + "' and l.CompanyCD='" + model.CompanyCD + "' and k.CompanyCD='" + model.CompanyCD + "'and a.ID=" + model.ID);
        ...
        public static string ISBigUseCountWhenCant(StorageOutSellModel model)
        {
            ...
            if (dtbatch.Rows.Count > 0)
            {
                for (int i = 0; i < dtbatch.Rows.Count; i++)
                {
                    ...
                    //sql.AppendLine(",Convert(numeric(22," + point + "),isnull(c.productcount,0)+isnull(roadcount,0)+isnull(incount,0)+isnull(a.ProductCount,0)-isnull(ordercount,0)-isnull(outcount,0)) as UseCount ");
                    
                    sql.AppendLine(",Convert(numeric(22," + point + "),isnull(c.productcount,0)+isnull(roadcount,0)+isnull(incount,0)) as UseCount ");
        ...
        #region 确认的时候判断,当物品允许负库存，是否大于可用库存（090807判断现有库存）
        /// <summary>
