...
namespace XBase.Data.Office.StorageManager
{
    public class PurchaseArriveInfoDBHelper
    {
        ...
        public static DataTable GetPurchaseArriveInfo(string CompanyCD, string ArriveNo, string Title)
        {
            ...
            sql.AppendLine("round((ISNULL(b.UsedUnitCount,0)-ISNULL(b.RejectCount,0)-ISNULL(b.BackCount,0)),3) as ProductCount");
            sql.AppendLine(",round(ISNULL(b.InCount,0),3) as InCount");
            sql.AppendLine(",round(b.UnitPrice,3) as UnitPrice,b.TotalPrice,b.Remark,b.FromBillID");
        ...
        {
            string sql = "select ProductID,ProductNo,ProductName,UnitID,round (ISNULL(ProductCount,0)-ISNULL(RejectCount,0)-ISNULL(BackCount,0),3)  as ProductCount,InCount,UnitPrice,TotalPrice,Remark,FromBillID,FromType,FromLineNo from officedba.PurchaseArriveDetail where CompanyCD='" + CompanyCD + "' and ArriveNo=" + ArriveNo;
            return SqlHelper.ExecuteSql(sql);
        }
        /// <summary>
        /// 根据传过来的明细ID数组来获取明细列表
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("select a.ProductCount as counts,a.ID,a.ArriveNo,a.SortNo as FromLineNo,a.ProductID,a.UsedUnitID,isnull(z.TypeName,'') ColorName ");
            sql.AppendLine(",isnull(a.Custom1,'') as Custom1,isnull(a.Custom2,'') as Custom2,isnull(a.Custom3,'') as Custom3,isnull(a.Custom4,'') as Custom4 ");
            sql.AppendLine(",isnull(a.Custom5,'') as Custom5,isnull(a.Custom6,'') as Custom6,isnull(a.Custom7,'') as Custom7,isnull(a.Custom8,'') as Custom8 ");
            sql.AppendLine(",isnull(a.Custom9,'') as Custom9,isnull(a.Custom10,'') as Custom10 ");
            sql.AppendLine(",ISNULL(p.ProdNo,'') as ProductNo");
            sql.AppendLine(",isnull(p.ExtField1,'') as ExtField1,isnull(p.ExtField2,'') as ExtField2,isnull(p.ExtField3,'') as ExtField3,isnull(p.ExtField4,'') as ExtField4 ");
            sql.AppendLine(",isnull(p.ExtField5,'') as ExtField5,isnull(p.ExtField6,'') as ExtField6,isnull(p.ExtField7,'') as ExtField7,isnull(p.ExtField8,'') as ExtField8 ");
            sql.AppendLine(",isnull(p.ExtField9,'') as ExtField9,isnull(p.ExtField10,'') as ExtField10 ");
            ...
            sql.AppendLine(",ISNULL(p.Specification,'') as Specification,ISNULL(a.StorageID,0) as StorageID,p.IsBatchNo");
            //edit 20110504 lcb StorageID用采购到货单明细中添加的仓库，不用物品的主放仓库
            sql.AppendLine(",(ISNULL(a.UsedUnitCount,0)-ISNULL(a.RejectCount,0)-ISNULL(a.BackCount,0)) as FromBillCount");
            sql.AppendLine(",ISNULL(a.InCount,0) as InCount");
            sql.AppendLine(",(ISNULL(a.UsedUnitCount,0)-ISNULL(a.RejectCount,0)-ISNULL(a.BackCount,0)-ISNULL(a.InCount,0)) as ProductCount");
            sql.AppendLine(",a.InCount,a.UsedPrice as UnitPrice,a.Remark,a.FromType");
            ...
            sql.AppendLine(" left join officedba.CodePublicType z on z.ID=p.ColorID");
            //sql.AppendLine(" left join (select baseunitid,unitid,exrate from officedba.UnitGroup un left join officedba.UnitGroupDetail dun on un.groupunitno=dun.groupunitno where un.companycd='" + CompanyCD + "' and dun.companycd='" + CompanyCD + "')ba on ba.baseunitid=p.unitid and a.UsedUnitID=ba.unitid");
