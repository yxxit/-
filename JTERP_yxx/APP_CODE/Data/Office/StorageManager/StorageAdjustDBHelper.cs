...
namespace XBase.Data.Office.StorageManager
{
    public class StorageAdjustDBHelper
    {
        ...
        //检查库存
        public static bool checkcount(string id,bool type,ref string msg)//true 确认，false 取消确认
        {
            string sql = @"select p.productname,isnull(s.productcount,0)-b.adjustcount num from officedba.StorageAdjust a left join officedba.StorageAdjustdetail b on a.adjustno=b.adjustno and a.companycd=b.companycd 
left join officedba.StorageProduct s on a.storageid=s.storageid and a.companycd=s.companycd and  b.productid=s.productid and isnull(b.batchno,'')=isnull(s.batchno,'')  left join officedba.productinfo p on p.id=b.productid where a.id=" + id + " ";
            if (type)
                sql += " and b.adjusttype=0 ";
            else
                sql += " and b.adjusttype=1 ";
        ...
        public static bool ConfirmBill(string id)
        {
            ArrayList sqllist = new ArrayList();
            ...
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@ID", id));
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@Confirmor", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID));
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@ConfirmDate", DateTime.Now.ToString("yyyy-MM-dd")));
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@ModifiedUserID", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).UserID));
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@ModifiedDate", DateTime.Now.ToString("yyyy-MM-dd")));
            sqlcomm.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
            sqllist.Add(sqlcomm);
            string get_data = "select a.adjustno,a.storageid,b.productid,b.adjusttype,isnull(b.adjustcount,0)adjustcount,isnull(b.costprice,0)costprice,isnull(b.batchno,'')batchno from officedba.StorageAdjust a left join officedba.StorageAdjustdetail b on a.adjustno=b.adjustno and a.companycd=b.companycd where a.id=" + id;
            DataTable dt = SqlHelper.ExecuteSql(get_data);
            
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                string upsql = "";
                if (dt.Rows[i]["adjusttype"].ToString() == "1")//增加
                {
                ...
                if (dt.Rows[i]["batchno"].ToString() != "")
                    upsql += " and BatchNo='" + dt.Rows[i]["batchno"].ToString() + "' "; 
                else
                    upsql += " and BatchNo is null ";

                SqlCommand upcomm = new SqlCommand();
                upcomm.CommandText = upsql;
                upcomm.Parameters.Add(SqlHelper.GetParameter("@ProductCount", dt.Rows[i]["adjustcount"].ToString()));
                upcomm.Parameters.Add(SqlHelper.GetParameter("@ProductID", dt.Rows[i]["productid"].ToString()));
                upcomm.Parameters.Add(SqlHelper.GetParameter("@StorageID", dt.Rows[i]["storageid"].ToString()));
                upcomm.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
                sqllist.Add(upcomm);
                #region 操作库存流水账                
                StorageAccountModel AccountM_ = new StorageAccountModel();
                AccountM_.BatchNo = dt.Rows[i]["batchno"].ToString();
                AccountM_.BillNo = dt.Rows[i]["adjustno"].ToString();
                AccountM_.BillType = 16;
                AccountM_.CompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                AccountM_.Creator = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID;
                AccountM_.HappenCount = Convert.ToDecimal(dt.Rows[i]["adjustcount"].ToString());
                AccountM_.ProductCount = Convert.ToDecimal(dt.Rows[i]["adjustcount"].ToString());
                AccountM_.HappenDate = System.DateTime.Now;
                AccountM_.PageUrl = "../Office/StorageManager/StorageAdjustAdd.aspx";
                AccountM_.Price = Convert.ToDecimal(dt.Rows[i]["costprice"].ToString());
                ...
                if (dt.Rows[i]["adjusttype"].ToString() == "1")
        ...
        /// <returns></returns>
        public static bool UnConfirmBill(string id)
        {
            ArrayList sqllist = new ArrayList();
            SqlCommand comm = new SqlCommand();
            ...
            if (BillStatus == "2")
            {
                ...
                #region 删除流水账 shjp add 2011-12-15
                SqlCommand del = new SqlCommand();
                del.CommandText = "delete  officedba.StorageAccount where companycd='" + ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD + "' and billtype=16 and billno='" + dt.Rows[0]["adjustno"].ToString() + "'";
                sqllist.Add(del);
                #endregion
                #region 撤消审批流程
                DataTable dtFlowInstance = Common.FlowDBHelper.GetFlowInstanceInfo(((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD, BillTypeFlag, BillTypeCode, int.Parse(id));
                if (dtFlowInstance.Rows.Count > 0)
                {
                    ...
                    commHis.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
                    ...
                    commHis.Parameters.Add(SqlHelper.GetParameter("@BillID",int.Parse(id)));
                    commHis.Parameters.Add(SqlHelper.GetParameter("@ModifiedUserID", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).UserID));
                    sqllist.Add(commHis);
                    #endregion
                    ...
                    commTask.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
                    commTask.Parameters.Add(SqlHelper.GetParameter("@tempFlowInstanceID", FlowInstanceID));
                    commTask.Parameters.Add(SqlHelper.GetParameter("@ModifiedUserID", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).UserID));
                    ...
                    commIns.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
                    ...
                    commIns.Parameters.Add(SqlHelper.GetParameter("@BillID", int.Parse(id)));
                    commIns.Parameters.Add(SqlHelper.GetParameter("@ModifiedUserID", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).UserID));
                    sqllist.Add(commIns);
                    #endregion
        ...
        public static DataTable GetAllAdjust(StorageAdjustModel model,string EFIndex,string EFDesc, string BetinTime, string EndTime, string FlowStatus, ref int TotalCount,string productname)
        {
            string sql = "";
            int BillTypeFlag = int.Parse(ConstUtil.CODING_RULE_Storage_NO);
            int BillTypeCode = int.Parse(ConstUtil.CODING_RULE_StoAdjust_NO);
            sql += " select * from (";
            sql += " SELECT DISTINCT s.[ID],p.ProductName,de.AdjustCount,de.CostPrice,de.CostPriceTotal,s.ModifiedDate                              ";
            sql += "       ,s.[AdjustNo]                                       ";
            ...
            sql += "   FROM [officedba].[StorageAdjust] as s left join officedba.StorageAdjustdetail de on de.companycd=s.companycd and de.adjustno=s.adjustno left join officedba.productinfo p on p.id=de.productid  left join officedba.StorageInfo as st on s.StorageID=st.ID                                                                ";
            ...
            sql += " (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=f.FlowNo  AND BillID=S.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ";
            sql += "or (S.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))";
            sql += "or (S.Executor IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))";
            ...
            if (model.Creator == -100)
            {
                dt1 = SqlHelper.ExecuteSearch(comm);
            }
            else
            {
                dt1 = SqlHelper.PagerWithCommand(comm, model.Creator, model.Confirmor, model.Attachment, ref TotalCount);
            }
            return dt1;
        }
        ...
        private static void GetExtAttrCmd(StorageAdjustModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@AdjustNo", model.AdjustNo);
