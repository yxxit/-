...
namespace XBase.Data.Office.StorageManager
{
    public class StorageInOtherDBHelper
    {
        ...
        public static DataTable GetStorageInOtherTableBycondition(string BatchNo,StorageInOtherModel model, string StorageID, string timeStart, string timeEnd, int pageIndex, int pageCount, string ord, ref int TotalCount,string time1,string time2,string prodid)
        {
            ...
            sql.AppendLine("FROM officedba.StorageInOther a   left join officedba.StorageInOtherdetail de on a.companycd=de.companycd and a.inno=de.inno left join officedba.productinfo p on p.id=de.productid        ");
        ...
        public static DataTable GetStorageInOtherTableBycondition(string BatchNo,StorageInOtherModel model, string StorageID, string timeStart, string timeEnd, string orderby,string time1,string time2,string prodid)
        {
            //入库单编号、入库单主题、交货人、验收人、人库人、入库时间、入库原因、入库数量、入库金额、摘要、单据状态。
            string CanUser = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID.ToString();
            StringBuilder sql = new StringBuilder();
        ...
        public static DataTable GetStorageInOtherDetailInfo(StorageInOtherModel model)
        {
            ...
            sql.AppendLine(@"   isnull(b.Custom1,'')Custom1,isnull(b.Custom2,'')Custom2,isnull(b.Custom3,'')Custom3,isnull(b.Custom4,'')Custom4,isnull(b.Custom5,'')Custom5,isnull(b.Custom6,'')Custom6,isnull(b.Custom7,'')Custom7,isnull(b.Custom8,'')Custom8,isnull(b.Custom9,'')Custom9,isnull(b.Custom10,'')Custom10,
                                isnull(b.Pnumber,'')Pnumber,isnull(b.Pnumberid,'')Pnumberid,isnull(b.AbrasionResist,'')AbrasionResist,isnull(b.AbrasionResistid,'')AbrasionResistid,
                                isnull(b.BalancePaper,'')BalancePaper,isnull(b.BalancePaperid,'')BalancePaperid,isnull(b.BaseMaterial,'')BaseMaterial,isnull(b.BaseMaterialid,'')BaseMaterialid,
        ...
        /// <returns></returns>
        public static bool InsertStorageInOther(StorageInOtherModel model, List<StorageInOtherDetailModel> modelList, Hashtable htExtAttr, out int IndexIDentity, string[] strCustomdetail)
        {
            ...
            if (modelList != null && modelList.Count > 0)//明细为空的时候
            {
                for (int i = 0; i < modelList.Count; i++)
                {
                    ...
                    {
                        strSqlDetail.Append(@",Pnumberid,Pnumber,AbrasionResistid,AbrasionResist,BalancePaperid,BalancePaper,BaseMaterialid,BaseMaterial,SurfaceTreatment");
                    }
        ...
        /// <param name="modelList"></param>
        /// <returns></returns>
        public static bool UpdateStorageInOther(StorageInOtherModel model, List<StorageInOtherDetailModel> modelList, Hashtable htExtAttr, string[] strCustomdetail)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        #endregion
        #region 取消确认
        public static bool Unconfirm(StorageInOtherModel model)
        {
            ArrayList lstConfirm = new ArrayList();
            StringBuilder sql = new StringBuilder();
            //修改单据状态
            ...
            comm1.CommandText = "delete  officedba.StorageAccount where companycd='" + model.CompanyCD + "' and billtype=5 and billno='" + dt.Rows[0]["InNo"].ToString() +"'";
        ...
        #region 判断单据是否可反确认
        public static bool CheckIsUnfirm(StorageInOtherModel model)
        {
            string sql = "select count(1) from officedba.StorageInRed where companycd='" + model.CompanyCD + "' and fromtype=3 and frombillid=" + model.ID;
        ...
        private static void EditInOtherDetailInfo(SqlCommand comm, StorageInOtherDetailModel model)
        {
            ...
            if (((UserInfoUtil)SessionUtil.Session["UserInfo"]).Version == "floor")
            {
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@Pnumberid ", model.Pnumberid));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@Pnumber ", model.Pnumber));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@AbrasionResist ", model.AbrasionResist));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@AbrasionResistid ", model.AbrasionResistid));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@BalancePaper ", model.BalancePaper));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@BalancePaperid ", model.BalancePaperid));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@BaseMaterial ", model.BaseMaterial));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@BaseMaterialid ", model.BaseMaterialid));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@SurfaceTreatment ", model.SurfaceTreatment));
            }
        ...
        public static DataTable GetStorageInOtherDetailInfo(string ID, string CompanyCD)
        {
            ...
            string sql = "";
            sql = "select * from officedba.V_StorageInOtherDetail where ID=" + ID + " and CompanyCD='" + CompanyCD + "'";
            if (UserInfo.Version == "floor")
            {
                sql = "select * from officedba.V_StorageInOtherDetail1 where ID=" + ID + " and CompanyCD='" + CompanyCD + "'";
            }
            return SqlHelper.ExecuteSql(sql);
        }
