    ...
    public class CheckReportDBHelper
    {
        ...
        public static DataTable GetCheckApplay(string CompanyCD, string method, string ReportStr)
        {
            ...
            if (dtt != null && dtt.Rows.Count > 0)
            {
                ...
                {
                    sql.AppendLine(" (q1.Creator IN  ");
                    sql.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sql.AppendLine("  WHERE DeptID IN (SELECT x.ID  ");
                    sql.AppendLine(" FROM officedba.DeptInfo x,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") y  ");
                    sql.AppendLine("  WHERE x.ID=y.ID) AND DeptID<>" + userInfo.DeptID.ToString() + " )) OR ");
                }
                if (dtt.Rows[0]["RoleRange"].ToString() == "2")
                {
                    ...
                    sql.AppendLine("  WHERE DeptID IN (SELECT x.ID  ");
                    sql.AppendLine(" FROM officedba.DeptInfo x,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") y  ");
                    sql.AppendLine("  WHERE x.ID=y.ID) ))  OR ");
                }
                if (dtt.Rows[0]["RoleRange"].ToString() == "3")
                {
                    ...
                    sql.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sql.AppendLine("  WHERE DeptID IN (SELECT x.ID  ");
                    sql.AppendLine(" FROM officedba.DeptInfo x,DepartInfo_Children(" + userInfo.BranchID.ToString() + ") y  ");
                    sql.AppendLine("  WHERE x.ID=y.ID)))  OR ");
                }
            }
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = q1.ID AND BillTypeFlag = " + BillTypeFlag + " AND BillTypeCode = " + BillTypeCode + ") AND BillID = q1.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (q1.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (q1.Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  OR (q1.Principal IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) )");//Principal
        ...
        public static DataTable GetMan(string CompanyCD, string method, string ReportStr)
        {
            ...
            if (method != "2")
            {
                sql.AppendLine("   and  md.ID in (select md2.ID from officedba.ManufactureTaskDetail as md2 left join officedba.ManufactureTask as m2 on m2.TaskNo=md2.TaskNo where m2.CompanyCD=@CompanyCD  ) ");
            }
        ...
        public static DataTable GetReportInfo(string CompanyCD, string method, string ReportStr)
        {
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance WHERE BillID = q.ID AND BillTypeFlag = " + BillTypeFlag + " AND BillTypeCode = " + BillTypeCode + ") AND BillID = q.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (" + userInfo.EmployeeID + " IN(select Checker from officedba.CheckReportDetail where ReportNo=q.ReportNo and q.CompanyCD=@CompanyCD)) or (q.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (q.ApplyUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))  OR (q.Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))   OR (q.Principal IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) )");//Principal
