...
namespace XBase.Data.Office.StorageManager
{
    ...
    public class StorageSearchDBHelper
    {
        ...
        public static DataTable GetProductStorageTableBycondition(StorageProductModel model, XBase.Model.Office.SupplyChain.ProductInfoModel pdtModel, string ProductCount1, string EFIndex, string EFDesc, int pageIndex, int pageCount, string ord, string BatchNo, ref int TotalCount,string isbatch)
        {
            ...
            if (!string.IsNullOrEmpty(EFIndex) && !string.IsNullOrEmpty(EFDesc))
            {
                ...
                sql.AppendLine("         ,d.Remark ,d.Size,d.BarCode,d.FromAddr,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardCost,0)) as StandardCost,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardSell,0)) as StandardSell");
                sql.AppendLine("         ,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.SellTax,0)) as SellTax,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.SellPrice,0)) as SellPrice,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardBuy,0)) as StandardBuy,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.TaxBuy,0)) as TaxBuy                                         ");

            }
            else
            {
                sql.AppendLine("SELECT a.ID  ,d.Remark ,d.Size,d.BarCode,d.FromAddr,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardCost,0)) as StandardCost,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardSell,0)) as StandardSell");
                sql.AppendLine("         ,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.SellTax,0)) as SellTax,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.SellPrice,0)) as SellPrice,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.StandardBuy,0)) as StandardBuy,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(d.TaxBuy,0)) as TaxBuy                                         ");
            }
            ...
            sql.AppendLine("			,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(a.ProductCount,0)+ISNULL(a.RoadCount,0)+ISNULL(a.InCount,0)-ISNULL(a.OrderCount,0)-ISNULL(a.OutCount,0)) as UseCount      ");
            sql.AppendLine("			,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(a.OrderCount,0)) as OrderCount      ");
            sql.AppendLine("			,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(a.RoadCount,0)) as RoadCount      ");
            sql.AppendLine("			,Convert(numeric(22," + userInfo.SelPoint + "),ISNULL(a.OutCount,0)) as OutCount,a.ProductID,g.TypeName as ColorName, g1.TypeName as  MaterialName  ");
            ...
            sql.AppendLine("left join officedba.CodeUnitType e on e.ID=d.UnitID	  left outer join officedba.CodePublicType g on d.ColorID=g.ID 	left outer join officedba.CodePublicType g1 on d.Material=g1.ID ");
            ...
            if (!string.IsNullOrEmpty(model.Prostatus))
            {
                if (model.Prostatus == "1")
                    sql.AppendLine(" and d.usedstatus=1");
                else if (model.Prostatus == "2")
                    sql.AppendLine(" and d.usedstatus=0");
            }
            if (!string.IsNullOrEmpty(model.Storagestatus))
            {
                if (model.Storagestatus == "0")
                    sql.AppendLine(" and isnull(a.ProductCount,0)>0");
            }
            ...
            string ResulSQL = "select a.*,Convert(numeric(22," + userInfo.SelPoint + "),ProductCount/ExRate) as StoreCount ,Convert(numeric(22," + userInfo.SelPoint + "),ProductCount/ExRate*SellPrice) TotalPrice from ( select a.*,isnull(h.ExRate,1) as ExRate,isnull(j.CodeName,'') as CodeName from (" + sql.ToString() + ") a left outer join officedba.ProductInfo g on a.ProductID=g.ID left outer join Officedba.UnitGroup p on (g.GroupUnitNo=p.GroupUnitNo and p.CompanyCD='" + model.CompanyCD + "') LEFT OUTER JOIN Officedba.UnitGroupDetail h  ON (p.GroupUnitNo=h.GroupUnitNo and h.CompanyCD='" + model.CompanyCD + "' and g.StockUnitID=h.UnitID)  left outer  join officedba.CodeUnitType j on h.UnitID=j.ID  ) a ";
            string strsql = "";
            if (isbatch == "1")
            {
                strsql = @"select ROW_NUMBER() OVER (ORDER BY ProductName) ID,[Size],BarCode,FromAddr,avg(StandardCost)StandardCost,avg(StandardSell)StandardSell,avg(SellTax)SellTax,avg(SellPrice)SellPrice,avg(StandardBuy)StandardBuy,avg(TaxBuy)TaxBuy,StorageNo,StorageName,ProductNo,ProductName,Specification,unitID,DeptName,
sum(productcount)ProductCount,sum(UseCount)UseCount,sum(OrderCount)OrderCount,sum(RoadCount)RoadCount,sum(OutCount)OutCount,ProductID,ColorName,MaterialName,avg(ExRate)ExRate,CodeName,sum(StoreCount)StoreCount,sum(TotalPrice)TotalPrice from (" + ResulSQL + @")  s
group by [Size],BarCode,FromAddr,StorageNo,StorageName,ProductNo,ProductName,Specification,unitID,DeptName,ProductID,ColorName,MaterialName,CodeName";
            }
            else
            {
                strsql=ResulSQL;
        ...
        //获取产品配置
        public static DataTable getfloorbatch(string proid,string batchno,string CompanyCD)
        {
            string sql = @"select b.id ID,isnull(b.Pnumber,'')Pnumber,isnull(b.AbrasionResist,'')AbrasionResist,isnull(b.BalancePaper,'')BalancePaper,isnull(b.BaseMaterial,'')BaseMaterial,
isnull(b.SurfaceTreatment,'')SurfaceTreatment,isnull(b.BackBottomPlate,'')BackBottomPlate,isnull(b.BuckleType,'')BuckleType,isnull(dbo.getCodeublic(b.Pakeages),'')Pakeages from officedba.ManufactureTask a left join officedba.ManufactureTaskdetail b on a.taskno=b.taskno and a.companycd=b.companycd where a.companycd='" +CompanyCD+@"' and b.productid="+proid+@" and a.id=
(select top 1 a.frombillid from officedba.StorageInProcess a left join officedba.StorageInProcessdetail b on a. inno=b.inno and a.companycd=b.companycd where a.companycd='"+CompanyCD+@"' and b.productid="+proid+@" and batchno='"+batchno+@"')";
