...
namespace XBase.Data.Office.StorageManager
{
    public class StrongeJournalDBHelper
    {
        ...
        public static DataTable GetDetailStrongJournal(string queryStr, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            SqlStr.AppendLine(" isnull(a.ProductCount,0) as NowProductCount,a.Price,a.Creator,a.ReMark,d.EmployeeName as CreatorName,f.TypeName as ColorName  ");
        ...
        /// <param name="TotalCount"></param>
        /// <returns></returns>
        public static DataTable GetDetailStrongJournalNew(string queryStr, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            StringBuilder SqlStr = new StringBuilder();
            SqlStr.AppendLine(" SELECT  a.ProductID,a.StorageID,isnull(a.HappenCount,0) as ProductCount,CONVERT(VARCHAR(10),a.HappenDate,21) as EnterDate ,a.BillType as typeflag ,");
            SqlStr.AppendLine(" case a.BillType when '1' then '期初库存录入' when '2' then '期初库存批量导入' when '3' then '采购入库单' when '4' then '生产完工入库单' when '5' then '其他入库单' ");
            SqlStr.AppendLine(" when '6' then '红冲入库单' when '7' then '销售出库单' when '8' then '其他出库单' when '9' then '红冲出库单' when '10' then '借货申请单' when '11' then '借货返还单' ");
            SqlStr.AppendLine(" when '12' then '调拨出库' when '13' then '调拨入库' when '14' then '日常调整单' when '15' then '期末盘点单' when '16' then '库存报损单' when '17' then '领料单' ");
            SqlStr.AppendLine(" when '18' then '退料单' when '19' then '配送单（配送出库）' when '20' then '配送退货单（验收入库）' when '21' then '门店销售管理' when '22' then '门店销售退货' ");
            SqlStr.AppendLine(" when '23' then '拆装入库单' when '24' then '拆装出库单' else '采购入库单' end as typeflagtext, ");
            SqlStr.AppendLine(" a.BillNo,isnull(a.PageUrl,'') as PageUrl, ");
            SqlStr.AppendLine(" isnull(b.ProdNo,'') as ProdNo, ");
            SqlStr.AppendLine(" isnull(b.ProductName,'') as ProductName,isnull(b.Specification,'') as Specification, ");
            SqlStr.AppendLine(" c.StorageName,c.StorageNo,isnull(b.Size,'') as ProductSize,e.CodeName, ");
