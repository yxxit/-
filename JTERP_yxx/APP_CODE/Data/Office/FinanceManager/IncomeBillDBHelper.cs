...
namespace XBase.Data.Office.FinanceManager
{
    public class IncomeBillDBHelper
    {
        ...
        public static bool ConfirmIncomeBill(string ID, string Confirmor, string billingid, string price, string incomeno)
        {
            string CompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
            int userID = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID;
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("Update officedba.IncomeBill");
            strSql.AppendLine("set ConfirmStatus='1' ,Confirmor=@Confirmor,");
            strSql.AppendLine("ConfirmDate=getdate()");
            if (billingid != "0" && billingid != "")
            {
                strSql.AppendLine(",naccounts=0.00");
            }
            strSql.AppendLine(",AccountsStatus=2 where ID  In( " + ID + ");");
            SqlParameter[] parms = new SqlParameter[1];
            ...
            if (billingid != "0" && billingid != "")
            {
                ...
                strSql.Append(string.Format(vals, CompanyCD, billingid, dt.Rows[0]["SourceDt"].ToString(),
                                    ...
                                    price == "" ? "0.00" : price,
                                    dt.Rows[0]["NAccounts"].ToString() == "" ? "0.00" : (double.Parse(dt.Rows[0]["NAccounts"].ToString()) - double.Parse(price)).ToString(),
                                    DateTime.Now,
                ...
                if (float.Parse(dt.Rows[0]["NAccounts"].ToString()) - float.Parse(price) > 0)
                {
                    accountsStatus = "2";
                }
                else
                ...
                strSql.AppendLine("Update officedba.Billing set YAccounts=isnull(YAccounts,0)+" + (price).ToString() + ",NAccounts=" + (float.Parse(dt.Rows[0]["NAccounts"].ToString()) - float.Parse(price)).ToString() + ",AccountsStatus='" + accountsStatus + "' where ID=" + billingid + ";");
            }
        ...
        /// <param name="Confirmor">确认人</param>
        /// <returns>true成功,false失败</returns>
        public static bool ReConfirmIncomeBill(string ID, string billingid, string price)
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("Update officedba.IncomeBill");
            ...
            if (billingid != "0" && billingid != "")
            {
                sql.AppendLine("naccounts=totalprice,");
            }
            sql.AppendLine("AccountsStatus=0 where ID  In( " + ID + ") ");
            string[] str = price.Split(',');
            price = "";
            for (int i = 0; i < str.Length; i++)
            {
            ...
            if (billingid != "0" && billingid != "")
            {
                ...
                sql.Append("delete officedba.BlendingDetails where billingid=" + billingid + " and ShouFuKuanID=" + ID + " and  PayOrInComeType=2 ;");

                sql.AppendLine("Update officedba.Billing set YAccounts=isnull(YAccounts,0)-" + (price).ToString() + ",NAccounts=" + (float.Parse(dt.Rows[0]["NAccounts"].ToString()) + float.Parse(price)).ToString() + ",AccountsStatus='2' where ID=" + billingid + ";");
            }
        ...
        public static DataTable GetIncomeBillInfoByID(string ID)
        {
            ...
            sql.AppendLine("as ExecutorName,a.ProjectID,t.ProjectName,isnull(a.PaymentTypes,0)PaymentTypes ");
        ...
        public static bool InsertIncomeBill(IncomeBillModel model, out int ID)
        {
            ...
            sql.AppendLine("AccountNo,ModifiedDate,ModifiedUserID,Summary,BlendingType,CurrencyType,CurrencyRate,CustID,FromTBName,FileName,ProjectID,PaymentTypes)");
            sql.AppendLine("values(@CompanyCD,@InComeNo,@AcceDate,");
            sql.AppendLine("@CustName,@BillingID,@TotalPrice,@AcceWay,");
            sql.AppendLine("@BankName,@Executor,@AccountNo,getdate(),@ModifiedUserID,@Summary,@BlendingType,@CurrencyType,@CurrencyRate,@CustID,@FromTBName,@FileName,@ProjectID,@PaymentTypes)");
            sql.AppendLine("set @IntID= @@IDENTITY");

            SqlCommand comm = new SqlCommand();
            comm.CommandText = sql.ToString();
            comm.Parameters.Add("@CompanyCD", SqlDbType.VarChar).Value = model.CompanyCD;
            comm.Parameters.Add("@InComeNo", SqlDbType.VarChar).Value = model.InComeNo;
            comm.Parameters.Add("@AcceDate", SqlDbType.VarChar).Value = model.AcceDate;
            comm.Parameters.Add("@CustName", SqlDbType.VarChar).Value = model.CustName;
            comm.Parameters.Add("@BillingID", SqlDbType.VarChar).Value = model.BillingID;
            comm.Parameters.Add("@TotalPrice", SqlDbType.VarChar).Value = model.TotalPrice;
            comm.Parameters.Add("@AcceWay", SqlDbType.Char).Value = model.AcceWay;
            comm.Parameters.Add("@BankName", SqlDbType.VarChar).Value = model.BankName;
            comm.Parameters.Add("@Executor", SqlDbType.VarChar).Value = model.Executor;
            comm.Parameters.Add("@AccountNo", SqlDbType.VarChar).Value = model.AccountNo;
            comm.Parameters.Add("@ModifiedUserID", SqlDbType.VarChar).Value = model.ModifiedUserID;
            comm.Parameters.Add("@Summary", SqlDbType.VarChar).Value = model.Summary;
            comm.Parameters.Add("@BlendingType", SqlDbType.Char).Value = model.BlendingType;
            comm.Parameters.Add("@CurrencyType", SqlDbType.Int).Value = model.CurrencyType;
            comm.Parameters.Add("@CurrencyRate", SqlDbType.Decimal).Value = model.CurrencyRate;

            comm.Parameters.Add("@CustID", SqlDbType.Int).Value = model.CustID;
            comm.Parameters.Add("@FromTBName", SqlDbType.VarChar).Value = model.FromTBName;
            comm.Parameters.Add("@FileName", SqlDbType.VarChar).Value = model.FileName;
            comm.Parameters.Add("@ProjectID", SqlDbType.Int).Value = model.ProjectID;
            comm.Parameters.Add("@PaymentTypes", SqlDbType.Int).Value = model.PaymentTypes;
        ...
        private static void UpdateBillingPrice(ArrayList lstCommand, int ID, decimal Price)
        {
            ...
            //定义参数
            cmd.Parameters.Add("@YAccounts", SqlDbType.Decimal).Value = Price;
            ...
            if (ProcPrice == 0)
            {
                cmd.Parameters.Add("@AccountsStatus", SqlDbType.Char).Value = ConstUtil.ACCOUNTS_STATUS_YJS;
            }
            else
            {
                cmd.Parameters.Add("@AccountsStatus", SqlDbType.Char).Value = ConstUtil.ACCOUNTS_STATUS_YJSZ;
            }

            cmd.Parameters.Add("@NAccounts", SqlDbType.Decimal).Value = ProcPrice;
            lstCommand.Add(cmd);
        }
        #endregion
        ...
        public static bool UpdateIncomeBill(IncomeBillModel model, decimal OldPrice)
        {
            ...
            sql.AppendLine("[BlendingType] = @BlendingType,CurrencyType=@CurrencyType,CurrencyRate=@CurrencyRate,CustID=@CustID,FromTBName=@FromTBName,FileName=@FileName,ProjectID=@ProjectID,PaymentTypes=@PaymentTypes");
            sql.AppendLine("where ID=@ID");
            ...
            comm.Parameters.Add("@AcceDate", SqlDbType.DateTime).Value = model.AcceDate;
            comm.Parameters.Add("@CustName", SqlDbType.VarChar).Value = model.CustName;
            comm.Parameters.Add("@BillingID", SqlDbType.Int).Value = model.BillingID;
            comm.Parameters.Add("@TotalPrice", SqlDbType.Decimal).Value = model.TotalPrice;
            comm.Parameters.Add("@AcceWay", SqlDbType.Char).Value = model.AcceWay;
            comm.Parameters.Add("@BankName", SqlDbType.VarChar).Value = model.BankName;
            comm.Parameters.Add("@Executor", SqlDbType.Int).Value = model.Executor;
            comm.Parameters.Add("@AccountNo", SqlDbType.VarChar).Value = model.AccountNo;
            comm.Parameters.Add("@ModifiedUserID", SqlDbType.VarChar).Value = model.ModifiedUserID;
            comm.Parameters.Add("@Summary", SqlDbType.VarChar).Value = model.Summary;
            comm.Parameters.Add("@BlendingType", SqlDbType.Char).Value = model.BlendingType;
            ...
            comm.Parameters.Add("@ProjectID", SqlDbType.Int).Value = model.ProjectID;
            comm.Parameters.Add("@PaymentTypes", SqlDbType.Int).Value = model.PaymentTypes;
            comm.Parameters.Add("@ID", SqlDbType.Int).Value = model.ID;
        ...
        private static void UpdateBillingPriceInfo(ArrayList lstCommand, int ID, decimal Price, decimal OldPrice)
        {
            ...
            cmd.CommandText = sql.ToString();
            //定义参数
            //cmd.Parameters.Add("@YAccounts", SqlDbType.Decimal).Value = OldPrice;
            cmd.Parameters.Add("@YAccounts", SqlDbType.Decimal).Value = diffAccounts;//update by moshenlin 2009-08-05
            cmd.Parameters.Add("@ID", SqlDbType.Int).Value = ID;
            //cmd.Parameters.Add("@NAccounts", SqlDbType.Decimal).Value = OldPrice;
            cmd.Parameters.Add("@NAccounts", SqlDbType.Decimal).Value = diffAccounts;//update by moshenlin 2009-08-05
        ...
        /// <returns>DataTable</returns>
        public static DataTable SearchIncomeInfo(string ProjectID, string CompanyCD, string IncomeBillNo, string AcceWay, string CustName, string TotalPrice
        ...
        {
            //StringBuilder sql = new StringBuilder();
            //sql.AppendLine("Update officedba.Billing  set  ");
            //sql.AppendLine(" where ID=@ID");
            //SqlCommand cmd = null;
            ...
            //        cmd.Parameters.Add("@YAccounts", SqlDbType.Decimal).Value = splitPrice;
            //        cmd.Parameters.Add("@NAccounts", SqlDbType.Decimal).Value = splitPrice;
