   ...
   public class PayBillDBHelper
    {
       ...
       public static bool InSertIntoPayBillInfo(PayBillModel model, out int IntID)
       {
           ...
           sql.AppendLine(" BankName,BankNo,ConfirmStatus,IsAccount,Remark,BlendingType,CurrencyType,CurrencyRate,CustID,FromTBName,FileName,ProjectID,PaymentType ) ");
           sql.AppendLine(" values (  @BillingID,@CompanyCD,@CreatDate,@PayNo,");
           sql.AppendLine("@CustName,@PayAmount,@PayDate,@AcceWay,@Executor,  ");
           sql.AppendLine(" @BankName,@BankNo,@ConfirmStatus,@IsAccout,@Remark,@BlendingType,@CurrencyType,@CurrencyRate,@CustID,@FromTBName,@FileName,@ProjectID,@PaymentType ) ");
           ...
           comm.Parameters.Add("@CreatDate", SqlDbType.DateTime).Value = model.CreatDate;
           comm.Parameters.Add("@PayNo", SqlDbType.VarChar).Value = model.PayNo;
           comm.Parameters.Add("@CustName", SqlDbType.VarChar).Value = model.CustName;
           comm.Parameters.Add("@PayAmount", SqlDbType.Decimal).Value = model.PayAmount;
           comm.Parameters.Add("@PayDate", SqlDbType.DateTime).Value = model.PayDate;
           ...
           comm.Parameters.Add("@BankNo", SqlDbType.VarChar).Value = model.BankNo;
           comm.Parameters.Add("@ConfirmStatus", SqlDbType.Char).Value = model.ConfirmStatus;
           comm.Parameters.Add("@IsAccout", SqlDbType.Char).Value = model.IsAccout;
           comm.Parameters.Add("@Remark", SqlDbType.VarChar).Value = model.Remark;
           ...
           comm.Parameters.Add("@PaymentType", SqlDbType.Char).Value = model.PaymentType;//款项类型
       ...
       private static void UpdateInsertBillingPrice(ArrayList lstCommand, int ID, decimal Price)
       {
           ...
           if (StatusAmount > 0)
           {
               comdd.Parameters.Add("@AccountsStatus", SqlDbType.Decimal).Value = ConstUtil.ACCOUNTS_STATUS_YJSZ;
               comdd.Parameters.Add("@ID1", SqlDbType.Int).Value = ID;
           }
           else
           {
               comdd.Parameters.Add("@AccountsStatus", SqlDbType.Decimal).Value = ConstUtil.ACCOUNTS_STATUS_YJS;
               comdd.Parameters.Add("@ID1", SqlDbType.Int).Value = ID;
           }
       ...
       //        com.Parameters.Add("@AccountsStatus", SqlDbType.Char).Value = "1";
       //        com.Parameters.Add("@ID1", SqlDbType.Int).Value = ID;
       //        lstCommand.Add(com);
       //    }
       ...
       //        comd.Parameters.Add("@AccountsStatus2", SqlDbType.Char).Value = "2";
       //        comd.Parameters.Add("@ID2", SqlDbType.Int).Value = ID;
       //        lstCommand.Add(comd);
       //    }
       //    else//未结算
       //    {
       ...
       //        comd3.Parameters.Add("@AccountsStatus3", SqlDbType.Char).Value = "0";
       //        comd3.Parameters.Add("@ID3", SqlDbType.Int).Value = ID;
       ...
       public static DataTable GetPayBillInfo(string queryStr)
       {
           ...
           sql.AppendLine(" isnull(a.AttestBillID,0) as AttestBillID,   ");
           sql.AppendLine(" case when a.AttestBillID is null then '' when a.AttestBillID is not null then c.AttestNo end as AttestNo ,");
           sql.AppendLine(" a.CurrencyType,isnull(a.CurrencyRate,1) as CurrencyRate ,h.CurrencyName,a.ProjectID,t.ProjectName,a.PaymentType ");
       ...
       public static bool UpdatePayBill(PayBillModel model, decimal DiffAmount)
       {
           ...
           comm.Parameters.Add("@BankNo", SqlDbType.VarChar).Value = model.BankNo;
           comm.Parameters.Add("@Remark", SqlDbType.VarChar).Value = model.Remark;
           comm.Parameters.Add("@ModifiedDate", SqlDbType.DateTime).Value = model.ModifiedDate;
           comm.Parameters.Add("@ModifiedUserID", SqlDbType.Int).Value = model.ModifiedUserID;
           comm.Parameters.Add("@BlendingType", SqlDbType.Char).Value = model.BlendingType;
           ...
           comm.Parameters.Add("@FileName", SqlDbType.VarChar).Value = model.FileName;
           comm.Parameters.Add("@ProjectID", SqlDbType.Int).Value = model.ProjectID;
           comm.Parameters.Add("@PaymentType", SqlDbType.Char).Value = model.PaymentType;
       ...
       public static void updateUpBillingStatus(ArrayList lstCommand, string BillingID, decimal DiffAmount)
       {
           ...
           cmd.CommandText = sql.ToString();
           //定义参数
           cmd.Parameters.Add("@DiffAmount", SqlDbType.Decimal).Value = DiffAmount;
           cmd.Parameters.Add("@ID", SqlDbType.Int).Value = BillingID;
           lstCommand.Add(cmd);
           ...
           {
               comdd.Parameters.Add("@AccountsStatus", SqlDbType.Decimal).Value = ConstUtil.ACCOUNTS_STATUS_YJSZ;
               comdd.Parameters.Add("@ID1", SqlDbType.Int).Value = BillingID;
           }
           else
           {
               comdd.Parameters.Add("@AccountsStatus", SqlDbType.Decimal).Value = ConstUtil.ACCOUNTS_STATUS_YJS;
       ...
       /// <param name="ids"></param>
       /// <returns></returns>
       public static bool Autidt(string ids,string billingid,string payno,string price)
       {
           ...
           string sql = string.Format(@"update officedba.PayBill set ConfirmStatus='1',Confirmor={0},ConfirmDate='{1}',AccountsStatus=2{3} where ID in ( {2} );", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).EmployeeID, DateTime.Now.ToString("yyyy-MM-dd"), ids,bill);
           ...
           if (billingid != "0" && billingid != "")
           {
               ...
               strSql.Append(string.Format(vals, CompanyCD, billingid, dt.Rows[0]["SourceDt"].ToString(),
                                   ...
                                   price == "" ? "0.00" : price,//每次勾兑的金额
                                   float.Parse(dt.Rows[0]["NAccounts"].ToString() == "" ? "0.00" : dt.Rows[0]["NAccounts"].ToString()) - float.Parse(price),//票据的余额
                                   DateTime.Now,
               ...
               strSql.AppendLine("Update officedba.Billing set YAccounts=isnull(YAccounts,0)+" + (float.Parse(price)).ToString() + ",NAccounts=isnull(NAccounts,0)-" + (float.Parse(price)).ToString() + ",AccountsStatus='" + accountsStatus + "' where ID=" + billingid + ";");
           }
       ...
       /// <param name="ids"></param>
       /// <returns></returns>
       public static bool AntiAutidt(string ids,string billingid,string price)
       {
           var bill = "";
           if (billingid != "0" && billingid != "")
           {
               bill = ",naccounts=payamount";
           }
           string sql = string.Format(@"update officedba.PayBill set ConfirmStatus='0',AccountsStatus=0,Confirmor=NULL,ConfirmDate=NULL{1} where ID in ( {0} );", ids, bill);
           string[] str = price.Split(',');
           price = "";
           for (int i = 0; i < str.Length; i++)
           {
           ...
           if (billingid != "0" && billingid != "")
           {
               ...
               sql+="delete officedba.BlendingDetails where billingid=" + billingid + " and ShouFuKuanID=" + ids + " and  PayOrInComeType=1 ;";

               sql += "Update officedba.Billing set YAccounts=isnull(YAccounts,0)-" + (price).ToString() + ",NAccounts=isnull(NAccounts,0)+" + float.Parse(price) + ",AccountsStatus='2' where ID=" + billingid + ";";
           }
           SqlParameter[] parms = new SqlParameter[0];
