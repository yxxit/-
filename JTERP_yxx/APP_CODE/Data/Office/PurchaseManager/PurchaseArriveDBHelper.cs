...
namespace XBase.Data.Office.PurchaseManager
{
    ...
    public class PurchaseArriveDBHelper
    {
        ...
        public static bool InsertPurchaseArrive(PurchaseArriveModel model, string DetailProductID, string DetailProductNo
            ...
            , string length, out string ID, Hashtable htExtAttr, string DetailStorageID, string[] strCustomdetail)
        {
            ...
            try
            {
                ...
                if (lengs > 0)
                {
                    ...
                    string[] StorageID = DetailStorageID.Split(',');//添加仓库 20110504 lcb
                    for (int i = 0; i < lengs; i++)
                    {
                        ...
                        //添加仓库 20110504 lcb
                        cmdsql.AppendLine("TaxPrice,TaxRate,TotalFee,TotalPrice,TotalTax,StorageID,Remark,UsedUnitCount,UsedUnitID,ExRate,UsedPrice");
                        //添加扩展项
                        ...
                        //添加仓库 20110504 lcb
                        cmdsql.AppendLine("@TaxPrice,@TaxRate,@TotalFee,@TotalPrice,@TotalTax,@StorageID,@Remark2,@UsedUnitCount,@UsedUnitID,@ExRate,@UsedPrice");
                        //添加扩展项
        ...
        private static void GetExtAttrCmd(PurchaseArriveModel model, Hashtable htExtAttr, SqlCommand cmd, string no)
        {
            try
            {
                ...
                {
                    cmd.Parameters.Add("@ArriveNo", no);
                }
                else
                {
                    cmd.Parameters.Add("@ArriveNo", model.ArriveNo);
                }
        ...
        public static bool UpdatePurchaseArrive(PurchaseArriveModel model, string DetailProductID
            ...
            , string no, Hashtable htExtAttr, string DetailStorageID, string[] strCustomdetail)
        {
            if (model.ID <= 0)
            {
                return false;
            ...
            sqlArrive.AppendLine("Attachment=@Attachment,MoneyType=@MoneyType,ArriveDate=@ArriveDate,ProjectID=@ProjectID ,ModifiedDate=getDate(),CanViewUser =@CanUserID,CanViewUserName =@UserName");
            
            sqlArrive.AppendLine("where CompanyCD=@CompanyCD and ArriveNo='" + no + "'and ID=" + model.ID);
        ...
        public static DataTable GetPurchaseOrderDetail(string ProductNo, string ProductName, string FromBillNo, string CompanyCD, int ProviderID, int CurrencyTypeID, string Rate, string PurchaseArriveEFIndex, string PurchaseArriveEFDesc)
        {
            try
            {
                ...
                sql.AppendLine("      ,B.StorageID as StorageID, si.StorageName as StorageName   ");
                ...
                sql.AppendLine("  left join officedba.storageproduct as sp on sp.productid=B.id   ");
                sql.AppendLine("  left join officedba.storageInfo as si on si.ID=B.storageID ");
                ...
                sql.AppendLine(" LEFT JOIN  officedba.FlowInstance AS F1 ON D.id = F1.BillID AND F1.BillTypeFlag = 6 AND F1.BillTypeCode = 4 ");
                sql.AppendLine(" AND F1.ID=(select max(ID) from officedba.FlowInstance AS F where  D.id = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode = 4) ");

                sql.AppendLine("WHERE D.BillStatus = 2  AND D.CompanyCD =@CompanyCD  and A.ProductCount>A.ArrivedCount ");
                sql.AppendLine("and ( ");
                ...
                sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=f1.FlowNo and BillID=D.ID  AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+d.CanViewUser+',')>0 )");

                sql.AppendLine("or (d.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (d.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        public static bool ConfirmPurchaseArrive(PurchaseArriveModel Model, string DetailProductCount, string DetailFromBillNo, string DetailFromLineNo, string length, out string strMsg, string DetailProductID, string DetailStorageID)
        {
            ...
            if (isCanDo(Model.ID, "1"))
            {
                ...
                string sql = "select c.orderno,b.fromlineno,b.productid,b.productcount,b.storageid from officedba.PurchaseArrive a left join officedba.PurchaseArrivedetail b on a.companycd=b.companycd and a.arriveno=b.arriveno left join officedba.PurchaseOrder c on c.id=b.frombillid where a.id=" + Model.ID;
                ...
                try
                {
                   ...
                   // int lengs = Convert.ToInt32(length);
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        //string[] FromLineNo = DetailFromLineNo.Split(',');
                        //string[] ProductID = DetailProductID.Split(',');//在途量的更新用 add 20110504 lcb
                        //string[] StorageID = DetailStorageID.Split(',');//在途量的更新用 add 20110504 lcb
                        string pp = "";
                        ...
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            ...
                            {
                                if (userInfo.IsOverOrder || IsCanConfirm(dt.Rows[i]["orderno"].ToString(), dt.Rows[i]["fromlineno"].ToString(), Model.CompanyCD, dt.Rows[i]["productcount"].ToString()))
                                {
                                    System.Text.StringBuilder cmdsql = new System.Text.StringBuilder();
                                    SqlCommand comms = new SqlCommand();
                                    ...
                                    comms.Parameters.Add(SqlHelper.GetParameter("@CompanyCD", Model.CompanyCD));
                                    comms.Parameters.Add(SqlHelper.GetParameter("@ProductCount", dt.Rows[i]["productcount"].ToString()));
                                    comms.Parameters.Add(SqlHelper.GetParameter("@FromBillNo", dt.Rows[i]["orderno"].ToString()));
                                    comms.Parameters.Add(SqlHelper.GetParameter("@FromLineNo", dt.Rows[i]["fromlineno"].ToString()));
                                    comms.CommandText = cmdsql.ToString();
                                    listADD.Add(comms);                                    
                                }
                                else
                            ...
                            //采购到货单中做在途量的更新 add 20110504 lcb
                            //回写库存分仓存量表
                            if (dt.Rows[i]["productid"].ToString() != "")
                            {
                                listADD.Add(WriteStorge(dt.Rows[i]["productid"].ToString(), dt.Rows[i]["productcount"].ToString(), dt.Rows[i]["storageid"].ToString()));
                            }
                        }
                    }
        ...
        #region 回写库存分仓存量表
        public static SqlCommand WriteStorge(string ProductID, string ProductCount, string StorageID)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            if (StorageID == "")
            ...
            if (IsExistInStorge(ProductID, int.Parse(StorageID)))
            {
                ...
                //edit 20110505 lcb 添加条件BatchNo=NULL，采购到货单没有批次
                sql.AppendLine(" AND CompanyCD = @CompanyCD AND BatchNo is NULL");
            ...
            else
            {
                ...
                sql.AppendLine(" SELECT @CompanyCD,@StorageID,@ProductID,@RoadCount ");
                    ...
                    try
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                SqlParameter[] param = { 
                                           new SqlParameter("@CompanyCD", Model.CompanyCD),
                                            new SqlParameter("@ProductCount", dt.Rows[i]["productcount"].ToString()),
                                             new SqlParameter("@FromBillNo", dt.Rows[i]["orderno"].ToString()),
                                              new SqlParameter("@FromLineNo", dt.Rows[i]["fromlineno"].ToString())
                                           };
                                ...
                                if (dt.Rows[i]["productid"].ToString() != "")
                                {
                                    listADD.Add(WriteStorgeDecr(dt.Rows[i]["productid"].ToString(), dt.Rows[i]["productcount"].ToString(), dt.Rows[i]["storageid"].ToString()));                                    
                                }
                            }
                        }
        ...
        #region 取消确认时回写库存
        public static SqlCommand WriteStorgeDecr(string ProductID, string ProductCount, string StorageID)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            if (IsExistInStorge(ProductID, int.Parse(StorageID)))
            ...
            else
            {
                ...
                sql.AppendLine("SELECT @CompanyCD,@StorageID,@ProductID,null,");
                ...
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@StorageID", StorageID));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProductID", ProductID));
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@RoadCount", (0 - Convert.ToInt32(ProductCount)).ToString()));
                comm.CommandText = sql.ToString();
            }
            return comm;
        }
        ...
        //判断这条物品在分仓存量表
        public static bool IsExistInStorge(string ProductID, int StorageID)
        {
            //edit 20110505 lcb 添加条件BatchNo=NULL，采购到货单没有批次
            String sql = "SELECT A.ID, A.StorageID FROM officedba.StorageProduct AS A INNER JOIN " +
                      "officedba.ProductInfo AS B ON A.ProductID = B.ID AND A.CompanyCD = B.CompanyCD " +
                      "WHERE A.CompanyCD=@CompanyCD AND A.ProductID=@ProductID AND A.StorageID=@StorageID AND BatchNo is NULL";            
        ...
        #region 查询到货通知列表所需数据
        public static DataTable SelectArriveList(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ArriveNo, string Title, string TypeID, string Purchaser, string FromType, string ProviderID, string BillStatus, string UsedStatus, string ProjectID, string EFIndex, string EFDesc,string time1,string time2,string prodid,string EFIndexP,string EFDescP)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            string sellpoint = ((XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"]).SelPoint;
            sql.AppendLine("SELECT distinct A.ID ,A.ArriveNo,de.productname,Convert(numeric(12," + sellpoint + "),de.unitprice) AS unitprice,Convert(numeric(12," + sellpoint + "),de.productcount) productcount,Convert(numeric(12," + sellpoint + "),de.totalprice) totalprice ,");
            sql.AppendLine(" isnull(A.Title,'') AS Title,A.Rate ,A.TypeID,isnull(B.TypeName,'') AS TypeName,A.Purchaser,isnull(C.EmployeeName,'')  AS PurchaserName ");
            sql.AppendLine("      ,A.FromType,case A.FromType when '0' then '无来源' when '1' then '采购订单'end AS FromTypeName ,isnull(de.FromBillID,0) AS FromBillID ,A.BillStatus");
            ...
            sql.AppendLine(" FROM officedba.PurchaseArrive AS A   left join officedba.PurchaseArrivedetail de on a.companycd=de.companycd and a.ArriveNo=de.ArriveNo                                                                  ");
            ...
            sql.AppendLine("LEFT JOIN officedba.ProductInfo AS p  ON  p.CompanyCD = de.CompanyCD AND de.productid = p.id             ");
            sql.AppendLine("LEFT JOIN officedba.PurchaseOrder AS E ON de.CompanyCD = E.CompanyCD   AND de.FromBillID=E.ID                 ");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F.FlowNo and BillID=A.ID  AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");

            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.CheckUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (time1 != "" && time2 != "")
            {
                sql.AppendLine(" and a.CreateDate between '" + time1 + "' and '" + time2 + "' ");
            }
            else if (time1 == "" && time2 != "")
            {
                sql.AppendLine(" and a.CreateDate<='" + time2 + "' ");
            }
            else if (time1 != "" && time2 == "")
            {
                sql.AppendLine(" and a.CreateDate>='" + time1 + "' ");
            }
            if (prodid != "")
            {
                sql.AppendLine(" and de.productname like'%" + prodid + "%'");
            }
            ...
            {
                sql.AppendLine(" and p.ExtField" + EFIndexP + " LIKE @EFDescP");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@EFDescP", "%" + EFDescP + "%"));
            }
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@CompanyCD", ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD));
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT A.ID ,B.Mobile,A.ArriveNo ,A.Title ,A.ProviderID,isnull(B.CustName,'') AS ProviderName ,isnull(B.Tel,'') AS Tel ,isnull(B.ContactName,'') AS ContactName ,isnull(B.Fax,'') AS Fax ,A.FromType,case A.FromType when '0' then '无来源' when '1' then '采购订单' end AS FromTypeName ,A.PayType,isnull(C.TypeName,'') AS PayTypeName,A.MoneyType,isnull(M.TypeName,'') AS  MoneyTypeName ");
        ...
        public static DataTable Details(int ID)
        {
            StringBuilder sql = new StringBuilder();
            string sellpoint = ((XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"]).SelPoint;
            sql.AppendLine(@"SELECT  distinct  isnull(A.Custom1,'')Custom1,isnull(A.Custom2,'')Custom2,isnull(A.Custom3,'')Custom3,isnull(A.Custom4,'')Custom4,isnull(A.Custom5,'')Custom5,isnull(A.Custom6,'')Custom6,isnull(A.Custom7,'')Custom7,isnull(A.Custom8,'')Custom8,isnull(A.Custom9,'')Custom9,isnull(A.Custom10,'')Custom10
                ,A.ID,A.CompanyCD,A.ArriveNo ,isnull(A.SortNo,0) AS SortNo ,A.FromBillID,A.ProductID,''as DeRemark ");
            sql.AppendLine(",A.ProductNo ,A.ProductName,Convert(numeric(12," + sellpoint + "),isnull(E.ProductCount,0)) AS ProductOrder,A.UnitID,Convert(numeric(12," + sellpoint + "),isnull(A.ProductCount,0)) AS ProductCount    ");
            sql.AppendLine(", B.Specification AS Specification,Convert(numeric(12," + sellpoint + "),isnull(A.ApplyCheckCount,0)) AS ApplyCheckCount,Convert(numeric(12," + sellpoint + "),isnull(A.CheckedCount,0)) AS  CheckedCount");
            sql.AppendLine(", Convert(numeric(12," + sellpoint + "),isnull(A.PassCount,0)) AS PassCount,Convert(numeric(12," + sellpoint + "),isnull(A.NotPassCount,0)) AS NotPassCount,Convert(numeric(12," + sellpoint + "),isnull(A.InCount,0)) AS InCount");
            sql.AppendLine(", Convert(numeric(12," + sellpoint + "),isnull(A.RejectCount,0)) AS RejectCount,Convert(numeric(12," + sellpoint + "),isnull(A.BackCount,0)) AS BackCount");
            sql.AppendLine(",ISNULL(cpt.TypeName,'') AS ColorName ");
            sql.AppendLine(",isnull(A.FromType,'') AS  FromType ,Convert(numeric(12,"+sellpoint+"),isnull(B.TaxRate,0)) AS HidTaxRate                     ");
            ...
            sql.AppendLine(",Convert(numeric(12," + sellpoint + "),A.UnitPrice) AS UnitPrice ,Convert(numeric(12," + sellpoint + "),A.TaxPrice) AS TaxPrice,Convert(numeric(12," + sellpoint + "),A.Discount) AS Discount,Convert(numeric(12," + sellpoint + "),A.TaxRate) AS TaxRate,Convert(numeric(12," + sellpoint + "),A.TotalFee) AS TotalFee,Convert(numeric(12," + sellpoint + "),A.TotalPrice) AS TotalPrice,Convert(numeric(12," + sellpoint + "),A.TotalTax) AS  TotalTax     ");
            sql.AppendLine(" ,Convert(numeric(12," + sellpoint + "),isnull(case when A.FromBillID is null then 0 when A.FromBillID = '' then 0                ");
            sql.AppendLine(" else (select ArrivedCount from officedba.PurchaseOrderDetail where OrderNo=C.OrderNo and SortNo=A.FromLineNo and CompanyCD='" + ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD + "') end,0)) AS ArrivedCount");
            sql.AppendLine(",(select CodeName from officedba.CodeUnitType where id=A.UsedUnitID) as UsedUnitName   ");
            //添加仓库 20110504 lcb
            sql.AppendLine(",A.Remark,isnull(C.OrderNo,'') AS FromBillNo,A.FromLineNo,A.UsedUnitID,Convert(numeric(12," + sellpoint + "),isnull(A.UsedUnitCount,0)) as UsedUnitCount,Convert(numeric(12," + sellpoint + "),isnull(A.UsedPrice,0)) as UsedPrice,isnull(A.StorageID,0) AS StorageID,isnull(dbo.getStorageName(a.storageid),'')StorageName   FROM officedba.PurchaseArriveDetail AS A     ");
            sql.AppendLine("LEFT JOIN officedba.ProductInfo   AS B on A.CompanyCD = B.CompanyCD  AND A.ProductID=B.ID    ");
            sql.AppendLine("LEFT JOIN officedba.CodePublicType cpt ON B.ColorID=cpt.ID                  ");
        ...
        {
            //所有引用追加条件CompanyCD edit 20110503 lcb
            bool IsCite = false;
            //入库引用
            if (!IsCite)
            {
                ...
                parameters[2].Value = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                IsCite = SqlHelper.Exists(sql, parameters);
            }
            //质量检测引用
            if (!IsCite)
            {
                string sql = "SELECT C.ID FROM officedba.QualityCheckApplyDetail AS C,officedba.QualityCheckApplay AS D WHERE C.FromType=@FromType AND C.FromBillID=@ID AND C.ApplyNo=D.ApplyNo AND C.CompanyCD=D.CompanyCD AND C.CompanyCD=@CompanyCD ";
                SqlParameter[] parameters = {
                    new SqlParameter("@FromType", SqlDbType.VarChar),
					new SqlParameter("@ID", Convert.ToString(SqlDbType.Int)),
                    new SqlParameter("@CompanyCD", SqlDbType.VarChar),};
        ...
        private static SqlCommand GetPurchaseArrive(string ProviderID, string ProductID, string StartConfirmDate
            , string EndConfirmDate, string CompanyCD, string orderBy, bool isDate, bool isPrint,string storageID)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            ...
            sql.AppendLine("      ,ISNULL([StorageName],'') AS StorageName ");
            if (isDate)
            {
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=officedba.V_PurchaseArriveCollect.FlowNo  and BillID=officedba.V_PurchaseArriveCollect.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+CanViewUser+',')>0 )");

            sql.AppendLine("or (Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (CheckUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))OR (Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        {
            SqlCommand comm = GetPurchaseArrive(ProviderID, ProductID, StartConfirmDate, EndConfirmDate, CompanyCD, orderBy, isDate, isPrint,storageID);
            if (isPrint)
            {
                return SqlHelper.ExecuteSearch(comm);
            }
        ...
        public static DataTable PurchaseArriveQuery(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProviderID, string StartConfirmDate, string EndConfirmDate, string CompanyCD)
        {
            ...
            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.CheckUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))OR (a.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
