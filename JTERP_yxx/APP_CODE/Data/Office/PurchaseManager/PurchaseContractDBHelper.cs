    ...
    public class PurchaseContractDBHelper
    {
        ...
        private static void GetExtAttrCmd(PurchaseContractModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@ContractNo", model.ContractNo );
            ...
            try
            {
                ...
                if (lengs > 0)
                {
                    ...
                    for (int i = 0; i < lengs; i++)
                    {
                        ...
                        cmdsql.AppendLine(GetBillTableCellsDBHelper.insertCustomFilder(strCustomdetail, i+1));
                        ...
                        cmdsql.AppendLine("            ,@ApplyReason ");
                        cmdsql.AppendLine(GetBillTableCellsDBHelper.insertCustomvalue(strCustomdetail, i+1));
                        cmdsql.AppendLine(")");
        ...
        public static DataTable GetApplySelectDetail(int Provider00ID, string ProductNo, string ProductName, string FromBillNo, string CompanyCD)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  F.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6            ");
            sql.AppendLine("AND F1.BillTypeCode = 1 AND F1.BillNo = F.ApplyNo                                                         ");
            sql.AppendLine(" AND F1.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F2 WHERE F.ID = F2.BillID AND F2.BillTypeFlag = 6 AND F2.BillTypeCode = 1 )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F1.FlowNo  and BillID=F.ID AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (f.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (f.ApplyUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        public static DataTable GetSellOrderDetail(string ProductNo, string ProductName, string FromBillNo, string CompanyCD,int ProviderID)
        {
            ...
            sql.AppendLine("left join officedba.CodePublicType HH on E.ColorID=HH.ID    ");
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  F.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6            ");
            sql.AppendLine("AND F1.BillTypeCode = 2 AND F1.BillNo = F.PlanNo                                                         ");
            sql.AppendLine(" AND F1.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F2 WHERE F.ID = F2.BillID AND F2.BillTypeFlag = 6 AND F2.BillTypeCode =2 )");
            ...
            sql.AppendLine("or (f.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (f.PlanUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (f.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        public static DataTable GetAskPriceOrderDetail(string ProductNo, string ProductName, string FromBillNo, string CompanyCD, int ProviderID,int CurrencyTypeID, string Rate)
        {
            ...
            {
                sql.AppendLine("      ,     Convert(numeric(14," + userInfo.SelPoint + "),isnull(a.UsedPrice,0)) as UnitPrice                     ");
            }
            else
            {
                sql.AppendLine("      ,     Convert(numeric(14," + userInfo.SelPoint + "),isnull(a.UnitPrice,0)) as UnitPrice                     ");
            ...
            sql.AppendLine("      ,       Convert(numeric(22," + userInfo.SelPoint + "),isnull(A.TaxPrice,0)) as TaxPrice                                ");
            sql.AppendLine("      ,              Convert(numeric(12," + userInfo.SelPoint + "),isnull(A.TaxRate,0)) as TaxRate                            ");
            ...
            sql.AppendLine("      ,isnull(H.DeptName,'')  AS  DeptIDName                                                     ");
            sql.AppendLine("      ,  Convert(numeric(14,4),isnull(F.isAddTax,0)) as isAddTax                                                            ");
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  F.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6            ");
            sql.AppendLine("AND F1.BillTypeCode = 3 AND F1.BillNo = F.AskNo                                                         ");
            sql.AppendLine(" AND F1.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F2 WHERE F.ID = F2.BillID AND F2.BillTypeFlag = 6 AND F2.BillTypeCode =3 )");
            ...
            sql.AppendLine("or (f.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (f.AskUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
        ...
        #region 查询合同列表所需数据
        public static DataTable SelectContractList(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ContractNo1, string Title, string TypeID, string DeptID, string Seller, string FromType, string ProviderID, string BillStatus, string UsedStatus,string  EFIndex ,string EFDesc ,string ProductName,string EFIndexP,string EFDescP)
        {
            UserInfoUtil userInfo1 = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            ...
            sql.AppendLine("  ,isnull(p.productname,'') productname,Convert(numeric(12," + userInfo1.SelPoint + "),de.productcount) productcount, ");
            if (userInfo1.IsMoreUnit == true)
            {
                sql.AppendLine(" Convert(numeric(12," + userInfo1.SelPoint + "),de.usedprice) AS unitprice");
            }
            else
            {
                sql.AppendLine(" Convert(numeric(12," + userInfo1.SelPoint + "),de.unitprice) AS unitprice");
            }
            sql.AppendLine(" ,Convert(numeric(12," + userInfo1.SelPoint + "),de.usedprice) AS usedprice ");
            sql.AppendLine("      ,Convert(numeric(12," + userInfo1.SelPoint + "),A.TotalPrice) AS TotalPrice,Convert(numeric(12," + userInfo1.SelPoint + "),A.TotalTax) AS TotalTax,Convert(numeric(12," + userInfo1.SelPoint + "),A.TotalFee) AS TotalFee,A.BillStatus   ");
            ...
            sql.AppendLine(" left join officedba.purchasecontractdetail as de on de.contractno=a.contractno and de.companycd=a.companycd ");
        ...
        public static DataTable SelectContract(int ID)
        {
            ...
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT A.ID ,A.ContractNo ,isnull(A.Title,'') as Title  ,A.ProviderID,isnull(B.CustName,'') AS ProviderName ,B.ArtiPerson,B.SendAddress,B.SetupAddress,B.ContactName,B.Tel,B.Fax,A.FromType,A.PayType,isnull(C.TypeName,'') AS PayTypeName,A.MoneyType,isnull(M.TypeName,'') AS  MoneyTypeName ");
        ...
        public static DataTable Details(int ID)
        {
            ...
            sql.AppendLine(",isnull(A.Custom1,'') Custom1,isnull(A.Custom2,'') Custom2,isnull(A.Custom3,'') Custom3,isnull(A.Custom4,'') Custom4 ");
            sql.AppendLine(",isnull(A.Custom5,'') Custom5,isnull(A.Custom6,'') Custom6,isnull(A.Custom7,'') Custom7,isnull(A.Custom8,'') Custom8 ");
            sql.AppendLine(",isnull(A.Custom9,'') Custom9,isnull(A.Custom10,'') Custom10 ");
            sql.AppendLine(",A.ProductNo ,A.ProductName, C.CASRegi,C.EngName,Convert(numeric(14," + userInfo.SelPoint + "),a.ProductCount) as ProductCount,A.UnitID                     ");
            ...
            if(userInfo.IsMoreUnit==true)
            {
                sql.AppendLine(",Convert(numeric(12," + userInfo.SelPoint + "),A.UsedPrice) AS UnitPrice ");
            }
            else
            {
                sql.AppendLine(",Convert(numeric(12," + userInfo.SelPoint + "),A.UnitPrice) AS UnitPrice ");
            }
            sql.AppendLine(",Convert(numeric(12," + userInfo.SelPoint + "),A.TaxPrice) AS TaxPrice,Convert(numeric(12," + userInfo.SelPoint + "),A.Discount) AS Discount");
            sql.AppendLine(" ,Convert(numeric(12," + userInfo.SelPoint + "),A.TaxRate) AS TaxRate,Convert(numeric(12," + userInfo.SelPoint + "),A.TotalFee) AS TotalFee,Convert(numeric(12," + userInfo.SelPoint + "),A.TotalPrice) AS TotalPrice,Convert(numeric(12," + userInfo.SelPoint + "),A.TotalTax ) AS  TotalTax ");
            sql.AppendLine(",isnull(CONVERT(varchar(100), A.RequireDate, 23),'') AS RequireDate,A.ApplyReason ,Convert(numeric(12," + userInfo.SelPoint + "),isnull(A.OrderCount,0)) AS OrderCount");
            ...
            sql.AppendLine(",isnull(A.Remark,'') AS Remark ,isnull(A.standard,'') AS standard,isnull(H.TypeName,'') as ColorName ,isnull(C.specification,'') Specification  FROM officedba.PurchaseContractDetail AS A     ");
