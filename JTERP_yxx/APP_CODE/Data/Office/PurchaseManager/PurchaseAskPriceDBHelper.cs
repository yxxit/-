...
namespace XBase.Data.Office.PurchaseManager
{
    public class PurchaseAskPriceDBHelper
    {
        ...
        public static SqlCommand InsertPurAskPricePri(PurchaseAskPriceModel PurchaseAskPriceM)
        {
            ...
            sql.AppendLine("           ,ProviderLinkID                ");
            sql.AppendLine("           ,ProviderLinkName                   ");
            sql.AppendLine("           ,ProviderLinkTel                ");
            sql.AppendLine("           ,ProviderLinkHandset              ");
            sql.AppendLine("           ,ProviderLinkAddress            ");
            ...
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderLinkID", PurchaseAskPriceM.ProviderLinkID));
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderLinkName", PurchaseAskPriceM.ProviderLinkName));
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderLinkTel", PurchaseAskPriceM.ProviderLinkTel));
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderLinkHandset", PurchaseAskPriceM.ProviderLinkHandset));
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderLinkAddress", PurchaseAskPriceM.ProviderLinkAddress));
            comm.Parameters.Add(SqlHelper.GetParameterFromString("@ProviderNo", PurchaseAskPriceM.ProviderNo));
        ...
        public static SqlCommand UpdatePurAskPricePri(PurchaseAskPriceModel PurchaseAskPriceM)
        {
            ...
            sql.AppendLine("      ,ProviderLinkID       =@ProviderLinkID        ");
            sql.AppendLine("      ,ProviderLinkName     =@ProviderLinkName      ");
            sql.AppendLine("      ,ProviderLinkTel     =@ProviderLinkTel      ");
            sql.AppendLine("      ,ProviderLinkHandset      =@ProviderLinkHandset     ");
            sql.AppendLine("      ,ProviderLinkAddress    =@ProviderLinkAddress ");
            sql.AppendLine("      ,ProviderNo    =@ProviderNo ");
        ...
        public static DataTable SelectPurAskPricePri(PurchaseAskPriceModel PurchaseAskPriceM, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            int ismoreunit = Convert.ToInt32(userInfo1.IsMoreUnit);
            #region SQL文
            StringBuilder sql = new StringBuilder();
            ...
            sql.AppendLine(",productname,productcount,unitprice  ");
            ...
            sql.AppendLine(",p.productname,ad.productcount ");
            if (ismoreunit == 1)
            {
                sql.AppendLine(",ad.usedprice as unitprice  ");
            }
            else
            {
                sql.AppendLine(",ad.unitprice as unitprice  ");
            }
            ...
            sql.AppendLine(" left join officedba.productinfo as p on p.id=ad.productid and p.companycd=ad.companycd  ");
            ...
            sql.AppendLine("AND F.BillTypeCode =3 AND F.BillNo = A.AskNo                                                         ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE A.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =3 )");
            ...
            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.AskUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            if (!string.IsNullOrEmpty(PurchaseAskPriceM.EFIndexP) && !string.IsNullOrEmpty(PurchaseAskPriceM.EFDescP))
            {
                sql.AppendLine(" and p.ExtField" + PurchaseAskPriceM.EFIndexP + " LIKE @EFDescP");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@EFDescP", "%" + PurchaseAskPriceM.EFDescP + "%"));
            }
            sql.AppendLine("    )AS E                                                                             ");
            ...
            if (PurchaseAskPriceM.ProdName != "")
            {
                sql.AppendLine(" and productname like @prodname");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@prodname","%"+PurchaseAskPriceM.ProdName+"%"));
            }
            #endregion
        ...
        public static DataTable GetPurAskPricePriByID(string ID)
        {
            ...
            sql.AppendLine("		 , isnull(B.CustName,'') AS ProviderName, isnull(B.WebSite,'') WebSite,B.ContactName,B.Mobile,B.Tel                            ");
            ...
            sql.AppendLine(" ,isnull(A.ProviderLinkID,'') AS ProviderLinkID");
            sql.AppendLine(" ,isnull(A.ProviderLinkName,'') AS ProviderLinkName");
            sql.AppendLine(" ,isnull(A.ProviderLinkTel,'') AS ProviderLinkTel");
            sql.AppendLine(" ,isnull(A.ProviderLinkHandset,'') AS ProviderLinkHandset");
            sql.AppendLine(" ,isnull(A.ProviderLinkAddress,'') AS ProviderLinkAddress");
            sql.AppendLine(" ,isnull(A.ProviderNo,'') AS ProviderNo");
        ...
        /// <returns>sqlcommend</returns>
        /// 
        public static SqlCommand InsertPurAskPriceDetail(PurchaseAskPriceDetailModel PurchaseAskPriceDetailM,string[] strCustomdetail,int j)
        {
            SqlCommand comm = new SqlCommand();
        ...
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            int ismoreunit = Convert.ToInt32(userInfo.IsMoreUnit);
           ...
           sql.AppendLine( " isnull(A.Custom9,'') Custom9,isnull(A.Custom10,'') Custom10 ");
            ...
            sql.AppendLine("		,isnull( B.CASRegi,'') as  CASRegi");
            ...
            if (ismoreunit == 1)
            {
                sql.AppendLine("		, Convert(numeric(14," + userInfo.SelPoint + "),a.UsedPrice) as UnitPrice      ");
            }
            else
            {
            ...
            sql.AppendLine("		, A.UsedUnitID,isnull(H.TypeName,'') as ColorName,isnull(sp.productcount,0) as nowproductcount, ct.TypeName as appearance     ");
            ...
            sql.AppendLine("left join officedba.CodePublicType as ct on ct.id=B.colorid and ct.companycd=B.companycd");
