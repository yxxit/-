...
namespace XBase.Data.Office.PurchaseManager
{
    public class PurchaseApplyDBHelper
    {
        ...
        public static DataTable SelectPrimary(PurchaseApplyModel PurchaseApplyM, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=E.FlowNo and BillID=A.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (A.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (A.ApplyUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
