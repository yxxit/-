    ...
    public class PurchaseRejectDBHelper
    {
        ...
        private static void GetExtAttrCmd(PurchaseRejectModel model, Hashtable htExtAttr, SqlCommand cmd)
        {
            try
            {
                ...
                cmd.Parameters.Add("@RejectNo", model.RejectNo);
        ...
        public static bool InsertPurchaseReject(PurchaseRejectModel model, string DetailProductID
            ...
            , string DetailUsedPrice, string length, out string ID, Hashtable htExtAttr, string[] strCustomdetail)
        {
            ...
            try
            {
                ...
                if (lengs > 0)
                {
                    ...
                    for (int i = 0; i < lengs; i++)
                    {
                        ...
                        //添加扩展项
                        cmdsql.Append(GetBillTableCellsDBHelper.insertCustomFilder(strCustomdetail, j));
                        ...
                        cmdsql.AppendLine("            ,@ApplyReason,@UsedUnitCount,@UsedUnitID,@ExRate,@UsedPrice");
                        //扩展项值
                        cmdsql.Append(GetBillTableCellsDBHelper.insertCustomvalue(strCustomdetail, j));
        ...
        public static bool UpdatePurchaseReject(PurchaseRejectModel model, string DetailProductID
            ...
            , string DetailUsedPrice, string length, string fflag2, string no, Hashtable htExtAttr, string[] strCustomdetail)
        {
            if (model.ID <= 0)
            {
                return false;
        ...
        public static DataTable GetPurchaseRejectDetail(string ProductNo, string ProductName, string FromBillNo, string CompanyCD, int ProviderID, int CurrencyTypeID, string Rate, string PurchaseArriveEFIndex, string PurchaseArriveEFDesc)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  d.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6                ");
            sql.AppendLine("AND F1.BillTypeCode = 5 AND F1.BillNo = d.ArriveNo                                                         ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE d.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode = 5 )");

            sql.AppendLine("WHERE D.BillStatus = 2 AND D.CompanyCD = '" + CompanyCD + "' and  isnull(A.ProductCount,0)- isnull(A.InCount,0)-isnull(A.BackCount,0)>0 ");
            sql.AppendLine("and ( ");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F1.FlowNo AND BillID=D.ID AND operateUserId = '" + userInfo.UserID + "')>0  ");
            ...
            sql.AppendLine(" AND B.ID is not null");
        ...
        public static DataTable GetPurchaseStorageDetail(string ProductNo, string ProductName, string FromBillNo, string CompanyCD, int ProviderID, int CurrencyTypeID, string Rate, string StorageEFIndex, string StorageEFDesc)
        {
            ...
            sql.AppendLine("(g.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) or (CHARINDEX('," + userInfo.UserID + ",',','+d.CanViewUser+',')>0 )");
            sql.AppendLine("or (d.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (d.Checker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))");
            sql.AppendLine("or (d.Taker IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            if (ProductNo != null && ProductNo != "")
            {
        ...
        public static bool ConfirmPurchaseReject(PurchaseRejectModel Model, string DetailBackCount, string DetailFromBillNo, string DetailFromLineNo, string DetailProductNo, string length, out string strMsg)
        {
            strMsg = "";
            bool result = true;
            if (isCanDo(Model.ID, "1") || isCanDo(Model.ID, "3"))
            {
        ...
        #region 查询退货列表所需数据
        public static DataTable SelectRejectList(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string RejectNo, string Title, string TypeID, string Purchaser, string FromType, string ProviderID, string BillStatus, string UsedStatus, string DeptID, string ProjectID, string EFIndex, string EFDesc,string time1,string time2,string prodid,string EFIndexP,string EFDescP)
        {
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            string sellpoint = ((XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"]).SelPoint;
            sql.AppendLine("SELECT distinct A.ID, de.productname,Convert(numeric(12," + sellpoint + "),de.unitprice) AS unitprice,");
            sql.AppendLine(" Convert(numeric(12," + sellpoint + "),isnull(de.backcount,0)) productcount,Convert(numeric(12," + sellpoint + "),de.totalprice) totalprice,");
            sql.AppendLine(" A.RejectNo ,isnull(A.Title,'') AS Title ,A.TypeID,isnull(B.TypeName,'') AS TypeName,A.Purchaser,isnull(C.EmployeeName,'')  AS PurchaserName ");
            ...
            sql.AppendLine(" FROM officedba.PurchaseReject AS A   left join officedba.PurchaseRejectdetail de on a.companycd=de.companycd and a.rejectno=de.rejectno                                                                    ");
            sql.AppendLine(" left join officedba.ProductInfo as p on p.companycd=de.companycd and p.id=de.productid ");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=f.FlowNo  AND BillID=A.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT A.ID ,A.RejectNo , B.Fax, B.Tel, B.Mobile, B.ContactName,A.Title ,A.ProviderID,isnull(B.CustName,'') AS ProviderName ,A.FromType,case A.FromType when '0' then '无来源' when '1' then '采购到货单' end AS FromTypeName,A.PayType,isnull(C.TypeName,'') AS PayTypeName,A.MoneyType,isnull(M.TypeName,'') AS  MoneyTypeName ");
        ...
        public static DataTable Details(int ID, string FormType)
        {
            ...
            {
                sql.AppendLine("SELECT   isnull(A.Custom1,'')Custom1,isnull(A.Custom2,'')Custom2,isnull(A.Custom3,'')Custom3,isnull(A.Custom4,'')Custom4,isnull(A.Custom5,'')Custom5,isnull(A.Custom6,'')Custom6,isnull(A.Custom7,'')Custom7,isnull(A.Custom8,'')Custom8,isnull(A.Custom9,'')Custom9,isnull(A.Custom10,'')Custom10,A.ID,A.CompanyCD,A.RejectNo ,isnull(A.SortNo,0) AS SortNo ,isnull(A.FromBillID,0) AS FromBillID,A.ProductID ");
        ...
        public static DataTable SelectPurchaseRejectOverview(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProductID, string Reason, string StartRejectDate, string EndRejectDate)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F ON  c.CompanyCD = F.CompanyCD  AND F.BillTypeFlag = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE + "' ");
            sql.AppendLine("AND F.BillTypeCode = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE_REJECT + "' AND F.BillNo = c.RejectNo          ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE c.ID = F.BillID AND F.BillTypeFlag = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE + "' AND F.BillTypeCode = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE_REJECT + "' )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=f.FlowNo  AND BillID=C.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (c.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (c.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            sql1.AppendLine("LEFT JOIN officedba.FlowInstance AS F ON  c.CompanyCD = F.CompanyCD  AND F.BillTypeFlag = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE + "' ");
            sql1.AppendLine("AND F.BillTypeCode = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE_REJECT + "' AND F.BillNo = c.RejectNo          ");
            sql1.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE c.ID = F.BillID AND F.BillTypeFlag = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE + "' AND F.BillTypeCode = '" + ConstUtil.BILL_TYPEFLAG_PURCHASE_REJECT + "' )");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["RoleRange"].ToString() == "1")
                {
                    sql1.AppendLine("(c.Creator IN  ");
                    sql1.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sql1.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sql1.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    sql1.AppendLine("  WHERE a.ID=b.ID) AND DeptID<>" + userInfo.DeptID.ToString() + " ))  OR  ");
                }
                if (dt.Rows[0]["RoleRange"].ToString() == "2")
                {
                    sql1.AppendLine("(c.Creator IN  ");
                    ...
                    sql1.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sql1.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.DeptID.ToString() + ") b  ");
                    sql1.AppendLine("  WHERE a.ID=b.ID) ))  OR  ");
                }
                if (dt.Rows[0]["RoleRange"].ToString() == "3")
                {
                    sql1.AppendLine("( c.Creator IN  ");
                    sql1.AppendLine("( SELECT ID FROM  officedba.EmployeeInfo ");
                    sql1.AppendLine("  WHERE DeptID IN (SELECT a.ID  ");
                    sql1.AppendLine(" FROM officedba.DeptInfo a,DepartInfo_Children(" + userInfo.BranchID.ToString() + ") b  ");
                    sql1.AppendLine("  WHERE a.ID=b.ID))) OR   ");
                }
            }
            sql1.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=f.FlowNo  AND BillID=C.ID  AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql1.AppendLine("or (c.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (c.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
