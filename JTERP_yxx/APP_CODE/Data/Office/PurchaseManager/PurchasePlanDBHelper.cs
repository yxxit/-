...
namespace XBase.Data.Office.PurchaseManager
{
    public  class PurchasePlanDBHelper
    {
            ...
            , int pageIndex, int pageCount, string OrderBy, string OrderByType, ref int totalCount, string PurchaseArriveEFIndex, string PurchaseArriveEFDesc)
        {
            ...
            parms[9] = new SqlParameter("@DeptID", userInfo.DeptID.ToString());
            parms[10] = new SqlParameter("@BranchID", userInfo.BranchID.ToString());
            parms[11] = new SqlParameter("@UserID", userInfo.UserID.ToString());
            parms[12] = MakeParam("@totalCount", SqlDbType.Int, 0, ParameterDirection.Output, null);
            DataTable dt = SqlHelper.ExecuteStoredProcedure("[officedba].[ProcgetPurApp]", parms);
            totalCount = int.Parse(parms[12].Value.ToString());
            return dt;
        }
        #endregion
        ...
        public static DataTable SelectPurchasePlanPrimary(PurchasePlanModel PurchasePlanM, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=d.FlowNo  AND BillID=A.ID AND operateUserId = '" + userInfo.UserID + "')>0 ");

            sql.AppendLine("or (A.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (A.PlanUserID IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "'))OR (A.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
