...
namespace XBase.Data.Office.PurchaseManager
{
    public class PurchaseOrderDBHelper
    {
            ...
            string CustName, string StartDate, string EndDate)
        {
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=officedba.GetPurchaseOrderInfo.FlowNo   and BillID=officedba.GetPurchaseOrderInfo.ID AND operateUserId = '" + userInfo.UserID + "')>0   or (CHARINDEX('," + userInfo.EmployeeID + ",',','+CanViewUser+',')>0 )");

            sql.AppendLine("or (Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            string CustName, string StartDate, string EndDate, int pageIndex, int pageSize, string OrderBy, ref int totalCount)
        {
            ...
            sql.AppendLine(" AND isopenbill<>'已建单' ");
            //指定命令的SQL文
            comm.CommandText = sql.ToString();
            //执行查询
        ...
        {  
            ...
            parms[10] = new SqlParameter("@DeptID", userInfo.DeptID.ToString());
            parms[11] = new SqlParameter("@BranchID", userInfo.BranchID.ToString());
            parms[12] = new SqlParameter("@UserID", userInfo.UserID.ToString());
            parms[13] = MakeParam("@totalCount", SqlDbType.Int, 0, ParameterDirection.Output, null);         
          
            DataTable dt = SqlHelper.ExecuteStoredProcedure("[officedba].[ProcgetPurPlan]", parms);
            
            totalCount = int.Parse(parms[13].Value.ToString());
            return dt;
        ...
        public static DataTable GetContractDetail(string CompanyCD,string ContractNo,string Title,int ProviderID,int Currency,int pageIndex,int pageSize,string OrderBy,out int totalRecord)
        {
            try
            {
                ...
                sql.AppendLine("        Convert(numeric(14," + userInfo.SelPoint + "),isnull(B.OrderCount,0)) as OrderCount        ,");
                if (userInfo.IsMoreUnit == true)
                {
                    sql.AppendLine(" Convert(numeric(14," + userInfo.SelPoint + "),isnull(B.UsedPrice,0)) as UnitPrice        ,");
                }
                else
                {
                    sql.AppendLine(" Convert(numeric(14," + userInfo.SelPoint + "),isnull(B.UnitPrice,0)) as UnitPrice        ,");
                }
                sql.AppendLine(" Convert(numeric(22," + userInfo.SelPoint + "),isnull(B.TaxPrice,0)) as TaxPrice, Convert(numeric(12," + userInfo.SelPoint + "),isnull(B.Discount,0)) as Discount, Convert(numeric(12," + userInfo.SelPoint + "),isnull(B.TaxRate,0)) as TaxRate  ,  Convert(numeric(22," + userInfo.SelPoint + "),isnull(B.TotalFee,0)) as TotalFee ,   Convert(numeric(22," + userInfo.SelPoint + "),isnull(B.TotalPrice,0)) as TotalPrice      , Convert(numeric(22," + userInfo.SelPoint + "),isnull(B.TotalTax,0)) as TotalTax        , CONVERT(VARCHAR(10),B.RequireDate,21) AS RequireDate , B.ApplyReason   ,B.UsedUnitID     ,Convert(numeric(14," + userInfo.SelPoint + "), isnull(B.UsedUnitCount,0) ) as UsedUnitCount ,  Convert(numeric(14," + userInfo.SelPoint + "),isnull(B.UsedPrice,0)) as UsedPrice   ,ISNULL(FF.CodeName,'') AS UsedUnitName,isnull(HF.TypeName,'') as ColorName                                                                   ");
                ...
                sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=(SELECT TOP 1 FlowNo FROM officedba.FlowInstance AS F WHERE A.ID = BillID AND BillTypeFlag = 6 AND BillTypeCode = 8 )   and BillID=A.ID AND operateUserId = '" + userInfo.UserID + "')>0 ");
                sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Seller IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
                ...
                DataTable dt1 = SqlHelper.ExecuteSql(sql.ToString(), parmList);
        ...
        #region select
        public static DataTable GetPurchaseOrder(PurchaseOrderModel PurchaseOrderM, int pageIndex, int pageCount, string OrderBy, ref int totalCount,string time1,string time2,string prodid)
        {
            SqlCommand comm = new SqlCommand();
            #region SQL文
            StringBuilder sql = new StringBuilder();
            sql.AppendLine(" SELECT     A.ID,de.productname,de.productcount,de.unitprice,de.totalprice totalprice1   ");
            ...
            sql.AppendLine(" FROM officedba.PurchaseOrder AS A   left join officedba.PurchaseOrderdetail de on a.companycd=de.companycd and a.orderno=de.orderno          ");
            sql.AppendLine(" left join officedba.ProductInfo p on p.id=de.ProductID and p.companycd=de.companycd ");
            ...
            sql.AppendLine("(select COUNT(*) from officedba.FlowTaskHistory where FlowNo=d.FlowNo AND BillID=a.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 )");

            sql.AppendLine("or (a.Creator IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')) OR (a.Purchaser IN (SELECT EmployeeID from  officedba.UserInfo where UserID = '" + userInfo.UserID + "')))");
            ...
            if (!string.IsNullOrEmpty(PurchaseOrderM.EFIndexP) && !string.IsNullOrEmpty(PurchaseOrderM.EFDescP))
            {
                sql.AppendLine(" and p.ExtField" + PurchaseOrderM.EFIndexP + " LIKE @EFDescP");
                comm.Parameters.Add(SqlHelper.GetParameterFromString("@EFDescP", "%" + PurchaseOrderM.EFDescP + "%"));
            }
            if (prodid != "")
            {
        ...
        public static DataTable GetPurchaseOrder(PurchaseOrderModel PurchaseOrderM, string OrderBy,string time1,string time2,string prodid)
        {
            SqlCommand comm = new SqlCommand();
            #region SQL文
            StringBuilder sql = new StringBuilder();
...
UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            ...
            sql.AppendLine(", isnull(C.Tel        ,'') AS Tel                              ");
            sql.AppendLine(", isnull(C.ContactName        ,'') AS ContactName                              ");
            sql.AppendLine(", isnull(C.Fax        ,'') AS Fax ,c.Mobile                             ");
        ...
        #region 采购订单明细表操作
        #region insert
        public static SqlCommand InsertPurchaseOrderDetail(PurchaseOrderDetailModel PurchaseOrderDetailM, string OrderNo, string[] strCustomdetail,int j)
        {
            SqlCommand comm = new SqlCommand();
            ...
            //sql.AppendLine("           ,ModifiedUserID)               ");
            //添加扩展项
            sql.Append(GetBillTableCellsDBHelper.insertCustomFilder(strCustomdetail, j));
            ...
            sql.AppendLine("           ,@ArrivedCount                ");
            //扩展项值
            sql.Append(GetBillTableCellsDBHelper.insertCustomvalue(strCustomdetail, j));
            sql.AppendLine("           )                ");
            //sql.AppendLine("           ,@ModifiedDate                 ");
            //sql.AppendLine("           ,@ModifiedUserID)              ");
        ...
        public static DataTable GetPurchaseOrderDetail(string ID)
        {
            ...
            SqlCommand comm = new SqlCommand();
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("SELECT  A.ID,isnull(a.Custom1,'')Custom1,isnull(a.Custom2,'')Custom2,isnull(a.Custom3,'')Custom3,isnull(a.Custom4,'')Custom4,isnull(a.Custom5,'')Custom5,isnull(a.Custom6,'')Custom6,isnull(a.Custom7,'')Custom7,isnull(a.Custom8,'')Custom8,isnull(a.Custom9,'')Custom9,isnull(a.Custom10,'')Custom10   ");
            sql.AppendLine("      , A.OrderNo                                                                          ");
            sql.AppendLine("      , '' as DeRemark                                                               ");
            sql.AppendLine("      , A.CompanyCD                                                                        ");
        ...
        {//取最大时期的含税价，同一天(最大日期)也有多条数据，则取最大ID为最新的一条数据
            ...
            sql.AppendLine("AND F.BillTypeCode = 4 AND F.BillNo = D.OrderNo                                                         ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE d.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F.FlowNo AND BillID=D.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+d.CanViewUser+',')>0 )");
        ...
        public static DataTable SelectHistoryAskPriceShowList(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProductID)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  A.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6                ");
            sql.AppendLine("AND F1.BillTypeCode = 4 AND F1.BillNo = D.OrderNo                                                         ");
        ...
        public static DataTable PurchasePriceAnalyse(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProductID, string StartOrderDate, string EndOrderDate, string CompanyCD)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F ON  C.CompanyCD = F.CompanyCD  AND F.BillTypeFlag = 6                ");
            sql.AppendLine("AND F.BillTypeCode = 4 AND F.BillNo =C.OrderNo                                                         ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE C.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
        ...
        public static DataTable PurchasePriceAnalysePrint(string ProductID, string StartOrderDate, string EndOrderDate, string CompanyCD, string orderBy)
        {
            ...
            sql.AppendLine("left join officedba.PurchaseOrder as H on G.OrderNo =H.OrderNo and G.CompanyCD =H.CompanyCD   ");
            sql.AppendLine("left join  officedba.ProviderInfo as I on H.ProviderID =I.id and H.CompanyCD =I.CompanyCD  ");
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  H.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6                ");
            sql.AppendLine("AND F1.BillTypeCode = 4 AND F1.BillNo = H.OrderNo                                                         ");
            sql.AppendLine(" AND F1.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE H.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F1.FlowNo  AND BillID=a.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+d.CanViewUser+',')>0 ) ");
        ...
        public static DataTable PurchaseHisPriceQuery(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProductID, string StartConfirmDate, string EndConfirmDate, string CompanyCD)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.CodeUnitType AS C ON A.CompanyCD = C.CompanyCD AND A.UnitID=C.ID                    ");
            sql.AppendLine("LEFT JOIN officedba.PurchaseOrder AS D ON A.CompanyCD = D.CompanyCD AND A.OrderNo=D.OrderNo             ");
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F ON  D.CompanyCD = 1.CompanyCD  AND F.BillTypeFlag = 6                ");
            sql.AppendLine("AND F.BillTypeCode = 4 AND F.BillNo = D.OrderNo                                                         ");
            sql.AppendLine(" AND F.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE D.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
        ...
        public static DataTable PurchaseHisPriceQueryPrint(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ProductID, string StartConfirmDate, string EndConfirmDate, string CompanyCD)
        {
            ...
            sql.AppendLine(" WHERE 1=1");
            sql.AppendLine("and  ((D.Creator IN  ");
            XBase.Common.UserInfoUtil userInfo = (XBase.Common.UserInfoUtil)XBase.Common.SessionUtil.Session["UserInfo"];
            DataTable dt = XBase.Data.Office.SystemManager.RoleInfoDBHelper.GetRoleInfoByUserID(userInfo.UserID);
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        public static DataTable SelectPurchaseOrder(string CompanyCD, string BillStatus, string StartDate, string EndDate, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS t ON  A.CompanyCD = t.CompanyCD  AND t.BillTypeFlag = 6                ");
            sql.AppendLine("AND t.BillTypeCode = 4 AND t.BillNo = A.OrderNo                                                         ");
            sql.AppendLine(" AND t.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE A.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=t.FlowNo AND BillID=a.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 ) ");
        ...
        public static DataTable SelectPurchaseOrder(string CompanyCD,string OrderBy, string BillStatus, string StartDate, string EndDate)
        {
            ...
            sql.AppendLine("LEFT JOIN officedba.FlowInstance AS F1 ON  A.CompanyCD = F1.CompanyCD  AND F1.BillTypeFlag = 6                ");
            sql.AppendLine("AND F1.BillTypeCode = 4 AND F1.BillNo = A.OrderNo                                                         ");
            sql.AppendLine(" AND F1.ID=(SELECT max(ID) FROM officedba.FlowInstance AS F WHERE A.ID = F.BillID AND F.BillTypeFlag = 6 AND F.BillTypeCode =4 )");
            ...
            sql.AppendLine(" (select COUNT(*) from officedba.FlowTaskHistory where FlowNo=F1.FlowNo  AND BillID=a.ID AND operateUserId = '" + userInfo.UserID + "')>0  or (CHARINDEX('," + userInfo.EmployeeID + ",',','+a.CanViewUser+',')>0 ) ");
        ...
        public static DataTable GetPurchasePrice(string CompanyCD, string ProductID, string StartDate, string EndDate, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            sql.AppendLine("(select COUNT(*) from officedba.FlowTaskHistory where FlowNo=[officedba].[V_PurchasePriceQuery].FlowNo  AND BillID=[officedba].[V_PurchasePriceQuery].ID AND operateUserId = '" + userInfo.UserID + "')>0 ");
        ...
        public static DataTable GetPurByProduct(string CompanyCD, string ProductID, string StartDate, string EndDate, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            ...
            string sql = "SELECT ProductID,ProductNo,ProductName,SUM(ProductCount) AS ProductCount,SUM(TotalFee) AS TotalFee FROM "
                         + " (SELECT     A.ProductID,A.CompanyCD, B.OrderDate, C.ProdNo AS ProductNo, C.ProductName, A.ProductCount, A.TotalFee*B.DisCount*B.Rate/100 AS TotalFee"
        ...
        //在途物品查询
        public static DataTable ProdOnRoadQry(string CompanyCD, string ProductID, string ProviderID, string StartDate, string EndDate
            , int pageIndex, int pageSize, string OrderBy, ref int TotalCount)
        {
            try
            {
                ...
                TotalCount = int.Parse(parms[8].Value.ToString());
                return dt;
            }
            catch (Exception ex)
            {
