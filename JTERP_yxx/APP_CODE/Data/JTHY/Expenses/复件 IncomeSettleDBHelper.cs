   ...
   public  class IncomeSettleDBHelper
   {
       ...
       public static DataTable SearchIncomeSetle(string ProviderName, string DateBegin, string DateEnd, int iscount)
       {
           ...
           try
           {
               ...
               sql1 += " p.CustName from jt_cgdh a";
               ...
               string sql2 = @"select  c.id,a.SendNo as ArriveNo,convert(varchar(12), a.CreateDate,23) CreateDate,c.SttlTotalPrice as sttlTotalPrice,c.SttlCount as sttlCounts,
                            a.ProviderID,c.ProductId,c.InCost as TaxPrice,c.ProductCount ,p.CustName 
                            from  jt_xsfh a  
               ...
               else
               {
                   ...
                   sql2 += "  and  c.SttlCount<c.ProductCount and   a.CompanyCD = '" + CompanyCD + "'";
               }
               if (ProviderName != "")
               {
