...
namespace XBase.Data.Common
{
    ...
    public class CodePublicTypeDBHelper
    {
        ...
        /// <returns></returns>
        public static DataTable GetLimitAreaInfoForDrp()
        {
            //定义查询SQL变量
            StringBuilder searchSql = new StringBuilder();
            //定义查询语句
            searchSql.AppendLine("SELECT distinct cInvDefine4                     ");

            searchSql.AppendLine("FROM                          ");
            searchSql.AppendLine("Inventory  where cInvDefine4 !='' ");
            ...
            //执行查询并返回的查询到的分类信息
            return YYSqlHelper.ExecuteSql(searchSql.ToString());
        }
