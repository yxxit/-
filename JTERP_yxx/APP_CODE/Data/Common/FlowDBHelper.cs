...
namespace XBase.Data.Common
{
    public class FlowDBHelper
    {
        ...
        public static DataSet GetFlow(string CompanyCD, int BillTypeFlag, int BillTypeCode, int BillID)
        {
            ...
            sql[0] = "select ID,FlowNo,FlowName,isFlowApply=0,isnull(IsMobileNotice,1) as IsMobileNotice from officedba.Flow where CompanyCD='" + CompanyCD + "' and BillTypeFlag=" + BillTypeFlag + " and BillTypeCode=" + BillTypeCode + " and UsedStatus=2 order by id desc ";
        ...
        /// <returns></returns>
        public static string FlowApplyAdd(string CompanyCD, string @FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string BillNo, string PageUrl, string ApplyUserId, string ModifiedUserID, string nextperson)
        {
            try
            {
                #region 设置参数
                ...
                param[9] = paramRetVal;
                param[10] = SqlHelper.GetParameter("@Receiver", nextperson); 
                #endregion
        ...
        /// <param name="ModifiedUserID"></param>
        /// <returns></returns>
        public static string FlowApprovalAdd(string CompanyCD, string @FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string State, string Note, string ModifiedUserID, int ModifiedEmployeeID, string nextperson)
        {
            try
            {
                #region 设置参数
        ...
        public static DataTable GetNextOperateList(string CompanyCD, int BillTypeFlag, int BillTypeCode, int BillID)
        {
            ...
            searchSql.AppendLine("				select max(StepNo) from officedba.FlowTaskList");//old: select StepNo from officedba.FlowTaskList
