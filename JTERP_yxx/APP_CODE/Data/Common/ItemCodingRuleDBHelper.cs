...
namespace XBase.Data.Common
{
    ...
    public class ItemCodingRuleDBHelper
    {
        ...
        #region 根据编码类型ID获取编码规则数据

        /// <summary>
        /// 根据编码类型ID获取编码规则数据
        /// </summary>
        /// <param name="codeID">编码类型</param>
        /// <param name="typeid">类型id</param>
        /// <returns></returns>
        public static DataTable GetCodingRuleInfoWithID(string codeID,string typeID,string CompanyCD,bool iscopy)
        {
            //定义查询SQL变量
            StringBuilder searchSql = new StringBuilder();
            //定义查询语句
            ...
            searchSql.AppendLine("	CodingType = @codeID     and  ");
            searchSql.AppendLine("	ItemTypeID = @typeID       ");

            //设置参数
            SqlParameter[] param = new SqlParameter[3];
            //编码规则ID
            param[0] = SqlHelper.GetParameter("@CompanyCD", CompanyCD);
            param[1] = SqlHelper.GetParameter("@CodeID", codeID);
            param[2] = SqlHelper.GetParameter("@typeID", typeID);
            ...
            if (data.Rows.Count > 0)
            {
                ...
                if (iscopy)
                {
                    ...
                    updateSql.AppendLine("    SET                          ");
                    updateSql.AppendLine("       LastNo = @LastNo          ");
                    updateSql.AppendLine("  WHERE CompanyCD = @CompanyCD  and   ");
                    updateSql.AppendLine("	CodingType = @codeID     and  ");
                    updateSql.AppendLine("	ItemTypeID = @typeID       ");
                    //设置参数
                    SqlParameter[] updatePram = new SqlParameter[4];
                    updatePram[0] = SqlHelper.GetParameter("@CompanyCD", CompanyCD);
                    //编码规则ID
                    updatePram[1] = SqlHelper.GetParameter("@CodeID", codeID);
                    //最大流水号
                    updatePram[2] = SqlHelper.GetParameter("@LastNo", updateNo);

                    updatePram[3] = SqlHelper.GetParameter("@typeID", typeID);
                    //执行更新
                    updateFlag = SqlHelper.ExecuteTransSql(updateSql.ToString(), updatePram) > 0 ? true : false;
                    if (updateFlag)
                    {
