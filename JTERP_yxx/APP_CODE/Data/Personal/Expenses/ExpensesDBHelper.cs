...
namespace XBase.Data.Personal.Expenses
{
    public class ExpensesDBHelper
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select exp.ID,exp.ExpCode,exp.Title,cft.Flag,cft.CodeName,exp.Applyor,e1.EmployeeName as ApplyorName,");
            ...
            strSql.Append(" left join officedba.CodeFeeType cft on exp.ExpType=cft.Flag");
            ...
            if (expApplyModel.ExpType != 0)
            {
                strSql.Append(" and exp.ExpType=" + expApplyModel.ExpType);
            }
            if (expApplyModel.IsReimburse != null)
            {
                strSql.Append(" and exp.IsReimburse='"+expApplyModel.IsReimburse+"'");
            }
            //if (expApplyModel.CustID != null)
            //{
            //    strSql.Append(" and exp.CustID='" + expApplyModel.CustID + "'");
            //}
            if (FlowStatus != null)
            {
                if (FlowStatus == "0")
        ...
        /// <param name="pageIndex"></param>
        /// <param name="pageCount"></param>
        /// <param name="ID">选项编号</param>
        /// <returns></returns>
        public static DataTable GetFeeInfoTableByCheckcondition(string ID, string CompanyCD)
        {
            string sql = "";

            sql += "select f.CodeName,f.ID,f.Flag,f.Description,c.TypeName";
            sql += " FROM officedba.CodeFeeType f ";
            sql += " inner join officedba.CodePublicType AS c ON f.Flag = c.ID AND f.CompanyCD =c.CompanyCD ";
            sql += " where f.CompanyCD=@CompanyCD and f.UsedStatus =1";//检索未被停用的物品
            string allid = "";
