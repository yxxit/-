...
namespace XBase.Data.Personal.Expenses
{
    public class ReimbursementDBHelper
    {
        ...
        public static DataTable GetReimbursementList(ReimbursementModel reimbModel, int empid, DateTime? ReimbDate1, string FlowStatus, decimal? ReimbAmount1, int pageIndex, int pageCount, string ord, ref int TotalCount)
        {
            ...
            strSql.Append(" reimb.UserReimbID,e2.EmployeeName as UserReimbName,cft.CodeName,cft.Flag,");
            strSql.Append(" convert(varchar(100), reimb.ReimbDate, 23) as ReimbDate, fd.ReimbAmount,fd.RestoreAmount,");
            ...
            strSql.Append(" left join officedba.FeeReturnDetail fd on reimb.ID=fd.ReimbID");
            strSql.Append(" left join officedba.CodeFeeType cft on cft.ID=fd.FeeNameID");
            ...
            if (reimbModel.ExpType != 0)
            {
                strSql.Append(" and cft.Flag =" + reimbModel.ExpType);
            }
            if (reimbModel.FeeName != null)
            {
                strSql.Append(" and cft.CodeName like '%" + reimbModel.FeeName + "%'");
            }
            if (FlowStatus != null)
            {
                if (FlowStatus == "0")
