...
namespace XBase.Data.DBHelper
{
    ...
    public class SqlHelper
    {
        ...
        /// <param name="sql">命令集</param>
        /// <returns>操作的记录数</returns>
        public static bool ExecuteWithSQL(string sql)
        {
            //创建连接
            using (SqlConnection conn = new SqlConnection(_connectionString))
            {
        ...
        /* Add by lp 2014-09-11 end */
        #region 构造一个新的SqlParameter,并初始化
        public static SqlParameter GetNewSqlParameter(string Key, object Value, SqlDbType type)
        {
            SqlParameter temp = new SqlParameter();
            temp = new SqlParameter(Key, type);
            temp.Value = Value;
        ...
        // public static DataSet ExecuteDataset_fast(string connectionString, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        public static DataSet ExecuteDataset_fast(CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                connection.Open();
                // 调用指定数据库连接字符串重载方法.
                return ExecuteDataset_fast(connection, commandType, commandText, commandParameters);
            }
        }
        ...
        /// <returns>返回一个包含结果集的DataSet</returns>
        public static DataSet ExecuteDataset_fast(SqlConnection connection, CommandType commandType, string commandText, params SqlParameter[] commandParameters)
        {
            if (connection == null) throw new ArgumentNullException("connection");
            // 预处理
            SqlCommand cmd = new SqlCommand();
            bool mustCloseConnection = false;
            PrepareCommand_fast(cmd, connection, (SqlTransaction)null, commandType, commandText, commandParameters, out mustCloseConnection);
        ...
        private static void PrepareCommand_fast(SqlCommand command, SqlConnection connection, SqlTransaction transaction, CommandType commandType, string commandText, SqlParameter[] commandParameters, out bool mustCloseConnection)
        {
