    ...
    public class ProductInfoBus
    {
        ...
        public static DataTable GetProductInfoTableBycondition1(ProductInfoModel model, string QueryID, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount, string gettype)
        {
            try
            {
                ...
                {
                    return ProductInfoDBHelper.GetProductInfoTableBycondition1(model, QueryID, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount, gettype);
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataTable GetProductInfoTableBycondition2(ProductInfoModel model, string QueryID, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                ...
                {
                    return ProductInfoDBHelper.GetProductInfoTableBycondition2(model, QueryID, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount);
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataTable GetProductInfoTableBycondition3(ProductInfoModel model, string QueryID, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                ...
                {
                    return ProductInfoDBHelper.GetProductInfoTableBycondition3(model, QueryID, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount);
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataTable GetProductInfoTableBycondition4(ProductInfoModel model, string QueryID, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                ...
                {
                    return ProductInfoDBHelper.GetProductInfoTableBycondition4(model, QueryID, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount);
                }
            }
            catch (Exception ex)
            {
        ...
        public static DataTable GetProductInfoTableByCheckcondition(string strid, string issc)
        {
            try
            {
                UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
                string CompanyCD = userInfo.CompanyCD;
                return ProductInfoDBHelper.GetProductInfoTableByCheckcondition(strid, CompanyCD, issc);
            }
            catch (Exception ex)
            {
                return null;
       ...
       // public static DataTable GetProductInfo(ProductInfoModel Model, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
...

        {
            try
            {
              //  return ProductInfoDBHelper.GetProductInfo(Model, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount);
                return ProductInfoDBHelper.GetProductInfo(Model, pageIndex, pageCount, OrderBy, ref totalCount);
            }
            catch (Exception ex)
            {
                return null;
        ...
        /// <param name="CompanyCD"></param>
        /// <returns></returns>
        public static string Existss(string ProductID)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            string CompanyCD = userInfo.CompanyCD;
            string[] IdS = null;
            ...
            {
                DataTable dt = ProductInfoDBHelper.Exists(IdS[i].ToString(), CompanyCD);
                if (dt.Rows.Count == 1)
                {
                    //Reason = "已经被" + dt.Rows[0]["billname"].ToString() + "中引用单据编号为" + dt.Rows[0]["BillNo"].ToString();
                    Reason = "已经被其他单据引用，无法删除！" ;
                    return Reason;
                }
                else if (dt.Rows.Count > 1)
                {
                    //Reason = "已经被" + dt.Rows[0]["billname"].ToString() + "中" + dt.Rows[0]["BillNo"].ToString() + "等多个模块、多个单据引用！";
                    Reason = "已经被其他单据引用，无法删除！";
                    return Reason;
                }
            }
        ...
        ////传入参数：id 和 公司id
        ////传出参数：true or false
        //public static Boolean JudgePY(string ID, string name,string companyCD)
        //{
        //    string[] ids = null;
        ...
        //    for (int i = 0; i < ids.Length; i++)
        //    {
        //        DataTable dt = ProductInfoDBHelper.PYShort(ids[i].ToString(), companyCD);
        //        if (dt.Rows[0][0].ToString() == "")
        //        {
        //            int no = Int32.Parse(ids[i].ToString());
        //            string PYShort = "";
        //            PYShort = XBase.Common.PYShortUtil.GetPYString(names[i].ToString());
        //            reason = ProductInfoDBHelper.UpdateProductPYShort(PYShort, no, companyCD);
        //        }
        //    }
        //        return reason;
        //}
        ...
        public static void dr_to_proinfo(DataRow dr, string companycd)//2010-10-20
        {
            ProductInfoDBHelper.dr_to_proinfo(dr, companycd);
        }
        public static DataSet ReadExcel_ds(string FilePath, string companycd)//2010-10-20
        {
            return ProductInfoDBHelper.ReadExcel_ds(FilePath, companycd);
        }
        ...
        public static DataTable GetBatch(string ProdNo, string ProductID, string ProdName, string batch, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                return ProductInfoDBHelper.GetBatch(ProdNo, ProductID, ProdName, batch, pageIndex, pageCount, OrderBy, ref totalCount);
            }
            catch (System.Exception ex)
            {
                throw ex;
        ...
        public static DataTable GetPnumber(string TypeID, string Name, string Remark, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                return ProductInfoDBHelper.GetPnumber(TypeID, Name, Remark, pageIndex, pageCount, OrderBy, ref totalCount);
            }
            catch (System.Exception ex)
            {
                throw ex;
        ...
        public static DataTable GetCodePublic(string TypeID, int pageIndex, int pageCount, string OrderBy, ref int totalCount)
        {
            try
            {
                return ProductInfoDBHelper.GetCodePublic(TypeID, pageIndex, pageCount, OrderBy, ref totalCount);
            }
            catch (System.Exception ex)
            {
                throw ex;
        ...
        {
            return ProductInfoDBHelper.GetCodeType(codename, compid);
        }
        #endregion
        ...
        /// <returns></returns>
        public static DataTable GetProductInfoTableByCheckcondition(string strid, string StorageID, string BranchID, string DeptID)
        {
            try
            {
                UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
                string CompanyCD = userInfo.CompanyCD;
                //bool IsBatchNo = userInfo.IsBatchNo;//是否启用批次
                bool IsBatchNo = false;
                bool IsBarCode = userInfo.IsBarCode;//是否启用条码
                return ProductInfoDBHelper.GetProductInfoTableByCheckcondition(strid, CompanyCD, StorageID, BranchID, DeptID, IsBatchNo, IsBarCode);
            }
            catch (Exception ex)
            {
                return null;
