...
namespace XBase.Business.Office.PurchaseManager
{
    ...
    public class PurchaseContractBus
    {
        ...
        #region 查询采购合同列表所需数据
        public static DataTable SelectPurchaseContract(int pageIndex, int pageCount, string orderBy, ref int TotalCount, string ContractNo1, string Title, string TypeID, string DeptID, string Seller, string FromType, string ProviderID, string BillStatus, string UsedStatus, string EFIndex ,string EFDesc,string productname,string EFIndexP,string EFDescP )
        {
            try
            {
                return PurchaseContractDBHelper.SelectContractList(pageIndex, pageCount, orderBy, ref TotalCount, ContractNo1, Title, TypeID, DeptID, Seller, FromType, ProviderID, BillStatus, UsedStatus, EFIndex, EFDesc,productname,EFIndexP,EFDescP);
            }
            catch (Exception ex)
            {
                throw ex;
