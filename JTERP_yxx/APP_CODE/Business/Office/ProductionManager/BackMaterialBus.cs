...
namespace XBase.Business.Office.ProductionManager
{
    public class BackMaterialBus
    {
        ...
        /// <returns></returns>
        public static bool InsertBackMaterial(BackMaterialModel model, Hashtable ht, out string ID, string[] strCustomdetail)
        {
            ...
            try
            {
                ...
                succ = BackMaterialDBHelper.InsertBackMaterial(model, ht, loginUserID, out ID, strCustomdetail);
                if (!succ)
        ...
        /// <returns></returns>
        public static bool UpdateBackMaterialInfo(BackMaterialModel model, Hashtable ht, string[] strCustomdetail)
        {
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            try
            {
                ...
                succ = BackMaterialDBHelper.UpdateBackMaterialInfo(model, ht, loginUserID, strCustomdetail);
                if (!succ)
                    logModel.Remark = ConstUtil.LOG_PROCESS_FAILED;
                else
                    logModel.Remark = ConstUtil.LOG_PROCESS_SUCCESS;
        ...
        public static DataTable GetBackMaterialListBycondition(BackMaterialModel model, string ReceiveDateStart, string ReceiveDateEnd, int BillTypeFlag, int BillTypeCode, int FlowStatus, string EFIndex, string EFDesc, int pageIndex, int pageCount, string OrderBy, ref int totalCount,string time1,string time2,string prodid,string storageid)
        {
            try
            {
                return BackMaterialDBHelper.GetBackMaterialListBycondition(model, ReceiveDateStart, ReceiveDateEnd, BillTypeFlag, BillTypeCode, FlowStatus, EFIndex, EFDesc, pageIndex, pageCount, OrderBy, ref totalCount,time1,time2,prodid,storageid);
            }
            catch (System.Exception ex)
            {
                throw ex;
