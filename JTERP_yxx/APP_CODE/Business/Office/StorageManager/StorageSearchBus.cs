...
namespace XBase.Business.Office.StorageManager
{
    public class StorageSearchBus
    {
        ...
        {
            return StorageSearchDBHelper.GetProductStorageTableBycondition(model, pdtModel, ProductCount1, EFIndex, EFDesc, pageIndex, pageCount, ord,BatchNo, ref TotalCount,isbatch);
        }
        ...
        //获取产品配置
        public static DataTable getfloorbatch(string proid,string batchno)
        {
            try
            {
                string CompanyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                return StorageSearchDBHelper.getfloorbatch(proid,batchno,CompanyCD);
            }
            catch (Exception ex)
            {
                throw ex;
