...
namespace XBase.Business.Office.StorageManager
{
    public class StorageOutOtherBus
    {
        ...
        {
            return StorageOutOtherDBHelper.GetStorageOutOtherTableBycondition(model, timeStart, timeEnd, EFIndex, EFDesc, pageIndex, pageCount, ord, ref TotalCount,time1,time2,prodid);
        }
        ...
        {
            return StorageOutOtherDBHelper.GetStorageOutOtherTableBycondition(model, IndexValue, TxtValue, timeStart, timeEnd,BatchNo, orderby,prodid,time1,time2);
        }
        #endregion
        ...
        /// <returns></returns>
        public static bool InsertStorageOutOther(StorageOutOtherModel model, Hashtable ht, List<StorageOutOtherDetailModel> modelList, out int IndexIDentity, string[] strCustomdetail)
        {
            IndexIDentity = 0;
            //获取登陆用户信息
            ...
            {
                //执行更新
                isSucc = StorageOutOtherDBHelper.InsertStorageOutOther(model, ht, modelList, out IndexIDentity, strCustomdetail);
        ...
        /// <param name="modelList"></param>
        /// <returns></returns>
        public static bool UpdateStorageOutOther(StorageOutOtherModel model, Hashtable ht, List<StorageOutOtherDetailModel> modelList, string[] strCustomdetail)
        {
            //获取登陆用户信息
            UserInfoUtil userInfo = (UserInfoUtil)SessionUtil.Session["UserInfo"];
            //设置公司代码
            ...
            {
                //执行更新
                isSucc = StorageOutOtherDBHelper.UpdateStorageOutOther(model, ht, modelList, strCustomdetail);
            }
            catch (Exception ex)
            {
                //输出日志
        ...
        {
            return StorageOutOtherDBHelper.CheckIsUnfirm(model);
        }
        #endregion
        #region 取消确认
        public static bool unconfirm(StorageOutOtherModel model)
        {
            ...
            {
                //执行更新
                isSucc = StorageOutOtherDBHelper.unconfirm(model);
            }
            catch (Exception ex)
            {
                //输出日志
