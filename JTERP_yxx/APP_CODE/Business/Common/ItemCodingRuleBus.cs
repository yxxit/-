...
namespace XBase.Business.Common
{
    ...
    public class ItemCodingRuleBus
    {
        ...
        /// <returns></returns>
        public static string GetCodeValue_jt(string codeID, string table, string column)
        {
            //获取单据编号
            string code = GetCodeValue(codeID);
            //校验是否已经存在
            bool isExist = PrimekeyVerifyBus.CheckCodeUniq_jt(table, column, code);
            //已经存在时
            if (!isExist)
            {
        ...
        /// <param name="column">列名</param>
        /// <returns></returns>
        public static string GetCodeValue(string codeID,string typeid, string table, string column,bool iscopy)
        {
            //获取单据编号
            string code = GetCodeValue(codeID,typeid,iscopy);
            //校验是否已经存在
            bool isExist = PrimekeyVerifyBus.CheckCodeUniq(table, column, code);
            //已经存在时
            if (!isExist)
            {
                return GetCodeValue(codeID,typeid,table, column,iscopy);
            }
            //不存在时
            else
            {
        ...
        /// 
        /// <returns></returns>
        public static string GetCodeValue(string codeID,string typeID,bool iscopy)
        {
            string companyCD = string.Empty;
            ...
            string codeValue = "";
            //获取编码规则详细设置内容
            DataTable ruleData = ItemCodingRuleDBHelper.GetCodingRuleInfoWithID(codeID,typeID,companyCD,iscopy);
            //数据存在时进行Code生成操作
            if (ruleData != null && ruleData.Rows.Count > 0)
            {
                //获取编码示例
