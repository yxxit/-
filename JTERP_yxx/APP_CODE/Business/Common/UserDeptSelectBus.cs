...
namespace XBase.Business.Common
{
    public class UserDeptSelectBus
    {
        ...
        //zyy 修改  2012-07-25
        public static DataTable GetUserInfo(string ShowType, string OprtType,string IsShow,string EmpType)
        {
            try
            {
                string companyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                ...
                if (IsShow == "1")
                {
                    if (EmpType == "1")
                    {
                        dt = UserDeptSelectDBHelper.GetUserInfoDiff(companyCD);
                    }
                    else
                    {
                    ...
                    if (ShowType == SHOWTYPE_CODE_USERS && OprtType == OPRT_CODE_SELECT)
                    {
                        foreach (DataRow rows in dt.Rows)
                        {
                            if (rows["Flag"].ToString().Trim() == "3")
                            {
                                if (EmpType == "1")
                                {
                                    if (!string.IsNullOrEmpty(rows["CustName"].ToString()))
                                    {
                                        rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "_" + rows["CustName"].ToString() + "'   >" + rows["EmployeesName"].ToString() + "_" + rows["CustName"].ToString() + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>已离职</font>";
                                    }
                                    else
                                    {
                                        rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "_本公司" + "'   >" + rows["EmployeesName"].ToString() + "_本公司" + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>已离职</font>";
                                    }
                                }
                                else
                                {
                                    rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "'   >" + rows["EmployeesName"].ToString() + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>已离职</font>";
                            ...
                            else
                            {
                                if (EmpType == "1")
                                {
                                    if (!string.IsNullOrEmpty(rows["CustName"].ToString()))
                                    {
                                        rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "_" + rows["CustName"].ToString() + "|" + rows["DeptID"].ToString() + "|" + rows["DeptName"].ToString() + "'   >" + rows["EmployeesName"].ToString() + "_" + rows["CustName"].ToString();
                                    }
                                    else
                                    {
                                        rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "_本公司" + "|" + rows["DeptID"].ToString() + "|" + rows["DeptName"].ToString() + "'   >" + rows["EmployeesName"].ToString() + "_本公司";
                                    }
                                }
                                else
                                {
                                    rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "|" + rows["DeptID"].ToString() + "|" + rows["DeptName"].ToString() + "'   >" + rows["EmployeesName"].ToString();
                                }
                            }
                        }
                    }
                        ...
                        foreach (DataRow rows in dt.Rows)
                        {
                            if (rows["Flag"].ToString().Trim() == "3")
                            {
                                if (EmpType == "1")
                                {
                                    if (!string.IsNullOrEmpty(rows["CustName"].ToString()))
                                    {
                                        rows["EmployeesName"] = rows["EmployeesName"].ToString() + "_" + rows["CustName"].ToString() + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>已离职</font>";
                                    }
                                    else
                                    {
                                        rows["EmployeesName"] = rows["EmployeesName"].ToString() + "_本公司" + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>已离职</font>";
                                    }
                                }
                                else
                                {
                    ...
                    !string.IsNullOrEmpty(OprtType))
                {
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        if (ShowType == SHOWTYEP_CODE_SELECT_DEPT && OprtType == OPRT_CODE_SELECT)
                        {
                            foreach (DataRow rows in dt.Rows)
                            {
                                rows["DeptName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();'  value='" + rows["ID"] + "|" + rows["DeptName"].ToString() + "'   >" + rows["DeptName"].ToString();
                            }
        ...
        //by:zyy
        public static DataTable GetCulture(string ShowType)
        {
            try
            {
                string companyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                DataTable dt = UserDeptSelectDBHelper.GetCulture(companyCD);
                if (dt != null && dt.Rows.Count > 0)
                {
                    if (ShowType == OPRT_CODE_SELECT)
                    {
                    ...
                    else if (ShowType == OPRT_CODE_SELECTS)
                    {
                        foreach (DataRow rows in dt.Rows)
                        {
                            rows["DeptName"] = "<input type='checkbox' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["DeptName"].ToString() + "'   >" + rows["DeptName"].ToString();
                        }
                    }
                }
                return dt;
        ...
        //by:zyy
        public static DataTable GetCultureInfo(string SupperTypeID)
        {
            try
            {
                string companyCD = ((UserInfoUtil)SessionUtil.Session["UserInfo"]).CompanyCD;
                ...
                dt = UserDeptSelectDBHelper.GetCultureInfo(companyCD, SupperTypeID);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow rows in dt.Rows)
                    {
                        ...
                        rows["EmployeesName"] = "<input type='radio' name='select'  id='chk_" + rows["ID"] + "' onclick='getUserOrdept();' value='" + rows["ID"] + "|" + rows["EmployeesName"].ToString() + "'   >" + rows["EmployeesName"].ToString();
