...
namespace XBase.Business.Common
{
    public class FlowBus
    {
        ...
        /// <returns></returns>
        public static string FlowApplyAdd(string CompanyCD, string FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string BillNo, string PageUrl, string ApplyUserID, string ModifiedUserID, int MsgRemind, string nextperson)
        {
            try
            {
                string strRetVal = "";
                strRetVal = FlowDBHelper.FlowApplyAdd(CompanyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID, BillNo, PageUrl, ApplyUserID, ModifiedUserID, nextperson);
                if (!String.IsNullOrEmpty(strRetVal))
                {
                    ...
                    if (int.Parse(retArray[0].ToString()) > 0)
                    {
                        if (MsgRemind > 0)
                        {
                            ...
                            //提交审批成功后发送站内消息提醒  bsd 2014-04-17
                            SendNoticeMsg(CompanyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID, BillNo, PageUrl, ApplyUserID, ModifiedUserID, nextperson);
                        }
                    }
                }
                return strRetVal;
        ...
        /// <param name="ApplyUserID"></param>
        /// <param name="ModifiedUserID"></param>
        /// <param name="nextperson"></param>
        private static void SendNoticeMsg(string CompanyCD, string FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string BillNo, string PageUrl, string ApplyUserID, string ModifiedUserID, string nextperson)
        {
            string billtype = "";
            if (BillTypeFlag == 30 && BillTypeCode == 1)
            {
                billtype = "销售合同";
            }
            else if (BillTypeFlag == 30 && BillTypeCode == 2)
            {
                billtype = "采购合同";
            }
            else
            ...
            string strMaxID = "select isnull(max(id),0)+1 as maxid from  officedba.MessageInputBox ";
            DataTable dtMaxID = SqlHelper.ExecuteSql(strMaxID);
            string MaxID = dtMaxID.Rows[0]["maxid"].ToString();

            //将提交人用户编码转换为职员编码
            string strEmployee = "select isnull(employeeid,0) employeeid  from officedba.userinfo where companycd='"+CompanyCD+"' and userid='"+ApplyUserID+"'";
            DataTable dtEmployee = SqlHelper.ExecuteSql(strEmployee);
            string EmployeeID = "0";
            if (dtEmployee.Rows.Count > 0)
            {
                EmployeeID = dtEmployee.Rows[0]["employeeid"].ToString();
            }
            string strSend = "insert into officedba.MessageInputBox"+
            "    (CompanyCD,Title,Content,FromID,SendUserID,ReceiveUserID,Status,ReadDate,CreateDate,MsgURL,ModifiedDate,ModifiedUserID)"+
            "    VALUES('"+CompanyCD+"','您有新的待审批任务','单据编号："+BillNo+"，单据类型："+billtype+"，请尽快处理',"+
            "    '" + MaxID + "','"+EmployeeID+"','" + nextperson + "','0','" + DateTime.Now.ToString() + "'," +
            "    '" + DateTime.Now.ToString() + "','','" + DateTime.Now.ToString() + "','" + ApplyUserID + "')";
            SqlHelper.ExecuteSql(strSend);

           

            strSend = "insert into officedba.MessageSendBox" +
            "    (CompanyCD,Title,Content,SendUserID,ReceiveUserID,Status,ReadDate,CreateDate,MsgURL,ModifiedDate,ModifiedUserID)" +
            "    VALUES('" + CompanyCD + "','您有新的待审批任务','单据编号：" + BillNo + "，单据类型：" + billtype + "，请尽快处理'," +
            "   '" + EmployeeID + "','" + nextperson + "','0','" + DateTime.Now.ToString() + "'," +
            "    '" + DateTime.Now.ToString() + "','','" + DateTime.Now.ToString() + "','" + ApplyUserID + "')";
            SqlHelper.ExecuteSql(strSend);
        ...
        /// <param name="ModifiedUserID"></param>
        /// <returns></returns>
        public static string FlowApproveAdd(string CompanyCD, string FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string BillNo, string State, string Note, string ModifiedUserID, int ModifiedEmployeeID, int MsgRemind, string nextperson)
        {
            try
            {
                string strRetVal = "";
                strRetVal = FlowDBHelper.FlowApprovalAdd(CompanyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID, State, Note, ModifiedUserID, ModifiedEmployeeID, nextperson);
                if (!String.IsNullOrEmpty(strRetVal))
                {
                    string[] retArray = strRetVal.Split('|');
                    if (int.Parse(retArray[0].ToString()) > 0)
                    {
                        if (MsgRemind > 0)
                        {
                            ...
                            //审批成功后发送站内短信 bsd 2014-04-17
                            SendNoticeApproveMsg(CompanyCD, FlowNo, BillTypeFlag, BillTypeCode, BillID, State, Note, ModifiedUserID, ModifiedEmployeeID, nextperson);
                        }
                    }
                }
                return strRetVal;
        ...
        private static void SendNoticeApproveMsg(string CompanyCD, string FlowNo, int BillTypeFlag, int BillTypeCode, int BillID, string State, string Note, string ModifiedUserID, int ModifiedEmployeeID, string nextperson)
        {
            string billtype = "";
            if (BillTypeFlag == 30 && BillTypeCode == 1)
            {
            ...
            string ApplyUserID = "";//流程发起人UserID
            string BillNo = "";
            string strFlowInstance = "select top 1 BillNo,ApplyUserId  from officedba.FlowInstance"+
           " where companycd='"+CompanyCD+"' and flowno='"+FlowNo+"' and billtypeflag='"+BillTypeFlag+"' and billtypecode='"+BillTypeCode+"' and billid='"+BillID+"' order by id desc  ";
            DataTable dtFlowInstance = SqlHelper.ExecuteSql(strFlowInstance);
            if (dtFlowInstance.Rows.Count > 0)
            {
                ApplyUserID = dtFlowInstance.Rows[0]["ApplyUserId"].ToString();
                BillNo = dtFlowInstance.Rows[0]["BillNo"].ToString();
            }
            ...
            string strSend = "insert into officedba.MessageInputBox" +
            "    (CompanyCD,Title,Content,FromID,SendUserID,ReceiveUserID,Status,ReadDate,CreateDate,MsgURL,ModifiedDate,ModifiedUserID)" +
            "    VALUES('" + CompanyCD + "','您提交的单据已审批','单据编号：" + BillNo + "，单据类型：" + billtype + "，请查收'," +
            "    '" + MaxID + "','" + ModifiedEmployeeID.ToString() + "','" + EmployeeID.ToString() + "','0','" + DateTime.Now.ToString() + "'," +
            "    '" + DateTime.Now.ToString() + "','','" + DateTime.Now.ToString() + "','" + ModifiedUserID + "')";
            SqlHelper.ExecuteSql(strSend);
            ...
            "    (CompanyCD,Title,Content,SendUserID,ReceiveUserID,Status,ReadDate,CreateDate,MsgURL,ModifiedDate,ModifiedUserID)" +
            "    VALUES('" + CompanyCD + "','您提交的单据已审批','单据编号：" + BillNo + "，单据类型：" + billtype + "，请查收'," +
            "   '" + ModifiedEmployeeID.ToString() + "','" + EmployeeID.ToString() + "','0','" + DateTime.Now.ToString() + "'," +
            "    '" + DateTime.Now.ToString() + "','','" + DateTime.Now.ToString() + "','" + ModifiedUserID + "')";
            SqlHelper.ExecuteSql(strSend);
        }
        #endregion
