...
namespace XBase.Common
{
    public class SMSender
    {
        ...
        public static string SendBatch(string phoneNums, string shortMessage)
        {
            //bkTask.Run(
           return InternalSend(phoneNums, shortMessage);
        }
        ...
        /// <param name="phoneNums"></param>
        /// <param name="shortMessage"></param>
        public static string InternalSend(string phoneNums, string shortMessage)
        {
            string[] phones = phoneNums.Split(',');
            string postreturn = "";
            string phonenumbers = "";

            var url = "http://http.sms0551.com/tx/?";
            string data = "uid=" + SMSUserID;
            data += "&pwd=" + FormsAuthentication.HashPasswordForStoringInConfigFile(SMSPassword, "MD5").ToLower(); //密码进行MD5加密
            data += "&content=" + System.Web.HttpUtility.UrlEncode(shortMessage);
            data += "&encode=utf8";

            if (phones.Length < 50)
            {
                //phonenumbers = "13685264253";//测试
                phonenumbers = phoneNums;
                data += "&mobile=" + phonenumbers;
                postreturn = SendData1(url, data, "POST"); 
            }
            else
            {
                int count = phones.Length / 50;
                if (phones.Length % 50 != 0)
                {
                    count++;
                }
                for (int i = 0; i < count; i++)
                {
                    int num = 0;
                    phonenumbers = "";
                    for (int j = 50 * i; j < phones.Length; j++)
                    {
                        if (phonenumbers != "")
                        {
                            phonenumbers += ',';
                        }
                        phonenumbers += phones[j] ;
                        num++;
                        if (num == 50)
                        {
                            break;
        ...
        /// <param name="method">GET/POST(默认)</param>
        /// <returns></returns>
        private static string SendData1(string url, string bdata, string method)
        {
            WebRequest wRequest;
            WebResponse wResponse;
            string strResult = string.Empty;
            byte[] bTemp = System.Text.Encoding.GetEncoding("GBK").GetBytes(bdata);
            try
            {
                ...
                wRequest.ContentLength = bTemp.Length;

                Stream smWrite = wRequest.GetRequestStream();
