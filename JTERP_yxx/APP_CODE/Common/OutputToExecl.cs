...
namespace XBase.Common
{
    public class OutputToExecl
    {
        ...
        #region 导出到Excel 无制表人信息（导出客户档案导入错误列表）
        /// <summary>
        /// 导出Excel
        /// </summary>
        /// <param name="page"></param>
        ...
        /// <param name="field">标题头对应字段名称</param>
        /// <param name="fileName">生成保存的文件名(例：WorkCenter,后面的.xls不需要)</param>
        public static void ExportToTable_Err(System.Web.UI.Page page, System.Data.DataTable dt, string[] header, string[] field, string fileName, string Promote)
        {
            #region 导出
            HttpResponse resp;
            ...
            for (int j = 0; j < dt.Rows.Count; j++)
            {
                for (int n = 0; n <= field.Length - 1; n++)
                {
                    if (n == field.Length - 1)
                    {
                        lsField_item += dt.Rows[j][field[n].ToString()].ToString().Trim() + Convert.ToChar(127) + Convert.ToChar(13); ;
                    }
                    else
                    {
                        lsField_item += dt.Rows[j][field[n].ToString()].ToString().Trim() + Convert.ToChar(127) + Convert.ToChar(9); ;
                    }
                }
                resp.Write(lsField_item);
                lsField_item = "";
        ...
        #region 使用excel组件导出  （往来单位档案错误列表导出专用）

        public static void ExportDataTableToExcel(System.Data.DataTable dbSource, string[] header)
        {
            System.Data.DataTable myTable = dbSource;
            ...
            try
            {
                ...
                {
                    colIndex = colIndex + 1;
                    xlApp.Cells[1, colIndex] = header[i].ToString();
                }
                ...
                xlApp.ActiveCell.Font.Bold = true;
                //字体
                xlApp.get_Range(xlApp.Cells[rowSum, 1], xlApp.Cells[rowSum, 1]).Font.Size = 10;
                //对齐方式
                xlApp.get_Range(xlApp.Cells[rowSum, 1], xlApp.Cells[rowSum, 1]).HorizontalAlignment = Microsoft.Office.Interop.Excel.Constants.xlLeft;
